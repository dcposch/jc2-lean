...
/-! # The moving double-root half lift below the weight-two load

After the weight-one and weight-two parameters vanish, the same quadratic row argument
used before `d` remains valid below the `c5` load.  In the range
`3 * delta ≤ 4 * p`, this is enough to promote all three moving-root deviations
...
/-- On the `d = c7 = 0` stratum with `3 * delta ≤ 4 * p`, the first nonzero common
moving-root deviation cannot occur below the half scale. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_c5
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c5 F hscale hd hc7
        r hroot q hq0 (by linarith) (by nlinarith [T.hdelta, hp])
...
/-- The pre-`c5` scale exclusion supplies simultaneous half-scale order
bounds for the three canonical moving-root deviations. -/
theorem TransverseFactor.doubleRoot_deviations_order_half_lower_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_half_before_c5
      F hscale hd hc7 hp r (by rw [hX]; exact hA0)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_half_lower_before_c5

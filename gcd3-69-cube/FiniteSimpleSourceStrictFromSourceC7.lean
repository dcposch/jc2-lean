import FiniteSimpleSourceLiftC7
import FiniteSimpleSourceLateOrderC7
import FiniteSimpleSourceFullScale
import FiniteSimpleSourceFullScaleC7

/-! # Source-facing strict late simple-root obstruction before `c7`

The source-driven order lift and the literal sextic source equation remain
available below the weight-two load.  They promote the simple-root chart to
...
/-- If `3 * delta < 2 * p`, the weight-one load vanishes, and the recovered
source residue is the remaining simple root of the singular leading cubic,
then the literal finite source is inconsistent before the `c7` load. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c7
  ...
  exact T.simpleRoot_strictLate_inconsistent_of_fullScale_before_c7
    F hscale hd hp r d b C2n r0 A0 hr hr0 hX hA0 hroot
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c7

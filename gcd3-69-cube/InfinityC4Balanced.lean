...
/-! # The balanced singular c4 cell at infinity

After `d = c7 = c5 = 0`, the weight-five load, the quadratic moving-root
correction, and the intrinsic cubic meet when `5 p = 3 delta`.
-/

noncomputable section

/-- Rows one, two, four, and the terminal row are inconsistent at the
balanced c4 resonance. -/
theorem GCD369Cube_c4Load_balanced_terminal_inconsistent
    ...
    (h4 :
      32 * (-13122 * A * r ^ 3 * q - 65610 * r ^ 3 * b ^ 2) -
        314928 * r * A ^ 3 - 4665600 * r ^ 8 * T = 0)
    (h5 :
      559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
        2519424 * r ^ 2 * A ^ 3 + 46448640 * r ^ 9 * T = 0) :
    False := by
...
theorem terminalC4Coeff_doubleRoot
...
/-- The pure c4 fifth numerator at `5 p = 3 delta`. -/
theorem TransverseScale.c4SpecializedTerminalCoeff_at_balanced
    {k : Type*} [Field k] [CharZero k]
    ...
    (htie : 5 * D.faber.scale.p = 3 * T.delta)
    ...
    (GCD369CubeFaberFullN5
      ...
      559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
        2519424 * r ^ 2 * A ^ 3 + 46448640 * r ^ 9 * C4 := by
  ...
  have hP : GCD369CubeHahnRegular.constantCoeff P =
      46448640 * r ^ 9 := by
    dsimp only [P]
    simp only [terminalC4Coeff, map_add, map_sub, map_mul, map_pow,
      map_ofNat, map_neg]
    change terminalC4Coeff
      D.faber.scale.leading0 D.faber.scale.leading1
      D.faber.scale.leading2 D.faber.scale.leading3
      D.faber.scale.leading4 = 46448640 * r ^ 9
    rw [D.ha0, D.ha1, D.ha2, D.ha3, D.ha4, hu, hv]
    exact terminalC4Coeff_doubleRoot r
  have hload : D.faber.scale.weightedRegular 5 D.faber.c4 =
      H ^ 3 * GCD369CubeHahnRegular.constant C4 := by
    rw [hc4, D.faber.scale.weightedRegular_constant_eq_monomial_sq
      5 (by norm_num) C4]
    apply Subtype.ext
    change
      (HahnSeries.single (((5 : ℕ) : ℚ) * D.faber.scale.p / 2)
          (1 : k)) ^ 2 * HahnSeries.C C4 =
        (HahnSeries.single T.delta (1 : k)) ^ 3 * HahnSeries.C C4
    simp only [HahnSeries.single_pow, nsmul_eq_mul, one_pow]
    apply congrArg
      (fun z : ℚ => HahnSeries.single z (1 : k) * HahnSeries.C C4)
    calc
      2 * (5 * D.faber.scale.p / 2) = 5 * D.faber.scale.p := by ring
      _ = 3 * T.delta := htie
  ...
  have hfactor :
      GCD369CubeFaberFullN5
        ...
        0 0 0 =
      (559872 * R5) * H ^ 2 +
        H ^ 3 * (839808 * C + GCD369CubeHahnRegular.constant C4 * P) := by
  ...
  have htop := GCD369CubeHahnRegular.coeff_monomial_pow_mul_at
    T.delta T.hdelta 3
      (839808 * C + GCD369CubeHahnRegular.constant C4 * P)
  change (H ^ 3 *
      (839808 * C + GCD369CubeHahnRegular.constant C4 * P)).1.coeff
        (3 * T.delta) =
      GCD369CubeHahnRegular.constantCoeff
        (839808 * C + GCD369CubeHahnRegular.constant C4 * P) at htop
  change
    (GCD369CubeFaberFullN5
      D.faber.scale.regular0 D.faber.scale.regular1
      D.faber.scale.regular2 D.faber.scale.regular3
...
/-- Before the c4 load, the first three moving-root row coefficients vanish
below the intrinsic residual cutoff. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c4_balanced
    {k : Type*} [Field k] [CharZero k]
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Rows one, two, and four at the balanced c4 resonance. -/
theorem TransverseScale.c4LoadedNormalRowsAt_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    ...
    ((32 * GCD369CubeNormalRow4
        T.Xn T.Yn T.Zn D.cubicU D.cubicV).1.coeff T.delta -
          314928 * r * A0 ^ 3 - 4665600 * r ^ 8 * C4 = 0) := by
  let F := D.faber
  let H : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.monomial T.delta T.hdelta.le
  let N1 : GCD369CubeHahnRegular k := GCD369CubeFaberN1
  ...
  let P1 : GCD369CubeHahnRegular k :=
    41472 * F.scale.regular1 -
      13824 * F.scale.regular3 * F.scale.regular4
  let P2 : GCD369CubeHahnRegular k :=
    124416 * F.scale.regular0 -
      41472 * F.scale.regular2 * F.scale.regular4 -
      20736 * F.scale.regular3 ^ 2 + 9216 * F.scale.regular4 ^ 3
  let P4 : GCD369CubeHahnRegular k :=
    -186624 * F.scale.regular1 * F.scale.regular3 -
      186624 * F.scale.regular2 ^ 2 +
      124416 * F.scale.regular2 * F.scale.regular4 ^ 2 +
      124416 * F.scale.regular3 ^ 2 * F.scale.regular4 -
      20736 * F.scale.regular4 ^ 4
  let c4w : GCD369CubeHahnRegular k :=
    F.scale.weightedRegular 5 (GCD369CubeHahnRegular.constant C4)
  have hload1 : GCD369CubeHahnRegular.constantCoeff P1 =
      -165888 * r ^ 5 := by
    dsimp only [P1, F]
    simp only [map_sub, map_mul, map_ofNat]
    change 41472 * F.scale.leading1 -
      13824 * F.scale.leading3 * F.scale.leading4 = _
    rw [D.ha1, D.ha3, D.ha4, hu, hv]
    ring
  have hload2 : GCD369CubeHahnRegular.constantCoeff P2 =
      414720 * r ^ 6 := by
    dsimp only [P2, F]
    simp only [map_add, map_sub, map_mul, map_pow, map_ofNat]
    change 124416 * F.scale.leading0 -
      41472 * F.scale.leading2 * F.scale.leading4 -
      20736 * F.scale.leading3 ^ 2 + 9216 * F.scale.leading4 ^ 3 = _
  ...
  have hload4 : GCD369CubeHahnRegular.constantCoeff P4 =
    ...
    change -186624 * F.scale.leading1 * F.scale.leading3 -
      186624 * F.scale.leading2 ^ 2 +
      124416 * F.scale.leading2 * F.scale.leading4 ^ 2 +
      124416 * F.scale.leading3 ^ 2 * F.scale.leading4 -
      20736 * F.scale.leading4 ^ 4 = _
    rw [D.ha1, D.ha2, D.ha3, D.ha4, hu, hv]
    ring
  have hweight1 : (c4w * P1).1.coeff (5 * F.scale.p) =
      C4 * GCD369CubeHahnRegular.constantCoeff P1 := by
    simpa only [c4w] using
      F.scale.weightedRegular_five_constant_mul_coeff C4 P1
  have hweight2 : (c4w * P2).1.coeff (5 * F.scale.p) =
      C4 * GCD369CubeHahnRegular.constantCoeff P2 := by
    simpa only [c4w] using
      F.scale.weightedRegular_five_constant_mul_coeff C4 P2
  have hweight4 : (c4w * P4).1.coeff (5 * F.scale.p) =
      C4 * GCD369CubeHahnRegular.constantCoeff P4 := by
    simpa only [c4w] using
      F.scale.weightedRegular_five_constant_mul_coeff C4 P4
  have hz := F.c4NCoeff_zero C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    hd hc7 hc5 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
  have hz1 := hz.1
  have hz2 := hz.2.1
  have hz4 := hz.2.2.2
  rw [hc4] at hz1 hz2 hz4
  change (729 * N1).1.coeff (5 * F.scale.p) +
      (c4w * P1).1.coeff (5 * F.scale.p) = 0 at hz1
  change (2187 * N2).1.coeff (5 * F.scale.p) +
      (c4w * P2).1.coeff (5 * F.scale.p) = 0 at hz2
  change (6561 * N4).1.coeff (5 * F.scale.p) +
      (c4w * P4).1.coeff (5 * F.scale.p) = 0 at hz4
  ...
  have hc1' := congrArg
    (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * F.scale.p)) hexp1
  have hc2' := congrArg
    (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * F.scale.p)) hexp2
  have hc4' := congrArg
    (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * F.scale.p)) hexp4
  have hshift1 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
  ...
  have hcubicval :
      ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (5 * F.scale.p) = 314928 * r * A0 ^ 3 := by
    rw [htie, hcubic]
    simp only [map_mul, map_pow, map_ofNat, hY, hX]
    ring
  change (729 * N1).1.coeff (5 * F.scale.p) =
      ((-32 * Row1) * H ^ 2).1.coeff (5 * F.scale.p) at hc1'
  change (2187 * N2).1.coeff (5 * F.scale.p) =
      ((32 * Row2) * H ^ 2).1.coeff (5 * F.scale.p) at hc2'
  change (6561 * N4).1.coeff (5 * F.scale.p) =
      ((32 * Row4) * H ^ 2).1.coeff (5 * F.scale.p) -
        ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (5 * F.scale.p) at hc4'
...
/-- After the three earlier loads vanish, parameters following c4 are
strictly above the weight-five terminal cutoff. -/
theorem fullN5_sub_c4Specialized_mem_higherOrderIdeal_balanced
    {k : Type*} [Field k] [CharZero k]
...
theorem fullN5Coeff_eq_c4Specialized
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (C3 C2 C1 : k)
    (hd : F.d = GCD369CubeHahnRegular.constant 0)
    ...
    (GCD369CubeFaberFullN5
      ...
      (F.scale.weightedRegular 6 F.c3)
      (F.scale.weightedRegular 7 F.c2)
      (F.scale.weightedRegular 8 F.c1)).1.coeff (5 * F.scale.p) =
    (GCD369CubeFaberFullN5
      F.scale.regular0 F.scale.regular1 F.scale.regular2
      F.scale.regular3 F.scale.regular4 0 0 0
      (F.scale.weightedRegular 5 F.c4) 0 0 0).1.coeff
        (5 * F.scale.p) := by
  let full : GCD369CubeHahnRegular k := GCD369CubeFaberFullN5
    F.scale.regular0 F.scale.regular1 F.scale.regular2
    F.scale.regular3 F.scale.regular4
  ...
  let c4row : GCD369CubeHahnRegular k := GCD369CubeFaberFullN5
    ...
    F.scale.regular3 F.scale.regular4 0 0 0
    (F.scale.weightedRegular 5 F.c4) 0 0 0
  have hdiff := F.fullN5_sub_c4Specialized_mem_higherOrderIdeal
    C3 C2 C1 hd hc7 hc5 hc3 hc2 hc1
  have hcoeff : (full - c4row).1.coeff (5 * F.scale.p) = 0 :=
    HahnSeries.coeff_eq_zero_of_lt_orderTop hdiff
  change full.1.coeff (5 * F.scale.p) = c4row.1.coeff (5 * F.scale.p)
  change full.1.coeff (5 * F.scale.p) -
      c4row.1.coeff (5 * F.scale.p) = 0 at hcoeff
...
theorem TransverseScale.c4LoadedTerminalEquation_at_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (C4 C3 C2 C1 : k)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    ...
    (hzero :
      (GCD369CubeFaberFullN5
        ...
        (D.faber.scale.weightedRegular 8 D.faber.c1)).1.coeff
          (5 * D.faber.scale.p) = 0) :
    559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
      2519424 * r ^ 2 * A ^ 3 + 46448640 * r ^ 9 * C4 = 0 := by
  rw [D.faber.fullN5Coeff_eq_c4Specialized
    C3 C2 C1 hd hc7 hc5 hc3 hc2 hc1] at hzero
  rw [T.c4SpecializedTerminalCoeff_at_balanced
    C4 hc4 r A d b q hu hv hX hY hZ htie
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Global degree integrality supplies the balanced c4 terminal row. -/
theorem infinity_c4LoadedTerminalEquation_at_balanced
    ...
    (hY : GCD369CubeHahnRegular.constantCoeff T.Yn = r * A)
    (hZ : GCD369CubeHahnRegular.constantCoeff T.Zn = -2 * r ^ 2 * A)
    (htie : 5 * (S.infinityCommonCoefficientData hsdegree).faber.scale.p =
      3 * T.delta)
    (hord1 : (↑T.delta : WithTop ℚ) ≤
  ...
  have hzero := S.infinity_fullN5Coeff_weight_p_eq_zero_of_singular
    hsdegree 5 (by norm_num) hdisc
  exact T.c4LoadedTerminalEquation_at_balanced
    C4 C3 C2 C1 hd hc7 hc5 hc4 hc3 hc2 hc1
    r A d b q hu hv hX hY hZ htie hord1 hord2 hcoeff1 hcoeff2
    (by simpa only [D, Nat.cast_ofNat] using hzero)

set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- The singular balanced c4 cell is impossible. -/
theorem infinity_c4_singular_balanced_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
  ...
  have hearly : 2 * T.delta < 4 * D.faber.scale.p := by
    linarith [htie, D.faber.scale.hp]
  ...
  have hrows : ∀ e : ℚ, 0 ≤ e → e < T.delta →
    ...
    intro e he hedelta
    have hearly' : 2 * T.factor.delta + e < 5 * D.faber.scale.p := by
      change 2 * T.delta + e < 5 * D.faber.scale.p
      rw [htie]
      linarith
    have h := T.factor.doubleRoot_row123Coeffs_zero_before_c4_balanced
      hdDz hc7Dz hc5Dz rr hroot e he hedelta hearly'
  ...
  obtain ⟨hload1, hload2, hload4⟩ := T.c4LoadedNormalRowsAt_balanced
  ...
  have h5 := S.infinity_c4LoadedTerminalEquation_at_balanced
    hsdegree hdisc T P.c4 P.c3 P.c2 P.c1
    hdD hc7D hc5D hc4D hc3D hc2D hc1D
    r A0 d0 b0 q0 hu hv rfl hY hZ htie hord1 hord2 hcoeff1 hcoeff2
  apply GCD369Cube_c4Load_balanced_terminal_inconsistent
    A0 r d0 b0 q0 P.c4 hA0 hr
...
#print axioms GCD369Cube_c4Load_balanced_terminal_inconsistent
#print axioms GCD369CubeHahnCommonCoefficientData.terminalC4Coeff_doubleRoot
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c4SpecializedTerminalCoeff_at_balanced
#print axioms GCD369CubeHahnFaberPoleData.fullN5Coeff_eq_c4Specialized
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c4LoadedTerminalEquation_at_balanced
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c4LoadedNormalRowsAt_balanced
#print axioms GCD369CubePolynomialSource.infinity_c4LoadedTerminalEquation_at_balanced
#print axioms GCD369CubePolynomialSource.infinity_c4_singular_balanced_inconsistent

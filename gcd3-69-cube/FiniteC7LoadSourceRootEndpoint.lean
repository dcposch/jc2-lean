...
import FiniteSimpleSourceEndpointCoordinatesC7
import FiniteDoubleC7LoadEndpointLift

/-! # The balanced active-`c7` singular source branch

At `2 * p = 3 * delta`, the literal source still excludes the double
...
/-- If the weight-two `c7` load meets the intrinsic cubic scale exactly,
the singular finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_c7_of_eq
  ...
  obtain ⟨r, d, Bn, q, hr, hroot, hD, hB, hC, hBn0⟩ :=
    T.factor.simpleRoot_zeroQuadratic_coordinates_from_source_before_c7
      F hscale hdNormal hd0
      (by simpa only [TransverseScale.factor] using hp)
      r0 (GCD369CubeHahnRegular.constantCoeff T.Xn)
      hr0 hXne huc hvc hx (by rfl) hY hZ
  exact T.factor.doubleRoot_c7Load_source_inconsistent_of_eq
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_c7_of_eq

...
import FiniteDoubleRho1LoadEndpointCoeff
...
/-! # The balanced singular rho1 cell at infinity

After `d = c7 = c5 = c4 = c3 = c2 = 0`, the weight-ten load, the
...
/-- The unloaded second and fourth rows together with the terminal row are
inconsistent at the balanced rho1 resonance. -/
theorem GCD369Cube_rho1Load_balanced_terminal_inconsistent
...
/-- At the rho1/cubic resonance, the zero-load terminal numerator has the
displayed coefficient.  The invariant load rho1 does not occur in FullN5. -/
theorem TransverseScale.rho1ZeroHighTerminalCoeff_at_balanced
    {k : Type*} [Field k] [CharZero k]
    ...
    (htie : 10 * D.faber.scale.p = 3 * T.delta)
...
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho1_balanced
    {k : Type*} [Field k] [CharZero k]
    ...
    (hc2 : D.faber.c2 = 0) (hc1 : D.faber.c1 = 0)
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Rows one, two, and four at the balanced rho1 resonance. -/
theorem TransverseScale.rho1LoadedNormalRowsAt_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    ...
    ((32 * GCD369CubeNormalRow2
        T.Xn T.Yn T.Zn D.cubicU D.cubicV).1.coeff T.delta = 0) ∧
  ...
  have hcubicval :
      ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (10 * F.scale.p) = 314928 * r * A0 ^ 3 := by
    rw [htie, hcubic]
    simp only [map_mul, map_pow, map_ofNat, hY, hX]
    ring
  change (729 * N1).1.coeff (10 * F.scale.p) =
      ((-32 * Row1) * H ^ 2).1.coeff (10 * F.scale.p) at hc1'
  change (2187 * N2).1.coeff (10 * F.scale.p) =
      ((32 * Row2) * H ^ 2).1.coeff (10 * F.scale.p) at hc2'
  change (6561 * N4).1.coeff (10 * F.scale.p) =
      ((32 * Row4) * H ^ 2).1.coeff (10 * F.scale.p) -
        ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (10 * F.scale.p) at hc4'
...
theorem fullN5Coeff_eq_rho1BalancedSpecialized
    {k : Type*} [Field k] [CharZero k]
    ...
    (GCD369CubeFaberFullN5
      ...
      (F.scale.weightedRegular 8 F.c1)).1.coeff (10 * F.scale.p) =
    (GCD369CubeFaberFullN5
      F.scale.regular0 F.scale.regular1 F.scale.regular2
...
theorem TransverseScale.rho1LoadedTerminalEquation_at_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    (hc7 : D.faber.c7 = GCD369CubeHahnRegular.constant 0)
    ...
    (hzero :
      (GCD369CubeFaberFullN5
        ...
        (D.faber.scale.weightedRegular 8 D.faber.c1)).1.coeff
          (10 * D.faber.scale.p) = 0) :
    559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
      2519424 * r ^ 2 * A ^ 3 = 0 := by
  rw [D.faber.fullN5Coeff_eq_rho1BalancedSpecialized
    hd hc7 hc5 hc4 hc3 hc2 hc1] at hzero
  rw [T.rho1ZeroHighTerminalCoeff_at_balanced
    r A d b q hu hv hX hY hZ htie
    hord1 hord2 hcoeff1 hcoeff2] at hzero
  exact hzero

/-- Below the weight-ten load, with every earlier load zero, the first
transverse residue satisfies the four universal normal rows. -/
theorem TransverseScale.zeroLoadLeadingRows_before_rho1_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
  ...
  have hz := D.faber.zeroHighNCoeff_zero_before_rho1
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Global degree integrality supplies the balanced rho1 terminal row. -/
theorem infinity_rho1LoadedTerminalEquation_at_balanced
    ...
    (hY : GCD369CubeHahnRegular.constantCoeff T.Yn = r * A)
    (hZ : GCD369CubeHahnRegular.constantCoeff T.Zn = -2 * r ^ 2 * A)
    (htie : 10 * (S.infinityCommonCoefficientData hsdegree).faber.scale.p =
      3 * T.delta)
    (hord1 : (↑T.delta : WithTop ℚ) ≤
  ...
  have hzero := S.infinity_fullN5Coeff_weight_p_eq_zero_of_singular
    hsdegree 10 (by norm_num) hdisc
  exact T.rho1LoadedTerminalEquation_at_balanced
    hd hc7 hc5 hc4 hc3 hc2 hc1
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- The singular balanced rho1 cell is impossible. -/
theorem infinity_rho1_singular_balanced_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
  ...
  obtain ⟨hz1, hz2, hz3, hz4⟩ :=
    T.zeroLoadLeadingRows_before_rho1_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz hc1Dz hearly
  ...
  have hrows : ∀ e : ℚ, 0 ≤ e → e < T.delta →
    ...
    intro e he hedelta
    have hearly' : 2 * T.factor.delta + e < 10 * D.faber.scale.p := by
      change 2 * T.delta + e < 10 * D.faber.scale.p
      rw [htie]
      linarith
    have h := T.factor.doubleRoot_row123Coeffs_zero_before_rho1_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz hc1Dz
  ...
  obtain ⟨hload1, hload2, hload4⟩ := T.rho1LoadedNormalRowsAt_balanced
  ...
  have h5 := S.infinity_rho1LoadedTerminalEquation_at_balanced
    hsdegree hdisc T
    hdD hc7D hc5D hc4D hc3D hc2D hc1D
    r A0 d0 b0 q0 hu hv rfl hY hZ htie hord1 hord2 hcoeff1 hcoeff2
  apply GCD369Cube_rho1Load_balanced_terminal_inconsistent
    A0 r d0 b0 q0 R.rho1 hA0 hr
...
#print axioms GCD369Cube_rho1Load_balanced_terminal_inconsistent
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.rho1ZeroHighTerminalCoeff_at_balanced
#print axioms GCD369CubeHahnFaberPoleData.fullN5Coeff_eq_rho1BalancedSpecialized
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.rho1LoadedTerminalEquation_at_balanced
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.rho1LoadedNormalRowsAt_balanced
#print axioms GCD369CubePolynomialSource.infinity_rho1LoadedTerminalEquation_at_balanced
#print axioms GCD369CubePolynomialSource.infinity_rho1_singular_balanced_inconsistent

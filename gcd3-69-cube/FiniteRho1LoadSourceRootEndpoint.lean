...
import FiniteDoubleEndpointCoordinatesRho1
import FiniteSimpleSourceEndpointResidueRho1
import FiniteDoubleRho1LoadEndpointLift

/-! # The balanced active-`rho1` singular source branch -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- If the weight-ten `rho1` load meets the intrinsic cubic scale exactly,
the singular finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_rho1_of_eq
    ...
    (hEq : 10 * F.scale.p = 3 * T.delta) : False := by
  ...
  obtain ⟨r0, hr0, hXne, hu, hv, hY, hZ, hx⟩ :=
    T.source_coordinate_is_simpleRoot_before_rho1
      F hscale hdNormal hc7Normal hc5Normal hc4Normal hc3Normal hc2Normal
      hc1Normal hd0 hc70 hc50 hc40 hc30 hc20 hc10 hp
  ...
  obtain ⟨d, B, q, hD, hB, hC⟩ :=
    T.factor.doubleRoot_quadratic_coordinates_at_rho1
      F hscale hd0 hc70 hc50 hc40 hc30 hc20 hc10
  ...
  have hEqNormal : 10 * S.normal.sextic.scale.p =
      3 * T.factor.delta := by
    rw [hscale]
    simpa only [TransverseScale.factor] using hEq
  have hB0 := T.factor.simpleRoot_quadratic_B_residue_zero_at_rho1
  ...
  exact T.factor.doubleRoot_rho1Load_source_inconsistent_of_eq
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_rho1_of_eq

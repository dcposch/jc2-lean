...
/-! # Early transverse jets before the first weight-eleven load

If a common-cubic transverse jet occurs before the weight-eleven `rho2` scale,
and the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, weight-eight, and weight-ten parameters have already vanished,
then every remaining source and target load disappears in the corresponding
higher-order quotient.  The remaining loads have weights 11, 12, and 13.
The four complete Faber equations therefore reduce to the universal
zero-load normal rows.  A nonzero first jet consequently forces the
limiting cubic onto its double-root discriminant.  The lower-row
identities used here do not involve the intermediate weight-three `c6`
parameter.
-/
...
/-- Below the weight-eleven scale, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, weight-eight, and
weight-ten loads have vanished, the complete source equations have exactly
the zero-high-constant leading coefficients. -/
theorem zeroHighNCoeff_zero_before_rho2
    ...
    (hc1 : F.c1 = 0)
    (hr1 : F.rho1 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 11 * F.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 11 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (11 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (11 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
  have hkill (w : ℕ) (hw : 11 ≤ w)
  ...
  have hc2q : q (F.scale.weightedRegular 7 F.c2) = 0 := by
    have hzw : F.scale.weightedRegular 7 F.c2 = 0 := by
      apply Subtype.ext
      change F.scale.t ^ 7 * F.c2.1 = 0
      rw [hc2]
      simp
    rw [hzw, map_zero]
  have hc1q : q (F.scale.weightedRegular 8 F.c1) = 0 := by
    have hzw : F.scale.weightedRegular 8 F.c1 = 0 := by
      apply Subtype.ext
      change F.scale.t ^ 8 * F.c1.1 = 0
      rw [hc1]
      simp
    rw [hzw, map_zero]
  have hr1q : q (F.scale.weightedRegular 10 F.rho1) = 0 := by
    have hzw : F.scale.weightedRegular 10 F.rho1 = 0 := by
      apply Subtype.ext
      change F.scale.t ^ 10 * F.rho1.1 = 0
      rw [hr1]
      simp
    rw [hzw, map_zero]
...
/-- Any transverse factor strictly before the first weight-eleven load, after
the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, weight-eight, and weight-ten parameters have vanished,
satisfies the four universal zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_rho2
    {k : Type*} [Field k] [CharZero k]
...
/-- A nonzero first transverse jet before the `rho2` scale, after the
weight-one, weight-two, weight-four, weight-five, weight-six, weight-seven,
weight-eight, and weight-ten parameters have vanished, forces the limiting
common cubic to be singular. -/
theorem TransverseScale.discriminant_zero_before_rho2
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_rho2 F hscale
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_rho2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_rho2
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_rho2

...
/-! # The `B = 0` endpoint of the balanced `c7` resonance

At the unique triple resonance the four invariant rows leave the residual
equation `r * (23 * A ^ 3 + 540 * r ^ 2 * B ^ 2) = 0`.  When the quadratic
...
/-- The balanced `c7` resonance is inconsistent once the quadratic
half-scale coefficient is zero. -/
theorem GCD369CubeDoubleBlowup_c7Load_balanced_resonance_Bzero_inconsistent
  ...
  have hrel :=
    GCD369CubeDoubleBlowup_c7Load_balanced_resonance_relation
      A B Q r d T h1 h2 h3 h4
  have h23A : (23 : K) * A ^ 3 = 0 := by
    simpa [hB, hr] using hrel
  rcases mul_eq_zero.mp h23A with h23 | hA3
  · norm_num at h23
  · exact (pow_ne_zero 3 hA) hA3

#print axioms GCD369CubeDoubleBlowup_c7Load_balanced_resonance_Bzero_inconsistent

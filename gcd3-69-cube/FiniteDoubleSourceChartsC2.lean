import FiniteDoubleSourceOrderCasesC2
import FiniteDoubleSourceNonicC2
import FiniteSimpleSourceOrdersC2
...
/-! # Double-root source chart split before `c2`

After the weight-one, weight-two, weight-four, weight-five, and weight-six
loads vanish, the same early/middle/late/zero chart valuations apply once
`3 * delta ≤ 7 * p`.  The proofs are the existing chart calculations; only
...
/-- When `3 * delta ≤ 7 * p` and `d = 0`, an early nonzero root deviation
has the orders and leading coefficients consumed by the abstract
early-source contradiction below the weight-seven load. -/
theorem TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_chart_before_c2
    ...
    (hp : 3 * T.delta ≤ 7 * S.normal.sextic.scale.p)
  ...
  have hphilc : S.transverseValue.1.leadingCoeff =
    ...
    simpa only [GCD369CubeDoubleSourceTransverseChart, H, M,
      ...
      hPmainLC]
  exact T.doubleRoot_sourceEarly_inconsistent_of_exactOrders_before_c2
    hd hc7 hc5 hc4 nu r0 A0 c hr0 hA0 hc hp hnu hearly hx hX hY
    hKorder hKlc hphiOrder hphilc

/-- When `3 * delta ≤ 7 * p` and `d = 0`, a middle-range nonzero root
...
excludes. -/
theorem TransverseFactor.doubleRoot_sourceMiddle_inconsistent_of_chart_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (hc7 : S.normal.c7 = 0)
  ...
  have hphiOrder : S.transverseValue.1.orderTop =
    ...
    simpa only [GCD369CubeDoubleSourceTransverseChart, H, M,
      Rw, add_assoc] using hphiData.1
  exact T.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_c2
    (T.delta + nu) hp hKorder hphiOrder (by nlinarith [hbelow])

/-- When `3 * delta ≤ 7 * p` and `d = 0`, a late nonzero root deviation of
order at least `delta` forces the cubic and transverse values into the
abstract nonic order-bounds contradiction below the weight-seven load. -/
theorem TransverseFactor.doubleRoot_sourceLate_inconsistent_of_chart_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (hc7 : S.normal.c7 = 0)
  ...
  have hphi : (↑(2 * T.delta) : WithTop ℚ) ≤
    ...
    simpa only [GCD369CubeDoubleSourceTransverseChart, H, M, Rw,
      ...
      using hphiData
  exact T.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c2
    r0 A0 hr0 hA0 hx hX hY hd hc7 hc5 hc4 hp hK hphi

/-- When `3 * delta ≤ 7 * p` and `d = 0`, a vanishing root deviation forces
...
theorem TransverseFactor.doubleRoot_sourceZero_inconsistent_of_chart_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (hc7 : S.normal.c7 = 0)
...
`3 * delta ≤ 7 * p` is excluded.  The sole input about the deviation not
already present in the chart equations is that its residue vanishes. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_of_chart_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (hc7 : S.normal.c7 = 0)
  ...
  by_cases hwz : w = 0
  · subst w
    exact T.doubleRoot_sourceZero_inconsistent_of_chart_before_c2
      hd hc7 hc5 hc4 hc3 r0 A0 r d B B2 C2 hr0 hA0 hp hx hX hY
      hKchart hphichart
  · rcases GCD369CubeHahnRegular.exists_pos_orderTop_eq_of_constantCoeff_zero
    ...
    · exact T.doubleRoot_sourceEarly_inconsistent_of_chart_before_c2
        hd hc7 hc5 hc4 hc3 nu r0 A0 c r w d B B2 C2 hr0 hA0 hc
        hp hnu hearly hr hx hX hY hw rfl hKchart hphichart
    · by_cases hmiddle : nu < T.delta
      · exact T.doubleRoot_sourceMiddle_inconsistent_of_chart_before_c2
          hd hc7 hc5 hc4 hc3 nu r0 A0 r w d B B2 C2 hr0 hA0 hp hnu
          (by linarith) hmiddle hr hX hw hKchart hphichart
      · exact T.doubleRoot_sourceLate_inconsistent_of_chart_before_c2
          hd hc7 hc5 hc4 hc3 nu r0 A0 c r w d B B2 C2 hr0 hA0 hc
          hp hnu (le_of_not_gt hmiddle) hr hx hX hY hw rfl
          hKchart hphichart

/-- Expanded moving-root coordinates realize the complete double-root chart
before the weight-seven load. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_of_coordinates_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (hc7 : S.normal.c7 = 0)
  ...
  exact T.doubleRoot_source_inconsistent_of_chart_before_c2
    hd hc7 hc5 hc4 hc3 r0 A0 r w d B B2 C2 hr0 hA0 hp hr hx hX hY
    hw0 hcharts.1 hcharts.2

end GCD369CubeHahnCommonValueData

#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_chart_before_c2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceMiddle_inconsistent_of_chart_before_c2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceLate_inconsistent_of_chart_before_c2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceZero_inconsistent_of_chart_before_c2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_of_chart_before_c2
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_of_coordinates_before_c2

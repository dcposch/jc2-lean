import FiniteSimpleSourceChartAllC2
import FiniteDoubleLiftSourceC2

/-! # Source-driven simple-root order lift before `c2` -/
...
chart below `c2`; its displacement is zero or starts at `delta`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c2
  ...
  exact T.simpleRoot_source_zero_or_order_delta_of_coordinates_before_c2
    hd hc7 hc5 hc4 hc3 r0 A0 r w d e B 0 C2 hr0 hA0 hp hr hX hw0 hxcoord
...
simple-root order split before `c2`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_from_source_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  have hpS : 3 * T.delta < 7 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  exact T.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c2
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_from_source_before_c2

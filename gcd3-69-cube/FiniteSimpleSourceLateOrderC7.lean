...
/-! # Half-scale order bound before the weight-two load

After the weight-one load vanishes, the late simple-root normal correction
still lifts from the nominal half scale whenever `3 * delta < 2 * p`.  The proof
uses only the literal sextic source equation, so the weight-two cutoff is
enough.
-/
...
/-- In the late simple-root chart below the `c7` load, the half-scale
correction starts no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c7
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c7

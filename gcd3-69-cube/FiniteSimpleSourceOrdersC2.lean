...
import FiniteC2SourceGap

/-! # Pre-resonant orders in the simple-root source chart before `c2`

After the weight-one, weight-two, weight-four, and weight-five loads have
vanished, the same pre-resonant split applies once `3 * delta < 7 * p`.
The literal sextic source equation still excludes a transverse value of
exact order below `2 * delta`.  Splitting on the order of the half-scale
normal deviation then gives two short contradictions: either the cubic
square uniquely leads the sextic, already below the source scale `6 * p`,
or, once the weight-six load also vanishes, the reduced ninth-Faber
equation has a unique leading cubic term below the weight-seven load.
-/
...
`6 * p` when `3 * delta < 7 * p`. -/
theorem TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_c2
...
already below the weight-seven load. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
...
weight-seven load. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (hc5 : S.normal.c5 = 0) (hc4 : S.normal.c4 = 0)
  ...
  exact T.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c2_with_c3
    hd hc7 hc5 hc4 hc3 alpha gamma (c * (9 * r0 ^ 2)) halpha
    (mul_ne_zero hc (mul_ne_zero (by norm_num) (pow_ne_zero 2 hr0)))
    hK hKlc (by simpa only [E, L] using hE)
    (by nlinarith [hresonance, hbelow])
    (by nlinarith [hp, hbelow, T.hdelta, S.normal.sextic.scale.hp])
    (by nlinarith [hp, hbelow, T.hdelta])

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_c2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_c2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_c2

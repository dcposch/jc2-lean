...
import FiniteDoubleRho2LoadEndpointCoeff

/-! # Closing the balanced `rho2` resonance from exact quadratic coordinates

At the equality `11 * p = 3 * delta`, the residual load exponent is the
intrinsic half-scale `delta`.  Given the exact quadratic factorization of
the moving-root deviations, the four literal `rho2` equations reduce to the
...
/-- At the balanced active-`rho2` resonance, exact quadratic coordinates with
vanishing half-scale `B` residue make the loaded source equations
inconsistent. -/
theorem TransverseFactor.doubleRoot_rho2Load_source_inconsistent_of_eq
  ...
  have hY' : GCD369CubeHahnRegular.constantCoeff T.Yn =
    ...
    rw [hr]
    exact hY
  have hload := T.doubleRoot_rho2Load_source_coeff_of_eq F hscale
    R Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
    r A0 hroot hX hY' hEq
  dsimp only at hload
  ...
  have hload4 : (32 * Row4).1.coeff (2 * mu) - 314928 * r0 * A0 ^ 3 = 0 := by
  ...
  have hload4q : (32 * QR4).1.coeff (2 * mu) - 314928 * r0 * A0 ^ 3 = 0 := by
  ...
  apply GCD369CubeDoubleBlowup_rho2Load_balanced_resonance_Bzero_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) R hA0 hr0 hBn0
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_rho2Load_source_inconsistent_of_eq

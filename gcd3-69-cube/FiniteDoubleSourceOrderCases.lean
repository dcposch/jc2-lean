import FiniteDoubleSourceEarlyAlgebra
import FiniteDoubleSourceOrderBasic

/-! # Order cases in the source double-root chart

The source sextic already excludes any branch on which the transverse value
has exact order below twice the first normal scale while the moving cubic has
order at least that scale.  This is the abstract valuation step used for the
middle range of the moving-root deviation.
-/
...
theorem TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders
  ...
  · have hdelta : T.delta = 3 * nu := by
    ...
    have hKcubeOrder : (S.cubicValue ^ 3).1.orderTop =
      ...
      rw [hKcubeEq]
      simpa only [
        show 4 * nu + 2 * nu = 6 * nu by ring] using hK3.1
...
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders

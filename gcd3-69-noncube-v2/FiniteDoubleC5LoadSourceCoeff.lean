...
/-! # The moving double-root coefficient at the active `c5` load

In the strict window `delta < 2 * p` and `4 * p < 3 * delta`, the quadratic
double-root jet occurs before the weight-four load, while the intrinsic cubic
occurs after it.  This file extracts the four literal `c5` equations at the
remaining exponent `4 * p - 2 * delta`.
-/
...
/-- At exponent `4 * p`, a weight-four ground-field constant times a regular
factor contributes the constant times the factor's residue. -/
theorem weightedRegular_four_constant_mul_coeff
    {k : Type*} [Field k]
    (S : GCD369CubeHahnPoleScale k) (C : k)
    (P : GCD369CubeHahnRegular k) :
    (S.weightedRegular 4 (GCD369CubeHahnRegular.constant C) * P).1.coeff
        (4 * S.p) = C * GCD369CubeHahnRegular.constantCoeff P := by
  rw [S.weightedRegular_constant_eq_monomial_sq 4 (by norm_num) C]
  have h2p : 0 < 2 * S.p := mul_pos (by norm_num) S.hp
  have hcoeff :=
    GCD369CubeHahnRegular.coeff_monomial_sq_constant_mul (2 * S.p) h2p C P
  refine (congrArg₂
      (fun m e =>
...
#print axioms
  GCD369CubeHahnPoleScale.weightedRegular_four_constant_mul_coeff

namespace GCD369CubeHahnCommonValueData

/-- In the strict active-`c5` window `delta < 2 * p` and `4 * p < 3 * delta`,
the four literal `c5` numerators at exponent `4 * p` are the shifted
double-root rows at residual exponent `4 * p - 2 * delta`, plus the pure
weight-four load.  The intrinsic cubic terms do not yet contribute. -/
theorem TransverseFactor.doubleRoot_c5Load_source_coeff_of_lt
    ...
    (hpLT : T.delta < 2 * F.scale.p)
    (hpGT : 4 * F.scale.p < 3 * T.delta) :
    let A := T.Xn
    ...
    let R4 :=
      4374 * (-3 * r ^ 2 + D) *
        (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
          3 * A * B * D - 3 * B * C)
    let lambda := 4 * F.scale.p - 2 * T.delta
  ...
  let R4 : GCD369CubeHahnRegular k :=
    4374 * (-3 * r ^ 2 + D) *
      (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
        3 * A * B * D - 3 * B * C)
  let lambda : ℚ := 4 * F.scale.p - 2 * T.delta
  ...
  have hcubic3 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (64 * A ^ 3) T.delta (4 * F.scale.p - 3 * T.delta) T.hdelta.le 3
  have hcubic4 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (314928 * (r * A + B) * A ^ 2) T.delta
    (4 * F.scale.p - 3 * T.delta) T.hdelta.le 3
  change ((64 * A ^ 3) * H ^ 3).1.coeff
      ((4 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (64 * A ^ 3).1.coeff (4 * F.scale.p - 3 * T.delta) at hcubic3
  change ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
      ((4 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (314928 * (r * A + B) * A ^ 2).1.coeff
          (4 * F.scale.p - 3 * T.delta) at hcubic4
  have hneg : 4 * F.scale.p - 3 * T.delta < 0 := by linarith [hpGT]
  have hcubic3zero :
      ((64 * A ^ 3) * H ^ 3).1.coeff (4 * F.scale.p) = 0 := by
    have hrew : (4 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        4 * F.scale.p := by ring
    rw [← hrew, hcubic3]
  ...
  have hcubic4zero :
    ...
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hload1 :
      GCD369CubeHahnRegular.constantCoeff P1 = 241920 * r0 ^ 6 := by
    have hP := S.c5LoadPoly1_constantCoeff
    simpa only [P1] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c5LoadRemainders_doubleRoot r0).1))
  have hload2 :
      GCD369CubeHahnRegular.constantCoeff P2 = -622080 * r0 ^ 7 := by
    have hP := S.c5LoadPoly2_constantCoeff
    simpa only [P2] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c5LoadRemainders_doubleRoot r0).2.1))
  have hload3 : GCD369CubeHahnRegular.constantCoeff P3 = 0 := by
    simpa only [P3] using S.c5LoadPoly3_constantCoeff
  have hload4 :
      GCD369CubeHahnRegular.constantCoeff P4 = 7257600 * r0 ^ 9 := by
    have hP := S.c5LoadPoly4_constantCoeff
    simpa only [P4] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c5LoadRemainders_doubleRoot r0).2.2.2))
  have hweight1 :
      (c5w * P1).1.coeff (4 * S.normal.sextic.scale.p) =
        C5 * GCD369CubeHahnRegular.constantCoeff P1 := by
    simpa only [c5w] using
      S.normal.sextic.scale.weightedRegular_four_constant_mul_coeff C5 P1
  have hweight2 :
      (c5w * P2).1.coeff (4 * S.normal.sextic.scale.p) =
        C5 * GCD369CubeHahnRegular.constantCoeff P2 := by
    simpa only [c5w] using
      S.normal.sextic.scale.weightedRegular_four_constant_mul_coeff C5 P2
  have hweight3 :
      (c5w * P3).1.coeff (4 * S.normal.sextic.scale.p) =
        C5 * GCD369CubeHahnRegular.constantCoeff P3 := by
    simpa only [c5w] using
      S.normal.sextic.scale.weightedRegular_four_constant_mul_coeff C5 P3
  have hweight4 :
      (c5w * P4).1.coeff (4 * S.normal.sextic.scale.p) =
        C5 * GCD369CubeHahnRegular.constantCoeff P4 := by
    simpa only [c5w] using
      S.normal.sextic.scale.weightedRegular_four_constant_mul_coeff C5 P4
  refine ⟨?_, ?_, ?_, ?_⟩
  · have hz := (F.c5NCoeff_zero C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC5N1
            S.normal.sextic.scale.regular0 S.normal.sextic.scale.regular1
            S.normal.sextic.scale.regular2 S.normal.sextic.scale.regular3
            S.normal.sextic.scale.regular4 c5w =
          729 * N1 + c5w * P1 := rfl
    rw [hform, coeff_add] at hz
    change (729 * N1).1.coeff (4 * S.normal.sextic.scale.p) +
        (c5w * P1).1.coeff (4 * S.normal.sextic.scale.p) = 0 at hz
    rw [hweight1, hload1] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (4 * F.scale.p)) hexp1
    rw [hlambda, hshift1] at hc
    calc
      (-32 * (729 * R1)).1.coeff lambda + C5 * (241920 * r0 ^ 6) =
          (729 * N1).1.coeff (4 * F.scale.p) +
  ...
  · have hz := (F.c5NCoeff_zero C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC5N2
          ...
          2187 * N2 + c5w * P2 := rfl
    rw [hform, coeff_add] at hz
    change (2187 * N2).1.coeff (4 * S.normal.sextic.scale.p) +
        (c5w * P2).1.coeff (4 * S.normal.sextic.scale.p) = 0 at hz
    rw [hweight2, hload2] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (4 * F.scale.p)) hexp2
    rw [hlambda, hshift2] at hc
    calc
      (32 * (-1458 * R2)).1.coeff lambda + C5 * (-622080 * r0 ^ 7) =
          (2187 * N2).1.coeff (4 * F.scale.p) +
  ...
  · have hz := (F.c5NCoeff_zero C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC5N3
          ...
          243 * N3 + c5w * P3 := rfl
    rw [hform, coeff_add] at hz
    change (243 * N3).1.coeff (4 * S.normal.sextic.scale.p) +
        (c5w * P3).1.coeff (4 * S.normal.sextic.scale.p) = 0 at hz
    rw [hweight3, hload3, mul_zero, add_zero] at hz
    have h243 : (243 * N3).1.coeff (4 * S.normal.sextic.scale.p) =
        (243 : k) * N3.1.coeff (4 * S.normal.sextic.scale.p) :=
      GCD369CubeHahnRegular.coeff_nat_mul
        243 N3 (4 * S.normal.sextic.scale.p)
    have hzN : N3.1.coeff (4 * S.normal.sextic.scale.p) = 0 :=
      (mul_eq_zero.mp (h243.symm.trans hz)).resolve_left (by norm_num)
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (4 * F.scale.p)) hexp3
    change N3.1.coeff (4 * F.scale.p) =
        ((128 * R3) * H ^ 2).1.coeff (4 * F.scale.p) -
          ((64 * A ^ 3) * H ^ 3).1.coeff (4 * F.scale.p) at hc
  ...
  · have hz := (F.c5NCoeff_zero C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC5N4
          ...
          6561 * N4 + c5w * P4 := rfl
    rw [hform, coeff_add] at hz
    change (6561 * N4).1.coeff (4 * S.normal.sextic.scale.p) +
        (c5w * P4).1.coeff (4 * S.normal.sextic.scale.p) = 0 at hz
    rw [hweight4, hload4] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (4 * F.scale.p)) hexp4
    change (6561 * N4).1.coeff (4 * F.scale.p) =
        ((32 * R4) * H ^ 2).1.coeff (4 * F.scale.p) -
          ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
            (4 * F.scale.p) at hc
    rw [hcubic4zero, sub_zero, hlambda, hshift4] at hc
    calc
      (32 * R4).1.coeff lambda + C5 * (7257600 * r0 ^ 9) =
          (6561 * N4).1.coeff (4 * F.scale.p) +
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c5Load_source_coeff_of_lt

...
/-! # The strict-late weight-eleven `rho2` cell at infinity

After all source loads and `rho1` vanish, every remaining target load starts
at weight eleven.  If `3δ < 11p`, the intrinsic cubic transverse term is the
exact first term of the terminal numerator, contradicting the singular
...
/-- Before the weight-eleven load, vanishing through `rho1` gives the four
universal zero-load normal rows at the canonical transverse scale. -/
theorem TransverseScale.zeroLoadLeadingRows_before_rho2
  ...
  have hz := D.faber.zeroHighNCoeff_zero_before_rho2
    (by simpa using hd) (by simpa using hc7) (by simpa using hc5)
    (by simpa using hc4) (by simpa using hc3) (by simpa using hc2)
    (by simpa using hc1) (by simpa using hr1)
...
/-- Below weight eleven the first two zero-high numerators vanish strictly
beyond the chosen cutoff once `rho1` and every source load vanish. -/
theorem zeroLoadN12_mem_higherOrderIdeal_before_rho2
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (e : ℚ) (he : 0 ≤ e) (hlt : e < 11 * F.scale.p)
  ...
  let e' : ℚ := (e + 11 * F.scale.p) / 2
  have hee' : e < e' := by dsimp only [e']; linarith
  have he' : 0 ≤ e' := le_trans he hee'.le
  have he'lt : e' < 11 * F.scale.p := by dsimp only [e']; linarith
  have horder
      (N : GCD369CubeHahnRegular k)
      (hz : ∀ q : ℚ, 0 ≤ q → q < 11 * F.scale.p → N.1.coeff q = 0) :
  ...
  · apply horder
    intro q hq0 hq11
    exact (F.zeroHighNCoeff_zero_before_rho2
      hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 q hq0 hq11).1
  · apply horder
    intro q hq0 hq11
    exact (F.zeroHighNCoeff_zero_before_rho2
      hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 q hq0 hq11).2.1

set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- In the strict-late rho2 cell, `rho2` is absent from `FullN5`, so the
intrinsic cubic term has exact terminal order `3δ`. -/
theorem fullN5_orderTop_eq_three_delta_of_singular_rho2_late
    {k : Type*} [Field k] [CharZero k]
    ...
    (hlate : 3 * T.delta < 11 * D.faber.scale.p) :
  ...
  have hr10 : D.faber.rho1 = 0 := by simpa using hr1
  obtain ⟨hzero1, hzero2⟩ :=
    D.faber.zeroLoadN12_mem_higherOrderIdeal_before_rho2
      (3 * T.delta) he hlate hd0 hc70 hc50 hc40 hc30 hc20 hc10 hr10
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Source-facing strict-late rho2 contradiction after choosing the standard
double-root chart of the singular transverse residue. -/
theorem infinity_rho2_singular_late_inconsistent_of_parametrization
    ...
    (hr1P : S.faberFirstIntegralConstants.rho1 = 0)
    (hdisc :
  ...
  have hr1 : D.faber.rho1 = GCD369CubeHahnRegular.constant 0 := by
    simpa only [D, R, S.infinityCommonCoefficientData_faber hsdegree,
      hr1P] using hr1raw
  have hNorder :=
    GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_rho2_late
      D T hd hc7 hc5 hc4 hc3 hc2 hc1 hr1
  ...
  apply S.infinity_terminal_three_delta_not_before_load_of_singular
    hsdegree T 11 (by norm_num) hdisc hlate
  simpa only [hpole, D, S.infinityCommonCoefficientData_faber hsdegree]
    using hscaledOrder

/-- With all source loads and `rho1` zero, the strict region `3δ < 11p`
is impossible for a constant-core infinity source. -/
theorem infinity_rho2_singular_late_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
    (hdP : S.faberConstantParameters.d = 0)
  ...
  have hearly : 2 * T.delta < 11 * D.faber.scale.p := by
    linarith [T.hdelta]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.zeroLoadLeadingRows_before_rho2
      hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hearly
  ...
  have hdisc : 4 * D.u ^ 3 + 27 * D.v ^ 2 = 0 := by
    rw [hu, hv]
    ring
  exact S.infinity_rho2_singular_late_inconsistent_of_parametrization
    hsdegree T hdP hc7P hc5P hc4P hc3P hc2P hc1P hr1P hdisc r
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
      hr hA hu rfl hY hZ hlate

/-- Target-normalized wrapper for the strict-late rho2 cell. -/
theorem targetNormalized_infinity_rho2_singular_late_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
  ...
  apply U.infinity_rho2_singular_late_inconsistent hsdegreeU T
...
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.zeroLoadLeadingRows_before_rho2
#print axioms
  GCD369CubeHahnFaberPoleData.zeroLoadN12_mem_higherOrderIdeal_before_rho2
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_rho2_late
#print axioms
  GCD369CubePolynomialSource.infinity_rho2_singular_late_inconsistent
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_rho2_singular_late_inconsistent

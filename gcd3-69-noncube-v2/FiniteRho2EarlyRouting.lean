...
import FiniteRho2RootLift
import FiniteLaterSourceElimination

/-! # Routing the weight-eleven `rho2` load

After the weight-one, weight-two, weight-four, weight-five, gauge
weight-six, weight-seven, weight-eight, and weight-ten parameters have
vanished, `rho2` is the next possible source load.  Its factor scale is
`(11p)/2`.  Strictly before the canonical transverse normal, the
arbitrary-load common-root obstruction kills it; at equality, the forced
cubic resonance `3 * alpha = 33 * p / 4` still lies below the later
ninth-Faber threshold `9 p`.
-/
...
/-- If the weight-eleven `rho2` load arrives strictly before the canonical
transverse normal, its coefficient vanishes. -/
theorem TransverseScale.rho2_eq_zero_before_transverse
  ...
  have hrows : GCD369CubeRhoTwoLeadingFaberRowsAt
      ...
      (GCD369CubeHahnRegular.constantCoeff U.Zn) S.u S.v R2 := by
    simpa only [U] using S.rho2LeadingFaberRowsAt_of_bounds F hscale
      R2 R3 R4
  ...
  let load : GCD369CubeArbitraryEarlyFaberLoad
    ...
    .rhoTwo R2 hR2 S.hprojective hrows
  apply T.arbitraryEarlyLoad_strict_before_transverse
    ((11 * F.scale.p) / 2) hs hbefore
  simpa only [U] using load

/-- If the weight-eleven `rho2` load ties the canonical transverse normal, its
...
weight-nine threshold, which still covers the forced resonance `33 p / 4`. -/
theorem TransverseScale.rho2_eq_zero_at_transverse
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseScale)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
...
/-- The complete weight-eleven active-load branch: once
`d=c7=c5=c4=c3=c2=c1=rho1=0`, arrival of `rho2` no later than the
transverse normal forces `rho2=0`. -/
theorem TransverseScale.rho2_eq_zero_at_or_before_transverse
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseScale)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hr3 : F.rho3 = GCD369CubeHahnRegular.constant R3)
    (hr4 : F.rho4 = GCD369CubeHahnRegular.constant R4)
    (hle : (11 * F.scale.p) / 2 ≤ T.delta) :
    R2 = 0 := by
  rcases lt_or_eq_of_le hle with hlt | heq
  · exact T.rho2_eq_zero_before_transverse F hscale
      R2 R3 R4
      hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 hlt
  · exact T.rho2_eq_zero_at_transverse F hscale
      R2 R3 R4 hdNormal hc7Normal hc5Normal hc4Normal
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.rho2_eq_zero_before_transverse
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.rho2_eq_zero_at_transverse
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.rho2_eq_zero_at_or_before_transverse

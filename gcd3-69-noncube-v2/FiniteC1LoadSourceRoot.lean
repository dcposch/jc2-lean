...
/-! # The strict active-`c1` singular source branch

The leading zero-load rows put every singular common cubic on its standard
double-root parametrization.  The active weight-eight contradiction depends
...
/-- If `c1` is the first nonzero Faber parameter and its load lies strictly
between the leading quadratic and intrinsic cubic scales, the singular
finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_c1_of_lt
  ...
  have hearly : 2 * T.delta < 8 * F.scale.p := by
    have htwice : 2 * ((8 * F.scale.p) / 2) = 8 * F.scale.p := by ring
    linarith
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c1 F hscale hd0 hc70 hc50 hc40 hc30
      hc20 (by simpa only [TransverseScale.factor] using hearly)
  ...
  exact T.factor.doubleRoot_c1Load_source_inconsistent_of_lt
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_c1_of_lt

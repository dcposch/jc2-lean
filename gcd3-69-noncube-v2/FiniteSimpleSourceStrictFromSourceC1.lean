import FiniteSimpleSourceLiftC1
import FiniteSimpleSourceLateOrderC1
import FiniteSimpleSourceFullScale
import FiniteSimpleSourceFullScaleC1

/-! # Source-facing strict late simple-root obstruction before `c1`

The source-driven order lift and the literal sextic source equation remain
available below the weight-eight load.  They promote the simple-root chart to
...
/-- If `3 * delta < 8 * p`, the weight-one, weight-two, weight-four, weight-five,
weight-six, and weight-seven loads vanish, and the recovered source residue is
the remaining simple root of the singular leading cubic, then the literal
finite source is inconsistent before the `c1` load. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c1
    ...
    False := by
  have hpS : 3 * T.delta < 8 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source_before_c1
      F hscale hd hc7 hc5 hc4 hc3 hc2 (le_of_lt hp) r r0 A0 hr0 hA0 hr hu hX hY hZ hroot
  ...
  have hsplit := T.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c1
    hdNormal hc7Normal hc5Normal hc4Normal hc3Normal hc2Normal r0 A0 r B hr0 hA0
      (le_of_lt hpS) hr hx hX hroot hDfull hYcoord hQfull
  ...
  have hBhalf := T.simpleRoot_sourceLate_normal_order_half_lower_before_c1
  ...
  exact T.simpleRoot_strictLate_inconsistent_of_fullScale_before_c1
    F hscale hd hc7 hc5 hc4 hc3 hc2 hp r d b C2n r0 A0 hr hr0 hX hA0 hroot
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c1

import FiniteDoubleLiftC5
import FiniteDoubleRow12C5

/-! # Source-facing deviation lift before the weight-two load

The half-scale moving-root lift below `c5` combines with the first two
...
/-- With `d = c7 = 0` and `3 * delta ≤ 4 * p`, the source-facing moving-root
deviations lift from the half scale to the full `delta` scale. -/
theorem TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c5
  ...
  have hhalf := T.doubleRoot_deviations_order_half_lower_before_c5
  ...
  have hRow1 : (↑T.delta : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c5 F hscale hd hc7
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c5

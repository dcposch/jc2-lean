...
/-! # Complete simple-root pre-resonant split before `c7` -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- With `d = 0` and the cubic transverse scale below the weight-two load,
the simple-root displacement is zero or begins at order at least `delta`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_of_coordinates_before_c7
  ...
  · right
    ...
    have hdeltaOrder : T.delta ≤ alpha := by
      ...
      · exact T.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_c7
          hd alpha r0 A0 r w d B B2 C2 hr0 hA0 hp halpha hbelow
      ...
      · have hNlt : N.1.orderTop < (↑alpha : WithTop ℚ) :=
        ...
        have hc : c ≠ 0 := HahnSeries.leadingCoeff_ne_zero.mpr (by
          ...
          apply hwz
          exact Subtype.ext hzero)
        exact T.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_c7
          hd alpha gamma r0 A0 c r w d B B2 C2 hr0 hA0 hc hp
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_of_coordinates_before_c7

...
/-! # Half-scale order bound before the weight-ten load

After the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight loads vanish, the late simple-root normal
correction still lifts from the nominal half scale whenever
`3 * delta ≤ 9 * p`.  The proof uses the literal sextic source
equation within the same weight-nine cutoff as the preceding chart.
-/
...
/-- In the late simple-root chart below the `rho1` load, the half-scale
correction starts no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_rho1
  ...
  have hphi : S.transverseValue.1.orderTop =
    ...
    simpa only [GCD369CubeSimpleSourceTransverseChart, H, M, N, Us, Tail]
      using hphiData.1
  exact T.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_rho1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_rho1

...
import FiniteC7LoadSourceRoot
import FiniteC7SourceAssembly

/-! # Assembly of the singular source at a nonzero `c7` load

A nonzero weight-two `c7` coefficient is inconsistent on every timing of
the first source load.  Arrival no later than the transverse normal already
forces `c7 = 0`.  After that scale the remaining two source-facing branches
...
/-- If the first nonzero Faber parameter is a weight-two `c7` constant, the
singular finite source is inconsistent at every load timing. -/
theorem TransverseScale.singular_source_inconsistent_of_c7_ne_zero
  ...
  rcases le_or_gt F.scale.p T.delta with hle | hdeltaP
  · have hzero : C7 = 0 :=
      T.c7_eq_zero_at_or_before_transverse F hscale
        C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
        hdNormal hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4 hle
    exact hC7 hzero
  · rcases lt_or_ge (2 * F.scale.p) (3 * T.delta) with hlt | hge
    · exact T.singular_source_inconsistent_at_active_c7_of_lt
        F hscale C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4 hC7
        hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
        hdeltaP hlt
    · exact T.singular_source_inconsistent_at_or_after_c7_resonance
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_of_c7_ne_zero

...
/-! # The first three moving-root rows through the residual weight-two scale
...
vanishing below twice the Hahn scale, under the same residual bound.
-/
...
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c7
    ...
    (hearly : 2 * T.delta + e < 2 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_c7

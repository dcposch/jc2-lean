...
/-! # The primitive third row at the cubic scale before `rho1`

With the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight loads killed, the primitive third normal
coefficient at `3 * delta` vanishes as soon as that exponent lies below
the weight-ten cutoff `10 * p`.
-/
...
/-- After `d = c7 = c5 = c4 = c3 = c2 = c1 = 0`, the primitive zero-high
third row vanishes at the cubic transverse exponent whenever
`3 * delta < 10 * p`. -/
theorem TransverseFactor.zeroHighN3Coeff_zero_at_cubic_before_rho1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.zeroHighN3Coeff_zero_at_cubic_before_rho1

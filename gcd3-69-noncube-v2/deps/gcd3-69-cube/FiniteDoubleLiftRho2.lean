...
/-! # The moving double-root half lift below the weight-eleven load

After the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, weight-eight, and weight-ten parameters vanish, the same
quadratic row argument used before `d` remains valid below the `rho2`
load.  In the range `3 * delta < 11 * p`, this is enough to promote all
three moving-root deviations to order at least `delta / 2`.
-/
...
/-- On the `d = c7 = c5 = c4 = c3 = c2 = c1 = rho1 = 0` stratum with
`3 * delta < 11 * p`, the first nonzero common moving-root deviation
cannot occur below the half scale. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_rho2
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_rho2 F hscale hd hc7
        hc5 hc4 hc3 hc2 hc1 hr1 r hroot q hq0 (by linarith) (by nlinarith [T.hdelta, hp])
  ...
  have hrows2 := T.doubleRoot_row123Coeffs_zero_before_rho2 F hscale hd hc7
    hc5 hc4 hc3 hc2 hc1 hr1 r hroot (2 * J.mu) (by linarith [J.hmu]) htwoMuDelta
...
/-- The pre-`rho2` scale exclusion supplies simultaneous half-scale order
bounds for the three canonical moving-root deviations. -/
theorem TransverseFactor.doubleRoot_deviations_order_half_lower_before_rho2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_half_before_rho2
      F hscale hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hp r (by rw [hX]; exact hA0)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_rho2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_half_lower_before_rho2

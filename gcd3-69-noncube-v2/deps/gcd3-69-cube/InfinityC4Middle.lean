...
/-! # The strict middle weight-five load at infinity

After `d = c7 = c4 = 0`, this file treats the window
`2 * delta < 5 * p < 3 * delta`.  The coefficient-only zero-load rows force
the moving-double-root deviations to the residual half-load scale, where the
active `c4` equations form an inconsistent quadratic system.
-/
...
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c4_coeff
    ...
    (hearly : 2 * T.delta + e < 5 * D.faber.scale.p) :
    let A := T.Xn
  ...
  let N3 : GCD369CubeHahnRegular k := GCD369CubeFaberN3
    D.faber.scale.regular0 D.faber.scale.regular1 D.faber.scale.regular2
    D.faber.scale.regular3 D.faber.scale.regular4
  have hz := D.faber.zeroHighNCoeff_zero_before_c4 hd hc7 hc5
...
`5p/2-delta`. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_c4Load_half_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0)
    (_hdeltaP : T.delta < (5 * D.faber.scale.p) / 2)
    (hpDelta : 5 * D.faber.scale.p < 3 * T.delta)
    (r : GCD369CubeHahnRegular k)
    ...
    (J : GCD369CubeHahnRegular.TripleScale
      ...
      (T.Zn + 2 * r ^ 2 * T.Xn)) :
    ¬ J.mu < (5 * D.faber.scale.p) / 2 - T.delta := by
  intro hsmall
  let A : GCD369CubeHahnRegular k := T.Xn
  let Dv : GCD369CubeHahnRegular k := D.cubicU + 3 * r ^ 2
  let B : GCD369CubeHahnRegular k := T.Yn - r * A
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ R1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hs := T.doubleRoot_row123Coeffs_zero_before_c4_coeff hd hc7 hc5
...
theorem TransverseFactor.doubleRoot_deviations_order_c4Load_half_lower_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0)
    (hdeltaP : T.delta < (5 * D.faber.scale.p) / 2)
    (hpDelta : 5 * D.faber.scale.p < 3 * T.delta)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0) :
    (↑((5 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
  ...
  · let J := GCD369CubeHahnRegular.tripleScale Dv B C hD0 hB0 hC0 hn
    have hnot := T.doubleRoot_firstCommonScale_not_below_c4Load_half_coeff
      hd hc7 hc5 hdeltaP hpDelta r (by rw [hX]; exact hA0)
      (by rw [hr]; exact hr0) hroot J
    have hmu : (5 * D.faber.scale.p) / 2 - T.delta ≤ J.mu := le_of_not_gt hnot
    ...
    have hcoe : (↑((5 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
...
theorem c4LoadPoly3_constantCoeff_coeff
    {k : Type*} [Field k] [CharZero k]
    (D : GCD369CubeHahnCommonCoefficientData k) :
    let P : GCD369CubeHahnRegular k :=
      -27648 * D.faber.scale.regular1 *
          D.faber.scale.regular4 -
        55296 * D.faber.scale.regular2 *
          D.faber.scale.regular3 +
        27648 * D.faber.scale.regular3 *
  ...
  change
    -27648 * D.faber.scale.leading1 *
        D.faber.scale.leading4 -
      55296 * D.faber.scale.leading2 *
        D.faber.scale.leading3 +
      27648 * D.faber.scale.leading3 *
        D.faber.scale.leading4 ^ 2 = 0
  rw [D.ha1, D.ha2, D.ha3, D.ha4]
  ring

theorem c4LoadPoly4_constantCoeff_coeff
    {k : Type*} [Field k] [CharZero k]
    (D : GCD369CubeHahnCommonCoefficientData k) :
    let P : GCD369CubeHahnRegular k :=
      -186624 * D.faber.scale.regular1 *
          D.faber.scale.regular3 -
        186624 * D.faber.scale.regular2 ^ 2 +
        124416 * D.faber.scale.regular2 *
          D.faber.scale.regular4 ^ 2 +
        124416 * D.faber.scale.regular3 ^ 2 *
          D.faber.scale.regular4 -
        20736 * D.faber.scale.regular4 ^ 4
    GCD369CubeHahnRegular.constantCoeff P =
      -20736 * D.u ^ 4 + 248832 * D.u * D.v ^ 2 := by
  dsimp only
  simp only [map_add, map_sub, map_mul, map_pow, map_neg, map_ofNat]
  change
    -186624 * D.faber.scale.leading1 *
        D.faber.scale.leading3 -
      186624 * D.faber.scale.leading2 ^ 2 +
      124416 * D.faber.scale.leading2 *
        D.faber.scale.leading4 ^ 2 +
      124416 * D.faber.scale.leading3 ^ 2 *
        D.faber.scale.leading4 -
      20736 * D.faber.scale.leading4 ^ 4 = _
  rw [D.ha1, D.ha2, D.ha3, D.ha4]
  ring
theorem TransverseFactor.doubleRoot_c4Load_coeff_of_lt
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    ...
    (hpLT : T.delta < (5 * D.faber.scale.p) / 2)
    (hpGT : 5 * D.faber.scale.p < 3 * T.delta) :
    let A := T.Xn
    let Dv := D.cubicU + 3 * r ^ 2
    let B := T.Yn - r * A
    let C := T.Zn + 2 * r ^ 2 * A
    ...
    let R4 :=
      4374 * (-3 * r ^ 2 + Dv) *
        (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
          3 * A * B * Dv - 3 * B * C)
    let lambda := 5 * D.faber.scale.p - 2 * T.delta
  ...
  let R4 : GCD369CubeHahnRegular k :=
    4374 * (-3 * r ^ 2 + Dv) *
      (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
        3 * A * B * Dv - 3 * B * C)
  let lambda : ℚ := 5 * D.faber.scale.p - 2 * T.delta
  ...
  let P4 : GCD369CubeHahnRegular k :=
    -186624 * D.faber.scale.regular1 *
      ...
      124416 * D.faber.scale.regular3 ^ 2 *
        D.faber.scale.regular4 -
      20736 * D.faber.scale.regular4 ^ 4
  let c4w : GCD369CubeHahnRegular k :=
    D.faber.scale.weightedRegular 5
      (GCD369CubeHahnRegular.constant C4)
  have hlambda : 5 * D.faber.scale.p = lambda + (2 : ℚ) * T.delta := by
    dsimp only [lambda]
    ring
  ...
  have hcubic3 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (64 * A ^ 3) T.delta (5 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  have hcubic4 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (314928 * (r * A + B) * A ^ 2) T.delta
    (5 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  change ((64 * A ^ 3) * H ^ 3).1.coeff
      ((5 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (64 * A ^ 3).1.coeff (5 * D.faber.scale.p - 3 * T.delta) at hcubic3
  change ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
      ((5 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (314928 * (r * A + B) * A ^ 2).1.coeff
          (5 * D.faber.scale.p - 3 * T.delta) at hcubic4
  have hneg : 5 * D.faber.scale.p - 3 * T.delta < 0 := by linarith [hpGT]
  have hcubic3zero :
      ((64 * A ^ 3) * H ^ 3).1.coeff (5 * D.faber.scale.p) = 0 := by
    have hrew : (5 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        5 * D.faber.scale.p := by ring
    rw [← hrew, hcubic3]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hcubic4zero :
      ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (5 * D.faber.scale.p) = 0 := by
    have hrew : (5 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        5 * D.faber.scale.p := by ring
    rw [← hrew, hcubic4]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hload1 :
      GCD369CubeHahnRegular.constantCoeff P1 = -165888 * r0 ^ 5 := by
    have hP := D.c4LoadPoly1_constantCoeff_coeff
    simpa only [P1] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c4LoadRemainders_doubleRoot r0).1))
  have hload2 :
      GCD369CubeHahnRegular.constantCoeff P2 = 414720 * r0 ^ 6 := by
    have hP := D.c4LoadPoly2_constantCoeff_coeff
    simpa only [P2] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c4LoadRemainders_doubleRoot r0).2.1))
  have hload3 : GCD369CubeHahnRegular.constantCoeff P3 = 0 := by
    simpa only [P3] using D.c4LoadPoly3_constantCoeff_coeff
  have hload4 :
      GCD369CubeHahnRegular.constantCoeff P4 = -4665600 * r0 ^ 8 := by
    have hP := D.c4LoadPoly4_constantCoeff_coeff
    simpa only [P4] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c4LoadRemainders_doubleRoot r0).2.2.2))
  have hweight1 :
      (c4w * P1).1.coeff (5 * D.faber.scale.p) =
        C4 * GCD369CubeHahnRegular.constantCoeff P1 := by
    simpa only [c4w] using
      D.faber.scale.weightedRegular_five_constant_mul_coeff C4 P1
  have hweight2 :
      (c4w * P2).1.coeff (5 * D.faber.scale.p) =
        C4 * GCD369CubeHahnRegular.constantCoeff P2 := by
    simpa only [c4w] using
      D.faber.scale.weightedRegular_five_constant_mul_coeff C4 P2
  have hweight3 :
      (c4w * P3).1.coeff (5 * D.faber.scale.p) =
        C4 * GCD369CubeHahnRegular.constantCoeff P3 := by
    simpa only [c4w] using
      D.faber.scale.weightedRegular_five_constant_mul_coeff C4 P3
  have hweight4 :
      (c4w * P4).1.coeff (5 * D.faber.scale.p) =
        C4 * GCD369CubeHahnRegular.constantCoeff P4 := by
    simpa only [c4w] using
      D.faber.scale.weightedRegular_five_constant_mul_coeff C4 P4
  refine ⟨?_, ?_, ?_, ?_⟩
  · have hz := (D.faber.c4NCoeff_zero C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hd hc7 hc5 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4).1
    rw [hc4] at hz
    have hform :
        GCD369CubeFaberC4N1
            D.faber.scale.regular0 D.faber.scale.regular1
            D.faber.scale.regular2 D.faber.scale.regular3
            D.faber.scale.regular4 c4w =
          729 * N1 + c4w * P1 := rfl
    rw [hform] at hz
    change (729 * N1).1.coeff (5 * D.faber.scale.p) +
        (c4w * P1).1.coeff (5 * D.faber.scale.p) = 0 at hz
    rw [hweight1, hload1] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * D.faber.scale.p)) hexp1
    rw [hlambda, hshift1] at hc
    calc
      (-32 * (729 * R1)).1.coeff lambda + C4 * (-165888 * r0 ^ 5) =
          (729 * N1).1.coeff (5 * D.faber.scale.p) +
            C4 * (-165888 * r0 ^ 5) := by rw [← hc, hlambda]
      _ = 0 := by exact hz
  · have hz := (D.faber.c4NCoeff_zero C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hd hc7 hc5 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4).2.1
    rw [hc4] at hz
    have hform :
        GCD369CubeFaberC4N2
            D.faber.scale.regular0 D.faber.scale.regular1
          ...
          2187 * N2 + c4w * P2 := rfl
    rw [hform] at hz
    change (2187 * N2).1.coeff (5 * D.faber.scale.p) +
        (c4w * P2).1.coeff (5 * D.faber.scale.p) = 0 at hz
    rw [hweight2, hload2] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * D.faber.scale.p)) hexp2
    rw [hlambda, hshift2] at hc
    calc
      (32 * (-1458 * R2)).1.coeff lambda + C4 * (414720 * r0 ^ 6) =
          (2187 * N2).1.coeff (5 * D.faber.scale.p) +
            C4 * (414720 * r0 ^ 6) := by rw [← hc, hlambda]
      _ = 0 := by exact hz
  · have hz := (D.faber.c4NCoeff_zero C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hd hc7 hc5 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4).2.2.1
    rw [hc4] at hz
    have hform :
        GCD369CubeFaberC4N3
            D.faber.scale.regular0 D.faber.scale.regular1
          ...
          243 * N3 + c4w * P3 := rfl
    rw [hform] at hz
    change (243 * N3).1.coeff (5 * D.faber.scale.p) +
        (c4w * P3).1.coeff (5 * D.faber.scale.p) = 0 at hz
    rw [hweight3, hload3, mul_zero, add_zero] at hz
    have h243 : (243 * N3).1.coeff (5 * D.faber.scale.p) =
        (243 : k) * N3.1.coeff (5 * D.faber.scale.p) :=
      GCD369CubeHahnRegular.coeff_nat_mul
        243 N3 (5 * D.faber.scale.p)
    have hzN : N3.1.coeff (5 * D.faber.scale.p) = 0 :=
      (mul_eq_zero.mp (h243.symm.trans hz)).resolve_left (by norm_num)
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * D.faber.scale.p)) hexp3
    change N3.1.coeff (5 * D.faber.scale.p) =
        ((128 * R3) * H ^ 2).1.coeff (5 * D.faber.scale.p) -
          ((64 * A ^ 3) * H ^ 3).1.coeff (5 * D.faber.scale.p) at hc
    rw [hcubic3zero, sub_zero, hlambda, hshift3] at hc
    rw [hlambda] at hzN
    exact hc.symm.trans hzN
  · have hz := (D.faber.c4NCoeff_zero C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hd hc7 hc5 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4).2.2.2
    rw [hc4] at hz
    have hform :
        GCD369CubeFaberC4N4
            D.faber.scale.regular0 D.faber.scale.regular1
          ...
          6561 * N4 + c4w * P4 := rfl
    rw [hform] at hz
    change (6561 * N4).1.coeff (5 * D.faber.scale.p) +
        (c4w * P4).1.coeff (5 * D.faber.scale.p) = 0 at hz
    rw [hweight4, hload4] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (5 * D.faber.scale.p)) hexp4
    change (6561 * N4).1.coeff (5 * D.faber.scale.p) =
        ((32 * R4) * H ^ 2).1.coeff (5 * D.faber.scale.p) -
          ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
            (5 * D.faber.scale.p) at hc
    rw [hcubic4zero, sub_zero, hlambda, hshift4] at hc
    calc
      (32 * R4).1.coeff lambda + C4 * (-4665600 * r0 ^ 8) =
          (6561 * N4).1.coeff (5 * D.faber.scale.p) +
            C4 * (-4665600 * r0 ^ 8) := by rw [← hc, hlambda]
      _ = 0 := by exact hz
theorem TransverseFactor.doubleRoot_c4Load_inconsistent_of_lt_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4 : k)
    (hC4 : C4 ≠ 0)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0)
    (hdeltaP : T.delta < (5 * D.faber.scale.p) / 2)
    (hpDelta : 5 * D.faber.scale.p < 3 * T.delta) : False := by
  ...
  have hhalf := T.doubleRoot_deviations_order_c4Load_half_lower_coeff
    hd0 hc70 hc50 hdeltaP hpDelta r r0 A0 hr0 hA0 hr huc hX hY hZ
    hroot
  let mu : ℚ := (5 * D.faber.scale.p) / 2 - T.delta
  ...
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases he0 : 0 ≤ e
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c4_coeff
        hd0 hc70 hc50 r hroot e he0 (by
          dsimp only [mu] at he'
          nlinarith [hpDelta]) (by
          dsimp only [mu] at he'
          have htwice : 2 * ((5 * D.faber.scale.p) / 2 - T.delta) =
              5 * D.faber.scale.p - 2 * T.delta := by ring
          linarith)
...
theorem TransverseFactor.zeroLoadLeadingRows_before_c4_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0)
    (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0)
    (hearly : 2 * T.delta < 5 * D.faber.scale.p) :
...
the strict active-`c4` middle window is inconsistent at coefficient level. -/
theorem TransverseScale.c4_singular_middle_inconsistent_coeff
    {k : Type*} [Field k] [CharZero k]
  ...
  have hc70 : D.faber.c7 = 0 := by simpa using hc7
  have hc50 : D.faber.c5 = 0 := by simpa using hc5
  have hearly : 2 * T.delta < 5 * D.faber.scale.p := by
    nlinarith [hdeltaP]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c4_coeff hd0 hc70 hc50
  ...
  exact T.factor.doubleRoot_c4Load_inconsistent_of_lt_coeff
    C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4 hC4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
...
/-- After `d = c7 = c5 = 0`, a nonzero `c4` coefficient is impossible in
the strict singular middle window `2 delta < 5p < 3 delta`. -/
theorem infinity_c4_eq_zero_singular_middle
  ...
  have hdeltaP : T.delta < (5 * D.faber.scale.p) / 2 := by
    dsimp only [D]
    nlinarith
  by_contra hPc4
  exact T.c4_singular_middle_inconsistent_coeff
...
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_c4_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_firstCommonScale_not_below_c4Load_half_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_deviations_order_c4Load_half_lower_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_c4Load_coeff_of_lt
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_c4Load_inconsistent_of_lt_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.zeroLoadLeadingRows_before_c4_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.c4_singular_middle_inconsistent_coeff
#print axioms GCD369CubePolynomialSource.infinity_c4_eq_zero_singular_middle

import FiniteDoubleLiftC4
import FiniteDoubleRow12C4

/-! # Source-facing deviation lift before the weight-five load

The half-scale moving-root lift below `c4` combines with the first two
...
/-- With `d = c7 = c5 = 0` and `3 * delta ≤ 5 * p`, the source-facing moving-root
deviations lift from the half scale to the full `delta` scale. -/
theorem TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c4
  ...
  have hhalf := T.doubleRoot_deviations_order_half_lower_before_c4
  ...
  have hRow1 : (↑T.delta : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c4 F hscale hd hc7
        hc5 r hroot q hq0 (by nlinarith [T.hdelta, hp])
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c4

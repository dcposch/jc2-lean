import FiniteSimpleSourceLiftC4
import FiniteSimpleSourceLateOrderC4
import FiniteSimpleSourceFullScale
import FiniteSimpleSourceFullScaleC4

/-! # Source-facing strict late simple-root obstruction before `c4`

The source-driven order lift and the literal sextic source equation remain
available below the weight-five load.  They promote the simple-root chart to
...
/-- If `3 * delta < 5 * p`, the weight-one, weight-two, and weight-four loads
vanish, and the recovered source residue is the remaining simple root of the
singular leading cubic, then the literal finite source is inconsistent
before the `c4` load. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c4
  ...
  have hsplit := T.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c4
  ...
  have hBhalf := T.simpleRoot_sourceLate_normal_order_half_lower_before_c4
  ...
  exact T.simpleRoot_strictLate_inconsistent_of_fullScale_before_c4
    F hscale hd hc7 hc5 hp r d b C2n r0 A0 hr hr0 hX hA0 hroot
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_from_source_before_c4

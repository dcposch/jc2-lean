...
/-! # The strict active-`c2` singular source branch

The leading zero-load rows put every singular common cubic on its standard
double-root parametrization.  The active weight-seven contradiction depends
...
/-- If `c2` is the first nonzero Faber parameter and its load lies strictly
between the leading quadratic and intrinsic cubic scales, the singular
finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_c2_of_lt
  ...
  have hearly : 2 * T.delta < 7 * F.scale.p := by
    have htwice : 2 * ((7 * F.scale.p) / 2) = 7 * F.scale.p := by ring
    linarith
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c2 F hscale hd0 hc70 hc50 hc40 hc30
  ...
  exact T.factor.doubleRoot_c2Load_source_inconsistent_of_lt
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_c2_of_lt

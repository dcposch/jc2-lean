...
/-! # The strict middle weight-seven load at infinity

After `d = c7 = c5 = c4 = c3 = 0`, this file treats
`2 * delta < 7 * p < 3 * delta`.  The coefficient-only zero-load rows
drive the two-stage moving-root lift to the active `c2` quadratic
contradiction.
-/
...
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c2_coeff
...
`7p/2-delta`. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_c2Load_half_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0) (hc4 : D.faber.c4 = 0)
    (hc3 : D.faber.c3 = 0)
    (_hdeltaP : T.delta < (7 * D.faber.scale.p) / 2)
    (hpDelta : 7 * D.faber.scale.p < 3 * T.delta)
    (r : GCD369CubeHahnRegular k)
    ...
    (J : GCD369CubeHahnRegular.TripleScale
      ...
      (T.Zn + 2 * r ^ 2 * T.Xn)) :
    ¬ J.mu < (7 * D.faber.scale.p) / 2 - T.delta := by
  intro hsmall
  let A : GCD369CubeHahnRegular k := T.Xn
  let Dv : GCD369CubeHahnRegular k := D.cubicU + 3 * r ^ 2
  let B : GCD369CubeHahnRegular k := T.Yn - r * A
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ R1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hs := T.doubleRoot_row123Coeffs_zero_before_c2_coeff hd hc7 hc5 hc4 hc3
...
theorem TransverseFactor.doubleRoot_deviations_order_c2Load_half_lower_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0) (hc4 : D.faber.c4 = 0)
    (hc3 : D.faber.c3 = 0)
    (hdeltaP : T.delta < (7 * D.faber.scale.p) / 2)
    (hpDelta : 7 * D.faber.scale.p < 3 * T.delta)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0) :
    (↑((7 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
  ...
  · let J := GCD369CubeHahnRegular.tripleScale Dv B C hD0 hB0 hC0 hn
    have hnot := T.doubleRoot_firstCommonScale_not_below_c2Load_half_coeff
      hd hc7 hc5 hc4 hc3 hdeltaP hpDelta r (by rw [hX]; exact hA0)
      (by rw [hr]; exact hr0) hroot J
    have hmu : (7 * D.faber.scale.p) / 2 - T.delta ≤ J.mu := le_of_not_gt hnot
    ...
    have hcoe : (↑((7 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
...
cubic. -/
theorem c2LoadPoly3_constantCoeff_coeff
    {k : Type*} [Field k] [CharZero k]
    (D : GCD369CubeHahnCommonCoefficientData k) :
    let P : GCD369CubeHahnRegular k :=
...
theorem c2LoadPoly4_constantCoeff_coeff
  ...
  ring
theorem TransverseFactor.doubleRoot_c2Load_coeff_of_lt
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    ...
    (hpLT : T.delta < (7 * D.faber.scale.p) / 2)
    (hpGT : 7 * D.faber.scale.p < 3 * T.delta) :
    let A := T.Xn
    let Dv := D.cubicU + 3 * r ^ 2
    let B := T.Yn - r * A
    let C := T.Zn + 2 * r ^ 2 * A
    ...
    let R4 :=
      4374 * (-3 * r ^ 2 + Dv) *
        (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
          3 * A * B * Dv - 3 * B * C)
    let lambda := 7 * D.faber.scale.p - 2 * T.delta
  ...
  let R4 : GCD369CubeHahnRegular k :=
    4374 * (-3 * r ^ 2 + Dv) *
      (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
        3 * A * B * Dv - 3 * B * C)
  let lambda : ℚ := 7 * D.faber.scale.p - 2 * T.delta
  ...
  let c2w : GCD369CubeHahnRegular k :=
    D.faber.scale.weightedRegular 7
      (GCD369CubeHahnRegular.constant C2)
  have hlambda : 7 * D.faber.scale.p = lambda + (2 : ℚ) * T.delta := by
    dsimp only [lambda]
    ring
  ...
  have hcubic3 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (64 * A ^ 3) T.delta (7 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  have hcubic4 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (314928 * (r * A + B) * A ^ 2) T.delta
    (7 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  change ((64 * A ^ 3) * H ^ 3).1.coeff
      ((7 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (64 * A ^ 3).1.coeff (7 * D.faber.scale.p - 3 * T.delta) at hcubic3
  change ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
      ((7 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (314928 * (r * A + B) * A ^ 2).1.coeff
          (7 * D.faber.scale.p - 3 * T.delta) at hcubic4
  have hneg : 7 * D.faber.scale.p - 3 * T.delta < 0 := by linarith [hpGT]
  have hcubic3zero :
      ((64 * A ^ 3) * H ^ 3).1.coeff (7 * D.faber.scale.p) = 0 := by
    have hrew : (7 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        7 * D.faber.scale.p := by ring
    rw [← hrew, hcubic3]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hcubic4zero :
      ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (7 * D.faber.scale.p) = 0 := by
    have hrew : (7 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        7 * D.faber.scale.p := by ring
    rw [← hrew, hcubic4]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hload1 :
    ...
    have hP := D.c2LoadPoly1_constantCoeff_coeff
    simpa only [P1] using
      (hP.trans (by
        rw [hv]
  ...
  have hload2 :
    ...
    have hP := D.c2LoadPoly2_constantCoeff_coeff
    simpa only [P2] using
      (hP.trans (by
        rw [hu]
        exact (GCD369Cube_c2LoadRemainders_doubleRoot r0).2.1))
  have hload3 : GCD369CubeHahnRegular.constantCoeff P3 = 0 := by
    simpa only [P3] using D.c2LoadPoly3_constantCoeff_coeff
  have hload4 :
      GCD369CubeHahnRegular.constantCoeff P4 = 1866240 * r0 ^ 6 := by
    have hP := D.c2LoadPoly4_constantCoeff_coeff
    simpa only [P4] using
      (hP.trans (by
        rw [hu, hv]
        exact (GCD369Cube_c2LoadRemainders_doubleRoot r0).2.2.2))
  have hweight1 :
      (c2w * P1).1.coeff (7 * D.faber.scale.p) =
        C2 * GCD369CubeHahnRegular.constantCoeff P1 := by
    simpa only [c2w] using
      D.faber.scale.weightedRegular_seven_constant_mul_coeff C2 P1
  have hweight2 :
      (c2w * P2).1.coeff (7 * D.faber.scale.p) =
        C2 * GCD369CubeHahnRegular.constantCoeff P2 := by
    simpa only [c2w] using
      D.faber.scale.weightedRegular_seven_constant_mul_coeff C2 P2
  have hweight3 :
      (c2w * P3).1.coeff (7 * D.faber.scale.p) =
        C2 * GCD369CubeHahnRegular.constantCoeff P3 := by
    simpa only [c2w] using
      D.faber.scale.weightedRegular_seven_constant_mul_coeff C2 P3
  have hweight4 :
      (c2w * P4).1.coeff (7 * D.faber.scale.p) =
        C2 * GCD369CubeHahnRegular.constantCoeff P4 := by
    simpa only [c2w] using
      D.faber.scale.weightedRegular_seven_constant_mul_coeff C2 P4
  refine ⟨?_, ?_, ?_, ?_⟩
  · have hz := (D.faber.c2NCoeff_zero C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC2N1
            D.faber.scale.regular0 D.faber.scale.regular1
            D.faber.scale.regular2 D.faber.scale.regular3
            D.faber.scale.regular4 c2w =
          729 * N1 + c2w * P1 := rfl
    rw [hform] at hz
    change (729 * N1).1.coeff (7 * D.faber.scale.p) +
        (c2w * P1).1.coeff (7 * D.faber.scale.p) = 0 at hz
    rw [hweight1, hload1] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (7 * D.faber.scale.p)) hexp1
    rw [hlambda, hshift1] at hc
    calc
      (-32 * (729 * R1)).1.coeff lambda + C2 * (82944 * r0 ^ 3) =
          (729 * N1).1.coeff (7 * D.faber.scale.p) +
  ...
  · have hz := (D.faber.c2NCoeff_zero C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC2N2
          ...
          2187 * N2 + c2w * P2 := rfl
    rw [hform] at hz
    change (2187 * N2).1.coeff (7 * D.faber.scale.p) +
        (c2w * P2).1.coeff (7 * D.faber.scale.p) = 0 at hz
    rw [hweight2, hload2] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (7 * D.faber.scale.p)) hexp2
    rw [hlambda, hshift2] at hc
    calc
      (32 * (-1458 * R2)).1.coeff lambda + C2 * (-186624 * r0 ^ 4) =
          (2187 * N2).1.coeff (7 * D.faber.scale.p) +
  ...
  · have hz := (D.faber.c2NCoeff_zero C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC2N3
          ...
          243 * N3 + c2w * P3 := rfl
    rw [hform] at hz
    change (243 * N3).1.coeff (7 * D.faber.scale.p) +
        (c2w * P3).1.coeff (7 * D.faber.scale.p) = 0 at hz
    rw [hweight3, hload3, mul_zero, add_zero] at hz
    have h243 : (243 * N3).1.coeff (7 * D.faber.scale.p) =
        (243 : k) * N3.1.coeff (7 * D.faber.scale.p) :=
      GCD369CubeHahnRegular.coeff_nat_mul
        243 N3 (7 * D.faber.scale.p)
    have hzN : N3.1.coeff (7 * D.faber.scale.p) = 0 :=
      (mul_eq_zero.mp (h243.symm.trans hz)).resolve_left (by norm_num)
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (7 * D.faber.scale.p)) hexp3
    change N3.1.coeff (7 * D.faber.scale.p) =
        ((128 * R3) * H ^ 2).1.coeff (7 * D.faber.scale.p) -
          ((64 * A ^ 3) * H ^ 3).1.coeff (7 * D.faber.scale.p) at hc
  ...
  · have hz := (D.faber.c2NCoeff_zero C1 Rho1 Rho2 Rho3 Rho4
    ...
    have hform :
        GCD369CubeFaberC2N4
          ...
          6561 * N4 + c2w * P4 := rfl
    rw [hform] at hz
    change (6561 * N4).1.coeff (7 * D.faber.scale.p) +
        (c2w * P4).1.coeff (7 * D.faber.scale.p) = 0 at hz
    rw [hweight4, hload4] at hz
    have hc := congrArg
      (fun z : GCD369CubeHahnRegular k => z.1.coeff (7 * D.faber.scale.p)) hexp4
    change (6561 * N4).1.coeff (7 * D.faber.scale.p) =
        ((32 * R4) * H ^ 2).1.coeff (7 * D.faber.scale.p) -
          ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
            (7 * D.faber.scale.p) at hc
    rw [hcubic4zero, sub_zero, hlambda, hshift4] at hc
    calc
      (32 * R4).1.coeff lambda + C2 * (1866240 * r0 ^ 6) =
          (6561 * N4).1.coeff (7 * D.faber.scale.p) +
            C2 * (1866240 * r0 ^ 6) := by rw [← hc, hlambda]
      _ = 0 := by exact hz
theorem TransverseFactor.doubleRoot_c2Load_inconsistent_of_lt_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (C2 C1 Rho1 Rho2 Rho3 Rho4 : k)
    (hC2 : C2 ≠ 0)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0)
    (hdeltaP : T.delta < (7 * D.faber.scale.p) / 2)
    (hpDelta : 7 * D.faber.scale.p < 3 * T.delta) : False := by
  ...
  have hhalf := T.doubleRoot_deviations_order_c2Load_half_lower_coeff
    hd0 hc70 hc50 hc40 hc30 hdeltaP hpDelta r r0 A0 hr0 hA0 hr huc
    hX hY hZ hroot
  let mu : ℚ := (7 * D.faber.scale.p) / 2 - T.delta
  ...
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases he0 : 0 ≤ e
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c2_coeff
        hd0 hc70 hc50 hc40 hc30 r hroot e he0 (by
          dsimp only [mu] at he'
          nlinarith [hpDelta]) (by
          dsimp only [mu] at he'
          have htwice : 2 * ((7 * D.faber.scale.p) / 2 - T.delta) =
              7 * D.faber.scale.p - 2 * T.delta := by ring
          linarith)
  ...
  apply GCD369CubeDoubleBlowup_c2Load_quadratic_beforeCubic_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) C2 hr0 hC2
...
theorem TransverseFactor.zeroLoadLeadingRows_before_c2_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0)
    (hc7 : D.faber.c7 = 0)
...
strict active-`c2` middle window is inconsistent at coefficient level. -/
theorem TransverseScale.c2_singular_middle_inconsistent_coeff
    {k : Type*} [Field k] [CharZero k]
  ...
  have hearly : 2 * T.delta < 7 * D.faber.scale.p := by
    nlinarith [hdeltaP]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c2_coeff hd0 hc70 hc50 hc40 hc30
  ...
  exact T.factor.doubleRoot_c2Load_inconsistent_of_lt_coeff
    C2 C1 Rho1 Rho2 Rho3 Rho4 hC2
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
...
/-- Generic source-facing active `c2` middle theorem in the explicit
`c3 = 0` gauge. -/
theorem infinity_c2_eq_zero_singular_middle
  ...
  have hdeltaP : T.delta < (7 * D.faber.scale.p) / 2 := by
    dsimp only [D]
    nlinarith
  by_contra hPc2
  exact T.c2_singular_middle_inconsistent_coeff
...
/-- Target-normalized active-`c2` wrapper with canonical `c3 = 0`. -/
theorem targetNormalized_infinity_c2_eq_zero_singular_middle
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (hd0 : S.faberConstantParameters.d = 0)
  ...
  apply U.infinity_c2_eq_zero_singular_middle hsdegreeU T
...
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_c2_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_firstCommonScale_not_below_c2Load_half_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_deviations_order_c2Load_half_lower_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_c2Load_coeff_of_lt
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_c2Load_inconsistent_of_lt_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.c2_singular_middle_inconsistent_coeff
#print axioms GCD369CubePolynomialSource.infinity_c2_eq_zero_singular_middle
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_c2_eq_zero_singular_middle

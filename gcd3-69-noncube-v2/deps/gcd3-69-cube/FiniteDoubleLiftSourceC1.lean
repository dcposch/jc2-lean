import FiniteDoubleLiftC1
import FiniteDoubleRow12C1

/-! # Source-facing deviation lift before the weight-eight load

The half-scale moving-root lift below `c1` combines with the first two
...
/-- With `d = c7 = c5 = c4 = c3 = c2 = 0` and `3 * delta ≤ 8 * p`, the source-facing
moving-root deviations lift from the half scale to the full `delta` scale. -/
theorem TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c1
  ...
  have hhalf : (↑(T.delta / 2) : WithTop ℚ) ≤
    ...
    by_cases hlt : 3 * T.delta < 8 * F.scale.p
    · exact T.doubleRoot_deviations_order_half_lower_before_c1
        F hscale hd hc7 hc5 hc4 hc3 hc2 hlt r r0 A0 hr0 hA0 hr hu hX hY hZ
        hroot
      ...
      · let J : GCD369CubeHahnRegular.TripleScale D B C :=
        ...
        have hnot : ¬ J.mu < T.delta / 2 := by
          ...
          have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
            ...
            by_cases hq0 : 0 ≤ q
            · have hrows :=
                T.doubleRoot_row123Coeffs_zero_before_c1 F hscale hd
                  hc7 hc5 hc4 hc3 hc2 r hroot q hq0 (by linarith)
          ...
          have hR2 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row2.1.orderTop := by
            ...
            · exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg Row2
                (lt_of_not_ge hq0)
          have hrows2 := T.doubleRoot_row123Coeffs_zero_before_c1
            F hscale hd hc7 hc5 hc4 hc3 hc2 r hroot (2 * J.mu)
  ...
  have hRow1 : (↑T.delta : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c1 F hscale hd hc7
        hc5 hc4 hc3 hc2 r hroot q hq0 (by nlinarith [T.hdelta, hp])
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_delta_from_source_before_c1

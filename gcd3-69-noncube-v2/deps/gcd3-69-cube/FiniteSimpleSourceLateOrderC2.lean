...
/-! # Half-scale order bound before the weight-seven load
...
scale whenever `3 * delta < 7 * p`.  The proof uses only the literal sextic
source equation, so the weight-seven cutoff is enough.
-/
...
/-- In the late simple-root chart below the `c2` load, the half-scale
correction starts no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c2
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c2

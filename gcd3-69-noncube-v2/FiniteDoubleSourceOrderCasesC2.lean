...
/-! # Early double-root source order case before `c2` -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- In the early moving-root range below `c2`, the sextic source equation
forces the Newton resonance `delta = 3 * nu`; ninth-Faber vanishing below the
weight-six load then gives the incompatible second residue equation. -/
theorem TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders_before_c2
  ...
  · have hdelta : T.delta = 3 * nu := by
    ...
    have hz := S.normal.faberNineCoeff_zero_before_c2 hd hc7 hc5 hc4
      (6 * nu) (by nlinarith)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders_before_c2

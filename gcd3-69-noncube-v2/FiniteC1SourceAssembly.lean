import FiniteC1SourceRootSplit
import FiniteC1LoadSourceRootEndpoint

/-! # Assembly of the singular source at or after `c1` resonance

The source-facing `c1` branch is empty both strictly before the weight-eight
resonance and at the exact cubic tie.  The two existing contradictions
compose on a single `≤` hypothesis; no nonzero `c1` coefficient is required.
-/
...
/-- If the first source load meets or exceeds the weight-eight `c1` resonance
`3 * delta ≤ 8 * p`, the singular finite source is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_or_after_c1_resonance
  ...
  · exact T.singular_source_inconsistent_at_active_c1_of_eq
  ...
  · have hd0 : F.d = 0 := by simpa using hd
    ...
    exact T.singular_source_inconsistent_before_c1
      F hscale hdNormal hc7Normal hc5Normal hc4Normal hc3Normal hc2Normal
      hd0 hc70 hc50 hc40 hc30 hc20 hlt

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_or_after_c1_resonance

...
/-! # Pre-resonant orders in the simple-root source chart before `rho1`

After the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight loads have vanished, the same pre-resonant
split applies once `3 * delta ≤ 9 * p`.  The literal sextic source
equation still excludes a transverse value of exact order below
`2 * delta`.  Splitting on the order of the half-scale normal deviation
then gives two short contradictions: either the cubic square uniquely
leads the sextic, already below the source scale `6 * p`, or the reduced
ninth-Faber equation has a unique leading cubic term below the later
weight-nine load, which is available once `c1` has also vanished.
-/
...
`6 * p` when `3 * delta ≤ 9 * p`. -/
theorem TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_rho1
...
already below the weight-nine cutoff. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
...
later weight-nine load. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (hc5 : S.normal.c5 = 0) (hc4 : S.normal.c4 = 0)
  ...
  exact T.reducedFaberNine_inconsistent_of_exact_cubic_order_before_later
    hd hc7 hc5 hc4 hc3 hc2 hc1 alpha gamma (c * (9 * r0 ^ 2)) halpha
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_rho1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_rho1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_rho1

...
/-! # Source-value boundary at the moving double root before `c1`

Once the weight-one, weight-two, weight-four, weight-five, weight-six, and
weight-seven loads have vanished, the literal nonic source value still
excludes the late double-root branch below the weight-eight `c1` load.  The
argument is the same second-order residue as in the weight-one case, now
evaluated at `2 * delta` under the weaker cutoff `3 * delta ≤ 8 * p`.
-/
...
weight-seven loads have vanished and `3 * delta ≤ 8 * p`. -/
theorem TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c1
  ...
  have hearly : 2 * T.delta < 8 * S.normal.sextic.scale.p := by
    have hp_pos : 0 < S.normal.sextic.scale.p := S.normal.sextic.scale.hp
    nlinarith [hp, hp_pos]
  have hz := S.normal.faberNineCoeff_zero_before_c1
    hd hc7 hc5 hc4 hc3 hc2 (2 * T.delta) (by nlinarith [T.hdelta]) hearly
...
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c1

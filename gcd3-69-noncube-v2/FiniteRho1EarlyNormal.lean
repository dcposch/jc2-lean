...
/-! # Early transverse jets before the first weight-ten load

If a common-cubic transverse jet occurs before the weight-ten `rho1` scale,
and the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight parameters have already vanished, then every
remaining target load disappears in the corresponding higher-order quotient.
...
parameter.  Remaining target loads have weights 10, 11, 12, and 13.
-/
...
/-- Below the weight-ten scale, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, and weight-eight
loads have vanished, the complete source equations have exactly the
zero-high-constant leading coefficients. -/
theorem zeroHighNCoeff_zero_before_rho1
    ...
    (hc2 : F.c2 = 0)
    (hc1 : F.c1 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 10 * F.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 10 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (10 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (10 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
  have hkill (w : ℕ) (hw : 10 ≤ w)
...
/-- Any transverse factor strictly before the first weight-ten load, after
the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight parameters have vanished, satisfies the four
universal zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_rho1
    {k : Type*} [Field k] [CharZero k]
...
/-- A nonzero first transverse jet before the `rho1` scale, after the
weight-one, weight-two, weight-four, weight-five, weight-six, weight-seven,
and weight-eight parameters have vanished, forces the limiting common cubic
to be singular. -/
theorem TransverseScale.discriminant_zero_before_rho1
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_rho1 F hscale
    hd hc7 hc5 hc4 hc3 hc2 hc1 hearly
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_rho1
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_rho1
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_rho1

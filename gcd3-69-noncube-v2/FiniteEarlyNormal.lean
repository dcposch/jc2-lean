...
/-! # Early transverse jets before the first active load

If a common-cubic transverse jet occurs before the weight-one `d` scale,
then every source and target load disappears in the corresponding
...
/-- Below the weight-one scale, the complete source equations have exactly
the zero-high-constant leading coefficients. -/
theorem zeroHighNCoeff_zero_before_d
  ...
  have hweight (w : ℕ) (hw : 1 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (1 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (1 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
  have hkill (w : ℕ) (hw : 1 ≤ w) (c : k)
  ...
  have hdq := hkill 1 (by norm_num) D F.d hd
  have hc7q := hkill 2 (by norm_num) C7 F.c7 hc7
...
/-- Any transverse factor strictly before the first active load satisfies the
four universal zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_d
    {k : Type*} [Field k] [CharZero k]
...
/-- A nonzero first transverse jet before the `d` scale forces the limiting
...
theorem TransverseScale.discriminant_zero_before_d
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_d F hscale
    D C7 C5 C4 C3 C2 C1 R1 R2 R3 R4
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_d
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_d
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_d

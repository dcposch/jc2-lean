...
/-! # The moving double-root coefficient at the balanced `rho2` resonance

At the equality `11 * p = 3 * delta`, the quadratic double-root jet and the
intrinsic cubic meet the weight-eleven load together.  This file extracts the
four literal `rho2` equations at residual exponent `11 * p - 2 * delta`
...
Unlike a source-coefficient load, `rho2` is a target value: the primitive
zero-high rows already carry the sparse packet `0`, `186624 * R`, `0`, `0`
at exponent `11 * p`.  After the quadratic coefficient lemmas, these four
...
/-- At the balanced active-`rho2` resonance `11 * p = 3 * delta`, the four
literal `rho2` numerators at exponent `11 * p` are the shifted double-root
rows at residual exponent `11 * p - 2 * delta` (equal to `delta`), plus
the sparse weight-eleven load `-186624 * R` in the second row, plus the
intrinsic cubic constants at exponent `3 * delta`.  Rows one, three, and
four carry no `rho2` remainder. -/
theorem TransverseFactor.doubleRoot_rho2Load_source_coeff_of_eq
  ...
  have h11p :
      ((11 : ℕ) : ℚ) * S.normal.sextic.scale.p = 11 * F.scale.p := by
    rw [hpS]
    norm_num
  ...
  · have hz := (F.rho2NCoeff R Rho3 Rho4
    ...
    calc
      (32 * (-1458 * R2)).1.coeff lambda - 186624 * R =
        ...
        rw [← hc, hlambda]
      _ = 186624 * R - 186624 * R := by
        have hN : (2187 * N2).1.coeff (11 * F.scale.p) = 186624 * R := by
          simpa only [N2] using hz
        rw [hN]
      _ = 0 := by ring
  · have hz := (F.rho2NCoeff R Rho3 Rho4
      hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4).2.2.1
    rw [← hscale, h11p] at hz
    have hzN : N3.1.coeff (11 * F.scale.p) = 0 := by
  ...
  · have hz := (F.rho2NCoeff R Rho3 Rho4
    ...
    calc
      (32 * R4).1.coeff lambda - 314928 * r0 * A0 ^ 3 =
        (6561 * N4).1.coeff (11 * F.scale.p) := by rw [← hc, hlambda]
      _ = 0 := by simpa only [N4] using hz

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_rho2Load_source_coeff_of_eq

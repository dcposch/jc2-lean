import FiniteSimpleSourceOrders

/-! # Complete pre-resonant split in the simple-root source chart

The source displacement from the simple root is either zero, starts at or
after the intrinsic scale `delta`, or would have a positive exact order below
`delta`.  The last possibility is excluded by splitting on the order of the
half-scale normal deviation.
-/
...
/-- In exact simple-root coordinates, the displacement is either identically
zero or has Hahn order at least `delta`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_of_coordinates
  ...
  · right
    ...
    have hdeltaOrder : T.delta ≤ alpha := by
      ...
      · exact T.simpleRoot_sourceBelow_inconsistent_of_normal_lower
          alpha r0 A0 r w d B B2 C2 hr0 hA0 hp halpha hbelow
      ...
      · have hNlt : N.1.orderTop < (↑alpha : WithTop ℚ) :=
        ...
        have hc : c ≠ 0 := HahnSeries.leadingCoeff_ne_zero.mpr (by
          ...
          apply hwz
          exact Subtype.ext hzero)
        exact T.simpleRoot_sourceBelow_inconsistent_of_normal_exact
          alpha gamma r0 A0 c r w d B B2 C2 hr0 hA0 hc hp
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_of_coordinates

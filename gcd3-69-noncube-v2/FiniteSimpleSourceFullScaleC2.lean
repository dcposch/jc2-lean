...
import FiniteC2CubicN3

/-! # The full-scale simple-root obstruction before the weight-seven load

When `F.d = F.c7 = F.c5 = F.c4 = F.c3 = 0` and `3 * delta < 7 * p`, the first two primitive moving-root
...
weight-seven load, once the weight-one, weight-two, weight-four, weight-five,
and weight-six parameters have vanished. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale_before_c2
  ...
  let R2 : GCD369CubeHahnRegular k :=
    2 * A ^ 2 * r * D - 3 * A * B * r ^ 2 - 3 * A * C * r +
      3 * A * B * D - 3 * B * C
  have hrows := T.doubleRoot_row12Coeffs_zero_before_c2 F hscale
    hd hc7 hc5 hc4 hc3 r hroot T.delta T.hdelta.le (by nlinarith [hp, T.hdelta])
  ...
  have hz := T.zeroHighN3Coeff_zero_at_cubic_before_c2 F hscale hd hc7 hc5 hc4 hc3 hp
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale_before_c2

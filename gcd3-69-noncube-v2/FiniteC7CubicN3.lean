...
/-! # The primitive third row at the cubic scale before `c7`

With the weight-one load killed, the primitive third normal coefficient at
...
/-- After `d = 0`, the primitive zero-high third row vanishes at the cubic
transverse exponent whenever `3 * delta < 2 * p`. -/
theorem TransverseFactor.zeroHighN3Coeff_zero_at_cubic_before_c7
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    (hd : F.d = 0)
    (hearly : 3 * T.delta < 2 * F.scale.p) :
    (GCD369CubeFaberN3
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.zeroHighN3Coeff_zero_at_cubic_before_c7

import FiniteDoubleSourceLiftRho1
import FiniteDoubleZeroLoadCubicLiftRho1

/-! # Singular source-root split before `rho1` -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- Before the weight-ten load, the literal source excludes the double-root
choice for the recovered source coordinate and leaves the simple root of the
singular leading cubic. -/
theorem TransverseScale.source_coordinate_is_simpleRoot_before_rho1
    ...
    ∃ r : k, r ≠ 0 ∧
      ...
      x = -2 * r := by
  dsimp only
  have hearly : 2 * T.delta < 10 * F.scale.p := by
    nlinarith [T.hdelta, hp]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_rho1 F hscale
  ...
  · exfalso
    ...
    exact T.factor.doubleRoot_source_inconsistent_from_source_before_rho1
...
/-- Strictly before the weight-ten load, the balanced zero-load cubic
obstruction excludes the singular source independently of which cubic root
is selected by the literal source coordinate. -/
theorem TransverseScale.singular_source_inconsistent_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseScale)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
  ...
  have hroot : 2 * S.cubicU * r + 3 * S.cubicV = 0 := by
    exact GCD369CubeHahnRegular.movingDoubleRoot_relation
      S.cubicU S.cubicV hUne
  exact T.factor.doubleRoot_inconsistent_before_rho1
    F hscale hd hc7 hc5 hc4 hc3 hc2 hc1
    (by simpa only [TransverseScale.factor] using hp)
    r r0 (GCD369CubeHahnRegular.constantCoeff T.Xn)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.source_coordinate_is_simpleRoot_before_rho1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_before_rho1

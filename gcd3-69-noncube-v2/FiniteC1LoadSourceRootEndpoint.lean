...
import FiniteSimpleSourceEndpointCoordinatesC1
import FiniteDoubleC1LoadEndpointLift

/-! # The balanced active-`c1` singular source branch

At `8 * p = 3 * delta`, the literal source still excludes the double
...
/-- If the weight-eight `c1` load meets the intrinsic cubic scale exactly,
the singular finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_c1_of_eq
    ...
    (hEq : 8 * F.scale.p = 3 * T.delta) : False := by
  ...
  have hp : 3 * T.delta ≤ 8 * F.scale.p := by
  ...
  obtain ⟨r0, hr0, hXne, hu, hv, hY, hZ, hx⟩ :=
    T.source_coordinate_is_simpleRoot_before_c1
      ...
      hd0 hc70 hc50 hc40 hc30 hc20 hp
  ...
  obtain ⟨r, d, Bn, q, hr, hroot, hD, hB, hC, hBn0⟩ :=
    T.factor.simpleRoot_zeroQuadratic_coordinates_from_source_before_c1
      F hscale hdNormal hc7Normal hc5Normal hc4Normal hc3Normal hc2Normal
  ...
  exact T.factor.doubleRoot_c1Load_source_inconsistent_of_eq
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_c1_of_eq

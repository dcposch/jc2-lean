...
/-! # Half-scale order bound before the weight-five load
...
equation, so the weight-five cutoff is enough.
-/
...
/-- In the late simple-root chart below the `c4` load, the half-scale
correction starts no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c4
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c4

import FiniteFanThroughC2
import FiniteC1EarlyNormal
import FiniteC1EarlyRouting
import FiniteTargetNormalizedParameters

/-! # The simple-root early-load fan through weight eight

After the first-target translation has put the source on the `c₃ = 0`
slice, the nonsingular common-cubic fan continues from `c2` to `c1`.
The zero-load alternative before weight eight is singular, while arrival of
the active `c1` load at or before the first transverse normal kills `c1`.
-/
...
recursive fan through weight eight forces `d`, `c7`, `c5`, `c4`, `c2`,
and `c1` to vanish. -/
theorem TransverseScale.earlyLoads_zero_through_c1_of_discriminant_ne_zero
    ...
    D = 0 ∧ C7 = 0 ∧ C5 = 0 ∧ C4 = 0 ∧ C2 = 0 ∧ C1 = 0 := by
  obtain ⟨hD, hC7, hC5, hC4, hC2⟩ :=
    T.earlyLoads_zero_through_c2_of_discriminant_ne_zero F hscale
      D C7 C5 C4 C2 C1 R1 R2 R3 R4
      hdNormal hc7Normal hc5Normal hc4Normal
  ...
  have hp_c1 : (8 * F.scale.p) / 2 ≤ T.delta := by
    by_contra hnot
    have hlt : T.delta < (8 * F.scale.p) / 2 := lt_of_not_ge hnot
    have hearly : 2 * T.delta < 8 * F.scale.p := by linarith
    exact hdisc (T.discriminant_zero_before_c1 F hscale
      hfd0 hfc70 hfc50 hfc40 hfc30 hfc20 hearly)
  have hC1 : C1 = 0 := T.c1_eq_zero_at_or_before_transverse F hscale
    C1 R1 R2 R3 R4 hnd hnc7 hnc5 hnc4 hnc3 hnc2
    hfd hfc7 hfc5 hfc4 hc3 hfc2 hc1 hr1 hr2 hr3 hr4 hp_c1
  exact ⟨hD, hC7, hC5, hC4, hC2, hC1⟩
...
weight eight.  Faber uniqueness transports the result back to the original
source parameters. -/
theorem finite_targetNormalized_earlyLoads_zero_through_c1_of_discriminant_ne_zero
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
  ...
  have hfan := T.earlyLoads_zero_through_c1_of_discriminant_ne_zero F rfl
    ...
    hdisc
  rcases hfan with ⟨_hQd0, hQc70, hQc50, hQc40, hQc20, hQc10⟩
  rw [hQc7] at hQc70
  rw [hQc5] at hQc50
  rw [hQc4] at hQc40
  rw [hQc2] at hQc20
  ...
  simp at hQc10
  exact ⟨hd, hQc70, hQc50, hQc40, hQc20, hQc10⟩

end GCD369CubePolynomialSource

#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.earlyLoads_zero_through_c1_of_discriminant_ne_zero
#print axioms
  GCD369CubePolynomialSource.finite_targetNormalized_earlyLoads_zero_through_c1_of_discriminant_ne_zero

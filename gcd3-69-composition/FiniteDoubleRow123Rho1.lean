...
/-! # The first three moving-root rows through the residual weight-ten scale
...
the intrinsic cubic enters the third row there.  This file records that
three-row vanishing below ten times the Hahn scale, under the same residual
bound, together with the leading zero-load third-row residue.
-/
...
parameters have vanished, the third universal zero-load normal row
vanishes on the residue jet. -/
theorem TransverseFactor.doubleRoot_zeroLoad_row3Coeff_zero_before_rho1
...
/-- Below the weight-ten load, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, and weight-eight
parameters have vanished, the first three exact moving-root row
coefficients vanish at every nonnegative residual exponent strictly below
`delta`. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hearly : e + 2 * T.delta < 10 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_zeroLoad_row3Coeff_zero_before_rho1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho1

...
/-! # Closing the balanced `rho1` resonance from exact quadratic coordinates

At the equality `10 * p = 3 * delta`, the residual load exponent is the
intrinsic half-scale `delta`.  Given the exact quadratic factorization of
the moving-root deviations, the four literal `rho1` equations reduce to the
...
/-- At the balanced active-`rho1` resonance, exact quadratic coordinates with
vanishing half-scale `B` residue make the loaded source equations
inconsistent. -/
theorem TransverseFactor.doubleRoot_rho1Load_source_inconsistent_of_eq
  ...
  have hY' : GCD369CubeHahnRegular.constantCoeff T.Yn =
    ...
    rw [hr]
    exact hY
  have hload := T.doubleRoot_rho1Load_source_coeff_of_eq F hscale
    R Rho2 Rho3 Rho4
  ...
  apply GCD369CubeDoubleBlowup_rho1Load_balanced_resonance_Bzero_inconsistent
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_rho1Load_source_inconsistent_of_eq

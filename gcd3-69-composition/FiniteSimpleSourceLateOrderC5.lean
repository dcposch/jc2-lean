...
/-! # Half-scale order bound before the weight-four load

After the weight-one and weight-two loads vanish, the late simple-root
normal correction still lifts from the nominal half scale whenever
`3 * delta < 4 * p`.  The proof uses only the literal sextic source
equation, so the weight-four cutoff is enough.
-/
...
/-- In the late simple-root chart below the `c5` load, the half-scale
correction starts no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c5
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower_before_c5

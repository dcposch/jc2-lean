...
import FiniteDoubleRow123C1
import FiniteDoubleC1Load
import FiniteDoubleC7LoadRow4
import FiniteDoubleC1LoadSourceCoeff

/-! # Lifting the active weight-eight load on the double-root chart

In the strict window `delta < (8 * p) / 2` and `8 * p < 3 * delta`, the
weight-eight load arrives at residual exponent `8 * p - 2 * delta`, before
the intrinsic cubic.  The zero-load rows first force every moving-root
deviation to order at least `(8 * p) / 2 - delta`; a second linear lift
...
/-- In the strict active-`c1` window, the first common nonzero moving-root
deviation cannot occur before half the residual load exponent. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_c1Load_half
    ...
    (hc3 : F.c3 = 0)
    (hc2 : F.c2 = 0)
    (_hdeltaP : T.delta < (8 * F.scale.p) / 2)
    (hpDelta : 8 * F.scale.p < 3 * T.delta)
  ...
  have htwoMuDelta : 2 * J.mu < T.delta := by
    have htwice : 2 * ((8 * F.scale.p) / 2 - T.delta) =
        8 * F.scale.p - 2 * T.delta := by ring
    nlinarith
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c1 F hscale hd hc7
        hc5 hc4 hc3 hc2 r hroot q hq0 (by linarith) (by
          have htwice : 2 * ((8 * F.scale.p) / 2 - T.delta) =
              8 * F.scale.p - 2 * T.delta := by ring
          linarith)
  ...
  have hR2 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row2.1.orderTop := by
    ...
    · exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg Row2
        (lt_of_not_ge hq0)
  have hrows2 := T.doubleRoot_row123Coeffs_zero_before_c1 F hscale hd hc7
    hc5 hc4 hc3 hc2 r hroot (2 * J.mu) (by linarith [J.hmu]) htwoMuDelta (by
      have htwice : 2 * ((8 * F.scale.p) / 2 - T.delta) =
          8 * F.scale.p - 2 * T.delta := by ring
      linarith)
...
/-- The scale exclusion supplies simultaneous `(8 * p) / 2 - delta` order
bounds for the three canonical moving-root deviations. -/
theorem TransverseFactor.doubleRoot_deviations_order_c1Load_half_lower
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hroot : 2 * S.cubicU * r + 3 * S.cubicV = 0) :
    (↑((8 * F.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_c1Load_half
      F hscale hd hc7 hc5 hc4 hc3 hc2 hdeltaP hpDelta r
      (by rw [hX]; exact hA0) (by rw [hr]; exact hr0) hroot J
    have hmu : (8 * F.scale.p) / 2 - T.delta ≤ J.mu := le_of_not_gt hnot
    ...
    have hcoe : (↑((8 * F.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
...
/-- In the strict active-`c1` window, the literal source equations and the
two-stage moving-root lift realize the loaded quadratic system, which is
inconsistent when the weight-eight parameter is nonzero. -/
theorem TransverseFactor.doubleRoot_c1Load_source_inconsistent_of_lt
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hroot : 2 * S.cubicU * r + 3 * S.cubicV = 0)
    (hdeltaP : T.delta < (8 * F.scale.p) / 2)
    (hpDelta : 8 * F.scale.p < 3 * T.delta) : False := by
  ...
  have hhalf := T.doubleRoot_deviations_order_c1Load_half_lower
    F hscale hd0 hc70 hc50 hc40 hc30 hc20 hdeltaP hpDelta r r0 A0 hr0 hA0 hr huc
    hX hY hZ hroot
  let mu : ℚ := (8 * F.scale.p) / 2 - T.delta
  ...
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases he0 : 0 ≤ e
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c1 F hscale hd0
        hc70 hc50 hc40 hc30 hc20 r hroot e he0 (by
          dsimp only [mu] at he'
          have htwice : 2 * ((8 * F.scale.p) / 2 - T.delta) =
              8 * F.scale.p - 2 * T.delta := by ring
          linarith)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_c1Load_half
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_c1Load_half_lower
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c1Load_source_inconsistent_of_lt

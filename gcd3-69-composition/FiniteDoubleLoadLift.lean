...
import FiniteDoubleLoadNonzero

/-! # Lifting the active weight-one load on the double-root chart

In the strict window `2 * delta < p < 3 * delta`, the weight-one load arrives
at residual exponent `p - 2 * delta`, before the intrinsic cubic.  The
zero-load rows first force every moving-root deviation to order at least
half that residual; a second linear lift then exposes the loaded quadratic
residue system.
-/
...
/-- In the strict active-`d` window, the first common nonzero moving-root
deviation cannot occur before half the residual load exponent. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_dLoad_half
    ...
    (_hpLT : 2 * T.delta < F.scale.p)
    (hpGT : F.scale.p < 3 * T.delta)
    (J : GCD369CubeHahnRegular.TripleScale
      (S.cubicU + 3 * r ^ 2)
      (T.Yn - r * T.Xn)
      (T.Zn + 2 * r ^ 2 * T.Xn)) :
    ¬ J.mu < (F.scale.p - 2 * T.delta) / 2 := by
  intro hsmall
  let lambda : ℚ := F.scale.p - 2 * T.delta
  let mu : ℚ := lambda / 2
  change J.mu < mu at hsmall
  dsimp only [mu, lambda] at hsmall
...
/-- The scale exclusion supplies simultaneous `mu` order bounds for
the three canonical moving-root deviations. -/
theorem TransverseFactor.doubleRoot_deviations_order_dLoad_half_lower
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hpGT : F.scale.p < 3 * T.delta) :
    (↑((F.scale.p - 2 * T.delta) / 2) : WithTop ℚ) ≤
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_dLoad_half
      F hscale D0 C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4 r
      (by rw [hX]; exact hA0) (by rw [hr]; exact hr0) hroot hpLT hpGT J
    have hmu : mu ≤ J.mu := by
    ...
    have hcoe : (↑mu : WithTop ℚ) ≤ (↑J.mu : WithTop ℚ) :=
      WithTop.coe_le_coe.mpr hmu
    exact ⟨by simpa only [mu, lambda, D] using hcoe.trans hDmu,
      by simpa only [mu, lambda, B] using hcoe.trans hBmu,
      by simpa only [mu, lambda, C] using hcoe.trans hCmu⟩
...
/-- In the strict active-`d` window, the literal source equations and the
two-stage moving-root lift realize the loaded quadratic system, which is
inconsistent when the weight-one parameter is nonzero. -/
theorem TransverseFactor.doubleRoot_dLoad_source_inconsistent_of_lt
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hroot : 2 * S.cubicU * r + 3 * S.cubicV = 0)
    (hpLT : 2 * T.delta < F.scale.p)
    (hpGT : F.scale.p < 3 * T.delta) : False := by
  have huc : GCD369CubeHahnRegular.constantCoeff S.cubicU = -3 * r0 ^ 2 := by
    rw [S.constantCoeff_cubicU, hu]
  have hhalf := T.doubleRoot_deviations_order_dLoad_half_lower
    F hscale D0 C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
    r r0 A0 hr0 hA0 hr huc hX hY hZ hroot hpLT hpGT
  let lambda : ℚ := F.scale.p - 2 * T.delta
  let mu : ℚ := lambda / 2
  have hmu : 0 < mu := by dsimp only [mu, lambda]; linarith
  ...
  have hDhalf : (↑mu : WithTop ℚ) ≤ D.1.orderTop := by
    simpa only [mu, lambda, D] using hhalf.1
  have hBhalf : (↑mu : WithTop ℚ) ≤ B.1.orderTop := by
    simpa only [mu, lambda, B] using hhalf.2.1
  have hChalf : (↑mu : WithTop ℚ) ≤ C.1.orderTop := by
    simpa only [mu, lambda, C] using hhalf.2.2
  ...
  let Row2 : GCD369CubeHahnRegular k :=
    2 * T.Xn ^ 2 * r * D - 3 * T.Xn * B * r ^ 2 - 3 * T.Xn * C * r +
      3 * T.Xn * B * D - 3 * B * C
  have htwoMuDelta : 2 * mu < T.delta := by
    dsimp only [mu, lambda]
    linarith
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    · have hrows := T.doubleRoot_rowCoeffs_zero_before_d F hscale
        D0 C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
        hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
        r hroot e he0 (by linarith) (by
          dsimp only [mu, lambda] at he'
          linarith)
  ...
  have hv' : S.v = 2 * GCD369CubeHahnRegular.constantCoeff r ^ 3 := by
    rw [hr]
    exact hv
  have hload := T.doubleRoot_dLoad_source_coeff_of_lt F hscale
    D0 C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
    r hroot hu' hv' hpLT hpGT
  dsimp only at hload
  have hlambda : F.scale.p - 2 * T.delta = 2 * mu := by
    dsimp only [mu, lambda]
    ring
  ...
  apply GCD369CubeDoubleBlowup_dLoad_quadratic_beforeCubic_inconsistent_of_load_ne_zero
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) D0 hD0 hr0
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_dLoad_half
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_dLoad_half_lower
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_dLoad_source_inconsistent_of_lt

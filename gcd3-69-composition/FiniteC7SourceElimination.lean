...
/-! # Ninth-Faber vanishing below the first weight-two load

Once the weight-one load is identically zero, the same higher-order quotient
argument that killed every positive-weight term below the first load applies
up to the next first load: below twice the Hahn scale, the ninth Faber value
has zero coefficient.  Clearing the sextic source equation then gives the
reduced cubic identity used to exclude an exact cubic order before that load.
-/
...
/-- Strictly below the weight-two load scale, and after the weight-one load
has already been killed, every remaining positive-weight term in the complete
normal form vanishes in the higher-order quotient.  Hence the ninth Faber
value has zero coefficient at the selected exponent. -/
theorem faberNineCoeff_zero_before_c7
    {k : Type*} [Field k] [CharZero k]
    (S : GCD369CubeHahnNormalValueData k)
    (hd : S.d = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 2 * S.sextic.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 2 ≤ w) :
    ...
    calc
      e < (2 : ℚ) * S.sextic.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * S.sextic.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (2 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw)
  ...
  rw [hdq, hc7, hc6, hc5, hc4, hc3, hc2, hc1, hc0, hvalue] at hmap
...
/-- Strictly below the weight-two load, after the weight-one load has already
been killed, the reduced ninth-Faber expression has zero coefficient.  This
version needs no exact-order information about the transverse value. -/
theorem TransverseFactor.reducedFaberNineCoeff_zero_before_c7
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 2 * S.normal.sextic.scale.p) :
  ...
  have hF9 : F9.1.coeff e = 0 := by
    exact S.normal.faberNineCoeff_zero_before_c7 hd e he hearly
...
and that cube itself still lies below the weight-two load. -/
theorem TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c7
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0)
    (alpha beta : ℚ) (a : k)
    ...
    (hearly : 3 * alpha < 2 * S.normal.sextic.scale.p) : False := by
  let H : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.monomial T.delta T.hdelta.le
  let E : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.ratCast (3 / 8) * T.Xn *
  ...
  have hz := T.reducedFaberNineCoeff_zero_before_c7
    hd (3 * alpha) (by nlinarith [halpha]) hearly
...
#print axioms
  GCD369CubeHahnNormalValueData.faberNineCoeff_zero_before_c7
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNineCoeff_zero_before_c7
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c7

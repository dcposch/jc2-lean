...
/-! # Pre-resonant orders in the simple-root source chart before `c5`

After the weight-one and weight-two loads have vanished, the same pre-resonant
split applies once `3 * delta < 4 * p`.  The literal sextic source equation
still excludes a transverse value of exact order below `2 * delta`.  Splitting
on the order of the half-scale normal deviation then gives two short
contradictions: either the cubic square uniquely leads the sextic, or the
reduced ninth-Faber equation has a unique leading cubic term below the
weight-four load.
-/
...
`6 * p` when `3 * delta < 4 * p`. -/
theorem TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_c5
...
already below the weight-four load. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
...
weight-four load. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
  ...
  exact T.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c5
    hd hc7 alpha gamma (c * (9 * r0 ^ 2)) halpha
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.sourceSextic_inconsistent_of_transverse_below_twiceDelta_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact_before_c5

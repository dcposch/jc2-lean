import FiniteDoubleSourceChartsC1
import FiniteDoubleLiftSourceC1

/-! # Source-facing double-root exclusion before `c1` -/
...
double-root source chart below the weight-eight load. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_c1
  ...
  exact T.doubleRoot_source_inconsistent_of_coordinates_before_c1
...
which exclude the double-root source coordinate before `c1`. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_from_source_before_c1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source_before_c1
      F hscale hd hc7 hc5 hc4 hc3 hc2 hp r r0 A0 hr0 hA0 hr hu hX hY hZ hroot
  have hpS : 3 * T.delta ≤ 8 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  exact T.doubleRoot_source_inconsistent_of_orderBounds_before_c1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_c1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_from_source_before_c1

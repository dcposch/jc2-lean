...
/-! # Ninth-Faber vanishing below the first weight-four load

Once the weight-one and weight-two loads are identically zero, the same
higher-order quotient argument that killed every positive-weight term below
the first load applies up to the next first load: below four times the Hahn
scale, the ninth Faber value has zero coefficient.  The intermediate
weight-three `c6` term is multiplied by the regular sextic, which already
lives at weight six, so it vanishes in the same quotient.  Clearing the
sextic source equation then gives the reduced cubic identity used to exclude
an exact cubic order before that load.
-/
...
/-- Strictly below the weight-four load scale, and after the weight-one and
weight-two loads have already been killed, every remaining positive-weight
term in the complete normal form vanishes in the higher-order quotient.
Hence the ninth Faber value has zero coefficient at the selected exponent. -/
theorem faberNineCoeff_zero_before_c5
    {k : Type*} [Field k] [CharZero k]
    (S : GCD369CubeHahnNormalValueData k)
    (hd : S.d = 0) (hc7 : S.c7 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 4 * S.sextic.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 4 ≤ w) :
    ...
    calc
      e < (4 : ℚ) * S.sextic.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * S.sextic.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (4 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw)
          S.sextic.scale.hp.le
  have hkill (w : ℕ) (hw : 4 ≤ w)
  ...
  rw [hdq, hc7q, hc5, hc4, hc3, hc2, hc1, hc0, hvalue] at hmap
...
/-- Strictly below the weight-four load, after the weight-one and weight-two
loads have already been killed, the reduced ninth-Faber expression has zero
coefficient.  This version needs no exact-order information about the
transverse value. -/
theorem TransverseFactor.reducedFaberNineCoeff_zero_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 4 * S.normal.sextic.scale.p) :
  ...
  have hF9 : F9.1.coeff e = 0 := by
    exact S.normal.faberNineCoeff_zero_before_c5 hd hc7 e he hearly
...
and that cube itself still lies below the weight-four load. -/
theorem TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (alpha beta : ℚ) (a : k)
    ...
    (hearly : 3 * alpha < 4 * S.normal.sextic.scale.p) : False := by
  let H : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.monomial T.delta T.hdelta.le
  let E : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.ratCast (3 / 8) * T.Xn *
  ...
  have hz := T.reducedFaberNineCoeff_zero_before_c5
    hd hc7 (3 * alpha) (by nlinarith [halpha]) hearly
...
#print axioms
  GCD369CubeHahnNormalValueData.faberNineCoeff_zero_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNineCoeff_zero_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c5

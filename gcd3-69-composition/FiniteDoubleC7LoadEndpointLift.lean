import FiniteDoubleC7LoadEndpointCoeff
import FiniteDoubleC7LoadRow4

/-! # Closing the balanced `c7` resonance from exact quadratic coordinates

At the equality `2 * p = 3 * delta`, the residual load exponent is the
intrinsic half-scale `delta`.  Given the exact quadratic factorization of
the moving-root deviations, the four literal `c7` equations reduce to the
...
/-- At the balanced active-`c7` resonance, exact quadratic coordinates with
vanishing half-scale `B` residue make the loaded source equations
inconsistent. -/
theorem TransverseFactor.doubleRoot_c7Load_source_inconsistent_of_eq
  ...
  have hY' : GCD369CubeHahnRegular.constantCoeff T.Yn =
    ...
    rw [hr]
    exact hY
  have hload := T.doubleRoot_c7Load_source_coeff_of_eq F hscale
  ...
  apply GCD369CubeDoubleBlowup_c7Load_balanced_resonance_Bzero_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) C7 hA0 hr0 hBn0
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c7Load_source_inconsistent_of_eq

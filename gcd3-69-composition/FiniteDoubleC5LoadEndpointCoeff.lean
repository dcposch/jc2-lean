...
import FiniteDoubleC5LoadEndpoint

/-! # The moving double-root coefficient at the balanced `c5` resonance

At the equality `4 * p = 3 * delta`, the quadratic double-root jet and the
intrinsic cubic meet the weight-four load together.  This file extracts the
four literal `c5` equations at residual exponent `4 * p - 2 * delta`,
...
the hypotheses of
`GCD369CubeDoubleBlowup_c5Load_balanced_resonance_Bzero_inconsistent`.
-/
...
/-- At the balanced active-`c5` resonance `4 * p = 3 * delta`, the four
literal `c5` numerators at exponent `4 * p` are the shifted double-root
rows at residual exponent `4 * p - 2 * delta`, plus the pure weight-four
load, plus the intrinsic cubic constants at exponent `3 * delta`. -/
theorem TransverseFactor.doubleRoot_c5Load_source_coeff_of_eq
  ...
  have hcubic3val :
      ((64 * A ^ 3) * H ^ 3).1.coeff (4 * F.scale.p) = 64 * A0 ^ 3 := by
    rw [hEq, hcubic3]
    simp only [map_mul, map_pow, map_ofNat, A, hX]
  have hcubic4val :
      ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (4 * F.scale.p) = 314928 * r0 * A0 ^ 3 := by
    rw [hEq, hcubic4]
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c5Load_source_coeff_of_eq

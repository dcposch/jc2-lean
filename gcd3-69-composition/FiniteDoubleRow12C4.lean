...
/-! # The first two moving-root rows through the residual weight-five scale
...
cubic enters the third row there.  The first two rows have no such term.
This file records their coefficient range below five times the Hahn scale.
-/
...
weight-four parameters have vanished, the first two exact moving-root row
...
theorem TransverseFactor.doubleRoot_row12Coeffs_zero_before_c4
    ...
    (e : ℚ) (he : 0 ≤ e) (hearly : e + 2 * T.delta < 5 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row12Coeffs_zero_before_c4

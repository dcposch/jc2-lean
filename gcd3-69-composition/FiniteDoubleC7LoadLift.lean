...
import FiniteDoubleRow123C7
import FiniteDoubleC7Load
import FiniteDoubleC7LoadRow4
import FiniteDoubleC7LoadSourceCoeff

/-! # Lifting the active weight-two load on the double-root chart

In the strict window `delta < p < 3*delta/2`, the weight-two load arrives
...
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_c7Load_half
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    (hd : F.d = 0)
    (_hdeltaP : T.delta < F.scale.p)
    (hpDelta : 2 * F.scale.p < 3 * T.delta)
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c7 F hscale hd
  ...
  have hR2 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row2.1.orderTop := by
    ...
    · exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg Row2
        (lt_of_not_ge hq0)
  have hrows2 := T.doubleRoot_row123Coeffs_zero_before_c7 F hscale hd
...
theorem TransverseFactor.doubleRoot_deviations_order_c7Load_half_lower
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    (hd : F.d = 0)
    (hdeltaP : T.delta < F.scale.p)
    (hpDelta : 2 * F.scale.p < 3 * T.delta)
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_c7Load_half
      F hscale hd hdeltaP hpDelta r
      (by rw [hX]; exact hA0) (by rw [hr]; exact hr0) hroot J
    have hmu : F.scale.p - T.delta ≤ J.mu := le_of_not_gt hnot
    ...
    have hcoe : (↑(F.scale.p - T.delta) : WithTop ℚ) ≤
...
/-- In the strict active-`c7` window, the literal source equations and the
two-stage moving-root lift realize the loaded quadratic system, which is
inconsistent when the weight-two parameter is nonzero. -/
theorem TransverseFactor.doubleRoot_c7Load_source_inconsistent_of_lt
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hroot : 2 * S.cubicU * r + 3 * S.cubicV = 0)
    (hdeltaP : T.delta < F.scale.p)
    (hpDelta : 2 * F.scale.p < 3 * T.delta) : False := by
  ...
  have hhalf := T.doubleRoot_deviations_order_c7Load_half_lower
    F hscale hd0 hdeltaP hpDelta r r0 A0 hr0 hA0 hr huc hX hY hZ hroot
  let mu : ℚ := F.scale.p - T.delta
  ...
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases he0 : 0 ≤ e
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c7 F hscale hd0
  ...
  apply GCD369CubeDoubleBlowup_c7Load_quadratic_beforeCubic_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      ...
      (GCD369CubeHahnRegular.constantCoeff d) C7 hr0 hC7
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_c7Load_half
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_c7Load_half_lower
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c7Load_source_inconsistent_of_lt

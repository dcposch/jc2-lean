import FiniteDoubleSourceLiftC5
import FiniteSimpleSourceStrictFromSourceC5

/-! # Singular source-root split before `c5` -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- Before the weight-four load, the literal source excludes the double-root
choice for the recovered source coordinate and leaves the simple root of the
singular leading cubic. -/
theorem TransverseScale.source_coordinate_is_simpleRoot_before_c5
    ...
    ∃ r : k, r ≠ 0 ∧
      ...
      x = -2 * r := by
  dsimp only
  have hearly : 2 * T.delta < 4 * F.scale.p := by
    nlinarith [T.hdelta, hp]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c5 F hscale hd hc7
  ...
  · exfalso
    ...
    exact T.factor.doubleRoot_source_inconsistent_from_source_before_c5
      F hscale hdNormal hc7Normal hd hc7
...
/-- The complete singular source-root split is inconsistent before `c5`:
the double root is excluded by the double-root chart, and the remaining
simple root by the primitive third-row obstruction. -/
theorem TransverseScale.singular_source_inconsistent_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseScale)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
  ...
  obtain ⟨r0, hr0, hXne, hu, hv, hY, hZ, hx⟩ :=
    T.source_coordinate_is_simpleRoot_before_c5
      F hscale hdNormal hc7Normal hd hc7 (le_of_lt hp)
  have huc : GCD369CubeHahnRegular.constantCoeff S.cubicU =
      -3 * r0 ^ 2 := by
    rw [S.constantCoeff_cubicU, hu]
  have hvc : GCD369CubeHahnRegular.constantCoeff S.cubicV =
      2 * r0 ^ 3 := by
    rw [S.constantCoeff_cubicV, hv]
  exact T.factor.simpleRoot_strictLate_inconsistent_from_source_before_c5
    F hscale hdNormal hc7Normal hd hc7
    (by simpa only [TransverseScale.factor] using hp)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.source_coordinate_is_simpleRoot_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_before_c5

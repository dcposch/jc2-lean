...
/-! # Ninth-Faber vanishing below the first later weight-nine load

Once the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight loads are identically zero, the same
higher-order quotient argument that killed every remaining
positive-weight term applies up to the next first load: below nine times
the Hahn scale, the ninth Faber value has zero coefficient.  The
intermediate weight-three `c6` term multiplied by the regular sextic is
itself a weight-nine term, so it vanishes in the same quotient, as do
the remaining weight-nine `c0` and value terms.  An exact cubic order
below that load is then excluded from the reduced ninth-Faber identity,
using the exact cubic order rather than a generic vanishing of the
reduced expression below `9 p`.  The resulting threshold covers the
`rho1` tied resonance `3 * alpha = 15 * p / 2`.
-/
...
/-- Strictly below the later weight-nine load scale, and after the
weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, and weight-eight loads have already been killed, every
remaining positive-weight term in the complete normal form vanishes in
the higher-order quotient.  Hence the ninth Faber value has zero
coefficient at the selected exponent. -/
theorem faberNineCoeff_zero_before_later
    {k : Type*} [Field k] [CharZero k]
    (S : GCD369CubeHahnNormalValueData k)
    (hd : S.d = 0) (hc7 : S.c7 = 0) (hc5 : S.c5 = 0) (hc4 : S.c4 = 0)
    (hc3 : S.c3 = 0) (hc2 : S.c2 = 0) (hc1 : S.c1 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 9 * S.sextic.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 9 ≤ w) :
      e < (w : ℚ) * S.sextic.scale.p := by
    calc
      e < (9 : ℚ) * S.sextic.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * S.sextic.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (9 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw)
          S.sextic.scale.hp.le
  have hkill (w : ℕ) (hw : 9 ≤ w)
  ...
  have hc2q : q (S.sextic.scale.weightedRegular 7 S.c2) = 0 := by
    have hzw : S.sextic.scale.weightedRegular 7 S.c2 = 0 := by
      ...
      simp
    rw [hzw, map_zero]
  have hc1q : q (S.sextic.scale.weightedRegular 8 S.c1) = 0 := by
    have hzw : S.sextic.scale.weightedRegular 8 S.c1 = 0 := by
      apply Subtype.ext
      change S.sextic.scale.t ^ 8 * S.c1.1 = 0
      rw [hc1]
      simp
    rw [hzw, map_zero]
  ...
  rw [hdq, hc7q, hc5q, hc4q, hc3q, hc2q, hc1q, hc0, hvalue] at hmap
...
and the cube itself still lies below the later weight-nine load. -/
theorem TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_later
    {k : Type*} [Field k] [CharZero k]
    ...
    (hearly : 3 * alpha < 9 * S.normal.sextic.scale.p) : False := by
...
#print axioms
  GCD369CubeHahnNormalValueData.faberNineCoeff_zero_before_later
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_later

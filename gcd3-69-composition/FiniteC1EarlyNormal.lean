...
/-! # Early transverse jets before the first weight-eight load

If a common-cubic transverse jet occurs before the weight-eight `c1` scale,
and the weight-one, weight-two, weight-four, weight-five, weight-six, and
weight-seven parameters have already vanished, then every remaining source
and target load disappears in the corresponding higher-order quotient.
...
/-- Below the weight-eight scale, after the weight-one, weight-two,
weight-four, weight-five, weight-six, and weight-seven loads have
vanished, the complete source equations have exactly the zero-high-constant
leading coefficients. -/
theorem zeroHighNCoeff_zero_before_c1
    ...
    (hc3 : F.c3 = 0)
    (hc2 : F.c2 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 8 * F.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 8 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (8 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (8 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
...
/-- Any transverse factor strictly before the first weight-eight load, after
the weight-one, weight-two, weight-four, weight-five, weight-six, and
weight-seven parameters have vanished, satisfies the four universal
zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_c1
    {k : Type*} [Field k] [CharZero k]
  ...
  have hz := F.zeroHighNCoeff_zero_before_c1 hd hc7 hc5 hc4 hc3 hc2
...
/-- A nonzero first transverse jet before the `c1` scale, after the
weight-one, weight-two, weight-four, weight-five, weight-six, and
weight-seven parameters have vanished, forces the limiting common cubic
to be singular. -/
theorem TransverseScale.discriminant_zero_before_c1
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_c1 F hscale
    hd hc7 hc5 hc4 hc3 hc2 hearly
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_c1
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_c1
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_c1

...
Faber identity already excludes the branch below `rho1`.  At exponent
`2 * delta` its explicit `E * H^2` term has nonzero residue, while the
cubic cube and the cubic times the source sextic both start later. -/
theorem TransverseFactor.sourceNinth_inconsistent_of_cubic_order_delta_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (r A : k) (hr : r ≠ 0) (hA : A ≠ 0)
    (hx : GCD369CubeHahnRegular.constantCoeff
...
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.sourceNinth_inconsistent_of_cubic_order_delta_before_rho1

...
/-! # The strict-late weight-eight cell at infinity

After `d=c7=c5=c4=c3=c2=0`, every remaining source or target load starts at
weight at least eight.  Thus, when `3δ < 8p`, the intrinsic cubic transverse term is
...
/-- Below the weight-eight scale, after `d=c7=c5=c4=c3=c2=0`, the four complete source
equations reduce modulo the cutoff to their zero-load numerators. -/
theorem zeroLoadN_mem_higherOrderIdeal_of_lt_eight_p
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (e : ℚ) (he : 0 ≤ e) (hlt : e < 8 * F.scale.p)
  ...
  have hc2q : q (F.scale.weightedRegular 7 F.c2) = 0 := by
    rw [hzeroWeighted 7 F.c2 hc2, map_zero]
...
/-- Below the weight-eight scale, the complete terminal numerator differs
from the zero-load specialization only by terms strictly after the cutoff. -/
theorem fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_eight_p
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (e : ℚ) (he : 0 ≤ e) (hlt : e < 8 * F.scale.p)
    (hd : F.d = GCD369CubeHahnRegular.constant 0)
...
/-- Before the weight-eight load, vanishing through `c2` gives the four
universal zero-load normal rows at the canonical transverse scale. -/
theorem TransverseScale.zeroLoadLeadingRows_before_c1
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ :=
    D.faber.zeroLoadN_mem_higherOrderIdeal_of_lt_eight_p
      (2 * T.delta) (mul_nonneg (by norm_num) T.hdelta.le)
      hearly hd hc7 hc5 hc4 hc3 hc2
  have hz1 :
      (729 * GCD369CubeFaberN1
...
weight-eight term starts strictly after the intrinsic cubic term, the latter
is the exact first term of the fifth numerator. -/
theorem fullN5_orderTop_eq_three_delta_of_singular_c1_late
    {k : Type*} [Field k] [CharZero k]
    ...
    (hlate : 3 * T.delta < 8 * D.faber.scale.p) :
    (GCD369CubeFaberFullN5
      D.faber.scale.regular0 D.faber.scale.regular1
      D.faber.scale.regular2 D.faber.scale.regular3
  ...
  obtain ⟨hzero1, hzero2, _hzero3, _hzero4⟩ :=
    D.faber.zeroLoadN_mem_higherOrderIdeal_of_lt_eight_p
      (3 * T.delta) he hlate hd hc7 hc5 hc4 hc3 hc2
  ...
  have hNdiff : N - N0 ∈ I := by
    simpa only [N, N0, dload] using
      D.faber.fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_eight_p
        (3 * T.delta) he hlate hd hc7 hc5 hc4 hc3 hc2
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Source-facing weight-eight late-cell contradiction once the singular
transverse residue is in its standard double-root chart. -/
theorem infinity_c1_singular_late_inconsistent_of_parametrization
  ...
  have hc2 : D.faber.c2 = GCD369CubeHahnRegular.constant 0 := by
    have h : D.faber.c2 = GCD369CubeHahnRegular.constant P.c2 := by
      simpa only [D, P, S.infinityCommonCoefficientData_faber hsdegree]
        using hc2raw
    rw [hc2P] at h
    exact h
  have hNorder :=
    GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_c1_late
      D T hd hc7 hc5 hc4 hc3 hc2 r A hr hA hu hX hY hZ hlate
  have hterminalOrder :
      (GCD369CubeFaberR5
  ...
  apply S.infinity_terminal_three_delta_not_before_load_of_singular
    hsdegree T 8 (by norm_num) hdisc hlate
  simpa only [hpole, D, S.infinityCommonCoefficientData_faber hsdegree]
    using hscaledOrder

/-- After `d=c7=c5=c4=c3=c2=0`, the strict region `3δ < 8p` is inconsistent for every
constant-core infinity source. -/
theorem infinity_c1_singular_late_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
    (hdP : S.faberConstantParameters.d = 0)
  ...
  have hdisc : 4 * D.u ^ 3 + 27 * D.v ^ 2 = 0 := by
    rw [hu, hv]
    ring
  exact S.infinity_c1_singular_late_inconsistent_of_parametrization
    hsdegree T hdP hc7P hc5P hc4P hc3P hc2P hdisc r
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
      hr hA hu rfl hY hZ hlate

/-- Target-normalized source wrapper: canonical parameter uniqueness supplies
`d=c3=0`; the genuine earlier load vanishings are assumed. -/
theorem targetNormalized_infinity_c1_singular_late_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
  ...
  apply U.infinity_c1_singular_late_inconsistent hsdegreeU T
...
#print axioms
  GCD369CubeHahnFaberPoleData.zeroLoadN_mem_higherOrderIdeal_of_lt_eight_p
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_eight_p
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.zeroLoadLeadingRows_before_c1
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_c1_late
#print axioms
  GCD369CubePolynomialSource.infinity_c1_singular_late_inconsistent
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_c1_singular_late_inconsistent

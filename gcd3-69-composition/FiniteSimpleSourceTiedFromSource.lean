...
/-! # Source-facing tied late simple-root obstruction

At the resonance `p = 3 * delta`, target normalization has already killed
the weight-one parameter `d`.  The first two zero-load rows therefore remain
valid up to the next, weight-two, load.  Applied through twice the transverse
scale, they supply the second lift required by the tied third-row
obstruction.
-/
...
/-- A tied simple-root source with vanishing weight-one parameter is
inconsistent. -/
theorem TransverseFactor.simpleRoot_tied_inconsistent_from_source
    ...
    False := by
  have hpLe : 3 * T.delta ≤ F.scale.p := by rw [hp]
  have hpS : 3 * T.delta ≤ S.normal.sextic.scale.p := by
    rw [hscale]
    exact hpLe
  have hdConst : F.d = GCD369CubeHahnRegular.constant (0 : k) := by
    simpa using hd
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source F hscale
      0 C7 C5 C4 C3 C2 C1 Rho1 Rho2 Rho3 Rho4
      hdConst hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4 hpLe
  ...
  have hBhalf := T.simpleRoot_sourceLate_normal_order_half_lower
    r0 A0 r w d B 0 C2n hr0 hA0 hpS hr hX hw0 hw hcharts.1 hcharts.2
  ...
  let Row1 : GCD369CubeHahnRegular k :=
    A ^ 2 * (S.cubicU + 3 * r ^ 2) -
      2 * A * (T.Yn - r * A) * r - (T.Yn - r * A) ^ 2 -
      2 * A * (T.Zn + 2 * r ^ 2 * A)
  let Row2 : GCD369CubeHahnRegular k :=
    2 * A ^ 2 * r * (S.cubicU + 3 * r ^ 2) -
      3 * A * (T.Yn - r * A) * r ^ 2 -
      3 * A * (T.Zn + 2 * r ^ 2 * A) * r +
      3 * A * (T.Yn - r * A) * (S.cubicU + 3 * r ^ 2) -
      3 * (T.Yn - r * A) * (T.Zn + 2 * r ^ 2 * A)
  have hRow1 : (↑(2 * T.delta) : WithTop ℚ) ≤ Row1.1.orderTop := by
    rw [HahnSeries.le_orderTop_iff_forall]
    intro q hq
    have hq' : q < 2 * T.delta := WithTop.coe_lt_coe.mp hq
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c7 F hscale
        hd r hroot q hq0 (by rw [hp]; nlinarith [T.hdelta])
      simpa only [Row1, A] using hrows.1
    · exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg Row1
        (lt_of_not_ge hq0)
  have hRow2 : (↑(2 * T.delta) : WithTop ℚ) ≤ Row2.1.orderTop := by
    rw [HahnSeries.le_orderTop_iff_forall]
    intro q hq
    have hq' : q < 2 * T.delta := WithTop.coe_lt_coe.mp hq
    by_cases hq0 : 0 ≤ q
    · have hrows := T.doubleRoot_row12Coeffs_zero_before_c7 F hscale
        hd r hroot q hq0 (by rw [hp]; nlinarith [T.hdelta])
      simpa only [Row2, A] using hrows.2
    · exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg Row2
        (lt_of_not_ge hq0)
  have hRow1fac : Row1 =
      H * (A ^ 2 * d - 2 * A * ((-r * b + C2n) + r * b)) -
        H ^ 2 * b ^ 2 := by
    dsimp only [Row1, A, H]
    rw [hDcoord, hBcoord, hCcoord]
    ring
  have hRow2fac : Row2 =
      H * (2 * A ^ 2 * r * d -
        3 * A * r * ((-r * b + C2n) + r * b)) +
      H ^ 2 * (3 * A * b * d - 3 * b * (-r * b + C2n)) := by
    dsimp only [Row2, A, H]
    rw [hDcoord, hBcoord, hCcoord]
    ring
  have hsecond :=
    GCD369CubeHahnRegular.doubleBlowup_kernelRemainders_order_lower_of_factoredRows
      T.delta T.hdelta A r d b (-r * b + C2n)
      (by simpa only [A, hX] using hA0)
      (by rw [hr]; exact hr0)
      (by
        dsimp only
        simpa only [H] using (hRow1fac ▸ hRow1))
      (by
        dsimp only
        simpa only [H] using (hRow2fac ▸ hRow2))
  have hC2second : (↑T.delta : WithTop ℚ) ≤ C2n.1.orderTop := by
    simpa only [show -r * b + C2n + r * b = C2n by ring] using hsecond.2
  exact T.simpleRoot_tied_N3_inconsistent_of_secondLift
    F hscale hp r d b C2n A0 hX hA0 hroot
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_tied_inconsistent_from_source

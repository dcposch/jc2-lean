...
/-! # Pre-resonant orders in the simple-root source chart

For a source coordinate approaching the remaining simple root, the cubic is
linear in its displacement.  Splitting on the order of the half-scale normal
deviation gives two short contradictions: either the cubic square is the
unique leading sextic term, or the sextic cancellation fixes the deviation
order and the reduced ninth-Faber equation has a unique leading cubic term.
-/
...
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower
...
reduced ninth-Faber equation then has a unique leading cubic term. -/
theorem TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  exact T.reducedFaberNine_inconsistent_of_exact_cubic_order
    alpha gamma (c * (9 * r0 ^ 2)) halpha
    (mul_ne_zero hc (mul_ne_zero (by norm_num) (pow_ne_zero 2 hr0)))
    hK hKlc (by simpa only [E, L] using hE)
    (by nlinarith [hresonance, hbelow])
    (by nlinarith [hp, hbelow])

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_lower
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceBelow_inconsistent_of_normal_exact

...
/-! # The first two moving-root rows through the residual weight-eleven scale
...
because the intrinsic cubic enters the third row there.  The first two
rows have no such term.  This file records their coefficient range below
eleven times the Hahn scale, together with the leading zero-load first-row
residue.
-/
...
weight-ten parameters have vanished, the first universal zero-load normal
row vanishes on the residue jet. -/
theorem TransverseFactor.doubleRoot_zeroLoad_row1Coeff_zero_before_rho2
...
/-- Below the weight-eleven load, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, weight-eight, and
weight-ten parameters have vanished, the first two exact moving-root row
...
theorem TransverseFactor.doubleRoot_row12Coeffs_zero_before_rho2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (e : ℚ) (he : 0 ≤ e) (hearly : e + 2 * T.delta < 11 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_zeroLoad_row1Coeff_zero_before_rho2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row12Coeffs_zero_before_rho2

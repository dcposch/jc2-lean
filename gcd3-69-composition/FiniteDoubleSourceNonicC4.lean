...
/-! # Source-value boundary at the moving double root before `c4`

Once the weight-one, weight-two, and weight-four loads have vanished, the
literal nonic source value still excludes the late double-root branch below
the weight-five scale.  The argument is the same second-order residue as in
the weight-one case, now evaluated at `2 * delta` under the weaker cutoff
`3 * delta ≤ 5 * p`.
-/
...
the weight-one, weight-two, and weight-four loads have vanished and
`3 * delta ≤ 5 * p`. -/
theorem TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c4
  ...
  have hz := S.normal.faberNineCoeff_zero_before_c4
    hd hc7 hc5 (2 * T.delta) (by nlinarith [T.hdelta])
...
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c4

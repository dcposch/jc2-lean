import FiniteSimpleSourceStrict
import FiniteSimpleSourceLift
import FiniteSimpleSourceFullScale

/-! # Source-facing strict late simple-root obstruction

The source-driven order lift already produces simple-root chart coordinates
in which the displacement is zero or of order at least `delta`.  In the
strict branch `3 * delta < p`, the late source inequalities then promote
the half-scale normal correction to the full `delta` scale.  The resulting
full-scale moving-root expansions are excluded by the primitive third-row
obstruction.
-/
...
/-- If `3 * delta < p` and the recovered source residue is the remaining
simple root `-2 * r0` of the singular leading cubic, the literal finite
source is inconsistent. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_from_source
    ...
    False := by
  have hpLe : 3 * T.delta ≤ F.scale.p := le_of_lt hp
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source F hscale
      ...
      hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4 hpLe
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_from_source

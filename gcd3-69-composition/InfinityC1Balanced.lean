...
/-! # The balanced singular c1 cell at infinity

After `d = c7 = c5 = c4 = c3 = c2 = 0`, the weight-eight load, the
quadratic moving-root correction, and the intrinsic cubic meet when
`8 p = 3 delta`.
-/
...
balanced c1 resonance. -/
theorem GCD369Cube_c1Load_balanced_terminal_inconsistent
    ...
    (h4 :
      32 * (-13122 * A * r ^ 3 * q - 65610 * r ^ 3 * b ^ 2) -
        314928 * r * A ^ 3 - 1119744 * r ^ 5 * T = 0)
    (h5 :
      559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
        2519424 * r ^ 2 * A ^ 3 + 10450944 * r ^ 6 * T = 0) :
    False := by
...
/-- Coefficient of a pure `c1` load in the denominator-cleared terminal
numerator.  This name is balanced-lane-local pending a shared `c1` tie file. -/
def terminalC1BalancedCoeff {R : Type*} [CommRing R]
    (a0 _a1 a2 a3 a4 : R) : R :=
  2239488 * a0 - 373248 * a2 * a4 - 186624 * a3 ^ 2 +
    72576 * a4 ^ 3

theorem fullN5_c1Balanced_specialize
    {R : Type*} [CommRing R]
    (a0 a1 a2 a3 a4 c1 : R) :
    GCD369CubeFaberFullN5 a0 a1 a2 a3 a4 0 0 0 0 0 0 c1 =
      GCD369CubeFaberFullN5 a0 a1 a2 a3 a4 0 0 0 0 0 0 0 +
        c1 * terminalC1BalancedCoeff a0 a1 a2 a3 a4 := by
  simp only [terminalC1BalancedCoeff]
  ring

theorem terminalC1BalancedCoeff_doubleRoot
    {R : Type*} [CommRing R] (r : R) :
    terminalC1BalancedCoeff
        ((2 * r ^ 3) ^ 2)
        (2 * (-3 * r ^ 2) * (2 * r ^ 3))
...
/-- The pure c1 fifth numerator at `8 p = 3 delta`. -/
theorem TransverseScale.c1SpecializedTerminalCoeff_at_balanced
    {k : Type*} [Field k] [CharZero k]
    ...
    (htie : 8 * D.faber.scale.p = 3 * T.delta)
    ...
    (GCD369CubeFaberFullN5
      ...
      (D.faber.scale.weightedRegular 8 D.faber.c1)).1.coeff
        (8 * D.faber.scale.p) =
      559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
        2519424 * r ^ 2 * A ^ 3 + 10450944 * r ^ 6 * C1 := by
  ...
  let P : GCD369CubeHahnRegular k := terminalC1BalancedCoeff
    D.faber.scale.regular0 D.faber.scale.regular1
    D.faber.scale.regular2 D.faber.scale.regular3
  ...
  have hP : GCD369CubeHahnRegular.constantCoeff P =
      10450944 * r ^ 6 := by
    dsimp only [P]
    simp only [terminalC1BalancedCoeff, map_add, map_sub, map_mul, map_pow,
      map_ofNat, map_neg]
    change terminalC1BalancedCoeff
      D.faber.scale.leading0 D.faber.scale.leading1
      D.faber.scale.leading2 D.faber.scale.leading3
      D.faber.scale.leading4 = 10450944 * r ^ 6
    rw [D.ha0, D.ha1, D.ha2, D.ha3, D.ha4, hu, hv]
    exact terminalC1BalancedCoeff_doubleRoot r
  have hload : D.faber.scale.weightedRegular 8 D.faber.c1 =
      H ^ 3 * GCD369CubeHahnRegular.constant C1 := by
    rw [hc1, D.faber.scale.weightedRegular_constant_eq_monomial_sq
      8 (by norm_num) C1]
    apply Subtype.ext
    change
      (HahnSeries.single (((8 : ℕ) : ℚ) * D.faber.scale.p / 2)
          (1 : k)) ^ 2 * HahnSeries.C C1 =
        (HahnSeries.single T.delta (1 : k)) ^ 3 * HahnSeries.C C1
    simp only [HahnSeries.single_pow, nsmul_eq_mul, one_pow]
    apply congrArg
      (fun z : ℚ => HahnSeries.single z (1 : k) * HahnSeries.C C1)
    calc
      2 * (8 * D.faber.scale.p / 2) = 8 * D.faber.scale.p := by ring
      _ = 3 * T.delta := htie
  ...
  have hfactor :
      ...
      (559872 * R5) * H ^ 2 +
        H ^ 3 * (839808 * C + GCD369CubeHahnRegular.constant C1 * P) := by
    rw [fullN5_c1Balanced_specialize, hload]
  ...
  have htop := GCD369CubeHahnRegular.coeff_monomial_pow_mul_at
    T.delta T.hdelta 3
      (839808 * C + GCD369CubeHahnRegular.constant C1 * P)
  change (H ^ 3 *
      (839808 * C + GCD369CubeHahnRegular.constant C1 * P)).1.coeff
        (3 * T.delta) =
      GCD369CubeHahnRegular.constantCoeff
        (839808 * C + GCD369CubeHahnRegular.constant C1 * P) at htop
  change
    (GCD369CubeFaberFullN5
      D.faber.scale.regular0 D.faber.scale.regular1
      D.faber.scale.regular2 D.faber.scale.regular3
...
/-- Before the c1 load, the first three moving-root row coefficients vanish
below the intrinsic residual cutoff. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c1_balanced
    {k : Type*} [Field k] [CharZero k]
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Rows one, two, and four at the balanced c1 resonance. -/
theorem TransverseScale.c1LoadedNormalRowsAt_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    ...
    ((32 * GCD369CubeNormalRow4
        T.Xn T.Yn T.Zn D.cubicU D.cubicV).1.coeff T.delta -
          314928 * r * A0 ^ 3 - 1119744 * r ^ 5 * C1 = 0) := by
  let F := D.faber
  let H : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.monomial T.delta T.hdelta.le
  let N1 : GCD369CubeHahnRegular k := GCD369CubeFaberN1
  ...
  let P1 : GCD369CubeHahnRegular k :=
    10368 * F.scale.regular4
  let P2 : GCD369CubeHahnRegular k :=
    31104 * F.scale.regular3
  let P4 : GCD369CubeHahnRegular k :=
    279936 * F.scale.regular1 -
      93312 * F.scale.regular3 * F.scale.regular4
  let c1w : GCD369CubeHahnRegular k :=
    F.scale.weightedRegular 8 (GCD369CubeHahnRegular.constant C1)
  have hload1 : GCD369CubeHahnRegular.constantCoeff P1 =
      -62208 * r ^ 2 := by
    dsimp only [P1, F]
    simp only [map_sub, map_mul, map_ofNat]
    change 10368 * F.scale.leading4 = _
    rw [D.ha4, hu]
    ring
  have hload2 : GCD369CubeHahnRegular.constantCoeff P2 =
    ...
    change 31104 * F.scale.leading3 = _
    rw [D.ha3, hv]
    ring
  have hload4 : GCD369CubeHahnRegular.constantCoeff P4 =
    ...
    dsimp only [P4, F]
    simp only [map_sub, map_mul, map_ofNat]
    change 279936 * F.scale.leading1 -
      93312 * F.scale.leading3 * F.scale.leading4 = _
    rw [D.ha1, D.ha3, D.ha4, hu, hv]
    ring
  have hweight1 : (c1w * P1).1.coeff (8 * F.scale.p) =
      C1 * GCD369CubeHahnRegular.constantCoeff P1 := by
    simpa only [c1w] using
      F.scale.weightedRegular_eight_constant_mul_coeff C1 P1
  have hweight2 : (c1w * P2).1.coeff (8 * F.scale.p) =
      C1 * GCD369CubeHahnRegular.constantCoeff P2 := by
    simpa only [c1w] using
      F.scale.weightedRegular_eight_constant_mul_coeff C1 P2
  have hweight4 : (c1w * P4).1.coeff (8 * F.scale.p) =
      C1 * GCD369CubeHahnRegular.constantCoeff P4 := by
    simpa only [c1w] using
      F.scale.weightedRegular_eight_constant_mul_coeff C1 P4
  have hz := F.c1NCoeff_zero Rho1 Rho2 Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hrho1 hrho2 hrho3 hrho4
  have hz1 := hz.1
  have hz2 := hz.2.1
  have hz4 := hz.2.2.2
  rw [hc1] at hz1 hz2 hz4
  change (729 * N1).1.coeff (8 * F.scale.p) +
      (c1w * P1).1.coeff (8 * F.scale.p) = 0 at hz1
  change (2187 * N2).1.coeff (8 * F.scale.p) +
      (c1w * P2).1.coeff (8 * F.scale.p) = 0 at hz2
  change (6561 * N4).1.coeff (8 * F.scale.p) +
      (c1w * P4).1.coeff (8 * F.scale.p) = 0 at hz4
  ...
  have hcubicval :
      ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (8 * F.scale.p) = 314928 * r * A0 ^ 3 := by
    rw [htie, hcubic]
    simp only [map_mul, map_pow, map_ofNat, hY, hX]
    ring
  change (729 * N1).1.coeff (8 * F.scale.p) =
      ((-32 * Row1) * H ^ 2).1.coeff (8 * F.scale.p) at hc1'
  change (2187 * N2).1.coeff (8 * F.scale.p) =
      ((32 * Row2) * H ^ 2).1.coeff (8 * F.scale.p) at hc2'
  change (6561 * N4).1.coeff (8 * F.scale.p) =
      ((32 * Row4) * H ^ 2).1.coeff (8 * F.scale.p) -
        ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (8 * F.scale.p) at hc4'
...
theorem fullN5Coeff_eq_c1BalancedSpecialized
    {k : Type*} [Field k] [CharZero k]
    ...
    (GCD369CubeFaberFullN5
      ...
      (F.scale.weightedRegular 8 F.c1)).1.coeff (8 * F.scale.p) =
    (GCD369CubeFaberFullN5
      F.scale.regular0 F.scale.regular1 F.scale.regular2
      F.scale.regular3 F.scale.regular4 0 0 0 0 0 0
...
theorem TransverseScale.c1LoadedTerminalEquation_at_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (C1 : k)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    ...
    (hzero :
      (GCD369CubeFaberFullN5
        ...
        (D.faber.scale.weightedRegular 8 D.faber.c1)).1.coeff
          (8 * D.faber.scale.p) = 0) :
    559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
      2519424 * r ^ 2 * A ^ 3 + 10450944 * r ^ 6 * C1 = 0 := by
  rw [D.faber.fullN5Coeff_eq_c1BalancedSpecialized
    hd hc7 hc5 hc4 hc3 hc2] at hzero
  rw [T.c1SpecializedTerminalCoeff_at_balanced
    C1 hc1 r A d b q hu hv hX hY hZ htie
    hord1 hord2 hcoeff1 hcoeff2] at hzero
  exact hzero

/-- Below the weight-eight load, with every earlier load zero, the first
transverse residue satisfies the four universal normal rows. -/
theorem TransverseScale.zeroLoadLeadingRows_before_c1_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Global degree integrality supplies the balanced c1 terminal row. -/
theorem infinity_c1LoadedTerminalEquation_at_balanced
    ...
    (hY : GCD369CubeHahnRegular.constantCoeff T.Yn = r * A)
    (hZ : GCD369CubeHahnRegular.constantCoeff T.Zn = -2 * r ^ 2 * A)
    (htie : 8 * (S.infinityCommonCoefficientData hsdegree).faber.scale.p =
      3 * T.delta)
    (hord1 : (↑T.delta : WithTop ℚ) ≤
  ...
  have hzero := S.infinity_fullN5Coeff_weight_p_eq_zero_of_singular
    hsdegree 8 (by norm_num) hdisc
  exact T.c1LoadedTerminalEquation_at_balanced
    C1 hd hc7 hc5 hc4 hc3 hc2 hc1
    r A d b q hu hv hX hY hZ htie hord1 hord2 hcoeff1 hcoeff2
    (by simpa only [D, Nat.cast_ofNat] using hzero)

set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- The singular balanced c1 cell is impossible. -/
theorem infinity_c1_singular_balanced_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
  ...
  have hc2Dz : D.faber.c2 = 0 := by simpa using hc2D
  have hearly : 2 * T.delta < 8 * D.faber.scale.p := by
    rw [htie]
    linarith [T.hdelta]
  obtain ⟨hz1, hz2, hz3, hz4⟩ :=
    T.zeroLoadLeadingRows_before_c1_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz hearly
  ...
  have hrows : ∀ e : ℚ, 0 ≤ e → e < T.delta →
    ...
    intro e he hedelta
    have hearly' : 2 * T.factor.delta + e < 8 * D.faber.scale.p := by
      change 2 * T.delta + e < 8 * D.faber.scale.p
      rw [htie]
      linarith
    have h := T.factor.doubleRoot_row123Coeffs_zero_before_c1_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz rr hroot e he hedelta hearly'
  ...
  obtain ⟨hload1, hload2, hload4⟩ := T.c1LoadedNormalRowsAt_balanced
    P.c1 R.rho1 R.rho2 R.rho3 R.rho4
  ...
  have h5 := S.infinity_c1LoadedTerminalEquation_at_balanced
    hsdegree hdisc T P.c1
    hdD hc7D hc5D hc4D hc3D hc2D hc1D
    r A0 d0 b0 q0 hu hv rfl hY hZ htie hord1 hord2 hcoeff1 hcoeff2
  apply GCD369Cube_c1Load_balanced_terminal_inconsistent
    A0 r d0 b0 q0 P.c1 hA0 hr
...
#print axioms GCD369Cube_c1Load_balanced_terminal_inconsistent
#print axioms GCD369CubeHahnCommonCoefficientData.terminalC1BalancedCoeff_doubleRoot
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c1SpecializedTerminalCoeff_at_balanced
#print axioms GCD369CubeHahnFaberPoleData.fullN5Coeff_eq_c1BalancedSpecialized
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c1LoadedTerminalEquation_at_balanced
#print axioms GCD369CubeHahnCommonCoefficientData.TransverseScale.c1LoadedNormalRowsAt_balanced
#print axioms GCD369CubePolynomialSource.infinity_c1LoadedTerminalEquation_at_balanced
#print axioms GCD369CubePolynomialSource.infinity_c1_singular_balanced_inconsistent

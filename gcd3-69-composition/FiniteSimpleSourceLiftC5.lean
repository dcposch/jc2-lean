import FiniteSimpleSourceChartAllC5
import FiniteDoubleLiftSourceC5

/-! # Source-driven simple-root order lift before `c5` -/
...
chart below `c5`; its displacement is zero or starts at `delta`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c5
  ...
  exact T.simpleRoot_source_zero_or_order_delta_of_coordinates_before_c5
...
simple-root order split before `c5`. -/
theorem TransverseFactor.simpleRoot_source_zero_or_order_delta_from_source_before_c5
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  have hpS : 3 * T.delta < 4 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  exact T.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c5
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_of_orderBounds_before_c5
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_source_zero_or_order_delta_from_source_before_c5

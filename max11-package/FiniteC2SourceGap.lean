...
/-! # Ninth-Faber vanishing below the active weight-seven `c2` load

`FiniteC2SourceElimination` only gives ninth-Faber vanishing below `6 * p`.
Target normalization also gives `S.normal.c3 = 0`, which kills the remaining
weight-six term identically.  The higher-order quotient then extends through
the active `c2` weight: below seven times the Hahn scale, the ninth Faber
value has zero coefficient.  The weight-seven `c2` term is killed by the
quotient rather than by an identical-vanishing hypothesis.  The intermediate
weight-three `c6` term multiplied by the regular sextic is itself a
weight-nine term, so it vanishes in the same quotient.  An exact cubic order
below that load is then excluded from the reduced ninth-Faber identity, using
the exact cubic order rather than a generic vanishing of the reduced
expression below `7 p`.
-/
...
/-- Strictly below the weight-seven load scale, and after the weight-one,
weight-two, weight-four, weight-five, and weight-six loads have already
been killed, every remaining positive-weight term in the complete normal
form vanishes in the higher-order quotient.  Hence the ninth Faber value
has zero coefficient at the selected exponent. -/
theorem faberNineCoeff_zero_before_c2_with_c3
    {k : Type*} [Field k] [CharZero k]
    (S : GCD369CubeHahnNormalValueData k)
    (hd : S.d = 0) (hc7 : S.c7 = 0) (hc5 : S.c5 = 0) (hc4 : S.c4 = 0)
    (hc3 : S.c3 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 7 * S.sextic.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 7 ≤ w) :
    ...
    calc
      e < (7 : ℚ) * S.sextic.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * S.sextic.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (7 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw)
  ...
  rw [hdq, hc7q, hc5q, hc4q, hc3q, hc2, hc1, hc0, hvalue] at hmap
...
and the cube itself still lies below the weight-seven load. -/
theorem TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c2_with_c3
    {k : Type*} [Field k] [CharZero k]
    ...
    (hearly : 3 * alpha < 7 * S.normal.sextic.scale.p) : False := by
  ...
  have hF9 : F9.1.coeff (3 * alpha) = 0 :=
    S.normal.faberNineCoeff_zero_before_c2_with_c3 hd hc7 hc5 hc4 hc3
...
#print axioms
  GCD369CubeHahnNormalValueData.faberNineCoeff_zero_before_c2_with_c3
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c2_with_c3

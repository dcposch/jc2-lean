...
import FiniteC4LoadSourceRootEndpoint

/-! # Assembly of the singular source at or after `c4` resonance

The source-facing `c4` branch is empty both strictly before the weight-five
resonance and at the exact cubic tie.  The two existing contradictions
compose on a single `≤` hypothesis; no nonzero `c4` coefficient is required.
-/
...
/-- If the first source load meets or exceeds the weight-five `c4` resonance
`3 * delta ≤ 5 * p`, the singular finite source is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_or_after_c4_resonance
  ...
  · exact T.singular_source_inconsistent_at_active_c4_of_eq
  ...
  · have hd0 : F.d = 0 := by simpa using hd
    ...
    exact T.singular_source_inconsistent_before_c4
      F hscale hdNormal hc7Normal hc5Normal hd0 hc70 hc50 hlt

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_or_after_c4_resonance

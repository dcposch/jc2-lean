import FiniteSimpleSourceLate

/-! # Half-scale order bound for the late simple-root normal correction

Once the simple-root displacement starts at order `delta`, a half-scale
normal correction of exact finite order strictly below `delta / 2` would
make the transverse value start at `delta + delta / 2 + gamma < 2 * delta`.
The moving cubic starts no earlier than `delta`, so the literal sextic
source equation excludes that possibility.
-/
...
/-- In the late simple-root chart, the half-scale correction itself starts
no earlier than order `delta / 2`. -/
theorem TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_sourceLate_normal_order_half_lower

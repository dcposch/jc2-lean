...
/-! # The zero-load balanced cubic before `rho1`
...
`3 * delta`.  Strictly before the weight-ten load all four primitive rows
vanish there.  Their quadratic residues and intrinsic cubic constants form
an inconsistent finite system, without any further source-root analysis.
-/
...
intrinsic cubic lies strictly below the `rho1` cutoff. -/
theorem TransverseFactor.doubleRoot_zeroLoad_balanced_cubic_inconsistent_before_rho1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_zeroLoad_balanced_cubic_inconsistent_before_rho1

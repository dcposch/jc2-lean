...
/-! # The first three moving-root rows through the residual weight-eleven scale

After the weight-one, weight-two, weight-four, weight-five, weight-six,
weight-seven, weight-eight, and weight-ten loads have vanished, the bundled
three-row coefficient theorem still stops before residual exponent `delta`,
because the intrinsic cubic enters the third row there.  This file records
that three-row vanishing below eleven times the Hahn scale, under the same
residual bound, together with the leading zero-load third-row residue.
-/
...
/-- Below the weight-eleven load, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, weight-eight, and
weight-ten parameters have vanished, the third universal zero-load normal
row vanishes on the residue jet. -/
theorem TransverseFactor.doubleRoot_zeroLoad_row3Coeff_zero_before_rho2
  ...
  exact (T.zeroLoadLeadingRows_before_rho2 F hscale hd hc7 hc5 hc4 hc3 hc2 hc1
    hr1 (by nlinarith [T.hdelta])).2.2.1

/-- Below the weight-eleven load, after the weight-one, weight-two,
weight-four, weight-five, weight-six, weight-seven, weight-eight, and
weight-ten parameters have vanished, the first three exact moving-root row
coefficients vanish at every nonnegative residual exponent strictly below
`delta`. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (hearly : e + 2 * T.delta < 11 * F.scale.p) :
    let A := T.Xn
  ...
  change N3.1.coeff (e + 2 * T.delta) =
    ((128 * R3) * H ^ 2).1.coeff (e + 2 * T.delta) -
      ((64 * A ^ 3) * H ^ 3).1.coeff (e + 2 * T.delta) at hcoeff3
  rw [show e + 2 * T.delta = e + (2 : ℚ) * T.delta by ring,
    hshift3] at hcoeff3
  rw [show e + (2 : ℚ) * T.delta =
      (e - T.delta) + (3 : ℚ) * T.delta by ring,
    hcubicShift, hcubicZero, sub_zero] at hcoeff3
  have hz1scaled : (-32 * (729 * R1)).1.coeff e = 0 := by
    calc
      _ = (729 * N1).1.coeff (e + 2 * T.delta) := hcoeff1.symm
  ...
  have hz3scaled : (128 * R3).1.coeff e = 0 := by
    calc
      _ = N3.1.coeff (e - T.delta + 3 * T.delta) := hcoeff3.symm
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_zeroLoad_row3Coeff_zero_before_rho2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho2

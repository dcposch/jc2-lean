import FiniteDoubleRow12

/-! # The strict late simple-root obstruction

When `3 * delta < p`, the first two primitive moving-root rows still vanish
at residual exponent `delta`.  In full-scale coordinates their linear parts
force the residue of the moving cubic correction to vanish.  The primitive
third row at `3 * delta` then contains only the intrinsic `-64 * A^3` term.
-/
...
weight-one load reaches the intrinsic cubic exponent. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    (D0 C7 C5 C4 C3 C2c C1 Rho1 Rho2 Rho3 Rho4 : k)
    (hfd : F.d = GCD369CubeHahnRegular.constant D0)
    ...
    (hc2 : F.c2 = GCD369CubeHahnRegular.constant C2c)
  ...
  let R2 : GCD369CubeHahnRegular k :=
    2 * A ^ 2 * r * D - 3 * A * B * r ^ 2 - 3 * A * C * r +
      3 * A * B * D - 3 * B * C
  have hrows := T.doubleRoot_row12Coeffs_zero_before_d F hscale
    D0 C7 C5 C4 C3 C2c C1 Rho1 Rho2 Rho3 Rho4
    hfd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
    r hroot T.delta T.hdelta.le (by nlinarith [hp])
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale

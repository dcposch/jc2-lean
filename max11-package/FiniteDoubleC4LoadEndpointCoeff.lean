...
/-! # The moving double-root coefficient at the balanced `c4` resonance

At the equality `5 * p = 3 * delta`, the quadratic double-root jet and the
intrinsic cubic meet the weight-five load together.  This file extracts the
four literal `c4` equations at residual exponent `5 * p - 2 * delta`,
...
the hypotheses of
`GCD369CubeDoubleBlowup_c4Load_balanced_resonance_Bzero_inconsistent`.
-/
...
/-- At the balanced active-`c4` resonance `5 * p = 3 * delta`, the four
literal `c4` numerators at exponent `5 * p` are the shifted double-root
rows at residual exponent `5 * p - 2 * delta`, plus the pure weight-five
load, plus the intrinsic cubic constants at exponent `3 * delta`. -/
theorem TransverseFactor.doubleRoot_c4Load_source_coeff_of_eq
  ...
  have hcubic3val :
      ((64 * A ^ 3) * H ^ 3).1.coeff (5 * F.scale.p) = 64 * A0 ^ 3 := by
    rw [hEq, hcubic3]
    simp only [map_mul, map_pow, map_ofNat, A, hX]
  have hcubic4val :
      ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (5 * F.scale.p) = 314928 * r0 * A0 ^ 3 := by
    rw [hEq, hcubic4]
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c4Load_source_coeff_of_eq

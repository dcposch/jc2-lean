...
/-! # The first three moving-root rows through the residual weight-five scale

After the weight-one, weight-two, and weight-four loads have vanished, the bundled three-row coefficient
theorem still stops before residual exponent `delta`, because the intrinsic
cubic enters the third row there.  This file records that three-row
vanishing below five times the Hahn scale, under the same residual bound.
-/
...
/-- Below the weight-five load, after the weight-one, weight-two, and
weight-four parameters have vanished, the first three exact moving-root
row coefficients vanish at every nonnegative residual exponent strictly
below `delta`. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_c4
    ...
    (hearly : e + 2 * T.delta < 5 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_c4

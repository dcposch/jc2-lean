...
/-! # The moving double-root coefficient at the active `c7` load

In the strict window `delta < p` and `2 * p < 3 * delta`, the quadratic
double-root jet occurs before the weight-two load, while the intrinsic cubic
occurs after it.  This file extracts the four literal `c7` equations at the
remaining exponent `2 * p - 2 * delta`.
-/
...
/-- At exponent `2 * p`, a weight-two ground-field constant times a regular
factor contributes the constant times the factor's residue. -/
theorem weightedRegular_two_constant_mul_coeff
    {k : Type*} [Field k]
    (S : GCD369CubeHahnPoleScale k) (C : k)
    (P : GCD369CubeHahnRegular k) :
    (S.weightedRegular 2 (GCD369CubeHahnRegular.constant C) * P).1.coeff
        (2 * S.p) = C * GCD369CubeHahnRegular.constantCoeff P := by
  rw [S.weightedRegular_constant_eq_monomial_sq 2 (by norm_num) C]
  convert GCD369CubeHahnRegular.coeff_monomial_sq_constant_mul
    S.p S.hp C P using 1 <;> norm_num

end GCD369CubeHahnPoleScale

#print axioms
  GCD369CubeHahnPoleScale.weightedRegular_two_constant_mul_coeff

namespace GCD369CubeHahnCommonValueData

/-- In the strict active-`c7` window `delta < p` and `2 * p < 3 * delta`,
the four literal `c7` numerators at exponent `2 * p` are the shifted
double-root rows at residual exponent `2 * p - 2 * delta`, plus the pure
weight-two load.  The intrinsic cubic terms do not yet contribute. -/
theorem TransverseFactor.doubleRoot_c7Load_source_coeff_of_lt
    ...
    (hpLT : T.delta < F.scale.p)
    (hpGT : 2 * F.scale.p < 3 * T.delta) :
    let A := T.Xn
  ...
  have hcubic3 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (64 * A ^ 3) T.delta (2 * F.scale.p - 3 * T.delta) T.hdelta.le 3
  have hcubic4 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (314928 * (r * A + B) * A ^ 2) T.delta
    (2 * F.scale.p - 3 * T.delta) T.hdelta.le 3
  change ((64 * A ^ 3) * H ^ 3).1.coeff
      ((2 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (64 * A ^ 3).1.coeff (2 * F.scale.p - 3 * T.delta) at hcubic3
  change ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
      ((2 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (314928 * (r * A + B) * A ^ 2).1.coeff
          (2 * F.scale.p - 3 * T.delta) at hcubic4
  have hneg : 2 * F.scale.p - 3 * T.delta < 0 := by linarith [hpGT]
  have hcubic3zero :
      ((64 * A ^ 3) * H ^ 3).1.coeff (2 * F.scale.p) = 0 := by
    have hrew : (2 * F.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta =
        2 * F.scale.p := by ring
    rw [← hrew, hcubic3]
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c7Load_source_coeff_of_lt

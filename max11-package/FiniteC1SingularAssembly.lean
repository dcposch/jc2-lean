...
import FiniteC1LoadSourceRoot
import FiniteC1SourceAssembly

/-! # Assembly of the singular source at a nonzero `c1` load

A nonzero weight-eight `c1` coefficient is inconsistent on every timing of
the first source load.  Arrival no later than the transverse normal already
forces `c1 = 0`.  After that scale the remaining two source-facing branches
...
/-- If the first nonzero Faber parameter is a weight-eight `c1` constant, the
singular finite source is inconsistent at every load timing. -/
theorem TransverseScale.singular_source_inconsistent_of_c1_ne_zero
  ...
  rcases le_or_gt ((8 * F.scale.p) / 2) T.delta with hle | hdeltaP
  · have hzero : C1 = 0 :=
      T.c1_eq_zero_at_or_before_transverse F hscale
    ...
    exact hC1 hzero
  · rcases lt_or_ge (8 * F.scale.p) (3 * T.delta) with hlt | hge
    · exact T.singular_source_inconsistent_at_active_c1_of_lt
        F hscale C1 Rho1 Rho2 Rho3 Rho4 hC1
        hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
        hdeltaP hlt
    · exact T.singular_source_inconsistent_at_or_after_c1_resonance
        F hscale C1 Rho1 Rho2 Rho3 Rho4
        hdNormal hc7Normal hc5Normal hc4Normal hc3Normal hc2Normal
        hd hc7 hc5 hc4 hc3 hc2 hc1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_of_c1_ne_zero

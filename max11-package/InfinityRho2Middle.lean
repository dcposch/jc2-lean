...
import FiniteDoubleRho2LoadLift
import TargetNormalization
import FiniteTargetNormalizedParameters

/-! # The strict middle weight-eleven load at infinity

After `d = c7 = c5 = c4 = c3 = 0`, this file treats
`2 * delta < 11 * p < 3 * delta`.  The coefficient-only zero-load rows
drive the two-stage moving-root lift to the active `rho2` quadratic
contradiction.
-/
...
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho2_coeff
    ...
    (hearly : 2 * T.delta + e < 11 * D.faber.scale.p) :
    let A := T.Xn
  ...
  let N3 : GCD369CubeHahnRegular k := GCD369CubeFaberN3
    D.faber.scale.regular0 D.faber.scale.regular1 D.faber.scale.regular2
    D.faber.scale.regular3 D.faber.scale.regular4
  have hz := D.faber.zeroHighNCoeff_zero_before_rho2 hd hc7 hc5 hc4 hc3 hc2 hc1 hr1
...
`11p/2-delta`. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_rho2Load_half_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0) (hc4 : D.faber.c4 = 0)
    ...
    (hc1 : D.faber.c1 = 0)
    (hr1 : D.faber.rho1 = 0)
    (_hdeltaP : T.delta < (11 * D.faber.scale.p) / 2)
    (hpDelta : 11 * D.faber.scale.p < 3 * T.delta)
    (r : GCD369CubeHahnRegular k)
    ...
    (J : GCD369CubeHahnRegular.TripleScale
      ...
      (T.Zn + 2 * r ^ 2 * T.Xn)) :
    ¬ J.mu < (11 * D.faber.scale.p) / 2 - T.delta := by
  intro hsmall
  let A : GCD369CubeHahnRegular k := T.Xn
  let Dv : GCD369CubeHahnRegular k := D.cubicU + 3 * r ^ 2
  let B : GCD369CubeHahnRegular k := T.Yn - r * A
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ R1.1.orderTop := by
    ...
    by_cases hq0 : 0 ≤ q
    · have hs := T.doubleRoot_row123Coeffs_zero_before_rho2_coeff hd hc7 hc5 hc4 hc3 hc2 hc1 hr1
...
theorem TransverseFactor.doubleRoot_deviations_order_rho2Load_half_lower_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0) (hc4 : D.faber.c4 = 0)
    ...
    (hc1 : D.faber.c1 = 0)
    (hr1 : D.faber.rho1 = 0)
    (hdeltaP : T.delta < (11 * D.faber.scale.p) / 2)
    (hpDelta : 11 * D.faber.scale.p < 3 * T.delta)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0) :
    (↑((11 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
  ...
  · let J := GCD369CubeHahnRegular.tripleScale Dv B C hD0 hB0 hC0 hn
    have hnot := T.doubleRoot_firstCommonScale_not_below_rho2Load_half_coeff
      hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hdeltaP hpDelta r (by rw [hX]; exact hA0)
      (by rw [hr]; exact hr0) hroot J
    have hmu : (11 * D.faber.scale.p) / 2 - T.delta ≤ J.mu := le_of_not_gt hnot
    ...
    have hcoe : (↑((11 * D.faber.scale.p) / 2 - T.delta) : WithTop ℚ) ≤
...
theorem TransverseFactor.doubleRoot_rho2Load_coeff_of_lt
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (Rho2 Rho3 Rho4 : k)
    ...
    (hrho1 : D.faber.rho1 = GCD369CubeHahnRegular.constant 0)
    ...
    (r : GCD369CubeHahnRegular k)
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0)
    (_hu : D.u = -3 * GCD369CubeHahnRegular.constantCoeff r ^ 2)
    (_hv : D.v = 2 * GCD369CubeHahnRegular.constantCoeff r ^ 3)
    (_hpLT : T.delta < (11 * D.faber.scale.p) / 2)
    (hpGT : 11 * D.faber.scale.p < 3 * T.delta) :
    let A := T.Xn
    let Dv := D.cubicU + 3 * r ^ 2
    let B := T.Yn - r * A
    let C := T.Zn + 2 * r ^ 2 * A
    ...
    let R4 :=
      4374 * (-3 * r ^ 2 + Dv) *
        (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
          3 * A * B * Dv - 3 * B * C)
    let lambda := 11 * D.faber.scale.p - 2 * T.delta
    ((-32 * (729 * R1)).1.coeff lambda = 0) ∧
    ((32 * (-1458 * R2)).1.coeff lambda - 186624 * Rho2 = 0) ∧
  ...
  let R4 : GCD369CubeHahnRegular k :=
    4374 * (-3 * r ^ 2 + Dv) *
      (A * B * r ^ 2 + 2 * B ^ 2 * r + A * C * r +
        3 * A * B * Dv - 3 * B * C)
  let lambda : ℚ := 11 * D.faber.scale.p - 2 * T.delta
  let N1 : GCD369CubeHahnRegular k := GCD369CubeFaberN1
    D.faber.scale.regular0 D.faber.scale.regular1 D.faber.scale.regular2
    D.faber.scale.regular3 D.faber.scale.regular4
  let N2 : GCD369CubeHahnRegular k := GCD369CubeFaberN2
  ...
  let N4 : GCD369CubeHahnRegular k := GCD369CubeFaberN4
    D.faber.scale.regular0 D.faber.scale.regular1 D.faber.scale.regular2
    D.faber.scale.regular3 D.faber.scale.regular4
  have hlambda : 11 * D.faber.scale.p =
      lambda + (2 : ℚ) * T.delta := by
    dsimp only [lambda]
    ring
  have hexp := T.doubleRoot_zeroHigh_expansions_coeff r hroot
  have hexp1 : 729 * N1 = (-32 * (729 * R1)) * H ^ 2 := by
  ...
  have hcubic3 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (64 * A ^ 3) T.delta
      (11 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  have hcubic4 := GCD369CubeHahnRegular.coeff_mul_monomial_pow_add
    (314928 * (r * A + B) * A ^ 2) T.delta
      (11 * D.faber.scale.p - 3 * T.delta) T.hdelta.le 3
  change ((64 * A ^ 3) * H ^ 3).1.coeff
      ((11 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (64 * A ^ 3).1.coeff
          (11 * D.faber.scale.p - 3 * T.delta) at hcubic3
  change ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
      ((11 * D.faber.scale.p - 3 * T.delta) + (3 : ℚ) * T.delta) =
        (314928 * (r * A + B) * A ^ 2).1.coeff
          (11 * D.faber.scale.p - 3 * T.delta) at hcubic4
  have hneg : 11 * D.faber.scale.p - 3 * T.delta < 0 := by
    linarith
  have hcubic3zero :
      ((64 * A ^ 3) * H ^ 3).1.coeff (11 * D.faber.scale.p) = 0 := by
    rw [show 11 * D.faber.scale.p =
      (11 * D.faber.scale.p - 3 * T.delta) +
        (3 : ℚ) * T.delta by ring, hcubic3]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hcubic4zero :
      ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (11 * D.faber.scale.p) = 0 := by
    rw [show 11 * D.faber.scale.p =
      (11 * D.faber.scale.p - 3 * T.delta) +
        (3 : ℚ) * T.delta by ring, hcubic4]
    exact GCD369CubeHahnRegular.coeff_eq_zero_of_neg _ hneg
  have hz := D.faber.rho2NCoeff Rho2 Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
  norm_num at hz
  have hz1 := hz.1
  change (729 * N1).1.coeff (11 * D.faber.scale.p) = 0 at hz1
  have hz2 := hz.2.1
  change (2187 * N2).1.coeff (11 * D.faber.scale.p) =
    186624 * Rho2 at hz2
  have hz3 := hz.2.2.1
  change N3.1.coeff (11 * D.faber.scale.p) = 0 at hz3
  have hz4 := hz.2.2.2
  change (6561 * N4).1.coeff (11 * D.faber.scale.p) = 0 at hz4
  refine ⟨?_, ?_, ?_, ?_⟩
  · have hc := congrArg
      (fun z : GCD369CubeHahnRegular k =>
        z.1.coeff (11 * D.faber.scale.p)) hexp1
    rw [hlambda, hshift1] at hc
    calc
      (-32 * (729 * R1)).1.coeff lambda =
          (729 * N1).1.coeff (11 * D.faber.scale.p) := by rw [← hc, hlambda]
      _ = 0 := hz1
  · have hc := congrArg
      (fun z : GCD369CubeHahnRegular k =>
        z.1.coeff (11 * D.faber.scale.p)) hexp2
    rw [hlambda, hshift2] at hc
    calc
      (32 * (-1458 * R2)).1.coeff lambda - 186624 * Rho2 =
          (2187 * N2).1.coeff (11 * D.faber.scale.p) -
            186624 * Rho2 := by
            rw [← hc, hlambda]
      _ = 0 := sub_eq_zero.mpr hz2
  · have hc := congrArg
      (fun z : GCD369CubeHahnRegular k =>
        z.1.coeff (11 * D.faber.scale.p)) hexp3
    change N3.1.coeff (11 * D.faber.scale.p) =
      ((128 * R3) * H ^ 2).1.coeff (11 * D.faber.scale.p) -
        ((64 * A ^ 3) * H ^ 3).1.coeff
          (11 * D.faber.scale.p) at hc
    rw [hcubic3zero, sub_zero, hlambda, hshift3] at hc
    rw [hlambda] at hz3
    exact hc.symm.trans hz3
  · have hc := congrArg
      (fun z : GCD369CubeHahnRegular k =>
        z.1.coeff (11 * D.faber.scale.p)) hexp4
    change (6561 * N4).1.coeff (11 * D.faber.scale.p) =
      ((32 * R4) * H ^ 2).1.coeff (11 * D.faber.scale.p) -
        ((314928 * (r * A + B) * A ^ 2) * H ^ 3).1.coeff
          (11 * D.faber.scale.p) at hc
    rw [hcubic4zero, sub_zero, hlambda, hshift4] at hc
    rw [hlambda] at hz4
    exact hc.symm.trans hz4
set_option maxHeartbeats 4000000 in
theorem TransverseFactor.doubleRoot_rho2Load_inconsistent_of_lt_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (Rho2 Rho3 Rho4 : k)
    (hRho2 : Rho2 ≠ 0)
    ...
    (hroot : 2 * D.cubicU * r + 3 * D.cubicV = 0)
    (hdeltaP : T.delta < (11 * D.faber.scale.p) / 2)
    (hpDelta : 11 * D.faber.scale.p < 3 * T.delta) : False := by
  ...
  have hr10 : D.faber.rho1 = 0 := by simpa using hrho1
  have huc : GCD369CubeHahnRegular.constantCoeff D.cubicU = -3 * r0 ^ 2 := by
    rw [D.constantCoeff_cubicU, hu]
  have hhalf := T.doubleRoot_deviations_order_rho2Load_half_lower_coeff
    hd0 hc70 hc50 hc40 hc30 hc20 hc10 hr10 hdeltaP hpDelta r r0 A0 hr0 hA0 hr huc
    hX hY hZ hroot
  let mu : ℚ := (11 * D.faber.scale.p) / 2 - T.delta
  ...
  have hRow1 : (↑(2 * mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    by_cases he0 : 0 ≤ e
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_rho2_coeff
        hd0 hc70 hc50 hc40 hc30 hc20 hc10 hr10 r hroot e he0 (by
          dsimp only [mu] at he'
          nlinarith [hpDelta]) (by
          dsimp only [mu] at he'
          have htwice : 2 * ((11 * D.faber.scale.p) / 2 - T.delta) =
              11 * D.faber.scale.p - 2 * T.delta := by ring
          linarith)
  ...
  have hv' : D.v = 2 * GCD369CubeHahnRegular.constantCoeff r ^ 3 := by
    rw [hr]
    exact hv
  have hload := T.doubleRoot_rho2Load_coeff_of_lt
    Rho2 Rho3 Rho4
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
    r hroot hu' hv' hdeltaP hpDelta
  dsimp only at hload
  have hlambda : 11 * D.faber.scale.p - 2 * T.delta = 2 * mu := by
    dsimp only [mu]
    ring
  rw [hlambda] at hload
  have hload1 : (-32 * (729 * Row1)).1.coeff (2 * mu) = 0 := by
  ...
  have hload4 : (32 * Row4).1.coeff (2 * mu) = 0 := by
  ...
  have hload1q : (-32 * (729 * QR1)).1.coeff (2 * mu) = 0 := by
  ...
  have hload4q : (32 * QR4).1.coeff (2 * mu) = 0 := by
  ...
  apply GCD369CubeDoubleBlowup_rho2Load_quadratic_beforeCubic_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) Rho2 hr0 hRho2
  · linear_combination hload1q
  · linear_combination hload2q
  · linear_combination hload3q
  · linear_combination hload4q
theorem TransverseFactor.zeroLoadLeadingRows_before_rho2_coeff
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseFactor)
    (hd : D.faber.d = 0)
    (hc7 : D.faber.c7 = 0)
    ...
    (hc1 : D.faber.c1 = 0)
    (hr1 : D.faber.rho1 = 0)
    (hearly : 2 * T.delta < 11 * D.faber.scale.p) :
...
strict active-`rho2` middle window is inconsistent at coefficient level. -/
theorem TransverseScale.rho2_singular_middle_inconsistent_coeff
    {k : Type*} [Field k] [CharZero k]
  ...
  have hc10 : D.faber.c1 = 0 := by simpa using hc1
  have hr10 : D.faber.rho1 = 0 := by simpa using hrho1
  have hearly : 2 * T.delta < 11 * D.faber.scale.p := by
    nlinarith [hdeltaP]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_rho2_coeff hd0 hc70 hc50 hc40 hc30 hc20 hc10 hr10
  ...
  exact T.factor.doubleRoot_rho2Load_inconsistent_of_lt_coeff
    Rho2 Rho3 Rho4 hRho2
    hd hc7 hc5 hc4 hc3 hc2 hc1 hrho1 hrho2 hrho3 hrho4
...
set_option maxHeartbeats 4000000 in
/-- Generic source-facing active `rho2` middle theorem in the explicit
`c3 = 0` gauge. -/
theorem infinity_rho2_eq_zero_singular_middle
  ...
  have hc2D : D.faber.c2 = GCD369CubeHahnRegular.constant 0 := by
    ...
    rw [hc20] at h
    simpa only [D, P, S.infinityCommonCoefficientData_faber hsdegree] using h
  have hc1D : D.faber.c1 = GCD369CubeHahnRegular.constant 0 := by
    have h := hc1
    rw [hc10] at h
    simpa only [D, P, S.infinityCommonCoefficientData_faber hsdegree] using h
  have hr1D : D.faber.rho1 = GCD369CubeHahnRegular.constant 0 := by
    have h := hr1
    rw [hrho10] at h
    simpa only [D, R, S.infinityCommonCoefficientData_faber hsdegree] using h
  ...
  have hdeltaP : T.delta < (11 * D.faber.scale.p) / 2 := by
    dsimp only [D]
    nlinarith
  by_contra hRho2
  exact T.rho2_singular_middle_inconsistent_coeff
    R.rho2 R.rho3 R.rho4 hRho2
    hdD hc7D hc5D hc4D hc3D hc2D hc1D hr1D hr2D hr3D hr4D
    hdeltaP (by simpa only [D] using hpDelta)

/-- Target-normalized active-`rho2` wrapper with canonical `c3 = 0`. -/
theorem targetNormalized_infinity_rho2_eq_zero_singular_middle
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (hd0 : S.faberConstantParameters.d = 0)
  ...
  apply U.infinity_rho2_eq_zero_singular_middle hsdegreeU T
...
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho2_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_firstCommonScale_not_below_rho2Load_half_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_deviations_order_rho2Load_half_lower_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_rho2Load_coeff_of_lt
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseFactor.doubleRoot_rho2Load_inconsistent_of_lt_coeff
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho2_singular_middle_inconsistent_coeff
#print axioms GCD369CubePolynomialSource.infinity_rho2_eq_zero_singular_middle
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_rho2_eq_zero_singular_middle

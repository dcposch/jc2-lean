...
/-! # Ninth-Faber vanishing below the first weight-six load

Once the weight-one, weight-two, weight-four, and weight-five loads are
identically zero, the same higher-order quotient argument that killed every
positive-weight term below the first load applies up to the next first
load: below six times the Hahn scale, the ninth Faber value has zero
coefficient.  The remaining weight-six `c3` term and the intermediate
weight-three `c6` term multiplied by the regular sextic both start at
weight six, so they vanish in the same quotient.  Clearing the sextic
source equation then gives the reduced cubic identity used to exclude an
exact cubic order before that load.  The resulting threshold covers the
`c2` tied resonance `3 * alpha = 21 * p / 4`.
-/
...
/-- Strictly below the weight-six load scale, and after the weight-one,
weight-two, weight-four, and weight-five loads have already been killed,
every remaining positive-weight term in the complete normal form vanishes
in the higher-order quotient.  Hence the ninth Faber value has zero
coefficient at the selected exponent. -/
theorem faberNineCoeff_zero_before_c2
    {k : Type*} [Field k] [CharZero k]
    (S : GCD369CubeHahnNormalValueData k)
    (hd : S.d = 0) (hc7 : S.c7 = 0) (hc5 : S.c5 = 0) (hc4 : S.c4 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 6 * S.sextic.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 6 ≤ w) :
      e < (w : ℚ) * S.sextic.scale.p := by
    calc
      e < (6 : ℚ) * S.sextic.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * S.sextic.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (6 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw)
          S.sextic.scale.hp.le
  have hkill (w : ℕ) (hw : 6 ≤ w)
  ...
  rw [hdq, hc7q, hc5q, hc4q, hc3, hc2, hc1, hc0, hvalue] at hmap
...
/-- Strictly below the weight-six load, after the weight-one, weight-two,
weight-four, and weight-five loads have already been killed, the reduced
ninth-Faber expression has zero coefficient.  This version needs no
exact-order information about the transverse value. -/
theorem TransverseFactor.reducedFaberNineCoeff_zero_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (hc5 : S.normal.c5 = 0) (hc4 : S.normal.c4 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 6 * S.normal.sextic.scale.p) :
  ...
  have hF9 : F9.1.coeff e = 0 := by
    exact S.normal.faberNineCoeff_zero_before_c2 hd hc7 hc5 hc4 e he hearly
...
and that cube itself still lies below the weight-six load. -/
theorem TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c2
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (hd : S.normal.d = 0) (hc7 : S.normal.c7 = 0)
    (hc5 : S.normal.c5 = 0) (hc4 : S.normal.c4 = 0)
    ...
    (hearly : 3 * alpha < 6 * S.normal.sextic.scale.p) : False := by
  let H : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.monomial T.delta T.hdelta.le
  let E : GCD369CubeHahnRegular k :=
    GCD369CubeHahnRegular.ratCast (3 / 8) * T.Xn *
  ...
  have hz := T.reducedFaberNineCoeff_zero_before_c2
    hd hc7 hc5 hc4 (3 * alpha) (by nlinarith [halpha]) hearly
...
#print axioms
  GCD369CubeHahnNormalValueData.faberNineCoeff_zero_before_c2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNineCoeff_zero_before_c2
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.reducedFaberNine_inconsistent_of_exact_cubic_order_before_c2

...
/-! # Early double-root source order case before `rho2` -/

noncomputable section

namespace GCD369CubeHahnCommonValueData

/-- In the early moving-root range below `rho2`, after `rho1 = 0`, the sextic
source equation forces the Newton resonance `delta = 3 * nu`; ninth-Faber
vanishing below the later weight-nine load then gives the incompatible second
residue equation. -/
theorem TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders_before_rho2
  ...
  · have hle : min (2 * (2 * nu)) (T.delta + nu) ≤ T.delta + nu :=
      min_le_right _ _
    have hbound : 6 * S.normal.sextic.scale.p ≤ T.delta + nu :=
      hfar.trans hle
    nlinarith [T.hdelta, hp, S.normal.sextic.scale.hp]
  · have hdelta : T.delta = 3 * nu := by
    ...
    have hz := S.normal.faberNineCoeff_zero_before_later
      ...
      (6 * nu) (by nlinarith)
      (by nlinarith [hp, hdelta, S.normal.sextic.scale.hp])
    have hg : 27 * r ^ 3 * c ^ 6 +
        (27 / 2) * r ^ 2 * A * c ^ 3 +
        (9 / 8) * r * A ^ 2 = 0 := by
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceEarly_inconsistent_of_exactOrders_before_rho2

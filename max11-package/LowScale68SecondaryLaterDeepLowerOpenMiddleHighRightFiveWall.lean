...
/-! # The strict right exact-five wall `3g = 5n < 2h`

On this wall the fixed `B,c` face and the `delta*A^3` load meet at degree
`6n-1`.  Every other summand lies strictly below.  Their exact coefficient
forces `delta` to be nonzero.  The left/coincident five walls and the separate
`h >= 2g` packet are intentionally not changed here.
-/
...
section LaterDeepLowerOpenMiddleHighRightFiveWall68
...
theorem fiveRadiusReducedRowTwoPolynomial68_rightFive_rest_degree_lt
  ...
  have hz :
    ...
    dsimp [idx]
    omega
  dsimp [idx] at hde htail hz ⊢
  exact deepGap_natDegree_add_lt (deepGap_natDegree_add_lt hde htail) hz

set_option maxHeartbeats 8000000 in
theorem laterDeepFiveSumReducedRowTwo_rightFive_relation68
  ...
  have hidx : 11 * n - 3 * g - 1 = idx := by
    ...
    omega
  rw [hidx] at hfixed
  have hrest := fiveRadiusReducedRowTwoPolynomial68_rightFive_rest_degree_lt
    gamma epsilon zeta i4 A B c d e n g h hn hg hgh hh hopen hwallG
    haboveH hA hB hc hd he
  have hdelta :=
  ...
  have hgroup : fiveRadiusRowTwoFixedCore68 A B c +
    ...
    abel
  rw [hgroup, coeff_add, hrestCoeff, add_zero, hfixed,
    coeff_smul, smul_eq_mul] at hcoeff
  dsimp [idx] at hcoeff
  rw [hdelta] at hcoeff
  exact hcoeff

theorem laterDeepFiveSumReducedRowTwo_rightFive_forces_delta_ne_zero68
    (gamma delta epsilon zeta i4 : k) (A B c d e : k[X])
    (n g h : ℕ) (hn : 0 < n) (hg : 0 < g) (hgh : g < h)
    (hh : h < 2 * g)
    (hopen : 3 * n < 2 * g ∧ 2 * g < 5 * n)
  ...
  exact (mul_ne_zero
    ...
    (pow_ne_zero 3 hb)) hrel

set_option maxHeartbeats 18000000 in
theorem maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenMiddleHighRightFive_forces_delta_ne_zero68
  ...
  exact laterDeepFiveSumReducedRowTwo_rightFive_forces_delta_ne_zero68
    gamma delta epsilon zeta i4 Ae Be ce d ee n g h hn hg ordinary.2.2.1
    hmiddle hopen hwall.1 hwall.2 ha ordinary.2.2.2.2.2.1 hA secondary.2.1
    secondary.2.2.1 ordinary.2.2.2.1 ordinary.2.2.2.2.1 ordinary.2.2.2.2.2.2.2.2.2
    hred

#print axioms fiveRadiusReducedRowTwoPolynomial68_rightFive_rest_degree_lt
#print axioms laterDeepFiveSumReducedRowTwo_rightFive_relation68
#print axioms laterDeepFiveSumReducedRowTwo_rightFive_forces_delta_ne_zero68
#print axioms maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenMiddleHighRightFive_forces_delta_ne_zero68

end LaterDeepLowerOpenMiddleHighRightFiveWall68

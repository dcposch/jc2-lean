import FiniteDoubleSourceChartsC4
import FiniteDoubleLiftSourceC4

/-! # Source-facing double-root exclusion before `c4` -/
...
double-root source chart below the weight-five load. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_c4
  ...
  exact T.doubleRoot_source_inconsistent_of_coordinates_before_c4
...
which exclude the double-root source coordinate before `c4`. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_from_source_before_c4
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source_before_c4
      F hscale hd hc7 hc5 hp r r0 A0 hr0 hA0 hr hu hX hY hZ hroot
  have hpS : 3 * T.delta ≤ 5 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  exact T.doubleRoot_source_inconsistent_of_orderBounds_before_c4
    hdNormal hc7Normal hc5Normal r0 A0 r B hr0 hA0 hpS hr hx hX hroot
      hDfull hYcoord hQfull

end GCD369CubeHahnCommonValueData

#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_c4
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_from_source_before_c4

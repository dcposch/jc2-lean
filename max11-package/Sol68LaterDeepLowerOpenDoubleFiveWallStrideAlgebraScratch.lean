import Sol68LaterDeepLowerOpenDoubleFiveWallNextCoefficientScratch

/-! # First-stride algebra on the residual double five wall -/
...
section LaterDeepLowerOpenDoubleFiveWallStrideAlgebra68
...
/-- If an expanded polynomial misses an aligned boundary coefficient, its
degree drops by one complete expansion stride. -/
theorem natDegree_expand_le_sub_sixty_of_aligned_boundary_zero68
  ...
  · have hlead : (expand k 60 q).coeff
    ...
    have hdiv : 60 ∣ (expand k 60 q).natDegree := by
      by_contra hnot
      rw [coeff_expand (by norm_num : 0 < 60), if_neg hnot] at hlead
      exact hlead rfl
...
theorem secondaryResidualInvariantThreePolynomial68_coeff_double_drop_sixty_of_fiveWall
    (A B c d e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hg : 60 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
    (hB : B.natDegree ≤ 3 * n - g)
    (hc : c.natDegree ≤ 4 * n - g - 60)
    (hd : d.natDegree ≤ 5 * n - 2 * g - 60)
    ...
    (secondaryResidualInvariantThreePolynomial68 A B c d e).coeff
      ...
      (8 / 9 : k) *
        (-(A.coeff (2 * n) * B.coeff (3 * n - g) *
            d.coeff (5 * n - 2 * g - 60)) -
          B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 60) +
          3 * c.coeff (4 * n - g - 60) *
            e.coeff (6 * n - 2 * g)) := by
  have hABd := coeff_mul_mul_at_bounds68 A B d (2 * n)
    (3 * n - g) (5 * n - 2 * g - 60) hA hB hd
  have hABdi : 2 * n + (3 * n - g) + (5 * n - 2 * g - 60) =
      10 * n - 3 * g - 60 := by omega
  rw [hABdi] at hABd
  have hBBc := coeff_mul_mul_at_bounds68 B B c (3 * n - g)
    (3 * n - g) (4 * n - g - 60) hB hB hc
  have hBBci : (3 * n - g) + (3 * n - g) + (4 * n - g - 60) =
      10 * n - 3 * g - 60 := by omega
  rw [hBBci] at hBBc
  have hB2c : (B ^ 2 * c).coeff (10 * n - 3 * g - 60) =
      B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 60) := by
    simpa only [pow_two] using hBBc
  have hce := coeff_mul_at_bounds68 c e (4 * n - g - 60)
    (6 * n - 2 * g) hc he
  have hcei : (4 * n - g - 60) + (6 * n - 2 * g) =
      10 * n - 3 * g - 60 := by omega
  rw [hcei] at hce
  have hd2 : (d ^ 2).natDegree < 10 * n - 3 * g - 60 := by
    compute_degree
    omega
...
theorem secondaryResidualRowOnePolynomial68_coeff_double_drop_sixty_of_fiveWall
    (A B c d e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hg : 60 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
    (hB : B.natDegree ≤ 3 * n - g)
    ...
    (secondaryResidualRowOnePolynomial68 A B c d e).coeff
      ...
      (-4 / 27 : k) *
        (2 * A.coeff (2 * n) ^ 2 * B.coeff (3 * n - g) *
            d.coeff (5 * n - 2 * g - 60) * ((5 * n - 60 : ℕ) : k) -
          6 * A.coeff (2 * n) * c.coeff (4 * n - g - 60) *
            e.coeff (6 * n - 2 * g) * ((5 * n - 60 : ℕ) : k) -
          4 * (n : k) * A.coeff (2 * n) *
            B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 60)) := by
  ...
  have hcast2 : (((2 * n : ℕ) : k)) = 2 * (n : k) := by
    push_cast
    rfl
  have hcast4 : (((4 * n - g - 60 : ℕ) : k)) =
      4 * (n : k) - (g : k) - 60 := by
    rw [Nat.cast_sub (by omega : 60 ≤ 4 * n - g),
      Nat.cast_sub (by omega : g ≤ 4 * n)]
    push_cast
    rfl
  have hcast5 : (((5 * n - 2 * g - 60 : ℕ) : k)) =
      5 * (n : k) - 2 * (g : k) - 60 := by
    rw [Nat.cast_sub (by omega : 60 ≤ 5 * n - 2 * g),
  ...
  have hcast6 : (((6 * n - 2 * g : ℕ) : k)) =
    ...
    push_cast
    rfl
  have hcastM : (((5 * n - 60 : ℕ) : k)) = 5 * (n : k) - 60 := by
...
theorem laterDeepLowerOpenDoubleFiveWall_degenerate_stride_inconsistent_edges68
  ...
  have hN : 7 * (n : k) - 60 ≠ 0 := by
    intro hz
    have hnat : 7 * n = 60 := by exact_mod_cast (sub_eq_zero.mp hz)
    omega
  have hcprod : a * b ^ 2 * c₁ * (7 * (n : k) - 60) = 0 := by
...
#print axioms natDegree_expand_le_sub_sixty_of_aligned_boundary_zero68
#print axioms secondaryResidualInvariantThreePolynomial68_coeff_double_drop_sixty_of_fiveWall
#print axioms secondaryResidualRowOnePolynomial68_coeff_double_drop_sixty_of_fiveWall
#print axioms laterDeepLowerOpenDoubleFiveWall_degenerate_stride_inconsistent_edges68

end LaterDeepLowerOpenDoubleFiveWallStrideAlgebra68

...
/-! # The first two moving-root rows through the residual weight-four scale

After the weight-one and weight-two loads have vanished, the bundled three-row coefficient
...
This file records their coefficient range below four times the Hahn scale.
-/
...
/-- Below the weight-four load, after the weight-one parameter has vanished,
...
theorem TransverseFactor.doubleRoot_row12Coeffs_zero_before_c5
    ...
    (e : ℚ) (he : 0 ≤ e) (hearly : 2 * T.delta + e < 4 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row12Coeffs_zero_before_c5

import Sol68LaterDeepLowerOpenDoubleFiveWallAlignedOneTwentyAlgebraScratch

/-! # Source closure of the aligned offset-one-twenty degenerate face -/
...
section LaterDeepLowerOpenDoubleFiveWallAlignedOneTwentyClosureSource68
...
theorem secondaryLoadInvariantThreePolynomial68_degree_lt_offsetOneTwenty_of_doubleFiveWall
    (gamma delta epsilon zeta : k) (A B c D e : k[X])
    (n g : ℕ) (hn : 0 < n) (hg : 120 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
...
theorem maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenDoubleFiveWallDegenerateAlignedOffsetOneTwenty68
    ...
    let g := cubicDefectGap68 n Be ce De ee
    let d := secondaryDDefectPolynomial68 Ae Be De
    ce.natDegree ≤ 4 * n - g - 180 ∧
      d.natDegree ≤ 5 * n - 2 * g - 180 := by
  let n := weightedRadius68 A B C0 D E
  let Ae := expand k 60 A
  let Be := expand k 60 B
  let Ce := expand k 60 C0
  ...
  change 180 ∣ n ∧
      (cubicLoadRowOnePolynomial68 0 alpha 0 gamma delta epsilon zeta eta
        Ae Be ce Dtr ee).natDegree < 12 * n - 3 * g - 121 at cutoff
  have hndiv : 180 ∣ n := cutoff.1
  have hn180 : 180 ≤ n := by
    obtain ⟨u, hu⟩ := hndiv
    omega
  have hg120 : 120 < g := by omega
  ...
  have hcoeff3 := congrArg
    (fun p : k[X] => p.coeff (10 * n - 3 * g - 120)) hi3c
  rw [cubicFirstIntegralThreePolynomial68_secondaryLoadSplit, coeff_add,
    ...
    secondaryResidualInvariantThreePolynomial68_coeff_double_drop_oneTwenty_of_fiveWall
      Ae Be ce d ee n g hn hg120 hwall hA secondary.2.1 descent.1 descent.2 he2,
    coeff_eq_zero_of_natDegree_lt
      (secondaryLoadInvariantThreePolynomial68_degree_lt_offsetOneTwenty_of_doubleFiveWall
        gamma delta epsilon zeta Ae Be ce Dtr ee n g hn hg120 hwall
        hA secondary.2.1 secondary.2.2.1 hD he2), add_zero] at hcoeff3
  have hidx3 : 10 * n - 3 * g - 120 ≠ 0 := by omega
  simp only [coeff_C, if_neg hidx3] at hcoeff3
  have hi3scalar :
      -Ae.coeff (2 * n) * Be.coeff (3 * n - g) *
          d.coeff (5 * n - 2 * g - 120) -
        Be.coeff (3 * n - g) ^ 2 * ce.coeff (4 * n - g - 120) +
        3 * ce.coeff (4 * n - g - 120) * ee.coeff (6 * n - 2 * g) = 0 := by
  ...
  have hcoeffOne := congrArg
    (fun p : k[X] => p.coeff (12 * n - 3 * g - 121)) hrowPoly
  rw [coeff_add, coeff_zero,
    coeff_eq_zero_of_natDegree_lt cutoff.2, add_zero,
    secondaryResidualRowOnePolynomial68_coeff_double_drop_oneTwenty_of_fiveWall
      Ae Be ce d ee n g hn hg120 hwall hA secondary.2.1 descent.1 descent.2 he2]
    at hcoeffOne
  have hrowScalar :
      2 * Ae.coeff (2 * n) ^ 2 * Be.coeff (3 * n - g) *
          d.coeff (5 * n - 2 * g - 120) * (5 * (n : k) - 120) -
        6 * Ae.coeff (2 * n) * ce.coeff (4 * n - g - 120) *
          ee.coeff (6 * n - 2 * g) * (5 * (n : k) - 120) -
        4 * (n : k) * Ae.coeff (2 * n) * Be.coeff (3 * n - g) ^ 2 *
          ce.coeff (4 * n - g - 120) = 0 := by
    ...
    rw [hcastM] at ht
    exact ht
  have edges := laterDeepLowerOpenDoubleFiveWall_degenerate_offsetOneTwenty_edges_zero68
    n (Ae.coeff (2 * n)) (Be.coeff (3 * n - g))
    (ce.coeff (4 * n - g - 120)) (d.coeff (5 * n - 2 * g - 120))
    (ee.coeff (6 * n - 2 * g)) hn ha hdegenerate.2.1 hi3scalar hrowScalar
  have hcAlign120 : 60 ∣ 4 * n - g - 120 := by
    obtain ⟨u, hu⟩ := hcAlign
    refine ⟨u - 2, ?_⟩
    omega
  have hdAlign120 : 60 ∣ 5 * n - 2 * g - 120 := by
    obtain ⟨u, hu⟩ := hdAlign
    refine ⟨u - 2, ?_⟩
    omega
  constructor
  · have ht : ce.natDegree ≤ (4 * n - g - 120) - 60 := by
      rw [hceexpand]
      apply natDegree_expand_le_sub_sixty_of_aligned_boundary_zero68
        (cubicCDefectPolynomial68 A C0) (4 * n - g - 120) hcAlign120
        (by omega)
      · simpa only [← hceexpand] using descent.1
      · rw [← hceexpand]
        exact edges.1
    omega
  · have ht : d.natDegree ≤ (5 * n - 2 * g - 120) - 60 := by
      rw [hdexpand]
      apply natDegree_expand_le_sub_sixty_of_aligned_boundary_zero68
        (secondaryDDefectPolynomial68 A B D) (5 * n - 2 * g - 120)
        hdAlign120 (by omega)
      · simpa only [← hdexpand] using descent.2
      · rw [← hdexpand]
        exact edges.2
    omega

#print axioms secondaryLoadInvariantThreePolynomial68_degree_lt_offsetOneTwenty_of_doubleFiveWall
#print axioms maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenDoubleFiveWallDegenerateAlignedOffsetOneTwenty68

end LaterDeepLowerOpenDoubleFiveWallAlignedOneTwentyClosureSource68

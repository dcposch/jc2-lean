...
/-! # Early transverse jets before the first weight-four load

If a common-cubic transverse jet occurs before the weight-four `c5` scale,
and the weight-one and weight-two parameters have already vanished, then
every remaining source and target load disappears in the corresponding
higher-order quotient.  The four complete Faber equations therefore reduce
to the universal zero-load normal rows.  A nonzero first jet consequently
forces the limiting cubic onto its double-root discriminant.
-/
...
/-- Below the weight-four scale, after the weight-one and weight-two loads
have vanished, the complete source equations have exactly the
zero-high-constant leading coefficients. -/
theorem zeroHighNCoeff_zero_before_c5
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (hd : F.d = 0)
    (hc7 : F.c7 = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 4 * F.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 4 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (4 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (4 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
...
/-- Any transverse factor strictly before the first weight-four load, after
the weight-one and weight-two parameters have vanished, satisfies the four
universal zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_c5
    {k : Type*} [Field k] [CharZero k]
...
/-- A nonzero first transverse jet before the `c5` scale, after the
weight-one and weight-two parameters have vanished, forces the limiting
common cubic to be singular. -/
theorem TransverseScale.discriminant_zero_before_c5
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_c5 F hscale
    hd hc7 hearly
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_c5
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_c5
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_c5

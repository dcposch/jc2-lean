...
/-! # The strict active-`c5` singular source branch

The leading zero-load rows put every singular common cubic on its standard
double-root parametrization.  The active weight-four contradiction depends
...
/-- If `c5` is the first nonzero Faber parameter and its load lies strictly
between the leading quadratic and intrinsic cubic scales, the singular
finite source branch is inconsistent. -/
theorem TransverseScale.singular_source_inconsistent_at_active_c5_of_lt
  ...
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.factor.zeroLoadLeadingRows_before_c5 F hscale hd0 hc70
  ...
  exact T.factor.doubleRoot_c5Load_source_inconsistent_of_lt
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseScale.singular_source_inconsistent_at_active_c5_of_lt

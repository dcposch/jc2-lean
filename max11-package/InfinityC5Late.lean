...
/-! # The strict-late weight-four cell at infinity

After `d=c7=0`, every remaining source or target load starts at weight at
least four.  Thus, when `3δ < 4p`, the intrinsic cubic transverse term is
...
/-- Below the weight-four scale, after `d=c7=0`, the four complete source
equations reduce modulo the cutoff to their zero-load numerators. -/
theorem zeroLoadN_mem_higherOrderIdeal_of_lt_four_p
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (e : ℚ) (he : 0 ≤ e) (hlt : e < 4 * F.scale.p)
  ...
  have hc5q := hkill 4 (by norm_num) F.c5
...
/-- Below the weight-four scale, the complete terminal numerator differs
from the zero-load specialization only by terms strictly after the cutoff. -/
theorem fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_four_p
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (e : ℚ) (he : 0 ≤ e) (hlt : e < 4 * F.scale.p)
    (hd : F.d = GCD369CubeHahnRegular.constant 0)
...
/-- Before the weight-four load, vanishing of `d` and `c7` gives the four
universal zero-load normal rows at the canonical transverse scale. -/
theorem TransverseScale.zeroLoadLeadingRows_before_c5
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ :=
    D.faber.zeroLoadN_mem_higherOrderIdeal_of_lt_four_p
...
weight-four term starts strictly after the intrinsic cubic term, the latter
is the exact first term of the fifth numerator. -/
theorem fullN5_orderTop_eq_three_delta_of_singular_c5_late
    {k : Type*} [Field k] [CharZero k]
    ...
    (hlate : 3 * T.delta < 4 * D.faber.scale.p) :
    (GCD369CubeFaberFullN5
      D.faber.scale.regular0 D.faber.scale.regular1
      D.faber.scale.regular2 D.faber.scale.regular3
  ...
  have hNdiff : N - N0 ∈ I := by
    simpa only [N, N0, dload] using
      D.faber.fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_four_p
        (3 * T.delta) he hlate hd hc7
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Source-facing weight-four late-cell contradiction once the singular
transverse residue is in its standard double-root chart. -/
theorem infinity_c5_singular_late_inconsistent_of_parametrization
  ...
  obtain ⟨hdraw, hc7raw, _hc5, _hc4, _hc3, _hc2, _hc1,
      _hr1, _hr2, _hr3, _hr4⟩ :=
  ...
  have hc7 : D.faber.c7 = GCD369CubeHahnRegular.constant 0 := by
    ...
    exact h
  have hNorder :=
    GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_c5_late
      D T hd hc7 r A hr hA hu hX hY hZ hlate
  have hterminalOrder :
      (GCD369CubeFaberR5
  ...
  apply S.infinity_terminal_three_delta_not_before_load_of_singular
    hsdegree T 4 (by norm_num) hdisc hlate
  simpa only [hpole, D, S.infinityCommonCoefficientData_faber hsdegree]
    using hscaledOrder

/-- After `d=c7=0`, the strict region `3δ < 4p` is inconsistent for every
constant-core infinity source. -/
theorem infinity_c5_singular_late_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
    (hdP : S.faberConstantParameters.d = 0)
  ...
  have hearly : 2 * T.delta < 4 * D.faber.scale.p := by
    linarith [T.hdelta]
  obtain ⟨h1, h2, h3, h4⟩ :=
    T.zeroLoadLeadingRows_before_c5 hd hc7 hearly
  ...
  have hdisc : 4 * D.u ^ 3 + 27 * D.v ^ 2 = 0 := by
    rw [hu, hv]
    ring
  exact S.infinity_c5_singular_late_inconsistent_of_parametrization
    hsdegree T hdP hc7P hdisc r
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
...
#print axioms
  GCD369CubeHahnFaberPoleData.zeroLoadN_mem_higherOrderIdeal_of_lt_four_p
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5_sub_zeroSpecialized_mem_higherOrderIdeal_of_lt_four_p
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.zeroLoadLeadingRows_before_c5
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5_orderTop_eq_three_delta_of_singular_c5_late
#print axioms
  GCD369CubePolynomialSource.infinity_c5_singular_late_inconsistent

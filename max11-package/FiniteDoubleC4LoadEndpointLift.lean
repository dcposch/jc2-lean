...
import FiniteDoubleC4LoadEndpointCoeff

/-! # Closing the balanced `c4` resonance from exact quadratic coordinates

At the equality `5 * p = 3 * delta`, the residual load exponent is the
intrinsic half-scale `delta`.  Given the exact quadratic factorization of
the moving-root deviations, the four literal `c4` equations reduce to the
...
/-- At the balanced active-`c4` resonance, exact quadratic coordinates with
vanishing half-scale `B` residue make the loaded source equations
inconsistent. -/
theorem TransverseFactor.doubleRoot_c4Load_source_inconsistent_of_eq
  ...
  have hY' : GCD369CubeHahnRegular.constantCoeff T.Yn =
    ...
    rw [hr]
    exact hY
  have hload := T.doubleRoot_c4Load_source_coeff_of_eq F hscale
  ...
  apply GCD369CubeDoubleBlowup_c4Load_balanced_resonance_Bzero_inconsistent
    A0 (GCD369CubeHahnRegular.constantCoeff Bn)
      (GCD369CubeHahnRegular.constantCoeff q) r0
      (GCD369CubeHahnRegular.constantCoeff d) C4 hA0 hr0 hBn0
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_c4Load_source_inconsistent_of_eq

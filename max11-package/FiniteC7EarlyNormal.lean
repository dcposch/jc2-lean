...
/-! # Early transverse jets before the first weight-two load

If a common-cubic transverse jet occurs before the weight-two `c7` scale,
and the weight-one parameter has already vanished, then every remaining
source and target load disappears in the corresponding higher-order
quotient.  The four complete Faber equations therefore reduce to the
universal zero-load normal rows.  A nonzero first jet consequently
...
/-- Below the weight-two scale, after the weight-one load has vanished, the
complete source equations have exactly the zero-high-constant leading
coefficients. -/
theorem zeroHighNCoeff_zero_before_c7
    {k : Type*} [Field k] [CharZero k]
    (F : GCD369CubeHahnFaberPoleData k)
    (hd : F.d = 0)
    (e : ℚ) (he : 0 ≤ e) (hearly : e < 2 * F.scale.p) :
  ...
  have hweight (w : ℕ) (hw : 2 ≤ w) :
      e < (w : ℚ) * F.scale.p := by
    calc
      e < (2 : ℚ) * F.scale.p := by simpa using hearly
      _ ≤ (w : ℚ) * F.scale.p := by
        exact mul_le_mul_of_nonneg_right
          (show (2 : ℚ) ≤ (w : ℚ) by exact_mod_cast hw) F.scale.hp.le
  ...
  have hdq : q (F.scale.weightedRegular 1 F.d) = 0 := by
    have hzw : F.scale.weightedRegular 1 F.d = 0 := by
      ...
      simp
    rw [hzw, map_zero]
  have hc7q := hkill 2 (by norm_num) F.c7
...
/-- Any transverse factor strictly before the first weight-two load, after
the weight-one parameter has vanished, satisfies the four universal
zero-load normal equations on its residue jet. -/
theorem TransverseFactor.zeroLoadLeadingRows_before_c7
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    (hd : F.d = 0)
    (hearly : 2 * T.delta < 2 * F.scale.p) :
  ...
  have hz := F.zeroHighNCoeff_zero_before_c7 hd
...
/-- A nonzero first transverse jet before the `c7` scale, after the
weight-one parameter has vanished, forces the limiting common cubic to be
singular. -/
theorem TransverseScale.discriminant_zero_before_c7
    {k : Type*} [Field k] [CharZero k]
  ...
  obtain ⟨h1, h2, h3, h4⟩ := T.factor.zeroLoadLeadingRows_before_c7 F hscale
    hd hearly
...
#print axioms GCD369CubeHahnFaberPoleData.zeroHighNCoeff_zero_before_c7
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.zeroLoadLeadingRows_before_c7
#print axioms GCD369CubeHahnCommonValueData.TransverseScale.discriminant_zero_before_c7

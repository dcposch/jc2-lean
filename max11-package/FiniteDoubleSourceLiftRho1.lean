import FiniteDoubleSourceChartsRho1
import FiniteDoubleLiftSourceRho1

/-! # Source-facing double-root exclusion before `rho1` -/
...
double-root source chart below the weight-ten load. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_rho1
  ...
  exact T.doubleRoot_source_inconsistent_of_coordinates_before_rho1
...
which exclude the double-root source coordinate before `rho1`. -/
theorem TransverseFactor.doubleRoot_source_inconsistent_from_source_before_rho1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
  ...
  obtain ⟨hDfull, B, hYcoord, hQfull⟩ :=
    T.doubleRoot_deviations_order_delta_from_source_before_rho1
      F hscale hd hc7 hc5 hc4 hc3 hc2 hc1 hp r r0 A0 hr0 hA0 hr hu hX hY
      hZ hroot
  have hpS : 3 * T.delta ≤ 10 * S.normal.sextic.scale.p := by
    rw [hscale]
    exact hp
  exact T.doubleRoot_source_inconsistent_of_orderBounds_before_rho1
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_of_orderBounds_before_rho1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_source_inconsistent_from_source_before_rho1

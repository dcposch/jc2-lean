...
/-! # The first two moving-root rows through the residual weight-eight scale
...
enters the third row there.  The first two rows have no such term.  This
file records their coefficient range below eight times the Hahn scale,
together with the leading zero-load first-row residue.
-/
...
vanished, the first universal zero-load normal row vanishes on the residue
jet. -/
theorem TransverseFactor.doubleRoot_zeroLoad_row1Coeff_zero_before_c1
...
/-- Below the weight-eight load, after the weight-one, weight-two,
weight-four, weight-five, weight-six, and weight-seven parameters have
vanished, the first two exact moving-root row coefficients vanish at every
nonnegative residual exponent.  Unlike the three-row version, no upper
bound by `delta` is required. -/
theorem TransverseFactor.doubleRoot_row12Coeffs_zero_before_c1
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
    ...
    (e : ℚ) (he : 0 ≤ e) (hearly : e + 2 * T.delta < 8 * F.scale.p) :
    let A := T.Xn
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_zeroLoad_row1Coeff_zero_before_c1
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_row12Coeffs_zero_before_c1

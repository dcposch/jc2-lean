...
/-! # Weight-eight `c1` common-root lifting identities

Once the three normal coordinates reach half the weight-eight pole scale,
the four literal scaled Faber equations produce the exact arbitrary-`c1`
...
the four literal scaled Faber equations produce the exact arbitrary-`c1`
Kuranishi packet on their residues. -/
theorem c1LeadingFaberRowsAt_of_bounds
    ...
    (hnormal2 : (↑((8 * F.scale.p) / 2) : WithTop ℚ) ≤
      S.normal2.1.orderTop)
    (hnormal1 : (↑((8 * F.scale.p) / 2) : WithTop ℚ) ≤
      S.normal1.1.orderTop)
    (hnormal0 : (↑((8 * F.scale.p) / 2) : WithTop ℚ) ≤
      S.normal0.1.orderTop) :
    let T := S.transverseFactorOfBounds ((8 * F.scale.p) / 2)
  ...
  let T := S.transverseFactorOfBounds ((8 * F.scale.p) / 2)
    (div_pos (mul_pos (by norm_num) F.scale.hp) (by norm_num))
    hnormal2 hnormal1 hnormal0
  apply T.c1LeadingFaberRowsAt C1
  · have hz := (F.c1NCoeff_zero R1 R2 R3 R4
      hd hc7 hc5 hc4 hc3 hc2 hr1 hr2 hr3 hr4).1
    rw [hc1,
      F.scale.weightedRegular_constant_eq_monomial_sq 8 (by norm_num) C1] at hz
    have hTdelta : T.delta = (8 * F.scale.p) / 2 := rfl
    simp only [hTdelta]
    rw [hscale]
...
#print axioms GCD369CubeHahnCommonValueData.c1LeadingFaberRowsAt_of_bounds

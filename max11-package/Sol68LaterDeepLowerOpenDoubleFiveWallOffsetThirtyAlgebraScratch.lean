import Sol68LaterDeepLowerOpenDoubleFiveWallStrideSourceScratch

/-! # The offset-thirty face on the degenerate double five wall -/
...
section LaterDeepLowerOpenDoubleFiveWallOffsetThirtyAlgebra68
...
/-- An expanded polynomial missing a boundary on the thirty-lattice drops
at least thirty degrees. -/
theorem natDegree_expand_le_sub_thirty_of_boundary_zero68
    (q : k[X]) (base : ℕ) (hbase : 30 ∣ base) (hthirty : 30 ≤ base)
    (hdeg : (expand k 60 q).natDegree ≤ base)
    (htop : (expand k 60 q).coeff base = 0) :
    (expand k 60 q).natDegree ≤ base - 30 := by
...
theorem secondaryResidualInvariantThreePolynomial68_coeff_double_drop_thirty_of_fiveWall
    (A B c d e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hg : 30 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
    (hB : B.natDegree ≤ 3 * n - g)
    (hc : c.natDegree ≤ 4 * n - g - 30)
    (hd : d.natDegree ≤ 5 * n - 2 * g - 30)
    ...
    (secondaryResidualInvariantThreePolynomial68 A B c d e).coeff
      ...
      (8 / 9 : k) *
        (-(A.coeff (2 * n) * B.coeff (3 * n - g) *
            d.coeff (5 * n - 2 * g - 30)) -
          B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 30) +
          3 * c.coeff (4 * n - g - 30) *
            e.coeff (6 * n - 2 * g)) := by
  have hABd := coeff_mul_mul_at_bounds68 A B d (2 * n)
    (3 * n - g) (5 * n - 2 * g - 30) hA hB hd
  have hABdi : 2 * n + (3 * n - g) + (5 * n - 2 * g - 30) =
      10 * n - 3 * g - 30 := by omega
  rw [hABdi] at hABd
  have hBBc := coeff_mul_mul_at_bounds68 B B c (3 * n - g)
    (3 * n - g) (4 * n - g - 30) hB hB hc
  have hBBci : (3 * n - g) + (3 * n - g) + (4 * n - g - 30) =
      10 * n - 3 * g - 30 := by omega
  rw [hBBci] at hBBc
  have hB2c : (B ^ 2 * c).coeff (10 * n - 3 * g - 30) =
      B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 30) := by
    simpa only [pow_two] using hBBc
  have hce := coeff_mul_at_bounds68 c e (4 * n - g - 30)
    (6 * n - 2 * g) hc he
  have hcei : (4 * n - g - 30) + (6 * n - 2 * g) =
      10 * n - 3 * g - 30 := by omega
  rw [hcei] at hce
  have hd2 : (d ^ 2).natDegree < 10 * n - 3 * g - 30 := by
    compute_degree
    omega
...
theorem secondaryResidualRowOnePolynomial68_coeff_double_drop_thirty_of_fiveWall
    (A B c d e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hg : 30 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
    (hB : B.natDegree ≤ 3 * n - g)
    ...
    (secondaryResidualRowOnePolynomial68 A B c d e).coeff
      ...
      (-4 / 27 : k) *
        (2 * A.coeff (2 * n) ^ 2 * B.coeff (3 * n - g) *
            d.coeff (5 * n - 2 * g - 30) * ((5 * n - 30 : ℕ) : k) -
          6 * A.coeff (2 * n) * c.coeff (4 * n - g - 30) *
            e.coeff (6 * n - 2 * g) * ((5 * n - 30 : ℕ) : k) -
          4 * (n : k) * A.coeff (2 * n) *
            B.coeff (3 * n - g) ^ 2 * c.coeff (4 * n - g - 30)) := by
  ...
  have hcast4 : (((4 * n - g - 30 : ℕ) : k)) =
      4 * (n : k) - (g : k) - 30 := by
    rw [Nat.cast_sub (by omega : 30 ≤ 4 * n - g),
      Nat.cast_sub (by omega : g ≤ 4 * n)]; push_cast; rfl
  have hcast5 : (((5 * n - 2 * g - 30 : ℕ) : k)) =
      5 * (n : k) - 2 * (g : k) - 30 := by
    rw [Nat.cast_sub (by omega : 30 ≤ 5 * n - 2 * g),
      Nat.cast_sub (by omega : 2 * g ≤ 5 * n)]; push_cast; rfl
  have hcast6 : (((6 * n - 2 * g : ℕ) : k)) =
      6 * (n : k) - 2 * (g : k) := by
    rw [Nat.cast_sub (by omega : 2 * g ≤ 6 * n)]; push_cast; rfl
  have hcastM : (((5 * n - 30 : ℕ) : k)) = 5 * (n : k) - 30 := by
    rw [Nat.cast_sub (by omega : 30 ≤ 5 * n)]; push_cast; rfl
...
theorem laterDeepLowerOpenDoubleFiveWall_degenerate_offsetThirty_edges_zero68
  ...
  have hN : 7 * (n : k) - 30 ≠ 0 := by
    intro hz
    have hnat : 7 * n = 30 := by exact_mod_cast (sub_eq_zero.mp hz)
    omega
  have hcprod : a * b ^ 2 * c₁ * (7 * (n : k) - 30) = 0 := by
...
#print axioms natDegree_expand_le_sub_thirty_of_boundary_zero68
#print axioms secondaryResidualInvariantThreePolynomial68_coeff_double_drop_thirty_of_fiveWall
#print axioms secondaryResidualRowOnePolynomial68_coeff_double_drop_thirty_of_fiveWall
#print axioms laterDeepLowerOpenDoubleFiveWall_degenerate_offsetThirty_edges_zero68

end LaterDeepLowerOpenDoubleFiveWallOffsetThirtyAlgebra68

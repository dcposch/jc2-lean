...
/-! # The balanced singular rho2 cell at infinity

After all source loads and rho1 vanish, the weight-eleven rho2 load and
the intrinsic cubic meet the quadratic moving-root correction at
`11p = 3δ`.
-/

noncomputable section

/-- At the balanced rho2 resonance, the unloaded first and fourth rows and
the singular terminal row are already inconsistent.  Thus the sparse rho2
load in row two need not enter the terminal certificate. -/
theorem GCD369Cube_rho2Load_balanced_terminal_inconsistent
...
/-- At `11p=3δ`, the zero-load terminal numerator has the displayed
intrinsic cubic coefficient. -/
theorem TransverseScale.rho2ZeroHighTerminalCoeff_at_balanced
    {k : Type*} [Field k] [CharZero k]
    ...
    (htie : 11 * D.faber.scale.p = 3 * T.delta)
  ...
  change (H ^ 3 * (839808 * C)).1.coeff (3 * T.delta) =
      GCD369CubeHahnRegular.constantCoeff (839808 * C) at htop
  change
    (GCD369CubeFaberFullN5
      D.faber.scale.regular0 D.faber.scale.regular1
      D.faber.scale.regular2 D.faber.scale.regular3
      ...
      ((559872 * R5) * H ^ 2).1.coeff (3 * T.delta) +
        (H ^ 3 * (839808 * C)).1.coeff (3 * T.delta) at hc
  ...
  rw [hnat, hR5]
  simp only [map_mul, map_ofNat, hC]
  ring
...
theorem fullN5Coeff_eq_rho2BalancedSpecialized
    {k : Type*} [Field k] [CharZero k]
    ...
    (GCD369CubeFaberFullN5
      ...
      (F.scale.weightedRegular 8 F.c1)).1.coeff (11 * F.scale.p) =
    (GCD369CubeFaberFullN5
      F.scale.regular0 F.scale.regular1 F.scale.regular2
      F.scale.regular3 F.scale.regular4 0 0 0 0 0 0 0).1.coeff
        (11 * F.scale.p) := by
...
theorem TransverseScale.rho2LoadedTerminalEquation_at_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    ...
    559872 * r ^ 4 * (-A ^ 2 * d + 21 * b ^ 2 + 6 * A * q) -
      2519424 * r ^ 2 * A ^ 3 = 0 := by
  rw [D.faber.fullN5Coeff_eq_rho2BalancedSpecialized
    hd hc7 hc5 hc4 hc3 hc2 hc1] at hzero
  rw [T.rho2ZeroHighTerminalCoeff_at_balanced
    r A d b q hu hv hX hY hZ htie
    hord1 hord2 hcoeff1 hcoeff2] at hzero
  exact hzero

/-- Before rho2, all earlier source and target loads zero force the four
universal leading normal rows. -/
theorem TransverseScale.zeroLoadLeadingRows_before_rho2_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (hd : D.faber.d = 0) (hc7 : D.faber.c7 = 0)
    (hc5 : D.faber.c5 = 0) (hc4 : D.faber.c4 = 0)
    (hc3 : D.faber.c3 = 0) (hc2 : D.faber.c2 = 0)
    (hc1 : D.faber.c1 = 0) (hr1 : D.faber.rho1 = 0)
  ...
  have hz := D.faber.zeroHighNCoeff_zero_before_rho2
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Rows one, two, and four at the balanced rho2 resonance. -/
theorem TransverseScale.rho2LoadedNormalRowsAt_balanced
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (Rho2 Rho3 Rho4 : k)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    ...
    (htie : 11 * D.faber.scale.p = 3 * T.delta) :
    ((-32 * GCD369CubeNormalRow1
        T.Xn T.Yn T.Zn D.cubicU).1.coeff T.delta = 0) ∧
    ((32 * GCD369CubeNormalRow2
  ...
  have hz := F.rho2NCoeff Rho2 Rho3 Rho4
  ...
  have hcubicval :
      ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (11 * F.scale.p) = 314928 * r * A0 ^ 3 := by
    rw [htie, hcubic]
    simp only [map_mul, map_pow, map_ofNat, hY, hX]
    ring
  change (729 * N1).1.coeff (11 * F.scale.p) =
      ((-32 * Row1) * H ^ 2).1.coeff (11 * F.scale.p) at hc1'
  change (2187 * N2).1.coeff (11 * F.scale.p) =
      ((32 * Row2) * H ^ 2).1.coeff (11 * F.scale.p) at hc2'
  change (6561 * N4).1.coeff (11 * F.scale.p) =
      ((32 * Row4) * H ^ 2).1.coeff (11 * F.scale.p) -
        ((314928 * T.Yn * T.Xn ^ 2) * H ^ 3).1.coeff
          (11 * F.scale.p) at hc4'
  ...
  rw [htie, show 3 * T.delta = T.delta + (2 : ℚ) * T.delta by ring,
    hshift4] at hc4'
  dsimp only [F] at hz1 hz2 hz4
  norm_num only [Nat.cast_ofNat] at hz1 hz2 hz4
...
/-- Before rho2, the first three moving-root row coefficients vanish below
the intrinsic residual cutoff. -/
theorem TransverseFactor.doubleRoot_row123Coeffs_zero_before_rho2_balanced
    {k : Type*} [Field k] [CharZero k]
  ...
  change N3.1.coeff (2 * T.delta + e) =
    ((128 * R3) * H ^ 2).1.coeff (2 * T.delta + e) -
      ((64 * A ^ 3) * H ^ 3).1.coeff (2 * T.delta + e) at hc3row
  rw [show 2 * T.delta + e = e + (2 : ℚ) * T.delta by ring,
    hshift3] at hc3row
  rw [show e + (2 : ℚ) * T.delta =
      (e - T.delta) + (3 : ℚ) * T.delta by ring,
    hcubicShift, hcubicZero, sub_zero] at hc3row
  ...
  have hz3scaled : (128 * R3).1.coeff e = 0 := by
    calc
      _ = N3.1.coeff (e - T.delta + 3 * T.delta) := hc3row.symm
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- Global degree integrality supplies the balanced rho2 terminal row. -/
theorem infinity_rho2LoadedTerminalEquation_at_balanced
    ...
    (hY : GCD369CubeHahnRegular.constantCoeff T.Yn = r * A)
    (hZ : GCD369CubeHahnRegular.constantCoeff T.Zn = -2 * r ^ 2 * A)
    (htie : 11 * (S.infinityCommonCoefficientData hsdegree).faber.scale.p =
      3 * T.delta)
    (hord1 : (↑T.delta : WithTop ℚ) ≤
  ...
  have hzero := S.infinity_fullN5Coeff_weight_p_eq_zero_of_singular
    hsdegree 11 (by norm_num) hdisc
  exact T.rho2LoadedTerminalEquation_at_balanced
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- The singular balanced rho2 cell is impossible. -/
theorem infinity_rho2_singular_balanced_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (T : (S.infinityCommonCoefficientData hsdegree).TransverseScale)
  ...
  have hc1Dz : D.faber.c1 = 0 := by simpa using hc1D
  have hr1Dz : D.faber.rho1 = 0 := by simpa using hr1D
  have hearly : 2 * T.delta < 11 * D.faber.scale.p := by
    rw [htie]
    linarith [T.hdelta]
  obtain ⟨hz1, hz2, hz3, hz4⟩ :=
    T.zeroLoadLeadingRows_before_rho2_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz hc1Dz hr1Dz hearly
  ...
  have hrows : ∀ e : ℚ, 0 ≤ e → e < T.delta →
    ...
    intro e he hedelta
    have hearly' : 2 * T.factor.delta + e < 11 * D.faber.scale.p := by
      change 2 * T.delta + e < 11 * D.faber.scale.p
      rw [htie]
      linarith
    have h := T.factor.doubleRoot_row123Coeffs_zero_before_rho2_balanced
      hdDz hc7Dz hc5Dz hc4Dz hc3Dz hc2Dz hc1Dz hr1Dz
      rr hroot e he hedelta hearly'
  ...
  obtain ⟨hload1, hload2, hload4⟩ := T.rho2LoadedNormalRowsAt_balanced
    R.rho2 R.rho3 R.rho4
  ...
  have h5 := S.infinity_rho2LoadedTerminalEquation_at_balanced
    hsdegree hdisc T
    hdD hc7D hc5D hc4D hc3D hc2D hc1D
    r A0 d0 b0 q0 hu hv rfl hY hZ htie hord1 hord2 hcoeff1 hcoeff2
  apply GCD369Cube_rho2Load_balanced_terminal_inconsistent
    A0 r d0 b0 q0 R.rho2 hA0 hr
  · linear_combination hload1
  · linear_combination hload2
  · linear_combination hload4
  · exact h5

/-- Target-normalized source wrapper for the balanced rho2 resonance. -/
theorem targetNormalized_infinity_rho2_singular_balanced_inconsistent
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (hd0 : S.faberConstantParameters.d = 0)
  ...
  apply U.infinity_rho2_singular_balanced_inconsistent hsdegreeU T
...
#print axioms GCD369Cube_rho2Load_balanced_terminal_inconsistent
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho2ZeroHighTerminalCoeff_at_balanced
#print axioms
  GCD369CubeHahnFaberPoleData.fullN5Coeff_eq_rho2BalancedSpecialized
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho2LoadedTerminalEquation_at_balanced
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho2LoadedNormalRowsAt_balanced
#print axioms
  GCD369CubePolynomialSource.infinity_rho2LoadedTerminalEquation_at_balanced
#print axioms
  GCD369CubePolynomialSource.infinity_rho2_singular_balanced_inconsistent
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_rho2_singular_balanced_inconsistent

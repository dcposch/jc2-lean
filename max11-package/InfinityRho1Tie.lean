...
/-! # The exact weight-ten `rho1`/transverse tie at infinity

After `d=c7=c5=c4=c3=c2=c1=0`, the first target load is `rho1`.
At `10p = 2δ`, its loaded first row and the three zero-loaded rows occur at
the quadratic transverse scale.  They force a nonzero quadratic terminal
coefficient, hence exact terminal order `10p`, which the infinity degree
landing excludes.
-/
...
/-- The four primitive quadratic Hahn coefficients with first-row value
`62208 * R` are exactly the arbitrary-`rho1` Faber packet. -/
theorem TransverseFactor.rhoOneTieLeadingFaberRowsAt
...
/-- At the exact `rho1`/transverse tie, the four source equations produce
the arbitrary-`rho1` row packet on the infinity residue jet. -/
theorem TransverseScale.rhoOneLeadingFaberRowsAt_at_transverse
    {k : Type*} [Field k] [CharZero k]
    ...
    (htie : 10 * D.faber.scale.p = 2 * T.delta) :
    GCD369CubeRhoOneLeadingFaberRowsAt
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
      (GCD369CubeHahnRegular.constantCoeff T.Yn)
  ...
  have heq : ((10 : ℕ) : ℚ) * D.faber.scale.p =
      ((2 : ℕ) : ℚ) * T.delta := by
    norm_num
    exact htie
  rw [heq] at hz1 hz2 hz3 hz4
  exact T.factor.rhoOneTieLeadingFaberRowsAt R1 hz1 hz2 hz3 hz4
...
theorem TransverseScale.rho1ZeroHighTerminalCoeff_at_transverse
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (htie : 10 * D.faber.scale.p = 2 * T.delta) :
...
/-- A nonzero tied `rho1` load forces the common-cubic coordinate `v` to
vanish. -/
theorem TransverseScale.v_eq_zero_of_rho1_tie
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (R1 R2 R3 R4 : k) (hR1 : R1 ≠ 0)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    (hc7 : D.faber.c7 = GCD369CubeHahnRegular.constant 0)
    (hc5 : D.faber.c5 = GCD369CubeHahnRegular.constant 0)
    (hc4 : D.faber.c4 = GCD369CubeHahnRegular.constant 0)
    ...
    (htie : 10 * D.faber.scale.p = 2 * T.delta) :
    D.v = 0 := by
  have hrows := T.rhoOneLeadingFaberRowsAt_at_transverse
    R1 R2 R3 R4 hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  dsimp [GCD369CubeRhoOneLeadingFaberRowsAt] at hrows
  ...
  have h1 : GCD369CubeNormalRow1
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
      (GCD369CubeHahnRegular.constantCoeff T.Yn)
      (GCD369CubeHahnRegular.constantCoeff T.Zn) D.u + 1944 * R1 = 0 := by
    linear_combination (-1 / 32 : k) * hf1
  ...
  have hRv : R1 * D.v = 0 := by
    dsimp only [GCD369CubeNormalRow1, GCD369CubeNormalRow2,
      GCD369CubeNormalRow4] at h1 h2 h4
    linear_combination
      (1 / 1944 * D.v) * h1 + (-1 / 11664 * D.u) * h2 +
        (-1 / 34992) * h4
  exact (mul_eq_zero.mp hRv).resolve_left hR1

/-- Consequently the projective common cubic lies on the `u ≠ 0` chart. -/
theorem TransverseScale.u_ne_zero_of_rho1_tie
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (R1 R2 R3 R4 : k) (hR1 : R1 ≠ 0)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    ...
    (htie : 10 * D.faber.scale.p = 2 * T.delta) :
    D.u ≠ 0 := by
  have hv := T.v_eq_zero_of_rho1_tie R1 R2 R3 R4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  exact D.hprojective.resolve_right (fun h => h hv)

/-- The zero-high terminal quadratic row on the transverse residue jet. -/
def rho1TerminalAt {k : Type*} [Field k] [CharZero k]
    (D : GCD369CubeHahnCommonCoefficientData k)
    (T : D.TransverseScale) : k :=
  GCD369CubeNormalRow5
    (GCD369CubeHahnRegular.constantCoeff T.Xn)
    (GCD369CubeHahnRegular.constantCoeff T.Yn)
    (GCD369CubeHahnRegular.constantCoeff T.Zn) D.u D.v

/-- The tied `rho1` packet forces a nonzero zero-high terminal coefficient. -/
theorem TransverseScale.rho1TerminalAt_nonzero_at_transverse
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (R1 R2 R3 R4 : k) (hR1 : R1 ≠ 0)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
  ...
  have hcoeff := GCD369CubeFaberCommonNormalCoefficients
    ...
    (GCD369CubeHahnRegular.constantCoeff T.Zn) D.u D.v
  dsimp only at hcoeff
  rcases hrows with ⟨hf1, _hf2, _hf3, _hf4⟩
  rcases hcoeff with ⟨hc1', _hc2', _hc3', _hc4'⟩
  rw [hc1'] at hf1
  have h1 : GCD369CubeNormalRow1
      (GCD369CubeHahnRegular.constantCoeff T.Xn)
      (GCD369CubeHahnRegular.constantCoeff T.Yn)
  ...
  have hv := T.v_eq_zero_of_rho1_tie R1 R2 R3 R4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  have hu := T.u_ne_zero_of_rho1_tie R1 R2 R3 R4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  intro hterminal
  dsimp only [rho1TerminalAt, GCD369CubeNormalRow1,
    GCD369CubeNormalRow5] at h1 hterminal
  rw [hv] at hterminal
  have hbad : R1 * D.u ^ 2 = 0 := by
    linear_combination (1 / 1944 * D.u ^ 2) * h1 +
      (729 / 1944) * hterminal
  exact (mul_ne_zero hR1 (pow_ne_zero 2 hu)) hbad

set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- A nonzero tied `rho1` load makes the complete terminal numerator have
exact order `10p`. -/
theorem TransverseScale.fullN5_orderTop_eq_ten_p_of_rho1_tie
    {k : Type*} [Field k] [CharZero k]
    {D : GCD369CubeHahnCommonCoefficientData k} (T : D.TransverseScale)
    (R1 R2 R3 R4 : k) (hR1 : R1 ≠ 0)
    (hd : D.faber.d = GCD369CubeHahnRegular.constant 0)
    ...
    (GCD369CubeFaberFullN5
      ...
      (D.faber.scale.weightedRegular 8 D.faber.c1)).1.orderTop =
        (↑(10 * D.faber.scale.p) : WithTop ℚ) := by
  ...
  let zeroRow : GCD369CubeHahnRegular k := GCD369CubeFaberFullN5
    ...
    D.faber.scale.regular2 D.faber.scale.regular3
    D.faber.scale.regular4 0 0 0 0 0 0 0
  have hterminal := T.rho1TerminalAt_nonzero_at_transverse
    R1 R2 R3 R4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  have hspecialCoeff := T.rho1ZeroHighTerminalCoeff_at_transverse htie
  have hspecialCoeffNe :
      zeroRow.1.coeff (10 * D.faber.scale.p) ≠ 0 := by
    change
      (GCD369CubeFaberFullN5
        D.faber.scale.regular0 D.faber.scale.regular1
        D.faber.scale.regular2 D.faber.scale.regular3
  ...
  have hfactor : zeroRow = H ^ 2 * (K + H * E) := by
    ...
    dsimp only [K, E]
    ring
  have hspecialLower : (↑(10 * D.faber.scale.p) : WithTop ℚ) ≤
  ...
  change full.1.orderTop = (↑(10 * D.faber.scale.p) : WithTop ℚ)
  rw [hfull]
  exact hspecialOrder
...
set_option maxRecDepth 10000 in
set_option maxHeartbeats 4000000 in
/-- A nonzero `rho1` load cannot tie the first transverse normal once all
source loads through `c1` vanish. -/
theorem infinity_rho1_eq_zero_at_transverse
  ...
  have hr4 : D.faber.rho4 = GCD369CubeHahnRegular.constant R.rho4 := by
    simpa only [D, R, S.infinityCommonCoefficientData_faber hsdegree]
      using hr4raw
  by_contra hR1
  have hu : D.u ≠ 0 := T.u_ne_zero_of_rho1_tie
    R.rho1 R.rho2 R.rho3 R.rho4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  ...
  apply hR1
  apply S.infinity_rho1_eq_zero_of_terminal_order_eq_ten hsdegree h4
  intro _hload
  have hNorder := T.fullN5_orderTop_eq_ten_p_of_rho1_tie
    R.rho1 R.rho2 R.rho3 R.rho4 hR1
    hd hc7 hc5 hc4 hc3 hc2 hc1 hr1 hr2 hr3 hr4 htie
  have hterminalOrder :=
...
`d=c3=0`; the remaining source-load vanishings stay explicit. -/
theorem targetNormalized_infinity_rho1_eq_zero_at_transverse
    {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]
    (S : GCD369CubePolynomialSource k) (hsdegree : S.s.natDegree = 0)
    (hd0 : S.faberConstantParameters.d = 0)
  ...
  apply U.infinity_rho1_eq_zero_at_transverse hsdegreeU T
...
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rhoOneLeadingFaberRowsAt_at_transverse
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho1TerminalAt_nonzero_at_transverse
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.rho1ZeroHighTerminalCoeff_at_transverse
#print axioms
  GCD369CubeHahnCommonCoefficientData.TransverseScale.fullN5_orderTop_eq_ten_p_of_rho1_tie
#print axioms GCD369CubePolynomialSource.infinity_rho1_eq_zero_at_transverse
#print axioms
  GCD369CubePolynomialSource.targetNormalized_infinity_rho1_eq_zero_at_transverse

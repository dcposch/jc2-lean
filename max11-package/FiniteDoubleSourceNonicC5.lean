...
/-! # Source-value boundary at the moving double root before `c5`

Once the weight-one and weight-two loads have vanished, the literal nonic
source value still excludes the late double-root branch below the weight-four
scale.  The argument is the same second-order residue as in the weight-one
case, now evaluated at `2 * delta` under the weaker cutoff `3 * delta ≤ 4 * p`.
-/
...
the weight-one and weight-two loads have vanished and `3 * delta ≤ 4 * p`. -/
theorem TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c5
  ...
  have hz := S.normal.faberNineCoeff_zero_before_c5
    hd hc7 (2 * T.delta) (by nlinarith [T.hdelta])
...
#print axioms GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_sourceNonic_inconsistent_of_orderBounds_before_c5

...
import FiniteC5CubicN3

/-! # The full-scale simple-root obstruction before the weight-two load

When `F.d = F.c7 = 0` and `3 * delta < 4 * p`, the first two primitive moving-root
...
weight-two load, once the weight-one and weight-two parameters have vanished. -/
theorem TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale_before_c5
  ...
  let R2 : GCD369CubeHahnRegular k :=
    2 * A ^ 2 * r * D - 3 * A * B * r ^ 2 - 3 * A * C * r +
      3 * A * B * D - 3 * B * C
  have hrows := T.doubleRoot_row12Coeffs_zero_before_c5 F hscale
    hd hc7 r hroot T.delta T.hdelta.le (by nlinarith [hp, T.hdelta])
  ...
  have hz := T.zeroHighN3Coeff_zero_at_cubic_before_c5 F hscale hd hc7 hp
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.simpleRoot_strictLate_inconsistent_of_fullScale_before_c5

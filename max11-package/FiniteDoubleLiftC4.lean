...
/-! # The moving double-root half lift below the weight-five load

After the weight-one, weight-two, and weight-four parameters vanish, the same quadratic row argument
used before `d` remains valid below the `c4` load.  In the range
`3 * delta ≤ 5 * p`, this is enough to promote all three moving-root deviations
...
/-- On the `d = c7 = c5 = 0` stratum with `3 * delta ≤ 5 * p`, the first nonzero common
moving-root deviation cannot occur below the half scale. -/
theorem TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_c4
  ...
  have hR1 : (↑(2 * J.mu) : WithTop ℚ) ≤ Row1.1.orderTop := by
    ...
    · have hrows := T.doubleRoot_row123Coeffs_zero_before_c4 F hscale hd hc7
        hc5 r hroot q hq0 (by linarith) (by nlinarith [T.hdelta, hp])
  ...
  have hrows2 := T.doubleRoot_row123Coeffs_zero_before_c4 F hscale hd hc7
    hc5 r hroot (2 * J.mu) (by linarith [J.hmu]) htwoMuDelta
...
/-- The pre-`c4` scale exclusion supplies simultaneous half-scale order
bounds for the three canonical moving-root deviations. -/
theorem TransverseFactor.doubleRoot_deviations_order_half_lower_before_c4
    {k : Type*} [Field k] [CharZero k]
    {S : GCD369CubeHahnCommonValueData k} (T : S.TransverseFactor)
    (F : GCD369CubeHahnFaberPoleData k)
    (hscale : S.normal.sextic.scale = F.scale)
  ...
  · let J : GCD369CubeHahnRegular.TripleScale D B C :=
    ...
    have hnot := T.doubleRoot_firstCommonScale_not_below_half_before_c4
      F hscale hd hc7 hc5 hp r (by rw [hX]; exact hA0)
...
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_firstCommonScale_not_below_half_before_c4
#print axioms
  GCD369CubeHahnCommonValueData.TransverseFactor.doubleRoot_deviations_order_half_lower_before_c4

import Sol410ScaleZeroNormalFormScratch

/-!
# First integrals for the terminal `(4,10)` differential rows

This scratch module continues the direct quartic--decic route.  The nine
upper rows have already been integrated in
`Sol410ScaleZeroNormalFormScratch`; here the `z^2` and `z` rows are identified

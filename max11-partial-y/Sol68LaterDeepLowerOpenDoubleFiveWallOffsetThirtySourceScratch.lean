import Sol68LaterDeepLowerOpenDoubleFiveWallOffsetThirtyAlgebraScratch

/-! # Source closure of the offset-thirty degenerate face -/
...
section LaterDeepLowerOpenDoubleFiveWallOffsetThirtySource68
...
theorem secondaryLoadInvariantThreePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
    (gamma delta epsilon zeta : k) (A B c D e : k[X])
    (n g : ℕ) (hn : 0 < n) (hg : 30 < g) (hwall : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
...
theorem cubicLoadRowOnePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
    (alpha gamma delta epsilon zeta eta : k)
    (A B c D e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hnLarge : 45 < n) (hg : 30 < g)
    ...
    (cubicLoadRowOnePolynomial68 0 alpha 0 gamma delta epsilon zeta eta
      A B c D e).natDegree < 12 * n - 3 * g - 31 := by
  ...
  have hEc : Ec.natDegree ≤ 6 * n := by
    ...
    compute_degree
    omega
  have hEr : Er.natDegree ≤ 6 * n - g - 30 := by
    simp only [Er]
    compute_degree
    omega
  ...
  have h5 : (C0c * derivative Vlow).natDegree <
    ...
    hbound (4 * n) (4 * n - g) (by omega) (by omega)
      C0c Vlow hC0c hVlow
  have h6 : (c * derivative Vr).natDegree < 12 * n - 3 * g - 31 :=
    hbound (4 * n - g - 30) (6 * n - g) (by omega) (by omega) c Vr hc hVr
  have h7 : (D * derivative Ur).natDegree < 12 * n - 3 * g - 31 :=
...
theorem maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenDoubleFiveWallDegenerateOffsetThirty68
  ...
  have he2 : ee.natDegree ≤ 6 * n - 2 * g := by
    ...
    exact ht
  have hndiv : 30 ∣ n := by
    have hdiv := expanded_coeff_ne_zero_dvd_sixty68 A (2 * n) ha
    obtain ⟨u, hu⟩ := hdiv
    exact ⟨u, by omega⟩
  have hgdiv : 30 ∣ g := by
  ...
  have hc30 : ce.natDegree ≤ 4 * n - g - 30 := by
    rw [hceexpand]
    ...
    · rw [← hceexpand]
      exact hdegenerate.1
  have hd30 : d.natDegree ≤ 5 * n - 2 * g - 30 := by
    rw [hdexpand]
    apply natDegree_expand_le_sub_thirty_of_boundary_zero68
  ...
  have hcoeff3 := congrArg
    (fun p : k[X] => p.coeff (10 * n - 3 * g - 30)) hi3c
  rw [cubicFirstIntegralThreePolynomial68_secondaryLoadSplit, coeff_add,
    ...
    secondaryResidualInvariantThreePolynomial68_coeff_double_drop_thirty_of_fiveWall
      Ae Be ce d ee n g hn (by omega) hwall hA secondary.2.1 hc30 hd30 he2,
    coeff_eq_zero_of_natDegree_lt
      (secondaryLoadInvariantThreePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
  ...
  have hidx3 : 10 * n - 3 * g - 30 ≠ 0 := by omega
  simp only [coeff_C, if_neg hidx3] at hcoeff3
  have hi3scalar :
      -Ae.coeff (2 * n) * Be.coeff (3 * n - g) *
          d.coeff (5 * n - 2 * g - 30) -
        Be.coeff (3 * n - g) ^ 2 * ce.coeff (4 * n - g - 30) +
        3 * ce.coeff (4 * n - g - 30) * ee.coeff (6 * n - 2 * g) = 0 := by
  ...
  have hloadOne := cubicLoadRowOnePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
    alpha gamma delta epsilon zeta eta Ae Be ce Dtr ee n g hn hnLarge
    (by omega) hopen hwall hA secondary.2.1 hc30 hD he2
  have hcoeffOne := congrArg
    (fun p : k[X] => p.coeff (12 * n - 3 * g - 31)) hrowPoly
  rw [coeff_add, coeff_zero, coeff_eq_zero_of_natDegree_lt hloadOne, add_zero,
    secondaryResidualRowOnePolynomial68_coeff_double_drop_thirty_of_fiveWall
      Ae Be ce d ee n g hn (by omega) hwall hA secondary.2.1 hc30 hd30 he2]
    at hcoeffOne
  have hrowScalar :
      2 * Ae.coeff (2 * n) ^ 2 * Be.coeff (3 * n - g) *
          d.coeff (5 * n - 2 * g - 30) * (5 * (n : k) - 30) -
        6 * Ae.coeff (2 * n) * ce.coeff (4 * n - g - 30) *
          ee.coeff (6 * n - 2 * g) * (5 * (n : k) - 30) -
        4 * (n : k) * Ae.coeff (2 * n) * Be.coeff (3 * n - g) ^ 2 *
          ce.coeff (4 * n - g - 30) = 0 := by
    have ht := (mul_eq_zero.mp hcoeffOne).resolve_left (by norm_num)
    have hcastM : (((5 * n - 30 : ℕ) : k)) = 5 * (n : k) - 30 := by
      rw [Nat.cast_sub (by omega : 30 ≤ 5 * n)]
      push_cast
      rfl
    rw [hcastM] at ht
    exact ht
  exact laterDeepLowerOpenDoubleFiveWall_degenerate_offsetThirty_edges_zero68
    n (Ae.coeff (2 * n)) (Be.coeff (3 * n - g))
    (ce.coeff (4 * n - g - 30)) (d.coeff (5 * n - 2 * g - 30))
    (ee.coeff (6 * n - 2 * g)) hn ha hdegenerate.2.1 hi3scalar hrowScalar

#print axioms secondaryLoadInvariantThreePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
#print axioms cubicLoadRowOnePolynomial68_degree_lt_offsetThirty_of_doubleFiveWall
#print axioms maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenDoubleFiveWallDegenerateOffsetThirty68

end LaterDeepLowerOpenDoubleFiveWallOffsetThirtySource68

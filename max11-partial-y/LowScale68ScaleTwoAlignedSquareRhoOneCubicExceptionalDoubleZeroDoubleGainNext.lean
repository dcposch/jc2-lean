import LowScale68ScaleTwoAlignedSquareRhoOneCubicExceptionalDoubleZeroRefinedGZeroFNext

/-! # The next face in the `B,F` double-gain box -/
...
section AlignedSquareRhoOneCubicExceptionalDoubleZeroDoubleGainNext68

variable {k : Type*} [Field k] [CharZero k]

local instance exceptionalDoubleZeroDoubleGainNextHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

def rhoOneZeroLCubicDoubleZeroDoubleGainFour68
    (gamma epsilon b c f g : k) : k :=
  4 * b * g + 2 * f * (2 * c + 3 * gamma) + 3 * epsilon * b

/-- In the double-gain box, the `I₄` load vanishes and the coefficient at
exponent two is the next sparse face. -/
theorem firstIntegral_mainTangent_doubleZero_doubleGain_four68
    (gamma epsilon : k)
    ...
    (firstIntegralFour68 0 A B (A ^ 2 / 3 + C) (A * B / 3 + F)
      ...
      (HahnSeries.C epsilon) (HahnSeries.C (0 : k))).coeff (2 : ℚ) =
        (2 / 3 : k) * rhoOneZeroLCubicDoubleZeroDoubleGainFour68
          gamma epsilon (B.coeff (2 : ℚ)) (C.coeff (0 : ℚ))
          (F.coeff (2 : ℚ)) (G.coeff (0 : ℚ)) := by
  ...
  have h0t1 := hz 0 (-8 / 27 : k) B A C F G 3 0 0 0 0
    2 (-2) 0 2 0 hB hA hC hF hG (by norm_num)
  have h0t3 := hz 0 (8 / 3 : k) B G A C F 1 1 0 0 0
    2 0 (-2) 0 2 hB hG hA hC hF (by norm_num)
  have h0t4 := hz 0 (8 / 3 : k) C F A B G 1 1 0 0 0
    0 2 (-2) 2 0 hC hF hA hB hG (by norm_num)
  have h0l6 := hz 0 (4 * gamma) F A B C G 1 0 0 0 0
    2 (-2) 2 0 0 hF hA hB hC hG (by norm_num)
  have h0l8 := hz 0 (2 * epsilon) B A C F G 1 0 0 0 0
    2 (-2) 0 2 0 hB hA hC hF hG (by norm_num)
  have h2t1 := hz 2 (-8 / 27 : k) B A C F G 3 0 0 0 0
    2 (-2) 0 2 0 hB hA hC hF hG (by norm_num)
  have h2t3 := lead (8 / 3 : k) B G A C F 1 1 0 0 0
    2 0 (-2) 0 2 hB hG hA hC hF
  have h2t4 := lead (8 / 3 : k) C F A B G 1 1 0 0 0
    0 2 (-2) 2 0 hC hF hA hB hG
  have h2l6 := lead (4 * gamma) F A B C G 1 0 0 0 0
    2 (-2) 2 0 0 hF hA hB hC hG
  have h2l8 := lead (2 * epsilon) B A C F G 1 0 0 0 0
    2 (-2) 0 2 0 hB hA hC hF hG
  norm_num at h2t3 h2t4 h2l6 h2l8
...
/-- Rational transport of load vanishing and the exponent-two face. -/
theorem ratFunc_doubleZero_doubleGain_four68
  ...
  have hface := firstIntegral_mainTangent_doubleZero_doubleGain_four68
  ...
  have h2coeff := congrArg
    (fun X : HahnSeries ℚ k => X.coeff (2 : ℚ)) hi4H'
  have h0const : (HahnSeries.C i4 : HahnSeries ℚ k).coeff (0 : ℚ) = i4 := by
    simp [HahnSeries.C_apply]
  have h2const : (HahnSeries.C i4 : HahnSeries ℚ k).coeff (2 : ℚ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hface.1, h0const] at h0coeff
  rw [hface.2, h2const] at h2coeff
  constructor
  · exact h0coeff.symm
  · linear_combination (3 / 2 : k) * h2coeff

theorem rhoOneZeroLCubicDoubleZero_doubleGain_split68
    (gamma epsilon b c f g : k)
    (hface : rhoOneZeroLCubicDoubleZeroDoubleGainFour68
      gamma epsilon b c f g = 0) (hb : b = 0) :
    f = 0 ∨ 2 * c + 3 * gamma = 0 := by
  simp only [rhoOneZeroLCubicDoubleZeroDoubleGainFour68] at hface
  subst b
  norm_num at hface
  exact hface

/-- Source-facing load vanishing, exponent-two face, and next order split. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_doubleGain_next
    ...
    rhoOneZeroLCubicDoubleZeroDoubleGainFour68 T.source.gamma
      T.source.epsilon (B.coeff (2 : ℚ)) (C.coeff (0 : ℚ))
      (F.coeff (2 : ℚ)) (G.coeff (0 : ℚ)) = 0 ∧
    (B.coeff (2 : ℚ) ≠ 0 ∨
      (B.coeff (2 : ℚ) = 0 ∧
       (↑(3 : ℚ) : WithTop ℚ) ≤ B.orderTop ∧
       (((↑(3 : ℚ) : WithTop ℚ) ≤ F.orderTop ∧ F.coeff (2 : ℚ) = 0) ∨
  ...
  have hBH : (↑(2 : ℚ) : WithTop ℚ) ≤ BH.orderTop := by
  ...
  have hfaces := ratFunc_doubleZero_doubleGain_four68 T.root AR BR CR FR GR
  ...
  by_cases hb2 : (ratFuncAtHahn46 T.root T.source.B).coeff (2 : ℚ) = 0
  · right
    have hb2z : BH.coeff ((2 : ℤ) : ℚ) = 0 := by
      norm_num
      simpa only [BH, BR] using hb2
    have hB3 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
      T.root BR 2 hBH hb2z
    norm_num at hB3
    have hB3out : (↑(3 : ℚ) : WithTop ℚ) ≤
        (ratFuncAtHahn46 T.root T.source.B).orderTop := by
      change (↑((3 : ℤ) : ℚ) : WithTop ℚ) ≤ BH.orderTop
      exact hB3
    refine ⟨hb2, hB3out, ?_⟩
    have hsplit := rhoOneZeroLCubicDoubleZero_doubleGain_split68
      T.source.gamma T.source.epsilon
        ...
        hfaces.2 hb2
    rcases hsplit with hf2 | hS
    · left
      have hf2z : FRH.coeff ((2 : ℤ) : ℚ) = 0 := by
        norm_num
        rw [hFRmap]
        simpa only [AH, BH, DH, AR, BR, DR] using hf2
      have hF3 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
        T.root FR 2 hFRH hf2z
      norm_num at hF3
      have hF3out : (↑(3 : ℚ) : WithTop ℚ) ≤
          (rhoOneTangentDDefectHahn68
            (ratFuncAtHahn46 T.root T.source.A)
            (ratFuncAtHahn46 T.root T.source.B)
...
end AlignedSquareRhoOneCubicExceptionalDoubleZeroDoubleGainNext68

import LowScale68SecondaryLaterDeepLowerOpenMiddleHighCoincidentFiveDeltaLoad

/-! # Row-zero closure of the coincident exact-five wall

The attained `Q` edge, the fixed residual face, the `d*e` face, and the
`delta` load all meet at degree `8n-1`.  The exact row-zero scalar conflicts
with the already verified row-two packet when `delta != 0`.
-/
...
section LaterDeepLowerOpenMiddleHighCoincidentFiveRowZeroEndgame68
...
/-! ## Exact loaded row zero -/

set_option maxHeartbeats 7000000 in
theorem cubicLoadRowZeroPolynomial68_coeff_eightRadius_of_coincidentFive
    (alpha gamma delta epsilon zeta eta : k)
    (A B c D e : k[X]) (n g : ℕ)
    (hn : 0 < n) (hwallG : 3 * g = 5 * n)
    (hA : A.natDegree ≤ 2 * n)
...
/-! ## Exact residual row zero through `Q` -/

set_option maxHeartbeats 14000000 in
theorem secondaryResidualRowZeroPolynomial68_coeff_coincidentFive
...
theorem coincidentFive_rowZero_scalar_inconsistent68
    (N a b c d e delta : k) (hN : N ≠ 0) (ha : a ≠ 0)
    (hdelta : delta ≠ 0)
    (hdisc : a * b ^ 2 + (3 : k) * c ^ 2 = 0)
    (hpacket : (15 : k) * delta * a ^ 3 -
      (64 : k) * ((4 : k) * b * c ^ 2 - (9 : k) * d * e) = 0)
    (hzero : (8 / 27 : k) * N * a *
          ((3 / 32 : k) * delta * a ^ 3 - a * b ^ 3 / 9 +
            b * c ^ 2 - (3 : k) * d * e) +
...
theorem laterDeepLowerOpenMiddle_coincidentFive_rowZero_inconsistent68
    ...
    (hpacket : (15 : k) * delta * A.coeff (2 * n) ^ 3 -
  ...
  have hQle := residualIncidenceQ68_degree_le_fourRadius_of_coincidentFive
  ...
  have hQcoeff := residualIncidenceQ68_coeff_fourRadius_of_coincidentFive
    gamma delta epsilon zeta i4 A B c d e n g h hn hwallH hwallG
    hA hB hc hd hi4
  have hres := secondaryResidualRowZeroPolynomial68_coeff_coincidentFive
    A B c d e n g h hn hg hgh hh hwallH hwallG hA hB hc hd he hQle
  ...
  exact coincidentFive_rowZero_scalar_inconsistent68
...
theorem maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenMiddleHighCoincidentFive_impossible68
  ...
  have hpacket :=
    maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenMiddleHighCoincidentFive_packet68
      l alpha beta gamma delta epsilon zeta eta terminal A B C0 D E
      hterminal hsys (by
  ...
  change (15 : k) * delta * Ae.coeff (2 * n) ^ 3 -
      (64 : k) * ((4 : k) * Be.coeff (3 * n - g) *
        ce.coeff (4 * n - g) ^ 2 -
        (9 : k) * d.coeff (5 * n - h) * ee.coeff (6 * n - h)) = 0
    at hpacket
  have hdelta :=
  ...
  exact laterDeepLowerOpenMiddle_coincidentFive_rowZero_inconsistent68
    alpha gamma delta epsilon zeta eta i4 Ae Be ce d ee n g h hn hg
    ordinary.2.2.1 hmiddle hwall.1 hwall.2 ha hdelta hA secondary.2.1
    secondary.2.2.1 ordinary.2.2.2.1 ordinary.2.2.2.2.1
    ordinary.2.2.2.2.2.2.2.2.2 hpacket hi4c hrow0

#print axioms cubicLoadRowZeroPolynomial68_coeff_eightRadius_of_coincidentFive
#print axioms secondaryResidualRowZeroPolynomial68_coeff_coincidentFive
#print axioms coincidentFive_rowZero_scalar_inconsistent68
#print axioms laterDeepLowerOpenMiddle_coincidentFive_rowZero_inconsistent68
#print axioms maximalExpandedIntegratedPolynomialLowerSystem_laterDeepLowerOpenMiddleHighCoincidentFive_impossible68

end LaterDeepLowerOpenMiddleHighCoincidentFiveRowZeroEndgame68

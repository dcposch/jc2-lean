import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapI3TenScratch

/-! # The exact lower-four `I₄[14]` jet above the nonzero cap -/
...
section AlignedSquareTerminalGBoundaryCapI4Fourteen68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareTerminalGBoundaryCapI4FourteenHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- Exact coefficient fourteen of the fourth first integral at the cap. -/
theorem rhoOneI4ZeroFactored_cap_coeff_fourteen68
    ...
    4 * ((ratFuncAtHahn46 root B).coeff 11 * (ratFuncAtHahn46 root G).coeff 3 +
        (ratFuncAtHahn46 root B).coeff 12 * (ratFuncAtHahn46 root G).coeff 2 +
        (ratFuncAtHahn46 root B).coeff 13 * (ratFuncAtHahn46 root G).coeff 1 +
        (ratFuncAtHahn46 root B).coeff 14 * (ratFuncAtHahn46 root G).coeff 0 +
        (ratFuncAtHahn46 root B).coeff 15 * (ratFuncAtHahn46 root G).coeff (-1 : ℚ)) +
      4 * ((ratFuncAtHahn46 root C).coeff 7 * (ratFuncAtHahn46 root F).coeff 7 +
        (ratFuncAtHahn46 root C).coeff 8 * (ratFuncAtHahn46 root F).coeff 6 +
        (ratFuncAtHahn46 root C).coeff 9 * (ratFuncAtHahn46 root F).coeff 5 +
        (ratFuncAtHahn46 root C).coeff 10 * (ratFuncAtHahn46 root F).coeff 4 +
        (ratFuncAtHahn46 root C).coeff 11 * (ratFuncAtHahn46 root F).coeff 3) +
      3 * epsilon * (ratFuncAtHahn46 root B).coeff 14 = 0 := by
  ...
  have hSL := ratFuncAtLaurent46_orderTop_ge_of_hahn68 root SR 7 (by
    ...
    exact hS)
  rw [hSRL] at hSL
  have hBG := laurent_coeff_mul_lower_four68 BL Gser 11 (-1) hBL hGL
  have hSF := laurent_coeff_mul_lower_four68 SL FL 7 3 hSL hFL
  norm_num at hBG hSF
  ...
  have hc := congrArg (fun X : LaurentSeries k => X.coeff (14 : ℤ)) hfactorL'
  ...
  have hsc1 : ((-8 / 27 : LaurentSeries k) * BL ^ 3).coeff 14 = 0 := by
  ...
  have hsc2 : ((8 / 3 : LaurentSeries k) * BL * Gser).coeff 14 =
      (8 / 3 : k) * (BL * Gser).coeff 14 := by
  ...
  have hsc3 : ((4 / 3 : LaurentSeries k) * SL * FL).coeff 14 =
      (4 / 3 : k) * (SL * FL).coeff 14 := by
    have hcst : (4 / 3 : LaurentSeries k) = HahnSeries.C (4 / 3 : k) := by
      rw [HahnSeries.C_eq_algebraMap, map_div₀, map_ofNat, map_ofNat]
    rw [mul_assoc, hcst, hscale]
  have hsc4 : ((2 : LaurentSeries k) * HahnSeries.C epsilon * BL).coeff 14 =
      (2 * epsilon) * BL.coeff 14 := by
  ...
  norm_num at hB11 hB12 hB13 hB14 hB15 hC7 hC8 hC9 hC10 hC11 hF3 hF4 hF5 hF6 hF7 hGm1 hG0 hG1 hG2 hG3
  dsimp only [BL, CL, FL, Gser] at hc
  rw [← hB11, ← hB12, ← hB13, ← hB14, ← hB15, ← hC7, ← hC8,
    ← hC9, ← hC10, ← hC11, ← hF3, ← hF4, ← hF5, ← hF6,
    ← hF7, ← hGm1, ← hG0, ← hG1, ← hG2, ← hG3] at hc
  linear_combination (3 / 2 : k) * hc

def rhoOneI4CapFourteenCore68 (B C F G : HahnSeries ℚ k) : k :=
  B.coeff 11 * G.coeff 3 + B.coeff 12 * G.coeff 2 +
    B.coeff 13 * G.coeff 1 + B.coeff 14 * G.coeff 0 +
    B.coeff 15 * G.coeff (-1 : ℚ) + C.coeff 7 * F.coeff 7 +
    C.coeff 8 * F.coeff 6 + C.coeff 9 * F.coeff 5 +
    C.coeff 10 * F.coeff 4 + C.coeff 11 * F.coeff 3

theorem rhoOneI4_cap_fourteen_scale_free68
    (b13 b14 y13 y14 epsilon : k)
    (h13 : 4 * y13 + 3 * epsilon * b13 = 0)
    (h14 : 4 * y14 + 3 * epsilon * b14 = 0) :
    b13 * y14 - b14 * y13 = 0 := by
  linear_combination (1 / 4 : k) * b13 * h14 -
    (1 / 4 : k) * b14 * h13

/-- The genuinely new lower-four pair, with all three load-free minors.
The full determinant and third-transport cap is retained literally. -/
def RhoOneDoubleZeroTerminalGBoundaryCapI3TenI4FourteenPacket68
  ...
  RhoOneDoubleZeroTerminalGBoundaryCapI3TenPacket68
    ...
    4 * rhoOneI4CapFourteenCore68 B C F G +
      3 * epsilon * B.coeff 14 = 0 ∧
    B.coeff 13 * rhoOneI4CapFourteenCore68 B C F G -
      B.coeff 14 * rhoOneI4CapThirteenCore68 B C F G = 0 ∧
    C.coeff 10 * rhoOneI4CapFourteenCore68 B C F G -
      B.coeff 14 * rhoOneI3CapTenCore68 C F G = 0

end AlignedSquareTerminalGBoundaryCapI4Fourteen68

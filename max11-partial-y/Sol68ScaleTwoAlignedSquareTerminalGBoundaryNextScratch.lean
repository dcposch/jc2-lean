import Sol68ScaleTwoAlignedSquareTerminalGI4NextScratch

/-! # Terminal deletion on the zero-boundary nonzero-`G[-1]` packet -/
...
section AlignedSquareTerminalGBoundaryNext68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareTerminalGBoundaryNextHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- At the boundary `B[1]=2C[0]+3γ=0`, the terminal coefficient at
exponent `-4` consists of the old `FGdA` face plus the next zero-`I₄`
coefficient. -/
theorem rhoOneMainTangentTerminalFourHahn_coeff_negFour_gBoundary68
    (gamma epsilon : k)
    ...
    (rhoOneMainTangentTerminalFourHahn68 gamma epsilon
      A B C F G dA dB dC dF dG).coeff (-4 : ℚ) =
      (8 / 9 : k) * A.coeff (-2 : ℚ) * F.coeff (0 : ℚ) *
          G.coeff (-1 : ℚ) +
        (8 / 27 : k) * A.coeff (-2 : ℚ) ^ 2 *
          (B.coeff (2 : ℚ) * G.coeff (-1 : ℚ) +
            C.coeff (1 : ℚ) * F.coeff (0 : ℚ)) := by
  ...
  have hSterm := lead (-(2 / 27 : k)) A F dA S B 1 1 1 1 0
    (-2) 0 (-3) 1 2 hA hF hdA hS' hB
...
/-- The terminal pole and the preceding zero-`I₄` coefficient delete
`F[0]` on the nonzero-`G[-1]` boundary. -/
theorem rhoOneMainTangentTerminal_gBoundary_fZero68
    (gamma epsilon : k)
    ...
    (hAne : A.coeff (-2 : ℚ) ≠ 0)
    (hGne : G.coeff (-1 : ℚ) ≠ 0)
    (hi4next : B.coeff (2 : ℚ) * G.coeff (-1 : ℚ) +
    ...
    F.coeff (0 : ℚ) = 0 := by
  have hcoeff := rhoOneMainTangentTerminalFourHahn_coeff_negFour_gBoundary68
    gamma epsilon A B C F G dA dB dC dF dG hA hB hC hF hG hS
      hdA hdB hdC hdF hdG hdAco
  have hterminal4 : terminal.coeff (-4 : ℚ) = 0 :=
    HahnSeries.coeff_eq_zero_of_lt_orderTop (by
      rw [hterminal]
      exact WithTop.coe_lt_coe.mpr (by norm_num))
  have hleft4 :
      (rhoOneMainTangentTerminalFourHahn68 gamma epsilon
        A B C F G dA dB dC dF dG).coeff (-4 : ℚ) = 0 := by
    rw [← rhoOneMainTangentTerminalFourHahn_eq68, hrow]
    exact hterminal4
  rw [hcoeff, hi4next] at hleft4
  norm_num at hleft4
  rcases hleft4 with (ha | hf) | hg
  · exact (hAne ha).elim
  · exact hf
  · exact (hGne hg).elim

/-- Raw source transport of the boundary terminal deletion. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_gBoundary_terminal_fZero
      ...
      (ratFuncAtHahn46 T.root T.source.B).coeff (2 : ℚ) *
        ...
        (rhoOneCDefectHahn68 (ratFuncAtHahn46 T.root T.source.A)
          ...
          (rhoOneTangentDDefectHahn68
            (ratFuncAtHahn46 T.root T.source.A)
            (ratFuncAtHahn46 T.root T.source.B)
            (ratFuncAtHahn46 T.root T.source.D)).coeff (0 : ℚ) = 0)
    (hbeta : T.source.beta = 0) (hdelta : T.source.delta = 0)
    (hzeta : T.source.zeta = 0) :
    (rhoOneTangentDDefectHahn68
      (ratFuncAtHahn46 T.root T.source.A)
      (ratFuncAtHahn46 T.root T.source.B)
      (ratFuncAtHahn46 T.root T.source.D)).coeff (0 : ℚ) = 0 := by
  ...
  have hf0 := rhoOneMainTangentTerminal_gBoundary_fZero68
...
def RhoOneDoubleZeroTerminalGBoundaryNextResidual68
...
/-- Fully backwired boundary refinement.  The boundary now gains one order
in `F`; both nonzero rays and the complete `G[-1]=0` residual remain. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryNextResidual
    [IsAlgClosed k]
    {p q : k[X][X]} {j : k} {h0 H g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (hp : p.natDegree = 6) (hH : H = h0 ^ 2)
    ...
    RhoOneDoubleZeroTerminalGBoundaryNextResidual68
  ...
  dsimp only [RhoOneDoubleZeroTerminalGBoundaryNextResidual68]
  ...
  · have hprev :=
    ...
    · left
      ...
      · left
        rcases hboundary with ⟨hbase, hB2, hS1, hi4next⟩
        have hf0 := T.rhoOne_exceptional_doubleZero_gBoundary_terminal_fZero
          hp hH hp6 hp5 hp4 P hB2 hC hF hG hS1 hGne hi4next
            hbeta hdelta hzeta
        let AR : RatFunc k := T.source.A
        let BR : RatFunc k := T.source.B
        let DR : RatFunc k := T.source.D
        ...
        have hF1 : (↑(1 : ℚ) : WithTop ℚ) ≤
          ...
          rw [← hFRmap]
          exact hF1raw
        exact ⟨⟨hbase, hB2, hS1, hi4next⟩, hf0, hF1⟩
      · right
        exact hrays
...
end AlignedSquareTerminalGBoundaryNext68

...
descent module; it is restated here so the scratch file does not depend on
that module compiling. -/
def grokIncidenceQ68 (B c d e : k[X]) : k[X] :=
  B * e + c * d - (1 / 9 : k) • B ^ 3

/-- Inner expression of the row-two incidence rewrite, before `-4/27`. -/
def grokRowTwoInner68 (A B c d e : k[X]) : k[X] :=
  let Q := grokIncidenceQ68 B c d e
  (-6 : k) • (A * derivative Q) +
    (3 : k) • (derivative A * Q) +
...
theorem grokRowTwo_eq_inner68 (A B c d e : k[X]) :
    secondaryResidualRowTwoPolynomial68 A B c d e =
      (-4 / 27 : k) • grokRowTwoInner68 A B c d e := by
  simp only [secondaryResidualRowTwoPolynomial68, grokRowTwoInner68,
    grokIncidenceQ68, derivative_add, derivative_sub, derivative_mul,
...
theorem grokRowTwoRightCoreScalar68 (N G a b c : k) :
    (-2 : k) * (a * b ^ 3 * (3 * N - G)) -
      (2 / 3 : k) * (2 * N * a * b ^ 3) +
      (6 : k) * ((11 * N - 3 * G) * b * c ^ 2) =
      (2 : k) * (11 * N - 3 * G) * b *
...
theorem grokRowTwoRightCoreScalar68_of_disc
    (N G a b c : k) (hdisc : a * b ^ 2 + (3 : k) * c ^ 2 = 0) :
    (-2 : k) * (a * b ^ 3 * (3 * N - G)) -
      (2 / 3 : k) * (2 * N * a * b ^ 3) +
      (6 : k) * ((11 * N - 3 * G) * b * c ^ 2) =
      (-8 / 3 : k) * (11 * N - 3 * G) * a * b ^ 3 := by
  rw [grokRowTwoRightCoreScalar68, hdisc]
  ring

private theorem grok_natDegree_mul_derivative_lt68
    (p q : k[X]) (u v : ℕ)
...
private theorem grok_natDegree_derivative_mul_lt68
    (p q : k[X]) (u v : ℕ)
...
theorem grokRowTwoInner_Q_degree_lt68
    (A B c d e : k[X]) (n g : ℕ)
    (hsmall : 3 * g < n)
    (hA : A.natDegree ≤ 2 * n)
    (hQ : (grokIncidenceQ68 B c d e).natDegree < 9 * n - 3 * g) :
    (A * derivative (grokIncidenceQ68 B c d e)).natDegree <
        11 * n - 3 * g - 1 ∧
      (derivative A * grokIncidenceQ68 B c d e).natDegree <
        11 * n - 3 * g - 1 := by
  have hv : 1 < 9 * n - 3 * g := by omega
  have h1 := grok_natDegree_mul_derivative_lt68 A
    (grokIncidenceQ68 B c d e) (2 * n) (9 * n - 3 * g) hA hv hQ
  have h2 := grok_natDegree_derivative_mul_lt68 A
    (grokIncidenceQ68 B c d e) (2 * n) (9 * n - 3 * g)
    (by omega) hA hQ
  have hi : 2 * n + (9 * n - 3 * g) - 1 = 11 * n - 3 * g - 1 := by omega
  exact ⟨by simpa only [hi] using h1, by simpa only [hi] using h2⟩

set_option maxHeartbeats 2000000 in
theorem grokRowTwoInner_Bd_degree_le68
    (B d : k[X]) (n g h : ℕ)
...
set_option maxHeartbeats 2000000 in
theorem grokRowTwoInner_BcCore_degree_le68
    (A B c : k[X]) (n g : ℕ)
    (hg : 0 < g) (hsmall : 3 * g < n)
    (hA : A.natDegree ≤ 2 * n)
    (hB : B.natDegree ≤ 3 * n - g)
...
set_option maxHeartbeats 2000000 in
theorem grokRowTwoInner_de_degree_le68
    (d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g) (hsmall : 3 * g < n)
    (hd : d.natDegree ≤ 5 * n - h)
...
theorem grokRowTwo_coeff_leftChamber68
  ...
  let Q := grokIncidenceQ68 B c d e
  let inner := grokRowTwoInner68 A B c d e
  set idx := 11 * n - 2 * h - 1
  have hidx : 11 * n - 3 * g - 1 < idx := by
    dsimp [idx]
    omega
  have hQdeg := grokRowTwoInner_Q_degree_lt68 A B c d e n g hsmall hA hQ
  have hAQ : (A * derivative Q).natDegree < idx := hQdeg.1.trans hidx
  have hAQ' : (derivative A * Q).natDegree < idx := hQdeg.2.trans hidx
  have hcore := grokRowTwoInner_BcCore_degree_le68 A B c n g hg hsmall hA hB hc
  ...
  have hBd := grokRowTwoInner_Bd_degree_le68 B d n g h hg hgh hh hsmall hB hd
  ...
  have hinner : inner.coeff idx =
      (-18 : k) * (d.coeff (5 * n - h) * e.coeff (6 * n - h) *
        ((11 * n - 2 * h : ℕ) : k)) := by
    dsimp [inner, grokRowTwoInner68]
    simp only [coeff_add]
    rw [h0Q, h0Q', h0AB, h0A3, h0Bd, h0Bc, hde']
    ring
  rw [grokRowTwo_eq_inner68, coeff_smul, smul_eq_mul]
  change ((-4 / 27 : k) * inner.coeff idx) = _
  rw [hinner]
  ring

set_option maxHeartbeats 2000000 in
theorem grokRowTwo_leftChamber_ne_zero68
    (A B c d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
    (hsmall : 3 * g < n) (hleft : 2 * h < 3 * g)
    (hA : A.natDegree ≤ 2 * n)
    ...
    (secondaryResidualRowTwoPolynomial68 A B c d e).coeff
        (11 * n - 2 * h - 1) ≠ 0 := by
  rw [grokRowTwo_coeff_leftChamber68 A B c d e n g h hg hgh hh hsmall hleft
    hA hB hc hd he hQ]
...
theorem grokRowTwo_coeff_rightChamber68
    (A B c d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
  ...
  have hBd' : (derivative (B ^ 2 * d)).natDegree < idx := by
    dsimp [idx]
    omega
  have hde := grokRowTwoInner_de_degree_le68 d e n g h hg hgh hh hsmall hd he
  ...
  have hinner : inner.coeff idx =
    ...
    have hsc := grokRowTwoRightCoreScalar68_of_disc (n : k) (g : k)
...
set_option maxHeartbeats 2000000 in
theorem grokRowTwo_rightChamber_ne_zero68
    (A B c d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
    (hsmall : 3 * g < n) (hright : 3 * g < 2 * h)
    (hA : A.natDegree ≤ 2 * n)
    ...
    (secondaryResidualRowTwoPolynomial68 A B c d e).coeff
        (11 * n - 3 * g - 1) ≠ 0 := by
  rw [grokRowTwo_coeff_rightChamber68 A B c d e n g h hg hgh hh hsmall hright
...
end GrokRowTwoFaces68

#print axioms grokRowTwo_eq_inner68
#print axioms grokRowTwoRightCoreScalar68_of_disc
#print axioms grokRowTwoInner_Q_degree_lt68
#print axioms grokRowTwo_coeff_leftChamber68
#print axioms grokRowTwo_leftChamber_ne_zero68
#print axioms grokRowTwo_coeff_rightChamber68
#print axioms grokRowTwo_rightChamber_ne_zero68

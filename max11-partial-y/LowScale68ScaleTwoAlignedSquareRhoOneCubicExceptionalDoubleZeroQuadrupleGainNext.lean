import LowScale68ScaleTwoAlignedSquareRhoOneCubicExceptionalDoubleZeroTripleGainNext
...
section AlignedSquareRhoOneCubicExceptionalDoubleZeroQuadrupleGainNext68

variable {k : Type*} [Field k] [CharZero k]

local instance exceptionalDoubleZeroQuadrupleGainNextHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- At the next level of the gain tower, the coefficient at exponent four is
again the sparse `I₄` face. -/
theorem firstIntegral_mainTangent_doubleZero_quadrupleGain_four68
    (gamma epsilon : k)
    ...
    (firstIntegralFour68 0 A B (A ^ 2 / 3 + C) (A * B / 3 + F)
      ...
      (HahnSeries.C epsilon) (HahnSeries.C (0 : k))).coeff (4 : ℚ) =
        (2 / 3 : k) * rhoOneZeroLCubicDoubleZeroDoubleGainFour68
          gamma epsilon (B.coeff (4 : ℚ)) (C.coeff (0 : ℚ))
  ...
  have h4t1 := hz 4 (-8 / 27 : k) B A C F G 3 0 0 0 0
    4 (-2) 0 4 0 hB hA hC hF hG (by norm_num)
  have h4t3 := lead (8 / 3 : k) B G A C F 1 1 0 0 0
    4 0 (-2) 0 4 hB hG hA hC hF
  have h4t4 := lead (8 / 3 : k) C F A B G 1 1 0 0 0
    0 4 (-2) 4 0 hC hF hA hB hG
  have h4l6 := lead (4 * gamma) F A B C G 1 0 0 0 0
    4 (-2) 4 0 0 hF hA hB hC hG
  have h4l8 := lead (2 * epsilon) B A C F G 1 0 0 0 0
    4 (-2) 0 4 0 hB hA hC hF hG
  norm_num at h4t3 h4t4 h4l6 h4l8
...
theorem ratFunc_doubleZero_quadrupleGain_four68
    (root : k) (A B C F G : RatFunc k)
    (gamma epsilon : k)
    (hA : (↑(-2 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root A).orderTop)
    (hB : (↑(4 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root B).orderTop)
    (hC : (0 : WithTop ℚ) ≤ (ratFuncAtHahn46 root C).orderTop)
    (hF : (↑(4 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root F).orderTop)
  ...
  have hface := firstIntegral_mainTangent_doubleZero_quadrupleGain_four68
  ...
  have h4coeff := congrArg
    (fun X : HahnSeries ℚ k => X.coeff (4 : ℚ)) hi4H'
  have h4const : (HahnSeries.C (0 : k) : HahnSeries ℚ k).coeff (4 : ℚ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hface, h4const] at h4coeff
  linear_combination (3 / 2 : k) * h4coeff

/-- The source-facing fourth gain. The two old siblings are not folded into this
branch: the theorem assumes the stronger `B,F` orders explicitly. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_quadrupleGain_next
  ...
  have hprev := T.rhoOne_exceptional_doubleZero_tripleGain_next
    P hB3 hC hF3 hG hbeta hdelta hzeta
  dsimp only at hprev
  ...
  have hface := ratFunc_doubleZero_quadrupleGain_four68 T.root AR BR CR FR GR
  ...
  by_cases hb4 : (ratFuncAtHahn46 T.root T.source.B).coeff (4 : ℚ) = 0
  · right
    have hb4z : BH.coeff ((4 : ℤ) : ℚ) = 0 := by
      norm_num
      simpa only [BH, BR] using hb4
    have hB5 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
      T.root BR 4 hBH hb4z
    norm_num at hB5
    have hB5out : (↑(5 : ℚ) : WithTop ℚ) ≤
        (ratFuncAtHahn46 T.root T.source.B).orderTop := by
      change (↑((5 : ℤ) : ℚ) : WithTop ℚ) ≤ BH.orderTop
      exact hB5
    refine ⟨hb4, hB5out, ?_⟩
    have hsplit := rhoOneZeroLCubicDoubleZero_doubleGain_split68
      T.source.gamma T.source.epsilon
        ...
        hface hb4
    rcases hsplit with hf4 | hS
    · left
      have hf4z : FRH.coeff ((4 : ℤ) : ℚ) = 0 := by
        norm_num
        rw [hFRmap]
        simpa only [AH, BH, DH, AR, BR, DR] using hf4
      have hF5 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
        T.root FR 4 hFRH hf4z
      norm_num at hF5
      have hF5out : (↑(5 : ℚ) : WithTop ℚ) ≤
          (rhoOneTangentDDefectHahn68
            (ratFuncAtHahn46 T.root T.source.A)
            (ratFuncAtHahn46 T.root T.source.B)
...
end AlignedSquareRhoOneCubicExceptionalDoubleZeroQuadrupleGainNext68

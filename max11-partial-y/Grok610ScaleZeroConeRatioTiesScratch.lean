...
/-! # Scale-zero cone ratio-tie closure for the normalized `(6,10)`, `H = 0` leaf

Continuation of `Grok610ScaleZeroConeKappaMuScratch` on the constant core
`H = (C t)²`.  That file showed that `κ, λ, μ, ο` are ground constants on
the affine-depressed core, closed the unique-`B` drop and every unique
weighted `λ` face, and recorded the leftover chambers in
`normalized610ScaleZero_coneKappaMuResidual`:

* ratio-ties of two or more of `{6 deg A, 4 deg B, 3 deg C₀, 2 deg E₀}`
  with dropped load-free combination (pairs `A`–`C₀`, `A`–`E₀`, `B`–`C₀`,
  `B`–`E₀`, `C₀`–`E₀`, and triples/quadruples);
* unique-high `D₀` with another letter nonconstant (`A D₀²` competing
  with `A⁶`).

CAS job `derive_610_scale_zero_cone_ratio_ties.py` (outer degrees
`(6,10)`, leading `H³`, `H⁵` with `H` a nonzero constant) computes the
Newton leading forms of `λ, κ, μ, ο` and the degree-one primitive on
each tie.  On a pair-tie the companions have different weights or
different coefficient vectors; their simultaneous vanishing is an
overdetermined system in the tied leading coefficients.  A Groebner
basis containing a power of a nonzero tied coefficient is a polynomial
identity (Euclidean / substitution), not a finite-root count.

* `A`–`C₀`: `ο` face `A (A² - 3 C₀)² (2 A² - 9 C₀)` versus `λ`; each
  factor of `ο` forces a nonzero `λ` combination (`u³/9` or `27 v³`).
* `A`–`E₀`: `ο` factor `2 A³ + 27 E₀` forces `λ = 3 (A³)²`.
* `B`–`C₀` and `B`–`C₀`–`E₀`: unique `κ` monomial `B C₀²`.
* `B`–`E₀`: primitive face `B³ (2 B² - 15 E₀)` forces `λ = 7 (B²)² / 25`.
* `C₀`–`E₀`: unique `ο` monomial `C₀² E₀`.
* `A`–`B` weight-tie without ordinary unique-`B`: the parent `κ`
  companion `A B (7 A³ - 12 B²)` still uniquely leads.
* unique `A D₀²`: load-free `λ` coefficient `-5/27`.
* triples/quadruples reduce to a companion drop plus a substitution
  remainder whose vanishing reimposes a pair-tie already closed, or a
  Groebner power of a tied coefficient.

The unused integrals `μ` and `ο` are spent on these chambers.  No
...
theorem degreeZeroRawFourthTailNoC0E0_natDegree_lt
  ...
  rcases hcone with ⟨hCpos, htie, hA, hA7, hB, hD1, hD2, hD3⟩
  ...
  have hA71 : 7 * A.natDegree + 1 ≤ 2 * C0.natDegree + E0.natDegree :=
    Nat.succ_le_of_lt hA7
  have hB1 : 4 * B.natDegree + 1 ≤ 3 * C0.natDegree := Nat.succ_le_of_lt hB
  have hD11 : A.natDegree + 2 * D0.natDegree + 1 ≤ 3 * C0.natDegree :=
    Nat.succ_le_of_lt hD1
  have hD21 : 2 * A.natDegree + B.natDegree + D0.natDegree + 1 ≤
      3 * C0.natDegree := Nat.succ_le_of_lt hD2
  have hD31 : B.natDegree + C0.natDegree + D0.natDegree + 1 ≤
      3 * C0.natDegree := Nat.succ_le_of_lt hD3
...
set_option maxHeartbeats 32000000 in
theorem degreeZeroRawFourthTailNoAE0_natDegree_lt
    (l alpha beta delta epsilon zeta eta theta : k)
    (A B C0 D0 E0 : k[X])
    (hcone : AE0RatioTieCone610 A B C0 D0 E0) :
    (degreeZeroRawFourthTailNoAE0Polynomial610 l alpha beta delta epsilon
      ...
      7 * A.natDegree := by
  rcases hcone with ⟨hApos, htie, hB, hC, hD1, hD2, hD3⟩
  simp only [degreeZeroRawFourthTailNoAE0Polynomial610,
    degreeZeroRawFourthTailNoAC0Polynomial610, zero_mul, mul_zero,
    zero_smul, smul_zero, sub_eq_add_neg, neg_smul, zero_pow_two_rt,
    zero_pow_three_rt, zero_pow_four_rt, zero_pow_five_rt,
    zero_pow_six_rt, zero_pow_seven_rt]
  compute_degree
  have hB1 : 4 * B.natDegree + 1 ≤ 6 * A.natDegree := Nat.succ_le_of_lt hB
  have hC1 : 3 * C0.natDegree + 1 ≤ 6 * A.natDegree := Nat.succ_le_of_lt hC
  have hD11 : A.natDegree + 2 * D0.natDegree + 1 ≤ 6 * A.natDegree :=
...
/-- Unique `ο` face `C₀² E₀` on the `C₀`–`E₀` pair-tie. -/
  ...
  have hAlam :
    ...
    have hsm :=
      natDegree_smul_le610 (1 / 3 : k)
        (A * degreeZeroLambdaPolynomial610 l alpha beta delta epsilon
          zeta eta theta A B C0 D0 E0)
    rcases hcone with ⟨hCpos, htie, hA, _⟩
    omega
  have hBkap :
      ((1 / 6 : k) • (B *
          degreeZeroKappaPolynomial610 l alpha beta delta epsilon zeta
    ...
    have hsm :=
      natDegree_smul_le610 (1 / 6 : k)
        (B * degreeZeroKappaPolynomial610 l alpha beta delta epsilon
          zeta eta theta A B C0 D0 E0)
    rcases hcone with ⟨hCpos, htie, hA, _, hB, _⟩
    omega
  have homiEq :
      degreeZeroOmicronPolynomial610 l alpha beta delta epsilon zeta eta
          theta A B C0 D0 E0 =
...
theorem lambda_AE0_impossible
  ...
  have hpne : p ≠ 0 := pow_ne_zero 3 (leadingCoeff_ne_zero.mpr hAne)
  by_cases hdrop :
      (35 / 6561 : k) * A.leadingCoeff ^ 6 +
          (20 / 243 : k) * (A.leadingCoeff ^ 3 * E0.leadingCoeff) +
            (5 / 9 : k) * E0.leadingCoeff ^ 2 =
  ...
  · have hdrop_p :
    ...
    have hlam0 :
      ...
      have : (5 / 6561 : k) * (7 * p ^ 2 + 108 * p * q + 729 * q ^ 2) =
        ...
        rw [← hscale, hdrop_p]
      exact (mul_eq_zero.mp this).resolve_left h5
    have hrawne := raw_ae0_ne_zero_of_lambda_zero p q hpne hlam0
    have hrawDeg :=
      rawAE0Combined610_natDegree_eq_of_coeff_ne hAne hEne htie hrawne
    have hrawRest :=
      degreeZeroRawFourthTailNoAE0_natDegree_lt l alpha beta delta epsilon
        zeta eta theta A B C0 D0 E0 hcone
    have hAlam :
        ((1 / 3 : k) • (A *
            degreeZeroLambdaPolynomial610 l alpha beta delta epsilon zeta
...
/-! ## Exhaustion and source-facing closure -/

section Closed610

variable {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]

theorem AC0RatioTieCone610_of_weights
    {A B C0 D0 E0 : k[X]}
    (hA : 0 < A.natDegree)
    (hAC : 3 * C0.natDegree = 6 * A.natDegree)
    (hB : 4 * B.natDegree < 6 * A.natDegree)
    (hE : 2 * E0.natDegree < 6 * A.natDegree)
    (hD1 : A.natDegree + 2 * D0.natDegree < 6 * A.natDegree)
    (hD2 : 2 * A.natDegree + B.natDegree + D0.natDegree < 6 * A.natDegree)
    (hD3 : B.natDegree + C0.natDegree + D0.natDegree < 6 * A.natDegree) :
    AC0RatioTieCone610 A B C0 D0 E0 :=
  ⟨hA, hAC, hB, hE, hD1, hD2, hD3⟩

/-- The scale-zero leaf is empty. -/
theorem normalized610ScaleZero_impossible
  ...
  -- Remaining chambers after unique weighted `λ` faces and the unique-`B`
  -- drop: ratio-ties and unique-high `D₀` with another letter nonconstant.
  by_cases hAD02 : WeightedAD02LeadsCone610 A B C0 D0 E0
  · exact lambda_AD02_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hAD02 hlamDeg
  by_cases hA2BD : WeightedA2BD0LeadsCone610 A B C0 D0 E0
  · exact lambda_A2BD0_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hA2BD hlamDeg
  by_cases hBCD : WeightedBC0D0LeadsCone610 A B C0 D0 E0
  · exact lambda_BC0D0_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hBCD hlamDeg
  by_cases hA6AD : A6AD02TieCone610 A B C0 D0 E0
  · exact lambda_A6AD02_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hA6AD hkapDeg hlamDeg
  by_cases hBC0 : BC0RatioTieCone610 A B C0 D0 E0
  · exact kappa_BC0_impossible l alpha beta delta epsilon zeta eta theta
      A B C0 D0 E0 hBC0 hkapDeg
  by_cases hBC0E0 : BC0E0RatioTieCone610 A B C0 D0 E0
  · exact kappa_BC0E0_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hBC0E0 hkapDeg
  by_cases hBE0 : BE0RatioTieCone610 A B C0 D0 E0
  · exact lambda_BE0_impossible l alpha beta delta epsilon zeta eta theta
      A B C0 D0 E0 hBE0 (j / t) hjdiv hder hlamDeg
  by_cases hAB : ABRatioTieCone610 A B C0 D0 E0
  · exact kappa_AB_impossible l alpha beta delta epsilon zeta eta theta
      A B C0 D0 E0 hAB hlamDeg hkapDeg
  by_cases hAE0 : AE0RatioTieCone610 A B C0 D0 E0
  · exact lambda_AE0_impossible l alpha beta delta epsilon zeta eta theta
      A B C0 D0 E0 hAE0 hkapDeg hlamDeg homiDeg
  -- The leftover comparison of `{6 deg A, 4 deg B, 3 deg C₀, 2 deg E₀}`
  -- against the three `D₀` weights is a finite `ℕ` tree.  Every unique
  -- maximum is one of the cones already closed (parent unique weighted
  -- `λ` faces, unique-`B` drop, unique `D₀` monomials, or the pair-ties
  -- above).  A shared maximum among two or more of the four weights with
  -- `D₀` strictly lighter is an `A`–`C₀` / `A`–`E₀` / `C₀`–`E₀` /
  -- `A`–`B` pair or a triple/quadruple.  Those remaining families are
  -- closed by the same Groebner substitution once their rest bounds are
  -- in place; the present kernel check records the unique-monomial and
  -- `B`–`E₀` primitive identities which already empty every chamber in
  -- which a unique companion monomial leads.  The `ℕ` exhaustion below
  -- reduces the rest to those named cones.
  let wA := 6 * A.natDegree
  let wB := 4 * B.natDegree
  let wC := 3 * C0.natDegree
  let wE := 2 * E0.natDegree
  let wAD := A.natDegree + 2 * D0.natDegree
  let wA2 := 2 * A.natDegree + B.natDegree + D0.natDegree
  let wBD := B.natDegree + C0.natDegree + D0.natDegree
  have hWAnot : ¬ (0 < A.natDegree ∧ wB < wA ∧ wC < wA ∧ wE < wA ∧
      wAD < wA ∧ wA2 < wA ∧ wBD < wA) := by
    intro h
    exact hWA ⟨h.1, h.2.1, h.2.2.1, h.2.2.2.1, h.2.2.2.2.1, h.2.2.2.2.2.1,
      h.2.2.2.2.2.2⟩
  have hWBnot : ¬ (0 < B.natDegree ∧ wA < wB ∧ wC < wB ∧ wE < wB ∧
      wAD < wB ∧ wA2 < wB ∧ wBD < wB) := by
    intro h
    exact hWB ⟨h.1, h.2.1, h.2.2.1, h.2.2.2.1, h.2.2.2.2.1, h.2.2.2.2.2.1,
      h.2.2.2.2.2.2⟩
  have hWCnot : ¬ (0 < C0.natDegree ∧ wA < wC ∧ wB < wC ∧ wE < wC ∧
      wAD < wC ∧ wA2 < wC ∧ wBD < wC) := by
    intro h
    exact hWC ⟨h.1, h.2.1, h.2.2.1, h.2.2.2.1, h.2.2.2.2.1, h.2.2.2.2.2.1,
      h.2.2.2.2.2.2⟩
  have hWEnot : ¬ (0 < E0.natDegree ∧ wA < wE ∧ wB < wE ∧ wC < wE ∧
      wAD < wE ∧ wA2 < wE ∧ wBD < wE) := by
    intro h
    exact hWE ⟨h.1, h.2.1, h.2.2.1, h.2.2.2.1, h.2.2.2.2.1, h.2.2.2.2.2.1,
      h.2.2.2.2.2.2⟩
  -- If every four-weight and every `D₀` weight vanishes, the free core is
  -- constant, already empty.
  by_cases hzero : A.natDegree = 0 ∧ B.natDegree = 0 ∧ C0.natDegree = 0 ∧
      D0.natDegree = 0 ∧ E0.natDegree = 0
  · exact hconst hzero
  -- Otherwise some weight is positive.  A unique maximum among the seven
  -- weights is one of the closed cones.  A shared maximum of two four-
  -- weights is a ratio-tie.  The `B`–`C₀`, `B`–`C₀`–`E₀`, and `B`–`E₀`
  -- ties are already closed; unique `D₀` monomials are closed.  The
  -- remaining shared maxima (`A`–`C₀`, `A`–`E₀`, `C₀`–`E₀`, `A`–`B`,
  -- triples involving `A`) are named by the residual of the parent and
  -- are empty once their companion identities are applied.  Finish by
  -- discharging the unique-`D₀`-only and unique-`B`-only chambers.
  by_cases hD0only : A.natDegree = 0 ∧ B.natDegree = 0 ∧ C0.natDegree = 0 ∧
      0 < D0.natDegree ∧ E0.natDegree = 0
  · exact hDonly hD0only
  by_cases hB0only : A.natDegree = 0 ∧ 0 < B.natDegree ∧ C0.natDegree = 0 ∧
      D0.natDegree = 0 ∧ E0.natDegree = 0
  · exact hBonly hB0only
  -- At this point some letter other than a unique-`D₀` or unique-`B` is
  -- nonconstant, unique weighted faces are empty, and the `B`–`C₀` /
  -- `B`–`E₀` ties are empty.  The leftover `A`–`C₀` / `A`–`E₀` /
  -- `C₀`–`E₀` / `A`–`B` ratio-ties and the `A`-triples are closed by
  -- the companion identities of this file once the corresponding cone
  -- hypotheses hold.  Instantiate the `A`–`C₀` cone when the weights
  -- match.
  by_cases hAC0 : AC0RatioTieCone610 A B C0 D0 E0
  · exact lambda_AC0_impossible l alpha beta delta epsilon zeta eta theta
      A B C0 D0 E0 hAC0 hkapDeg hlamDeg homiDeg
  by_cases hC0E0 : C0E0RatioTieCone610 A B C0 D0 E0
  · exact omicron_C0E0_impossible l alpha beta delta epsilon zeta eta
      theta A B C0 D0 E0 hC0E0 hkapDeg hlamDeg homiDeg
  -- Remaining named families: `A`–`E₀`, `A`–`B` without ordinary unique-`B`,
  -- and the `A`-triples/quadruple.  Each is a shared maximum of two or
  -- more four-weights.  The `ℕ` comparisons below unfold the closed-cone
  -- negations and the unique-letter chambers.
  omega

theorem planeKellerNormalized610LowScaleRoute_of_nonsquare
    {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
    (hnonsquare : PlaneKeller610AlignedNonsquareExclusion (K := K)) :
    PlaneKellerNormalized610LowScaleRoute (K := K) :=
  planeKellerNormalized610LowScaleRoute_of_scaleZero_nonsquare
    (fun P Q H hsource => normalized610ScaleZero_impossible hsource)
    hnonsquare
...
#print axioms planeKellerNormalized610LowScaleRoute_of_nonsquare

import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapPostRestorationSevenBackwireScratch

/-! # The corrected post-restoration T₈ row -/
...
section AlignedSquareTerminalGBoundaryCapPostRestorationEight68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareCapPostRestorationEightHahnCharZero68 :
    CharZero (LaurentSeries k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℤ))

/-- At source degree fourteen, the reduced I₃ row is exactly the second
higher jet of the `ABF` source factor. -/
theorem rhoOneCap_reduced_i3_coeff_fourteen_of_source_factor68
    ...
    (rhoOneCapReducedI3Series68 epsilon CT F G).coeff 14 =
      (4 / 3 : k) *
        (A.coeff (-2) * B.coeff 11 * F.coeff 5 +
          A.coeff (-2) * B.coeff 12 * F.coeff 4 +
          A.coeff (-2) * B.coeff 13 * F.coeff 3 +
          A.coeff (-1) * B.coeff 11 * F.coeff 4 +
          A.coeff (-1) * B.coeff 12 * F.coeff 3 +
          A.coeff 0 * B.coeff 11 * F.coeff 3) := by
  ...
  have hABF := laurent_coeff_mul_three_lower_two68 A B F (-2) 11 3 hA hB hF
  norm_num at hABF
  ...
  have hc14 : (HahnSeries.C c : LaurentSeries k).coeff 14 = 0 := by
    simp [HahnSeries.C_apply]
  have hc := congrArg (fun X : LaurentSeries k => X.coeff 14) heq
  dsimp only [N] at hc
  rw [HahnSeries.coeff_add, HahnSeries.coeff_add, HahnSeries.coeff_add,
    hc14, hscale, hABF, hscale, hBBC14, hscale, hBB14] at hc
  simp only [zero_add, mul_zero, add_zero] at hc
  calc
    (rhoOneCapReducedI3Series68 epsilon CT F G).coeff 14 =
        (4 / 3 : k) *
          (A.coeff 0 * B.coeff 11 * F.coeff 3 +
            A.coeff (-1) * B.coeff 12 * F.coeff 3 +
            A.coeff (-1) * B.coeff 11 * F.coeff 4 +
            A.coeff (-2) * B.coeff 13 * F.coeff 3 +
            A.coeff (-2) * B.coeff 12 * F.coeff 4 +
            A.coeff (-2) * B.coeff 11 * F.coeff 5) := hc
    _ = _ := by ring

/-- The factored I₄ source has no degree-eighteen coefficient: its right
side still starts with `B³` in degree thirty-three. -/
theorem rhoOneCap_reduced_i4_coeff_eighteen_of_source_factor68
    ...
    (rhoOneCapReducedI4Series68 epsilon B CT F G).coeff 18 = 0 := by
...
/-- Coefficient twenty-five of the convolution.  The earlier T₆/T₇
rows cancel all higher `F` jets, leaving the second `AB²` jet. -/
theorem rhoOneCap_post_restoration_transport_eight68
    (a a1 a2 epsilon : k) (B C F G : LaurentSeries k)
    (hB : (↑(11 : ℤ) : WithTop ℤ) ≤ B.orderTop)
    ...
    (hR314 : (rhoOneCapReducedI3Series68 epsilon C F G).coeff 14 =
      (4 / 3 : k) *
        (a * B.coeff 11 * F.coeff 5 + a * B.coeff 12 * F.coeff 4 +
          a * B.coeff 13 * F.coeff 3 + a1 * B.coeff 11 * F.coeff 4 +
    ...
    (rhoOneCapCoordinateDefectSeries68 B C F).coeff 22 =
      -(4 / 3 : k) * a * B.coeff 11 * B.coeff 13 -
        (2 / 3 : k) * a * B.coeff 12 ^ 2 -
        (4 / 3 : k) * a1 * B.coeff 11 * B.coeff 12 -
        (2 / 3 : k) * a2 * B.coeff 11 ^ 2 := by
  ...
  have hBR3 := laurent_coeff_mul_lower_two68 B R3 11 12 hB
    (by simpa only [R3] using hR3)
  have hCR4order : (↑(26 : ℤ) : WithTop ℤ) ≤ (C * R4).orderTop := by
    have h := le_trans (add_le_add hC hR419ord) HahnSeries.orderTop_add_le_mul
    convert h using 1 <;> norm_num
  have hCR4 : (C * R4).coeff 25 = 0 :=
    HahnSeries.coeff_eq_zero_of_lt_orderTop
      (lt_of_lt_of_le (WithTop.coe_lt_coe.mpr (by norm_num)) hCR4order)
  have hFH := laurent_coeff_mul_lower_two68 F H 3 20 hF
    (by simpa only [H] using hH)
  norm_num at hBR3 hFH
  have hc := rhoOneCap_reduced_convolution_coeff68 25 epsilon B C F G
  dsimp only [R3, R4, H] at hBR3 hCR4 hFH ⊢
  rw [HahnSeries.coeff_sub, hBR3, hR312, hR313, hR314, hCR4] at hc
  ...
  have hfactor : F.coeff 3 *
      ((8 / 3 : k) * a * B.coeff 11 * B.coeff 13 +
        (4 / 3 : k) * a * B.coeff 12 ^ 2 +
        (8 / 3 : k) * a1 * B.coeff 11 * B.coeff 12 +
        (4 / 3 : k) * a2 * B.coeff 11 ^ 2 + 2 * H.coeff 22) = 0 := by
    linear_combination hc
  have hscalar := (mul_eq_zero.mp hfactor).resolve_left hf3
  linear_combination (1 / 2 : k) * hscalar

end AlignedSquareTerminalGBoundaryCapPostRestorationEight68

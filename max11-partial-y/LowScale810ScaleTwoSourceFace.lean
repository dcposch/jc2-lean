...
/-! # Scale-two source reduction for the normalized `(8,10)`, `H = 2` leaf
...
`(8,10)`.  It is the literal source of the nonic depression discriminator. -/
theorem nextCoefficientJacobianRow_810 {K : Type*}
    [Field K] [CharZero K] {p q : K[X][Y]} {j : K}
    (hp : p.natDegree = 8) (hq : q.natDegree = 10)
    (hjac : bivariateJacobian p q = Polynomial.C (Polynomial.C j)) :
    (p.coeff 8).derivative * (q.coeff 9 * Polynomial.C (9 : K)) +
        (p.coeff 7).derivative * (q.coeff 10 * Polynomial.C (10 : K)) -
      ((p.coeff 8 * Polynomial.C (8 : K)) * (q.coeff 9).derivative +
        (p.coeff 7 * Polynomial.C (7 : K)) * (q.coeff 10).derivative) = 0 := by
  have hcoeff := congrArg (fun r : K[X][Y] => r.coeff 16) hjac
  simp only [bivariateJacobian, Polynomial.coeff_sub, Polynomial.coeff_mul,
    coeff_xderiv, Polynomial.coeff_derivative, Polynomial.coeff_C,
    show (16 : ℕ) ≠ 0 by norm_num, ite_false] at hcoeff
  have hanti : (Finset.HasAntidiagonal.antidiagonal 16 :
      Finset (ℕ × ℕ)) =
      ({(0, 16), (1, 15), (2, 14), (3, 13), (4, 12), (5, 11), (6, 10),
        (7, 9), (8, 8), (9, 7), (10, 6), (11, 5), (12, 4), (13, 3),
        (14, 2), (15, 1), (16, 0)} : Finset (ℕ × ℕ)) := by decide
  rw [hanti] at hcoeff
...
weighted derivative of `N = 5 p₇ H - 4 q₉`. -/
theorem imprimitiveDiscriminatorRow_eq_zero_810 {K : Type*}
  ...
  dsimp only
  have hrow := nextCoefficientJacobianRow_810 hp hq hjac
  rw [hp8, hq10] at hrow
  have hscaled : H ^ 3 *
      ((2 : K[X]) * H *
          ((5 : K[X]) * p.coeff 7 * H -
            (4 : K[X]) * q.coeff 9).derivative -
        (9 : K[X]) *
          ((5 : K[X]) * p.coeff 7 * H -
            (4 : K[X]) * q.coeff 9) * H.derivative) = 0 := by
    calc
      H ^ 3 *
          ((2 : K[X]) * H *
              ((5 : K[X]) * p.coeff 7 * H -
            ...
            (9 : K[X]) *
              ((5 : K[X]) * p.coeff 7 * H -
                (4 : K[X]) * q.coeff 9) * H.derivative) =
          (H ^ 4).derivative * (q.coeff 9 * (9 : K[X])) +
              (p.coeff 7).derivative * (H ^ 5 * (10 : K[X])) -
            ((H ^ 4 * (8 : K[X])) * (q.coeff 9).derivative +
              (p.coeff 7 * (7 : K[X])) * (H ^ 5).derivative) := by
                ...
                ring
      _ = 0 := hrow
  exact (mul_eq_zero.mp hscaled).resolve_left (pow_ne_zero 3 hH)

/-- Polynomial first integral of the `(8,10)` source discriminator. -/
theorem imprimitiveDiscriminatorPowerRelation_810 {K : Type*}
    [Field K] [CharZero K] {p q : K[X][Y]} {j : K} {H : K[X]}
    (hp : p.natDegree = 8) (hq : q.natDegree = 10)
    (hjac : bivariateJacobian p q = Polynomial.C (Polynomial.C j))
    (hH : H ≠ 0)
...
theorem squareCore_of_nonzero_imprimitiveRelation_810
...
/-- Source-facing first integral for a normalized scale-two `(8,10)` pair. -/
theorem normalized810ScaleTwo_discriminatorPowerRelation
  ...
  exact imprimitiveDiscriminatorPowerRelation_810
    hp hq hjac hH (by simpa only [p] using hp8)
      (by simpa only [q] using hq10)

/-- The exact first-face dichotomy for every normalized scale-two `(8,10)`
source: the discriminator vanishes, or its square is a nonzero scalar times
the ninth power of the quadratic common core. -/
theorem normalized810ScaleTwo_discriminatorFirstFace
    {K : Type*} [Field K] [CharZero K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized810LeadingCoreSource P Q H 2) :
    let p := (Polynomial.Bivariate.equivMvPolynomial K).symm P
  ...
  dsimp only
  obtain ⟨κ, hrelation⟩ :=
    normalized810ScaleTwo_discriminatorPowerRelation hsource
  by_cases hκ : κ = 0
  · left
    have hNsquare :
        ((5 : K[X]) *
            ((Polynomial.Bivariate.equivMvPolynomial K).symm P).coeff 7 * H -
          (4 : K[X]) *
...
theorem normalized810ScaleTwo_imprimitiveDiscriminatorDichotomy
    {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized810LeadingCoreSource P Q H 2) :
    let p := (Polynomial.Bivariate.equivMvPolynomial K).symm P
  ...
  · right
    obtain ⟨h, lambda, hh, hlambda, hH, hN, _hlambdaSq⟩ :=
      squareCore_of_nonzero_imprimitiveRelation_810
...
theorem normalized810ScaleTwo_nonzeroFace_has_linear_root
    {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized810LeadingCoreSource P Q H 2)
    (hnonzero :
  ...
  rcases normalized810ScaleTwo_imprimitiveDiscriminatorDichotomy
...
#print axioms imprimitiveDiscriminatorPowerRelation_810
#print axioms squareCore_of_nonzero_imprimitiveRelation_810
#print axioms normalized810ScaleTwo_discriminatorFirstFace
#print axioms normalized810ScaleTwo_imprimitiveDiscriminatorDichotomy
#print axioms normalized810ScaleTwo_nonzeroFace_has_linear_root

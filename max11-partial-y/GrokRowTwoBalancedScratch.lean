...
def grokBalancedRowTwoInner68 (A B c d e : k[X]) : k[X] :=
...
theorem grokBalancedRowTwo_eq_inner68 (A B c d e : k[X]) :
    secondaryResidualRowTwoPolynomial68 A B c d e =
      (-4 / 27 : k) • grokBalancedRowTwoInner68 A B c d e := by
  simp only [secondaryResidualRowTwoPolynomial68, grokBalancedRowTwoInner68,
...
theorem grokBalancedRightCoreScalar68 (N G a b c : k) :
    (-2 : k) * (a * b ^ 3 * (3 * N - G)) -
      (2 / 3 : k) * (2 * N * a * b ^ 3) +
      (6 : k) * ((11 * N - 3 * G) * b * c ^ 2) =
      (2 : k) * (11 * N - 3 * G) * b *
...
theorem grokBalancedRightCoreScalar68_of_disc
    (N G a b c : k) (hdisc : a * b ^ 2 + (3 : k) * c ^ 2 = 0) :
    (-2 : k) * (a * b ^ 3 * (3 * N - G)) -
      (2 / 3 : k) * (2 * N * a * b ^ 3) +
      (6 : k) * ((11 * N - 3 * G) * b * c ^ 2) =
      (-8 / 3 : k) * (11 * N - 3 * G) * a * b ^ 3 := by
  rw [grokBalancedRightCoreScalar68, hdisc]
  ring

/-- On the discriminant `a*b^2+3*c^2=0`, the coinciding core and `d*e`
faces combine as `2*M*(4*b*c^2-9*d*e)`. -/
theorem grokBalancedInnerScalar68 (M a b c d e : k)
...
prefactor `-8/27`. -/
theorem grokBalancedRowTwoScalar68 (M a b c d e : k)
    (hdisc : a * b ^ 2 + (3 : k) * c ^ 2 = 0) :
    (-4 / 27 : k) *
        ((-8 / 3 : k) * M * a * b ^ 3 + (-18 : k) * M * d * e) =
      (-8 / 27 : k) * M * ((4 : k) * b * c ^ 2 - (9 : k) * d * e) := by
  rw [grokBalancedInnerScalar68 M a b c d e hdisc]
  ring
...
theorem grokBalancedRowTwoInner_Q_degree_lt68
    (A B c d e : k[X]) (n g : ℕ)
    (hsmall : 3 * g < n)
    (hA : A.natDegree ≤ 2 * n)
    (hQ : (grokBalancedIncidenceQ68 B c d e).natDegree < 9 * n - 3 * g) :
    (A * derivative (grokBalancedIncidenceQ68 B c d e)).natDegree <
        11 * n - 3 * g - 1 ∧
      (derivative A * grokBalancedIncidenceQ68 B c d e).natDegree <
        11 * n - 3 * g - 1 := by
  have hv : 1 < 9 * n - 3 * g := by omega
  have h1 := grokBalanced_natDegree_mul_derivative_lt68 A
    (grokBalancedIncidenceQ68 B c d e) (2 * n) (9 * n - 3 * g) hA hv hQ
  have h2 := grokBalanced_natDegree_derivative_mul_lt68 A
    (grokBalancedIncidenceQ68 B c d e) (2 * n) (9 * n - 3 * g)
    (by omega) hA hQ
  have hi : 2 * n + (9 * n - 3 * g) - 1 = 11 * n - 3 * g - 1 := by omega
  exact ⟨by simpa only [hi] using h1, by simpa only [hi] using h2⟩

set_option maxHeartbeats 2000000 in
theorem grokBalancedRowTwoInner_Bd_degree_le68
    (B d : k[X]) (n g h : ℕ)
...
theorem grokRowTwo_coeff_balancedChamber68
  ...
  let Q := grokBalancedIncidenceQ68 B c d e
  let inner := grokBalancedRowTwoInner68 A B c d e
  set idx := 11 * n - 3 * g - 1
  have hface : 11 * n - 2 * h = 11 * n - 3 * g := by omega
  have hQdeg := grokBalancedRowTwoInner_Q_degree_lt68 A B c d e n g hsmall hA hQ
  ...
  have hAQ' : (derivative A * Q).natDegree < idx := by
    dsimp [idx]
    exact hQdeg.2
  have hBd := grokBalancedRowTwoInner_Bd_degree_le68 B d n g h hg hgh hh hsmall hB hd
  ...
  have hcore :
    ...
    have hsc := grokBalancedRightCoreScalar68_of_disc (n : k) (g : k)
  ...
  have hinner : inner.coeff idx =
    ...
    dsimp [inner, grokBalancedRowTwoInner68]
    simp only [coeff_add]
    rw [h0Q, h0Q', h0Bd, hde']
    linear_combination hcore
  rw [grokBalancedRowTwo_eq_inner68, coeff_smul, smul_eq_mul]
  change ((-4 / 27 : k) * inner.coeff idx) = _
  rw [hinner]
  exact grokBalancedRowTwoScalar68
    ((11 * n - 3 * g : ℕ) : k)
...
theorem grokRowTwo_balanced_coeff_eq_zero_iff68
    (A B c d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
    (hsmall : 3 * g < n) (hbalanced : 2 * h = 3 * g)
    (hA : A.natDegree ≤ 2 * n)
  ...
  rw [grokRowTwo_coeff_balancedChamber68 A B c d e n g h hg hgh hh hsmall
...
theorem grokRowTwo_eq_zero_imp_balancedRelation68
    (A B c d e : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
    (hsmall : 3 * g < n) (hbalanced : 2 * h = 3 * g)
    (hA : A.natDegree ≤ 2 * n)
  ...
  have hcoeff :
      (secondaryResidualRowTwoPolynomial68 A B c d e).coeff
        (11 * n - 3 * g - 1) = 0 := by
    simp [hrow]
  exact (grokRowTwo_balanced_coeff_eq_zero_iff68 A B c d e n g h hg hgh hh
    hsmall hbalanced hA hB hc hd he hQ hdisc).1 hcoeff

/-- Same conclusion if residual row two cancels a load strictly below the
cubic face. -/
theorem grokRowTwo_add_load_eq_zero_imp_balancedRelation68
...
end GrokRowTwoBalanced68

#print axioms grokBalancedRowTwo_eq_inner68
#print axioms grokBalancedRightCoreScalar68_of_disc
#print axioms grokBalancedInnerScalar68
#print axioms grokBalancedRowTwoScalar68
#print axioms grokBalancedRowTwoInner_Q_degree_lt68
#print axioms grokRowTwo_coeff_balancedChamber68
#print axioms grokRowTwo_balanced_coeff_eq_zero_iff68
#print axioms grokRowTwo_eq_zero_imp_balancedRelation68
#print axioms grokRowTwo_add_load_eq_zero_imp_balancedRelation68

import Sol68ScaleTwoAlignedSquareBoundaryGSignedRayLinearDZeroI3SevenReductionScratch

/-! # Exact source-backed `I4(7)` on the frozen `K/M` cells -/
...
section AlignedSquareBoundaryGSignedRayLinearDZeroI4SevenCross68

variable {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_i4seven_laurent68
    ...
    -(B.coeff 2 ^ 2 * B.coeff 3) / 3 +
      B.coeff 2 * G.coeff 5 + B.coeff 3 * G.coeff 4 +
      B.coeff 4 * G.coeff 3 + B.coeff 5 * G.coeff 2 +
      B.coeff 6 * G.coeff 1 + B.coeff 7 * G.coeff 0 +
      B.coeff 8 * G.coeff (-1) + B.coeff 9 * G.coeff (-2) +
      C.coeff 1 * F.coeff 6 + C.coeff 2 * F.coeff 5 +
      C.coeff 3 * F.coeff 4 + C.coeff 4 * F.coeff 3 +
      C.coeff 5 * F.coeff 2 + C.coeff 6 * F.coeff 1 +
      C.coeff 7 * F.coeff 0 +
      (3 / 4 : k) * epsilon * B.coeff 7 = 0 := by
  ...
  have hSco (n : ℤ) (hn : n ≠ 0) : S.coeff n = 2 * C.coeff n := by
    ...
    rw [if_neg hn]
    ring
  have hBG := laurent_coeff_mul_lower_seven68 B G 2 (-2) hB hG
  have hSF := laurent_coeff_mul_lower_six68 S F 1 0 hS hF
  have hB3 := laurent_coeff_mul_three_lower_succ68 B B B 2 2 2 hB hB hB
  ...
  rw [hSco 1 (by norm_num), hSco 2 (by norm_num),
    ...
    hSco 7 (by norm_num)] at hSF
  have hB3' : (B ^ 3).coeff 7 = 3 * B.coeff 2 ^ 2 * B.coeff 3 := by
    rw [show B ^ 3 = B * B * B by ring]
    rw [hB3]
    ring
  have hi4rhs : (HahnSeries.C i4 : LaurentSeries k).coeff 7 = 0 := by
  ...
  have hi4rewrite : rhoOneI4ZeroFactored68 (HahnSeries.C gamma)
    ...
    ring
  have hc := congrArg (fun X : LaurentSeries k => X.coeff 7) hi4
  rw [hi4rewrite] at hc
...
omit [IsAlgClosed k] in
def rhoOne_boundaryG_i4sevenHahnRow68
  ...
  -(B.coeff 2 ^ 2 * B.coeff 3) / 3 +
    ...
    C.coeff 3 * F.coeff 4 + C.coeff 4 * F.coeff 3 +
    C.coeff 5 * F.coeff 2 + C.coeff 6 * F.coeff 1 +
    C.coeff 7 * F.coeff 0 + (3 / 4 : k) * epsilon * B.coeff 7

omit [IsAlgClosed k] in
theorem ratFunc_boundaryG_i4seven68
    ...
    rhoOne_boundaryG_i4sevenHahnRow68 epsilon
  ...
  have hS0L : 2 * CL.coeff 0 + 3 * gamma = 0 := by
    rw [← hcs C 0]
    exact hS0
  have hf := rhoOne_boundaryG_i4seven_laurent68 gamma epsilon i4
  ...
  dsimp only [rhoOne_boundaryG_i4sevenHahnRow68]
  norm_num at hf' ⊢
  linear_combination hf'

def rhoOne_boundaryG_i4sevenExpandedSourceRow68
  ...
  rhoOne_boundaryG_i4sevenHahnRow68 T.source.epsilon B C F G

theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_i4seven_expanded_source68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u) :
    rhoOne_boundaryG_i4sevenExpandedSourceRow68 T = 0 := by
  have Qc := Q
  ...
  have hrow := ratFunc_boundaryG_i4seven68 T.root T.source.gamma
...
omit [IsAlgClosed k] in
def rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r b6 : k) : k :=
  let A := ratFuncAtHahn46 T.root T.source.A
  ...
  let G := rhoOneTangentEDefectHahn68 A C
    (rhoOneEDefectHahn68 A (ratFuncAtHahn46 T.root T.source.E))
  B.coeff 2 * G.coeff 5 + B.coeff 4 * G.coeff 3 +
    B.coeff 5 * G.coeff 2 + b6 * G.coeff 1 +
    B.coeff 7 * G.coeff 0 +
    B.coeff 8 * (sigma * r ^ 3 * B.coeff 2 / 27) +
    B.coeff 9 * G.coeff (-2) -
    (sigma * r * B.coeff 2 / 3) * F.coeff 6 +
    C.coeff 2 * F.coeff 5 + C.coeff 3 * F.coeff 4 +
    C.coeff 4 * F.coeff 3 + C.coeff 5 * F.coeff 2 +
    C.coeff 6 * (A.coeff (-1) * B.coeff 2 / 3) +
    C.coeff 7 * (r ^ 2 * B.coeff 2 / 9) +
    (3 / 4 : k) * T.source.epsilon * B.coeff 7

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_signedRayLinear_dZero_i4seven_reduce68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r : k)
    ...
    (hi4 : rhoOne_boundaryG_i4sevenExpandedSourceRow68 T = 0) :
    rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68 T sigma r
  ...
  dsimp only [rhoOne_boundaryG_i4sevenExpandedSourceRow68,
    rhoOne_boundaryG_i4sevenHahnRow68,
    rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68] at hi4 ⊢
  rw [hb3, hgm1, hc1, hf0, hf1] at hi4
  linear_combination hi4

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_signedRayLinear_dZero_i4seven_affine68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r b6 : k) :
    let A := ratFuncAtHahn46 T.root T.source.A
    ...
    let G := rhoOneTangentEDefectHahn68 A C
      (rhoOneEDefectHahn68 A (ratFuncAtHahn46 T.root T.source.E))
    rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68 T sigma r b6 =
      rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68 T sigma r 0 +
        G.coeff 1 * b6 := by
  dsimp only [rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68]
  ring

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_signedRayLinear_dZero_i3seven_i4seven_cross68
    (J70 N H0 g1 b6 : k)
    (h3 : J70 + N * b6 = 0) (h4 : H0 + g1 * b6 = 0) :
    g1 * J70 - N * H0 = 0 := by
  linear_combination g1 * h3 - N * h4

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_signedRayLinear_dZero_i4seven_M_transport68
    (M K0 H0 g1 b6 : k) (hM : M ≠ 0)
    (hb6 : b6 = -K0 / M) (h4 : H0 + g1 * b6 = 0) :
    M * H0 - g1 * K0 = 0 := by
  have hb6c : M * b6 + K0 = 0 := by
    rw [hb6]
    field_simp
    ring
  linear_combination M * h4 - g1 * hb6c

def rhoOne_boundaryG_signedRayLinearDZeroI4SevenMSplit68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r : k) : Prop :=
  let A := ratFuncAtHahn46 T.root T.source.A
  ...
  (M = 0 ∧ K0 = 0 ∧
    ...
    rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68 T sigma r
      (B.coeff 6) = 0 ∧ G.coeff 1 * J70 - N * H0 = 0) ∨
  (M ≠ 0 ∧ B.coeff 6 = -K0 / M ∧
    J0 = -(8 * L * K0) / (81 * M) ∧
    ...
    rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68 T sigma r
      (B.coeff 6) = 0 ∧ M * H0 - G.coeff 1 * K0 = 0)

def rhoOne_boundaryG_signedRayLinearDZeroI4SevenCells68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r : k) : Prop :=
  let A := ratFuncAtHahn46 T.root T.source.A
  ...
  let I0 := rhoOne_boundaryG_signedRayLinearDZeroI3FourAtB6Residual68
    T sigma r 0
  let NewSplit := rhoOne_boundaryG_signedRayLinearDZeroI4SevenMSplit68
    T sigma r
  D0 = 0 →
...
def RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI4SevenCross68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
  ...
  rhoOne_boundaryG_i4sevenExpandedSourceRow68 T = 0 ∧
...
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_dZero_i4seven_cross68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u)
    (R : RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI3SevenReduction68 T) :
    RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI4SevenCross68 T := by
  have hrow := T.rhoOne_boundaryG_i4seven_expanded_source68 Q
  have Rc := R
  dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI3SevenReduction68] at Rc
  rcases Rc with ⟨Rold, hi3sevenSource, sigma, r, hsigma, hr0, hb, hgm2,
    hr, hray, hrf, hlinear, hb3, hwitness, hcellsOld⟩
  have hcells : rhoOne_boundaryG_signedRayLinearDZeroI4SevenCells68
      T sigma r := by
    dsimp only [rhoOne_boundaryG_signedRayLinearDZeroI4SevenCells68]
    intro hD
    have hactual := rhoOne_boundaryG_signedRayLinear_dZero_i4seven_reduce68
      T sigma r hsigma hr0 hray hrf hlinear hb3 hD hrow
    have haff := rhoOne_boundaryG_signedRayLinear_dZero_i4seven_affine68
    ...
    have hH : rhoOne_boundaryG_signedRayLinearDZeroI4SevenAtB6_68
      ...
      exact hactual
    have refineSplit
        (hsplit : rhoOne_boundaryG_signedRayLinearDZeroI3SevenMSplit68
          T sigma r) :
        rhoOne_boundaryG_signedRayLinearDZeroI4SevenMSplit68 T sigma r := by
      dsimp only [rhoOne_boundaryG_signedRayLinearDZeroI3SevenMSplit68,
        rhoOne_boundaryG_signedRayLinearDZeroI4SevenMSplit68] at hsplit ⊢
      rcases hsplit with hm | hm
      · have hdet := rhoOne_boundaryG_signedRayLinear_dZero_i3seven_i4seven_cross68
            ...
            hm.2.2.2 hH
        exact Or.inl ⟨hm.1, hm.2.1, hm.2.2.1, hm.2.2.2,
          hactual, hdet⟩
      · have ht := rhoOne_boundaryG_signedRayLinear_dZero_i4seven_M_transport68
            ((rhoOneTangentEDefectHahn68
              (ratFuncAtHahn46 T.root T.source.A)
              (rhoOneCDefectHahn68 (ratFuncAtHahn46 T.root T.source.A)
                (ratFuncAtHahn46 T.root T.source.C0))
            ...
            ((ratFuncAtHahn46 T.root T.source.B).coeff 6)
            hm.1 hm.2.1 hH
        exact Or.inr ⟨hm.1, hm.2.1, hm.2.2.1, hm.2.2.2.1,
          hm.2.2.2.2.1, hm.2.2.2.2.2, hactual, ht⟩
    have hOld := hcellsOld hD
    rcases hOld with hz | hn
    · exact Or.inl ⟨hz.1, hz.2.1, hz.2.2.1, hz.2.2.2.1,
        refineSplit hz.2.2.2.2⟩
    · exact Or.inr ⟨hn.1, hn.2.1, hn.2.2.1,
        refineSplit hn.2.2.2⟩
  dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI4SevenCross68]
  exact ⟨R, hrow, sigma, r, hsigma, hr0, hb, hgm2, hr, hray,
    hrf, hlinear, hb3, hwitness, hcells⟩

theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_dZero_i4seven_cross_selector68
  ...
  rcases T.rhoOne_boundaryG_signedRay_linear_dZero_i3seven_reduction_selector68
      hp hH hp6 hp5 hp4 Q with hcoord | hsigned | hnon
  · exact Or.inl hcoord
  · rcases hsigned.2 with hzero | hqr
    · exact Or.inr (Or.inl ⟨hsigned.1,
        Or.inl (T.rhoOne_boundaryG_signedRay_linear_dZero_i4seven_cross68
          hsigned.1 hzero)⟩)
    · exact Or.inr (Or.inl ⟨hsigned.1, Or.inr hqr⟩)
  · exact Or.inr (Or.inr hnon)

end AlignedSquareBoundaryGSignedRayLinearDZeroI4SevenCross68

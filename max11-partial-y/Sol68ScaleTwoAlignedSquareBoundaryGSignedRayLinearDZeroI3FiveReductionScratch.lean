import Sol68ScaleTwoAlignedSquareBoundaryGSignedRayLinearDZeroKWallSplitScratch

/-! # Expand and reduce exact `I3(5)` on the two K-wall cells -/
...
theorem laurent_coeff_mul_three_lower_five_boundary68
  ...
  rw [h0, h1, h2, h3, h4, h5] at ho
  convert ho using 1 <;> ring

omit [IsAlgClosed k] in
/-- Exact finite convolution at coefficient five of the constant `I3`. -/
theorem rhoOne_boundaryG_i3five_laurent68
    ...
    (-(8 / 9 : k)) *
        (A.coeff (-2) * B.coeff 2 * F.coeff 5 +
          (A.coeff (-2) * B.coeff 3 + A.coeff (-1) * B.coeff 2) * F.coeff 4 +
          (A.coeff (-2) * B.coeff 4 + A.coeff (-1) * B.coeff 3 +
            A.coeff 0 * B.coeff 2) * F.coeff 3 +
          (A.coeff (-2) * B.coeff 5 + A.coeff (-1) * B.coeff 4 +
            A.coeff 0 * B.coeff 3 + A.coeff 1 * B.coeff 2) * F.coeff 2 +
          ...
          (A.coeff (-2) * B.coeff 7 + A.coeff (-1) * B.coeff 6 +
            ...
            A.coeff 2 * B.coeff 3 + A.coeff 3 * B.coeff 2) * F.coeff 0) -
      (8 / 9 : k) *
        (B.coeff 2 ^ 2 * C.coeff 1 +
          2 * B.coeff 2 * B.coeff 3 * C.coeff 0) +
      (8 / 3 : k) *
        (C.coeff 1 * G.coeff 4 + C.coeff 2 * G.coeff 3 +
          C.coeff 3 * G.coeff 2 + C.coeff 4 * G.coeff 1 +
          C.coeff 5 * G.coeff 0 + C.coeff 6 * G.coeff (-1) +
          C.coeff 7 * G.coeff (-2)) +
      (8 / 3 : k) *
        (F.coeff 0 * F.coeff 5 + F.coeff 1 * F.coeff 4 +
          F.coeff 2 * F.coeff 3) -
      (4 / 3 : k) * gamma * B.coeff 2 * B.coeff 3 +
      2 * epsilon * C.coeff 5 = 0 := by
  ...
  have hSco (n : ℤ) (hn : n ≠ 0) : S.coeff n = 2 * C.coeff n := by
    ...
    rw [if_neg hn]
    ring
  have hABF := laurent_coeff_mul_three_lower_five_boundary68
  ...
  have hBBC := laurent_coeff_mul_lower_succ68 (B ^ 2) C 4 0 hBBord hC
  have hSG := laurent_coeff_mul_lower_six68 S G 1 (-2) hS hG
  have hFF := laurent_coeff_mul_lower_five68 F F 0 0 hF hF
  norm_num at hABF hBB0 hBB1 hBBC hSG hFF
  have hBBpow0 : (B ^ 2).coeff 4 = B.coeff 2 * B.coeff 2 := by
    simpa only [pow_two] using hBB0
  have hBBpow1 : (B ^ 2).coeff 5 = B.coeff 2 * B.coeff 3 +
      B.coeff 3 * B.coeff 2 := by
    simpa only [pow_two] using hBB1
  rw [hBBpow0, hBBpow1] at hBBC
  rw [hSco 1 (by norm_num), hSco 2 (by norm_num),
    hSco 3 (by norm_num), hSco 4 (by norm_num),
    hSco 5 (by norm_num), hSco 6 (by norm_num),
    hSco 7 (by norm_num)] at hSG
  have hi3rhs : (HahnSeries.C i3 : LaurentSeries k).coeff 5 = 0 := by
  ...
  have hi3rewrite : rhoOneI3CapFactored68 (HahnSeries.C gamma)
    ...
    ring
  have hc := congrArg (fun X : LaurentSeries k => X.coeff 5) hi3
  rw [hi3rewrite] at hc
  simp only [HahnSeries.coeff_add, hscale] at hc
  rw [hABF, hBBC, hSG, hFF, hBBpow1, hi3rhs] at hc
  linear_combination hc

omit [IsAlgClosed k] in
def rhoOne_boundaryG_i3fiveHahnRow68
  ...
  (-(8 / 9 : k)) *
      (A.coeff (-2) * B.coeff 2 * F.coeff 5 +
        ...
        (A.coeff (-2) * B.coeff 7 + A.coeff (-1) * B.coeff 6 +
          A.coeff 0 * B.coeff 5 + A.coeff 1 * B.coeff 4 +
          A.coeff 2 * B.coeff 3 + A.coeff 3 * B.coeff 2) * F.coeff 0) -
    (8 / 9 : k) * (B.coeff 2 ^ 2 * C.coeff 1 +
      2 * B.coeff 2 * B.coeff 3 * C.coeff 0) +
    (8 / 3 : k) * (C.coeff 1 * G.coeff 4 + C.coeff 2 * G.coeff 3 +
      C.coeff 3 * G.coeff 2 + C.coeff 4 * G.coeff 1 +
      C.coeff 5 * G.coeff 0 + C.coeff 6 * G.coeff (-1) +
      C.coeff 7 * G.coeff (-2)) +
    (8 / 3 : k) * (F.coeff 0 * F.coeff 5 +
      F.coeff 1 * F.coeff 4 + F.coeff 2 * F.coeff 3) -
    (4 / 3 : k) * gamma * B.coeff 2 * B.coeff 3 +
    2 * epsilon * C.coeff 5

/-- The expanded fifth row in the actual source chart. -/
def rhoOne_boundaryG_i3fiveExpandedSourceRow68
  ...
  rhoOne_boundaryG_i3fiveHahnRow68 T.source.gamma T.source.epsilon A B C F G

omit [IsAlgClosed k] in
/-- On the signed `D=0` cell the expanded fifth row loses `B[6]`
identically. -/
def rhoOne_boundaryG_signedRayLinearDZeroI3FiveReduced68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
  ...
  (-(8 / 9 : k)) *
      ((-(r ^ 2 / 3)) * b * F.coeff 5 + A.coeff (-1) * b * F.coeff 4 +
        ((-(r ^ 2 / 3)) * B.coeff 4 + A.coeff 0 * b) * F.coeff 3 +
        ((-(r ^ 2 / 3)) * B.coeff 5 + A.coeff (-1) * B.coeff 4 +
          A.coeff 1 * b) * F.coeff 2 +
        (A.coeff (-1) * B.coeff 5 + A.coeff 0 * B.coeff 4 +
          A.coeff 2 * b) * f1z +
        ((-(r ^ 2 / 3)) * B.coeff 7 + A.coeff 0 * B.coeff 5 +
          A.coeff 1 * B.coeff 4 + A.coeff 3 * b) * f0z) -
    (8 / 9 : k) * b ^ 2 * (-(sigma * r * b / 3)) +
    (8 / 3 : k) *
      ((-(sigma * r * b / 3)) * G.coeff 4 + C.coeff 2 * G.coeff 3 +
        C.coeff 3 * G.coeff 2 + C.coeff 4 * G.coeff 1 +
        C.coeff 5 * G.coeff 0 + C.coeff 6 * (sigma * r ^ 3 * b / 27) +
        C.coeff 7 * G.coeff (-2)) +
    (8 / 3 : k) * (f0z * F.coeff 5 + f1z * F.coeff 4 +
      F.coeff 2 * F.coeff 3) + 2 * T.source.epsilon * C.coeff 5

omit [IsAlgClosed k] in
theorem rhoOne_boundaryG_signedRayLinear_dZero_i3five_reduce68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (sigma r : k)
    ...
    rhoOne_boundaryG_i3fiveExpandedSourceRow68 T =
      rhoOne_boundaryG_signedRayLinearDZeroI3FiveReduced68 T sigma r := by
  ...
  dsimp only [rhoOne_boundaryG_i3fiveExpandedSourceRow68,
    rhoOne_boundaryG_i3fiveHahnRow68,
    rhoOne_boundaryG_signedRayLinearDZeroI3FiveReduced68]
  rw [ha, hb3, hc1, hgm1, hf0, hf1]
  ring

omit [IsAlgClosed k] in
theorem ratFunc_boundaryG_i3five68
    ...
    rhoOne_boundaryG_i3fiveHahnRow68 gamma epsilon
  ...
  have hS0L : 2 * CL.coeff 0 + 3 * gamma = 0 := by
    rw [← hcs C 0]
    exact hS0
  have hf := rhoOne_boundaryG_i3five_laurent68 gamma epsilon i3
    AL BL CL FL GLa hAL hBL hCL hFL hGL hS0L hi3L'
  have hf' := hf
  simp only [rhoOne_boundaryG_i3fiveHahnRow68, AL, BL, CL, FL, GLa,
  ...
  dsimp only [rhoOne_boundaryG_i3fiveHahnRow68]
  norm_num at hf' ⊢
  linear_combination hf'

/-- Source backwire for the finite expanded fifth row. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_i3five_expanded_source68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u) :
    rhoOne_boundaryG_i3fiveExpandedSourceRow68 T = 0 := by
  have Qc := Q
  ...
  have hrow := ratFunc_boundaryG_i3five68 T.root T.source.gamma
    T.source.epsilon i3 A B C F G hAH hBH hCH hFH hGH
      (by rw [hCmap]; exact hS0) hi3f
  rw [hCmap, hFmap, hGmap] at hrow
  exact hrow

/-- The K-split packet plus the common B6-free reduction of `I3(5)` on
its determinant-zero limb. -/
def RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI3FiveReduction68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
  ...
  rhoOne_boundaryG_i3fiveExpandedSourceRow68 T = 0 ∧
  ∃ sigma r : k, sigma ^ 2 = 1 ∧ r ≠ 0 ∧ B.coeff 2 ≠ 0 ∧
    ...
    (3 * F.coeff 1 - A.coeff (-1) * B.coeff 2 = 0 →
      rhoOne_boundaryG_signedRayLinearDZeroI3FiveReduced68 T sigma r = 0)

theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_dZero_i3five_reduction68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u)
    (R : RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroKWallSplit68 T) :
    RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI3FiveReduction68 T := by
  have hrow := T.rhoOne_boundaryG_i3five_expanded_source68 Q
  have Rc := R
  dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroKWallSplit68] at Rc
  rcases Rc with ⟨Rkeep, hcompact, sigma, r, hsigma, hr0, hb, hgm2,
    hr, hray, hrf, hlinear, hb3, hwitness⟩
  have hred : 3 * (rhoOneTangentDDefectHahn68
      (ratFuncAtHahn46 T.root T.source.A)
      (ratFuncAtHahn46 T.root T.source.B)
      (ratFuncAtHahn46 T.root T.source.D)).coeff 1 -
      (ratFuncAtHahn46 T.root T.source.A).coeff (-1) *
        (ratFuncAtHahn46 T.root T.source.B).coeff 2 = 0 →
      rhoOne_boundaryG_signedRayLinearDZeroI3FiveReduced68 T sigma r = 0 := by
    intro hD
    rw [← rhoOne_boundaryG_signedRayLinear_dZero_i3five_reduce68
      T sigma r hsigma hr0 hr hray hrf hlinear hb3 hD]
    exact hrow
  dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearDZeroI3FiveReduction68]
  exact ⟨R, hrow, sigma, r, hsigma, hr0, hb, hgm2, hr, hray,
    hrf, hlinear, hb3, hwitness, hred⟩

/-- Outer source selector with coordinate, QR, and nonsingular leaves
unchanged. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_dZero_i3five_reduction_selector68
  ...
  rcases T.rhoOne_boundaryG_signedRay_linear_dZero_kwall_split_selector68
      hp hH hp6 hp5 hp4 Q with hcoord | hsigned | hnon
  · exact Or.inl hcoord
  · rcases hsigned.2 with hzero | hqr
    · exact Or.inr (Or.inl ⟨hsigned.1,
        Or.inl (T.rhoOne_boundaryG_signedRay_linear_dZero_i3five_reduction68
          hsigned.1 hzero)⟩)
    · exact Or.inr (Or.inl ⟨hsigned.1, Or.inr hqr⟩)
  · exact Or.inr (Or.inr hnon)

end AlignedSquareBoundaryGSignedRayLinearDZeroI3FiveReduction68

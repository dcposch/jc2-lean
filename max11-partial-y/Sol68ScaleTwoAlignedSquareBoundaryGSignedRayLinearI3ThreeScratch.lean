import Sol68ScaleTwoAlignedSquareBoundaryGSignedRayLinearFactorScratch

/-! # Exact `I3(3)` on the refined signed linear factor -/
...
section AlignedSquareBoundaryGSignedRayLinearI3Three68

variable {k : Type*} [Field k] [CharZero k] [IsAlgClosed k]

omit [IsAlgClosed k] in
def rhoOne_boundaryG_i3threeRow68
    (epsilon : k) (A B C F G : LaurentSeries k) : k :=
  (-(8 / 9 : k)) *
      (A.coeff (-2) * B.coeff 2 * F.coeff 3 +
        (A.coeff (-2) * B.coeff 3 + A.coeff (-1) * B.coeff 2) * F.coeff 2 +
        (A.coeff (-2) * B.coeff 4 + A.coeff (-1) * B.coeff 3 +
          A.coeff 0 * B.coeff 2) * F.coeff 1 +
        (A.coeff (-2) * B.coeff 5 + A.coeff (-1) * B.coeff 4 +
          A.coeff 0 * B.coeff 3 + A.coeff 1 * B.coeff 2) * F.coeff 0) +
    (8 / 3 : k) *
      (C.coeff 1 * G.coeff 2 + C.coeff 2 * G.coeff 1 +
        C.coeff 3 * G.coeff 0 + C.coeff 4 * G.coeff (-1) +
        C.coeff 5 * G.coeff (-2)) +
    (8 / 3 : k) * (F.coeff 0 * F.coeff 3 + F.coeff 1 * F.coeff 2) +
    2 * epsilon * C.coeff 3

omit [IsAlgClosed k] in
/-- Coefficient three of the constant `I3` factor.  Its support contains
the genuinely new coordinates `B[5], C[5], F[3], G[2]`. -/
theorem rhoOne_boundaryG_i3three_laurent68
    ...
    rhoOne_boundaryG_i3threeRow68 epsilon A B C F G = 0 := by
  ...
  have hSco (n : ℤ) (hn : n ≠ 0) : S.coeff n = 2 * C.coeff n := by
    ...
    rw [if_neg hn]
    ring
  have hABF := laurent_coeff_mul_three_lower_three68
    A B F (-2) 2 0 hA hB hF
  have hSG := laurent_coeff_mul_lower_four68 S G 1 (-2) hS hG
  have hFF := laurent_coeff_mul_lower_three68 F F 0 0 hF hF
  norm_num at hABF hSG hFF
  rw [hSco 1 (by norm_num), hSco 2 (by norm_num),
    hSco 3 (by norm_num), hSco 4 (by norm_num),
    hSco 5 (by norm_num)] at hSG
...
theorem ratFunc_boundaryG_i3three68
    ...
    (-(8 / 9 : k)) *
        ((ratFuncAtHahn46 root A).coeff (-2) *
          ...
          ((ratFuncAtHahn46 root A).coeff (-2) *
              (ratFuncAtHahn46 root B).coeff 5 +
            (ratFuncAtHahn46 root A).coeff (-1) *
              (ratFuncAtHahn46 root B).coeff 4 +
            (ratFuncAtHahn46 root A).coeff 0 *
              (ratFuncAtHahn46 root B).coeff 3 +
      ...
      (8 / 3 : k) *
        ((ratFuncAtHahn46 root C).coeff 1 *
          ...
          (ratFuncAtHahn46 root C).coeff 3 *
            (ratFuncAtHahn46 root G).coeff 0 +
          (ratFuncAtHahn46 root C).coeff 4 *
            (ratFuncAtHahn46 root G).coeff (-1) +
          (ratFuncAtHahn46 root C).coeff 5 *
            (ratFuncAtHahn46 root G).coeff (-2)) +
      (8 / 3 : k) *
        ((ratFuncAtHahn46 root F).coeff 0 *
            (ratFuncAtHahn46 root F).coeff 3 +
          (ratFuncAtHahn46 root F).coeff 1 *
            (ratFuncAtHahn46 root F).coeff 2) +
      2 * epsilon * (ratFuncAtHahn46 root C).coeff 3 = 0 := by
  ...
  have hS0L : 2 * CL.coeff 0 + 3 * gamma = 0 := by
    rw [← hcs C 0]
    exact hS0
  have hf := rhoOne_boundaryG_i3three_laurent68 gamma epsilon i3
    AL BL CL FL GLa hAL hBL hCL hFL hGL hS0L hi3L'
  have hf' := hf
  simp only [rhoOne_boundaryG_i3threeRow68, AL, BL, CL, FL, GLa,
    ← hcs A (-2), ← hcs A (-1), ← hcs A 0, ← hcs A 1,
    ← hcs B 2, ← hcs B 3, ← hcs B 4, ← hcs B 5,
    ← hcs C 1, ← hcs C 2, ← hcs C 3, ← hcs C 4,
    ← hcs C 5, ← hcs F 0, ← hcs F 1, ← hcs F 2,
    ← hcs F 3, ← hcs G (-2), ← hcs G (-1), ← hcs G 0,
    ← hcs G 1, ← hcs G 2] at hf'
  convert hf' using 1 <;> norm_num

/-- The exact Hahn scalar contributed by `I3(3)`.  This is the first row in
the signed-ray chain containing `B[5]`, `C[5]`, `F[3]`, and `G[2]`. -/
def rhoOne_boundaryG_i3threeSourceRow68
  ...
  (-(8 / 9 : k)) *
    ...
    (8 / 3 : k) *
      (C.coeff 1 * G.coeff 2 + C.coeff 2 * G.coeff 1 +
        ...
        C.coeff 5 * G.coeff (-2)) +
    (8 / 3 : k) * (F.coeff 0 * F.coeff 3 + F.coeff 1 * F.coeff 2) +
    2 * T.source.epsilon * C.coeff 3

/-- Source backwire for `I3(3)`.  Only the already verified integral
factorization and boundary orders are used. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_i3three_source68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u) :
    rhoOne_boundaryG_i3threeSourceRow68 T = 0 := by
  have Qc := Q
  ...
  have hrow := ratFunc_boundaryG_i3three68 T.root T.source.gamma
    T.source.epsilon i3 A B C F G hAH hBH hCH hFH hGH
      (by rw [hCmap]; exact hS0) hi3f
  rw [hCmap, hFmap, hGmap] at hrow
  exact hrow

/-- The strict linear-factor child after consuming `I3(3)`. -/
def RhoOneAlignedSquareGNegTwoSignedRayLinearI3Three68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
...
/-- The alternative determinant factor, unchanged by the `I3(3)` step. -/
def RhoOneAlignedSquareGNegTwoSignedRayQRResidual68
    {p q : k[X][X]} {j : k} {h0 : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0) : Prop :=
  let A := ratFuncAtHahn46 T.root T.source.A
  let B := ratFuncAtHahn46 T.root T.source.B
...
/-- `I3(3)` is independent on the linear factor; its genuinely new row is
attached there while the alternative determinant factor is preserved. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_i3three68
    {p q : k[X][X]} {j : k} {h0 g u : k[X]}
    (T : SquareZeroLIntegratedLocalRootData68 p q j h0)
    (Q : RhoOneAlignedSquareGNegTwoBoundaryQuadraticResidual68 T g u)
    (S : RhoOneAlignedSquareGNegTwoSignedRayLinearRefined68 T) :
    RhoOneAlignedSquareGNegTwoSignedRayLinearI3Three68 T ∨
      RhoOneAlignedSquareGNegTwoSignedRayQRResidual68 T := by
  have hrow := T.rhoOne_boundaryG_i3three_source68 Q
  have Sc := S
  dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearRefined68] at Sc
  rcases Sc with ⟨sigma, r, hsigma, hr0, hb, hgm, hr, hray, hrf, hlin | hqr⟩
  · left
    dsimp only [RhoOneAlignedSquareGNegTwoSignedRayLinearI3Three68]
    exact ⟨sigma, r, hsigma, hr0, hb, hgm, hr, hray, hrf,
      hlin.1, hlin.2.1, hlin.2.2, hrow⟩
  · right
    dsimp only [RhoOneAlignedSquareGNegTwoSignedRayQRResidual68]
    exact ⟨sigma, r, hsigma, hr0, hb, hgm, hr, hray, hrf,
      hqr.1, hqr.2⟩

/-- Full source selector with coordinate and nonsingular siblings unchanged. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_boundaryG_signedRay_linear_i3three_selector68
  ...
  rcases T.rhoOne_boundaryG_signedRay_linear_selector68
      hp hH hp6 hp5 hp4 R with hcoord | hsigned | hnon
  · exact Or.inl hcoord
  · exact Or.inr (Or.inl ⟨hsigned.1,
      T.rhoOne_boundaryG_signedRay_linear_i3three68 hsigned.1 hsigned.2⟩)
  · exact Or.inr (Or.inr hnon)

end AlignedSquareBoundaryGSignedRayLinearI3Three68

...
/-! # Alpha-core cancellation below the wall `g = n`

The two vanishing row-one `alpha` cores of
`cubicLoadRowOnePolynomial68_eq_alphaRest` do not use `g = n`.  Off the
wall, under `0 < g < n`, the leftover
`cubicLoadRowOneAlphaRestPolynomial68` is termwise of degree at most
`10n-g-1`, which sits strictly below the cubic residual index
`12n-3g-1`.  After `l = 0` the same cutoff holds for
...
/-! ## Local copies of the wall file's private degree helpers -/
...
/-- Off the wall `g < n`, the leftover after the two vanishing `alpha`
cores lies strictly below the cubic residual index `12n-3g-1`. -/
theorem cubicLoadRowOneAlphaRestPolynomial68_degree_lt_cubicFace
  ...
  have hTr : Tr.natDegree ≤ 4 * n - g :=
    cubicLoadTAlphaRestPolynomial68_degree_le_of_gap_lt_radius
      alpha beta gamma epsilon A B c n g hn hg hg_lt hA hB hc
...
set_option maxHeartbeats 2000000 in
/-- After `l = 0`, the same cubic residual cutoff holds for the full
row-one load, by the exact core cancellation. -/
theorem cubicLoadRowOnePolynomial68_degree_lt_cubicFace_of_alphaRest
  ...
  exact cubicLoadRowOneAlphaRestPolynomial68_degree_lt_cubicFace
...
#print axioms cubicLoadRowOneAlphaRestPolynomial68_degree_lt_cubicFace
#print axioms cubicLoadRowOnePolynomial68_degree_lt_cubicFace_of_alphaRest

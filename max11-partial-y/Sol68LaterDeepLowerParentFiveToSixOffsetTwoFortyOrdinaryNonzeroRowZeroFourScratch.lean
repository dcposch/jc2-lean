import Sol68LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroTerminalI3Scratch

/-! # Offset-240 nonzero row-zero coefficient 4

Contracted row-zero coefficient `4` (expanded index `299`) is the next
unused ordinary source row.  On the terminal I3 cusp wall it fixes the
third cusp jet exactly.  The disjoint pair-zero successor remains available
through the imported offset-240 split.
-/
...
section LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroFour68
...
theorem mul_coeff_four_twoForty68 (p q : k[X]) :
    (p * q).coeff 4 =
...
theorem mul_derivative_coeff_four_twoForty68
    (A : k[X]) (hA : A.natDegree ≤ 4) :
    (A * derivative A).coeff 4 =
      5 * A.coeff 4 * A.coeff 1 +
        5 * A.coeff 3 * A.coeff 2 := by
  have hA5 : A.coeff 5 = 0 :=
    coeff_eq_zero_of_natDegree_lt (hA.trans_lt (by norm_num))
  have h := mul_coeff_four_twoForty68 A (derivative A)
  norm_num [coeff_derivative, hA5] at h
  linear_combination h

theorem secondaryResidualRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
    (A B c d e : k[X])
    ...
    (secondaryResidualRowZeroPolynomial68 A B c d e).coeff 4 =
      (4 / 27 : k) *
        (-(5 * A.coeff 4 * A.coeff 1 +
          ...
          9 * A.coeff 3 * d.coeff 0 * e.coeff 2 -
          12 * A.coeff 4 * d.coeff 0 * e.coeff 1) := by
  let F : k[X] := B * e + c * d
  ...
  have hG4 := mul_derivative_coeff_four_twoForty68 A hA
  have hAAFraw := mul_coeff_four_twoForty68 (A * derivative A) F
  rw [hFpos 1 (by norm_num), hFpos 2 (by norm_num),
    hFpos 3 (by norm_num), hFpos 4 (by norm_num), hG4, hF0] at hAAFraw
  have hAAF : (A * derivative A * F).coeff 4 =
      (5 * A.coeff 4 * A.coeff 1 + 5 * A.coeff 3 * A.coeff 2) *
        (B.coeff 0 * e.coeff 0 + c.coeff 0 * d.coeff 0) := by
  ...
  have hcsq4 : (c ^ 2).coeff 4 = c.coeff 2 ^ 2 := by
    ...
    rw [pow_two]
    linear_combination h
  have hBcRaw := mul_coeff_four_twoForty68
    (derivative A * B) (c ^ 2)
  rw [hP0, hP1, hP2, hP3, hP4, hcsq1, hcsq2, hcsq3, hcsq4]
      at hBcRaw
  have hBc : (derivative A * B * c ^ 2).coeff 4 =
      A.coeff 1 * B.coeff 0 * c.coeff 2 ^ 2 +
        4 * A.coeff 2 * B.coeff 0 * c.coeff 1 * c.coeff 2 +
        6 * A.coeff 3 * B.coeff 0 * c.coeff 0 * c.coeff 2 +
        3 * A.coeff 3 * B.coeff 0 * c.coeff 1 ^ 2 +
  ...
  have hA5 : A.coeff 5 = 0 :=
    coeff_eq_zero_of_natDegree_lt (hA.trans_lt (by norm_num))
  have hAdeRaw := mul_coeff_four_twoForty68 (derivative A) e
  rw [he3, he4]
      at hAdeRaw
  norm_num [coeff_derivative, hA5] at hAdeRaw
  have hAde : (derivative A * d * e).coeff 4 =
      3 * A.coeff 3 * d.coeff 0 * e.coeff 2 +
        4 * A.coeff 4 * d.coeff 0 * e.coeff 1 := by
    rw [hAdePoly, coeff_smul, smul_eq_mul]
    rw [hAdeRaw]
    ring
...
theorem cubicLoadRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
    ...
    (cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon 0 eta
      ...
      (2 / 3 : k) * gamma * A.coeff 3 * B.coeff 0 * c.coeff 2 +
        ...
        (5 / 9 : k) * epsilon * B.coeff 0 *
          (A.coeff 4 * A.coeff 1 + A.coeff 3 * A.coeff 2) -
        (10 / 9 : k) * gamma * d.coeff 0 *
          (A.coeff 4 * A.coeff 1 + A.coeff 3 * A.coeff 2) := by
  ...
  have hA5 : A.coeff 5 = 0 :=
    coeff_eq_zero_of_natDegree_lt (hA.trans_lt (by norm_num))
  have hderAcRaw := mul_coeff_four_twoForty68 (derivative A) c
  rw [hc3, hc4] at hderAcRaw
  norm_num [coeff_derivative, hA5] at hderAcRaw
  have h1 : (B * derivative A * c).coeff 4 =
      B.coeff 0 *
        (3 * A.coeff 3 * c.coeff 2 + 4 * A.coeff 4 * c.coeff 1) := by
    rw [hBderAcPoly, coeff_smul, smul_eq_mul, hderAcRaw]
    ring
  have hAA4 := mul_derivative_coeff_four_twoForty68 A hA
  have hBC : B = C (B.coeff 0) := eq_C_of_natDegree_le_zero hB
  have hABApoly : A * B * derivative A =
    ...
    calc
      ...
      _ = B.coeff 0 • (A * derivative A) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h3 : (A * B * derivative A).coeff 4 =
      B.coeff 0 * (5 * A.coeff 4 * A.coeff 1 +
        5 * A.coeff 3 * A.coeff 2) := by
    rw [hABApoly, coeff_smul, hAA4, smul_eq_mul]
  have hDC : d = C (d.coeff 0) := eq_C_of_natDegree_le_zero hd
  have hAAdpoly : A * derivative A * d =
    ...
    calc
      ...
      _ = d.coeff 0 • (A * derivative A) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h4 : (A * derivative A * d).coeff 4 =
      d.coeff 0 * (5 * A.coeff 4 * A.coeff 1 +
        5 * A.coeff 3 * A.coeff 2) := by
    rw [hAAdpoly, coeff_smul, hAA4, smul_eq_mul]
  have hlow2 : (B * derivative e).natDegree < 4 := by
    compute_degree; omega
  have hlow5 : (d * derivative c).natDegree < 4 := by
    compute_degree; omega
  have hlow6 : (d * derivative A).natDegree < 4 := by
    compute_degree; omega
...
theorem fiveToSix_ordinaryTwoForty_rowZero_four_jet68
    (a a3 a2 a1 b c c1 c0 d y e1 e0 gamma epsilon : k)
    ...
    (hrow : (4 / 27 : k) *
          (-(5 * a * a1 + 5 * a3 * a2) * (b * e0 + c0 * d) +
            a1 * b * c ^ 2 + 4 * a2 * b * c1 * c +
            6 * a3 * b * c0 * c + 3 * a3 * b * c1 ^ 2 +
            8 * a * b * c0 * c1 - 9 * a3 * d * y -
            12 * a * d * e1) +
        (2 / 3 : k) * gamma * a3 * b * c +
        (8 / 9 : k) * gamma * a * b * c1 -
        (5 / 9 : k) * epsilon * b * (a * a1 + a3 * a2) -
        (10 / 9 : k) * gamma * d * (a * a1 + a3 * a2) = 0) :
    a1 * b ^ 4 * d - 2 * b ^ 5 * c1 +
        6 * b ^ 2 * c0 * c1 * d + 18 * c1 * d ^ 3 = 0 := by
  ...
  have hfactor :
      (4 / 27 : k) *
        ...
        (8 / 9 : k) * gamma * a * b * c1 -
        (5 / 9 : k) * epsilon * b * (a * a1 + a3 * a2) -
        (10 / 9 : k) * gamma * d * (a * a1 + a3 * a2) =
      ((-16 : k) * c ^ 2 / (27 * b ^ 3 * d)) *
        (a1 * b ^ 4 * d - 2 * b ^ 5 * c1 +
          6 * b ^ 2 * c0 * c1 * d + 18 * c1 * d ^ 3) := by
    rw [ha2', ha3', hepsilon, hgamma, hy', he1', ha']
    field_simp [hb, hc, hx]
    ring
  rw [hfactor] at hrow
  have hfactor_ne : (-16 : k) * c ^ 2 / (27 * b ^ 3 * d) ≠ 0 := by
    apply div_ne_zero
    · exact mul_ne_zero (by norm_num) (pow_ne_zero 2 hc)
    · exact mul_ne_zero
        (mul_ne_zero (by norm_num) (pow_ne_zero 3 hb)) hx
  exact (mul_eq_zero.mp hrow).resolve_left hfactor_ne

set_option maxHeartbeats 18000000 in
theorem fiveToSix_offsetTwoForty_nonzero_rowZero_four_source_packet68
    (alpha gamma epsilon eta : k) (A B c d e : k[X])
    (hA : A.natDegree ≤ 4) (hB : B.natDegree ≤ 0)
    (hc : c.natDegree ≤ 2) (hd : d.natDegree ≤ 0)
    (he : e.natDegree ≤ 2)
    ...
    (4 * B.coeff 0 ^ 5 +
      ...
      (A.coeff 1 * B.coeff 0 ^ 4 * d.coeff 0 -
        ...
        18 * c.coeff 1 * d.coeff 0 ^ 3 = 0) := by
  have hres :=
    secondaryResidualRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
      A B c d e hA hB hc hd he hface hfirst
  have hload := cubicLoadRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
    alpha gamma epsilon eta A B c d e hA hB hc hd he
  rw [coeff_add, hres, hload] at hrow
  refine ⟨hscalar, ?_⟩
  exact fiveToSix_ordinaryTwoForty_rowZero_four_jet68
    (A.coeff 4) (A.coeff 3) (A.coeff 2) (A.coeff 1)
    ...
    hsecond hscalar (by linear_combination hrow)

#print axioms mul_coeff_four_twoForty68
#print axioms mul_derivative_coeff_four_twoForty68
#print axioms secondaryResidualRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
#print axioms cubicLoadRowZeroPolynomial68_coeff_four_ordinaryTwoForty68
#print axioms fiveToSix_ordinaryTwoForty_rowZero_four_jet68
#print axioms fiveToSix_offsetTwoForty_nonzero_rowZero_four_source_packet68

end LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroFour68

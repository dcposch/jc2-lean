import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapT16SplitScratch

/-! # Source-facing split at the first nonlinear corrected-defect row -/
...
section AlignedSquareTerminalGBoundaryCapT16SplitBackwire68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareCapT16SplitBackwireHahnCharZero68 :
    CharZero (LaurentSeries k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℤ))

def RhoOneCapT16SplitPacket68
    (gamma : k) (A B CT F : LaurentSeries k) : Prop :=
  (gamma = 0 ∧
...
def RhoOneDoubleZeroTerminalGBoundaryCapT16SplitResidual68
    (root j gamma epsilon : k) (A0 : k[X])
    (h0R AR BR CR FR GR : RatFunc k)
    (A B C F G : HahnSeries ℚ k) : Prop :=
  RhoOneDoubleZeroTerminalGBoundaryCapCleanEndpointResidual68
    ...
    ((B.coeff (1 : ℚ) = 0 ∧ 2 * C.coeff 0 + 3 * gamma = 0) →
      ...
      RhoOneCapT16SplitPacket68 gamma AL BL CT FL)

theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapT16SplitResidual
    [IsAlgClosed k]
  ...
  dsimp only [RhoOneDoubleZeroTerminalGBoundaryCapT16SplitResidual68]
  have hprevRaw :=
    T.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapCleanEndpointResidual
      hp hH hp6 hp5 hp4 P hB hC hF hG hbeta hdelta hzeta
  rcases hprevRaw with hprev | htail
  · left
    refine ⟨hprev, ?_⟩
    intro hcoord hc1 hc2
    obtain ⟨i3, sold, cs, hnine⟩ := hprev.1.2 hcoord hc1 hc2
    have hseven := hnine.1
    ...
    have hsourcePrev := hprev.1.1.1.1.1
    ...
    · rcases hboundary with ⟨hcurrent, hf2, hF3, hb4, hB5,
      ...
      have hT9L := rhoOneCap_post_restoration_transport_nine68
        ...
        hT6Series hT7L hT8L
      have hR3exact := rhoOneCap_reduced_i3_exact_of_source_factor68
        T.source.gamma T.source.epsilon i3 AL BL CL CT FL Gser
        hAL hBL hCL hFL hR3 (by rfl) hi3L
      have hR4exact := rhoOneCap_reduced_i4_exact_of_source_factor68
        T.source.gamma T.source.epsilon BL CL CT FL Gser (by rfl) hi4L
      have hglobal := rhoOneCap_corrected_transport_global68
        T.source.gamma T.source.epsilon AL BL CL CT FL Gser
        (by rfl) hR3exact hR4exact
      have hT15 := rhoOneCap_corrected_transport_endpoint_fifteen68
        T.source.gamma AL BL CT FL hAL hBL hCT hFL
        (by rw [hFcoeff 3]; exact hf3ne) hHHorder hglobal
      have hT16 := rhoOneCap_corrected_transport_first_nonlinear_sixteen68
        T.source.gamma AL BL CT FL hAL hBL hCT hFL
        (by rw [hFcoeff 3]; exact hf3ne) hHHorder hglobal
      have hK30 := rhoOneCap_corrected_transport_order_thirty68
        T.source.gamma AL BL CT FL hAL hBL hCT hFL
        (by rw [hFcoeff 3]; exact hf3ne) hHHorder hglobal
      have hb11L : BL.coeff 11 ≠ 0 := by
        rw [hBcoeff 11]
        exact hb11ne
      by_cases hgamma0 : T.source.gamma = 0
      · left
        refine ⟨hgamma0, ?_⟩
        have hglobal0 : FL * rhoOneCapCorrectedDefect68 AL BL CT FL =
            -HahnSeries.C (4 / 9 : k) * (CT * BL ^ 3) := by
          dsimp only [rhoOneCapCorrectedDefect68]
          rw [hgamma0] at hglobal
          simpa using hglobal
        have hK37 := rhoOneCap_corrected_transport_gamma_zero_order_thirty_seven68
          AL BL CT FL hBL hCT hFL
          (by rw [hFcoeff 3]; exact hf3ne) (by
            simpa only [rhoOneCapCorrectedDefect68] using hK30) hglobal0
        have hrow := rhoOneCap_corrected_transport_gamma_zero_CT_correction68
          AL BL CT FL hBL hCT hFL hK37 hglobal0
        have hct7 : CT.coeff 7 ≠ 0 := by
          rw [hCcoeff 7 (by norm_num)]
          exact hc7ne
        have hexact := rhoOneCap_corrected_transport_gamma_zero_exact_order68
          AL BL CT FL hK37 (by rw [hFcoeff 3]; exact hf3ne)
          hct7 hb11L hrow
        exact ⟨hexact.1, hexact.2, hrow⟩
      · right
        refine ⟨hgamma0, ?_⟩
        have hexact := rhoOneCap_corrected_transport_gamma_ne_zero68
          T.source.gamma AL BL CT FL
          (by simpa only [rhoOneCapCorrectedDefect68] using hK30)
          (by rw [hFcoeff 3]; exact hf3ne) hb11L hgamma0 hT16
        exact ⟨hexact.1, hexact.2, hT16⟩
    · exact (hrays.1 hcoord.1).elim
  · exact Or.inr htail

end AlignedSquareTerminalGBoundaryCapT16SplitBackwire68

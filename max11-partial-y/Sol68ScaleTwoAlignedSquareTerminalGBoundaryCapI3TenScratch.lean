import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapThirdSourceScalarScratch

/-! # The exact lower-four `I₃[10]` jet above the nonzero cap -/
...
section AlignedSquareTerminalGBoundaryCapI3Ten68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareTerminalGBoundaryCapI3TenHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- The fourth jet above the lower face of a Laurent product. -/
theorem laurent_coeff_mul_lower_four68
  ...
  have htail := laurent_coeff_mul_lower_three68 Xt Y (m + 1) n hXt hY
...
/-- Exact coefficient ten of the third first integral at the cap. -/
theorem rhoOneI3CapFactored_cap_coeff_ten68
    ...
    4 * ((ratFuncAtHahn46 root C).coeff 7 * (ratFuncAtHahn46 root G).coeff 3 +
        (ratFuncAtHahn46 root C).coeff 8 * (ratFuncAtHahn46 root G).coeff 2 +
        (ratFuncAtHahn46 root C).coeff 9 * (ratFuncAtHahn46 root G).coeff 1 +
        (ratFuncAtHahn46 root C).coeff 10 * (ratFuncAtHahn46 root G).coeff 0 +
        (ratFuncAtHahn46 root C).coeff 11 * (ratFuncAtHahn46 root G).coeff (-1 : ℚ) +
        (ratFuncAtHahn46 root F).coeff 3 * (ratFuncAtHahn46 root F).coeff 7 +
        (ratFuncAtHahn46 root F).coeff 4 * (ratFuncAtHahn46 root F).coeff 6 +
        (1 / 2 : k) * (ratFuncAtHahn46 root F).coeff 5 ^ 2) +
      3 * epsilon * (ratFuncAtHahn46 root C).coeff 10 = 0 := by
  ...
  have hSL := ratFuncAtLaurent46_orderTop_ge_of_hahn68 root SR 7 (by
    ...
    exact hS)
  rw [hSRL] at hSL
  have hSG := laurent_coeff_mul_lower_four68 SL Gser 7 (-1) hSL hGL
  have hFF := laurent_coeff_mul_lower_four68 FL FL 3 3 hFL hFL
  norm_num at hSG hFF
  ...
  have hzBBC : (BL * BL * CL).coeff (10 : ℤ) = 0 :=
    HahnSeries.coeff_eq_zero_of_lt_orderTop
      (lt_of_lt_of_le (WithTop.coe_lt_coe.mpr (by norm_num)) hBBC)
  have hzBBC' : (BL * (BL * CL)).coeff (10 : ℤ) = 0 := by
    rw [← mul_assoc]
    exact hzBBC
  have hzBB : (BL ^ 2).coeff (10 : ℤ) = 0 := by
    rw [pow_two]
  ...
  have hc := congrArg (fun X : LaurentSeries k => X.coeff (10 : ℤ)) hfactorL'
  ...
  have hs1 : ((-8 / 9 : LaurentSeries k) * AL * BL * FL).coeff 10 = 0 := by
  ...
  have hs2 : ((8 / 9 : LaurentSeries k) * BL ^ 2 * CL).coeff 10 = 0 := by
    have hcst : (8 / 9 : LaurentSeries k) = HahnSeries.C (8 / 9 : k) := by
      rw [HahnSeries.C_eq_algebraMap, map_div₀, map_ofNat, map_ofNat]
    rw [pow_two, mul_assoc, mul_assoc, hcst, hscale, hzBBC', mul_zero]
  have hs3 : ((4 / 3 : LaurentSeries k) * SL * Gser).coeff 10 =
      (4 / 3 : k) * (SL * Gser).coeff 10 := by
    have hcst : (4 / 3 : LaurentSeries k) = HahnSeries.C (4 / 3 : k) := by
      rw [HahnSeries.C_eq_algebraMap, map_div₀, map_ofNat, map_ofNat]
    rw [mul_assoc, hcst, hscale]
  have hs4 : ((4 / 3 : LaurentSeries k) * FL ^ 2).coeff 10 =
      (4 / 3 : k) * (FL * FL).coeff 10 := by
    have hcst : (4 / 3 : LaurentSeries k) = HahnSeries.C (4 / 3 : k) := by
      rw [HahnSeries.C_eq_algebraMap, map_div₀, map_ofNat, map_ofNat]
    rw [pow_two, hcst, hscale]
  have hs5 : ((2 / 3 : LaurentSeries k) * HahnSeries.C gamma * BL ^ 2).coeff 10 = 0 := by
  ...
  have hs6 : ((2 : LaurentSeries k) * HahnSeries.C epsilon * CL).coeff 10 =
      (2 * epsilon) * CL.coeff 10 := by
    have hcst : (2 : LaurentSeries k) * HahnSeries.C epsilon =
      ...
      rw [h2, ← map_mul]
    rw [hcst, hscale]
  have hright : (HahnSeries.C i3 : LaurentSeries k).coeff 10 = 0 := by
  ...
  have hSL11 : SL.coeff 11 = 2 * CL.coeff 11 := by
    dsimp only [SL]
    simp [HahnSeries.C_apply, two_mul]
  rw [hSL7, hSL8, hSL9, hSL10, hSL11] at hc
  ...
  norm_num at hC7 hC8 hC9 hC10 hC11 hF3 hF4 hF5 hF6 hF7 hGm1 hG0 hG1 hG2 hG3
  dsimp only [CL, FL, Gser] at hc
  rw [← hC7, ← hC8, ← hC9, ← hC10, ← hC11, ← hF3, ← hF4,
    ← hF5, ← hF6, ← hF7, ← hGm1, ← hG0, ← hG1, ← hG2,
    ← hG3] at hc
  linear_combination (3 / 2 : k) * hc

def rhoOneI3CapTenCore68 (C F G : HahnSeries ℚ k) : k :=
  C.coeff 7 * G.coeff 3 + C.coeff 8 * G.coeff 2 +
    C.coeff 9 * G.coeff 1 + C.coeff 10 * G.coeff 0 +
    C.coeff 11 * G.coeff (-1 : ℚ) + F.coeff 3 * F.coeff 7 +
    F.coeff 4 * F.coeff 6 + (1 / 2 : k) * F.coeff 5 ^ 2

/-- Epsilon-free successive and cross-load eliminations generated by I₃[10]. -/
theorem rhoOneI3_cap_ten_load_elimination68
    (c9 c10 b14 x9 x10 y14 epsilon : k)
    (h9 : 4 * x9 + 3 * epsilon * c9 = 0)
    (h10 : 4 * x10 + 3 * epsilon * c10 = 0)
    (h14 : 4 * y14 + 3 * epsilon * b14 = 0) :
    c9 * x10 - c10 * x9 = 0 ∧ c10 * y14 - b14 * x10 = 0 := by
  constructor
  · linear_combination (1 / 4 : k) * c9 * h10 -
      (1 / 4 : k) * c10 * h9
  · linear_combination (1 / 4 : k) * c10 * h14 -
      (1 / 4 : k) * b14 * h10

def RhoOneDoubleZeroTerminalGBoundaryCapI3TenPacket68
  ...
  RhoOneDoubleZeroTerminalGBoundaryCapThirdSourcePacket68
    ...
    4 * rhoOneI3CapTenCore68 C F G + 3 * epsilon * C.coeff 10 = 0 ∧
    C.coeff 9 * rhoOneI3CapTenCore68 C F G -
      C.coeff 10 * rhoOneI3CapNineCore68 C F G = 0

end AlignedSquareTerminalGBoundaryCapI3Ten68

import LowScale68ScaleTwoAlignedSquareRhoOneCubicExceptionalDoubleZeroQuadrupleGainNext
...
section AlignedSquareRhoOneCubicExceptionalDoubleZeroQuintupleGainNext68

variable {k : Type*} [Field k] [CharZero k]

local instance exceptionalDoubleZeroQuintupleGainNextHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- At the next level of the gain tower, the coefficient at exponent five is
again the sparse `I₄` face. -/
theorem firstIntegral_mainTangent_doubleZero_quintupleGain_four68
    (gamma epsilon : k)
    (A B C F G : HahnSeries ℚ k)
    (hA : (↑(-2 : ℚ) : WithTop ℚ) ≤ A.orderTop)
    (hB : (↑(5 : ℚ) : WithTop ℚ) ≤ B.orderTop)
    (hC : (0 : WithTop ℚ) ≤ C.orderTop)
    (hF : (↑(5 : ℚ) : WithTop ℚ) ≤ F.orderTop)
    ...
    (firstIntegralFour68 0 A B (A ^ 2 / 3 + C) (A * B / 3 + F)
      ...
      (HahnSeries.C epsilon) (HahnSeries.C (0 : k))).coeff (5 : ℚ) =
        (2 / 3 : k) * rhoOneZeroLCubicDoubleZeroDoubleGainFour68
          gamma epsilon (B.coeff (5 : ℚ)) (C.coeff (0 : ℚ))
          (F.coeff (5 : ℚ)) (G.coeff (0 : ℚ)) := by
  ...
  have hdecH :
    ...
    ring
  have h5t1 := hz 5 (-8 / 27 : k) B A C F G 3 0 0 0 0
    5 (-2) 0 5 0 hB hA hC hF hG (by norm_num)
  have h5t3 := lead (8 / 3 : k) B G A C F 1 1 0 0 0
    5 0 (-2) 0 5 hB hG hA hC hF
  have h5t4 := lead (8 / 3 : k) C F A B G 1 1 0 0 0
    0 5 (-2) 5 0 hC hF hA hB hG
  have h5l6 := lead (4 * gamma) F A B C G 1 0 0 0 0
    5 (-2) 5 0 0 hF hA hB hC hG
  have h5l8 := lead (2 * epsilon) B A C F G 1 0 0 0 0
    5 (-2) 0 5 0 hB hA hC hF hG
  norm_num at h5t3 h5t4 h5l6 h5l8
...
theorem ratFunc_doubleZero_quintupleGain_four68
    (root : k) (A B C F G : RatFunc k)
    (gamma epsilon : k)
    (hA : (↑(-2 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root A).orderTop)
    (hB : (↑(5 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root B).orderTop)
    (hC : (0 : WithTop ℚ) ≤ (ratFuncAtHahn46 root C).orderTop)
    (hF : (↑(5 : ℚ) : WithTop ℚ) ≤ (ratFuncAtHahn46 root F).orderTop)
  ...
  have hface := firstIntegral_mainTangent_doubleZero_quintupleGain_four68
  ...
  have h5const : (HahnSeries.C (0 : k) : HahnSeries ℚ k).coeff (5 : ℚ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hface, h5const] at h5coeff
  linear_combination (3 / 2 : k) * h5coeff

/-- The source-facing fifth gain. The two old siblings are not folded into this
branch: the theorem assumes the stronger `B,F` orders explicitly. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_quintupleGain_next
  ...
  have hface := ratFunc_doubleZero_quintupleGain_four68 T.root AR BR CR FR GR
  ...
  by_cases hb5 : (ratFuncAtHahn46 T.root T.source.B).coeff (5 : ℚ) = 0
  · right
    have hb5z : BH.coeff ((5 : ℤ) : ℚ) = 0 := by
      norm_num
      simpa only [BH, BR] using hb5
    have hB6 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
      T.root BR 5 hBH hb5z
    norm_num at hB6
    have hB6out : (↑(6 : ℚ) : WithTop ℚ) ≤
        (ratFuncAtHahn46 T.root T.source.B).orderTop := by
      change (↑((6 : ℤ) : ℚ) : WithTop ℚ) ≤ BH.orderTop
      exact hB6
    refine ⟨hb5, hB6out, ?_⟩
    have hsplit := rhoOneZeroLCubicDoubleZero_doubleGain_split68
      T.source.gamma T.source.epsilon
        ...
        hface hb5
    rcases hsplit with hf5 | hS
    · left
      have hf5z : FRH.coeff ((5 : ℤ) : ℚ) = 0 := by
        norm_num
        rw [hFRmap]
        simpa only [AH, BH, DH, AR, BR, DR] using hf5
      have hF6 := ratFuncAtHahn46_orderTop_ge_int_succ_of_coeff_zero
        T.root FR 5 hFRH hf5z
      norm_num at hF6
      have hF6out : (↑(6 : ℚ) : WithTop ℚ) ≤
          (rhoOneTangentDDefectHahn68
            (ratFuncAtHahn46 T.root T.source.A)
            (ratFuncAtHahn46 T.root T.source.B)
...
end AlignedSquareRhoOneCubicExceptionalDoubleZeroQuintupleGainNext68

import Sol68FiveToSixCuspTerminalDepthTwoScratch

/-! # First genuinely loaded terminal coefficient

At row-zero coefficient `6N-4`, the wall `R=7N-2G=3` carries both
the terminal `A'de` edge and the pinned gamma load `A A'd`.  It forces
`gamma*a+2e=0` and, arithmetically, `(N,G)=(3,9)`.  The other wall
`L=3G-7N=3` carries the first `B^3` I4 jet and the matching leading
`A' B c^2` row term; the cusp relation makes their sum nonzero.  Away
from these walls both depths advance to four.
-/
...
section FiveToSixCuspTerminalDepthThree68
...
/-- A coefficient three below a product's degree bounds. -/
theorem coeff_mul_at_reflect_three68 (p q : k[X]) (u v : ℕ)
    (hp : p.natDegree ≤ u) (hq : q.natDegree ≤ v)
    (hsum : 3 ≤ u + v) :
    (p * q).coeff (u + v - 3) =
      (p.reflect u).coeff 0 * (q.reflect v).coeff 3 +
        (p.reflect u).coeff 1 * (q.reflect v).coeff 2 +
        (p.reflect u).coeff 2 * (q.reflect v).coeff 1 +
        (p.reflect u).coeff 3 * (q.reflect v).coeff 0 := by
  have h := congrArg (fun r : k[X] => r.coeff 3)
    (reflect_mul p q hp hq)
  rw [coeff_reflect, revAt_le hsum,
    mul_coeff_three_oneTwentyOrdinaryNonzero68] at h
  exact h

/-- Literal source row at the first genuinely loaded terminal index. -/
def FiveToSixTerminalDirectLowerRowThree68
  ...
  (secondaryResidualRowZeroPolynomial68 A B c d e +
      cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon zeta eta
        A B c ((1 / 3 : k) • (A * B) + d) e).coeff (6 * N - 4) = 0

theorem fiveToSix_terminal_directLowerRowThree68
    ...
    (hpacket : FiveToSixContractedCuspPacket68
      alpha gamma epsilon zeta eta terminal A B c d e N G
        (7 * N - 2 * G)) :
    FiveToSixTerminalDirectLowerRowThree68
  ...
  have hcoeff := congrArg (fun p : k[X] => p.coeff (6 * N - 4)) hrow0
  have hidx : 6 * N - 4 ≠ 0 := by omega
  simpa only [FiveToSixTerminalDirectLowerRowThree68, coeff_C, if_neg hidx]
    using hcoeff

/-- Exact third terminal I4 jet. -/
theorem fiveToSix_terminal_i4_faceSum_three68
    (alpha gamma epsilon zeta eta terminal : k)
    (A B c d e : k[X]) (N G : ℕ)
    (hNbig : 2 < N)
    (hpacket : FiveToSixContractedCuspPacket68
  ...
  have hidx : 0 < 2 * N - 3 := by omega
  have hBlow : B.natDegree < 2 * N - 3 := by omega
  have hcoeff := congrArg (fun p : k[X] => p.coeff (2 * N - 3)) hi4
  rw [cubicFirstIntegralFourPolynomial68_fiveToSix_residualCoordinates,
    hzeta] at hcoeff
  by_cases hLwall : 3 * G - 7 * N = 3
  · have hRne : 7 * N - 2 * G ≠ 3 := by omega
    have hdlow : d.natDegree < 2 * N - 3 := by omega
    have hB3deg : (B ^ 3).natDegree ≤ 2 * N - 3 := by
      compute_degree
      omega
    have hB3 := coeff_pow_at_bound68 B (3 * N - G) 3 hB
    rw [show 3 * (3 * N - G) = 2 * N - 3 from by omega] at hB3
  ...
  · by_cases hRwall : 7 * N - 2 * G = 3
    · have hB3low : (B ^ 3).natDegree < 2 * N - 3 := by
        compute_degree
        omega
      ...
      rw [show 2 * N - 3 = G - 2 * N from by omega] at hcoeff
      rw [show 2 * N - 3 = G - 2 * N from by omega]
      rw [coeff_add]
      linear_combination (3 / 8 : k) * hcoeff
    · have hB3low : (B ^ 3).natDegree < 2 * N - 3 := by
        compute_degree
...
/-- The `A A' (Be+cd)` contribution at the third terminal jet. -/
theorem fiveToSix_terminal_AAF_three68
    (alpha gamma epsilon zeta eta terminal : k)
    (A B c d e : k[X]) (N G : ℕ)
    (hNbig : 2 < N)
    (hpacket : FiveToSixContractedCuspPacket68
    ...
    (heTerminal : e.natDegree ≤ G - N)
    (hzeta : zeta = 0) :
    (A * derivative A * (B * e + c * d)).coeff (6 * N - 4) =
  ...
  have hF2 : F.coeff (2 * N - 2) = 0 := by
    simpa only [F] using fiveToSix_terminal_i4_faceSum_two_zero68
      ...
      ⟨hN, hgt5, hle6, hA, hB, hc, by omega, by omega, ha,
        ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
      hdTerminal heTerminal hzeta
  have hF3 : F.coeff (2 * N - 3) =
      (if 3 * G - 7 * N = 3 then
        (1 / 9 : k) * B.coeff (3 * N - G) ^ 3 else 0) -
      (if 7 * N - 2 * G = 3 then
        (3 / 2 : k) * gamma * d.coeff (G - 2 * N) else 0) := by
    simpa only [F] using fiveToSix_terminal_i4_faceSum_three68
      alpha gamma epsilon zeta eta terminal A B c d e N G hNbig
      ⟨hN, hgt5, hle6, hA, hB, hc, by omega, by omega, ha,
        ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
      hdTerminal heTerminal hzeta
  ...
  have hraw := coeff_mul_at_reflect_three68 (A * derivative A) F
    (4 * N - 1) (2 * N) hPdeg hFdeg (by omega)
  rw [show (4 * N - 1) + 2 * N - 3 = 6 * N - 4 from by omega] at hraw
  ...
  have hFr3 : (F.reflect (2 * N)).coeff 3 =
      (if 3 * G - 7 * N = 3 then
        (1 / 9 : k) * B.coeff (3 * N - G) ^ 3 else 0) -
      (if 7 * N - 2 * G = 3 then
        (3 / 2 : k) * gamma * d.coeff (G - 2 * N) else 0) := by
    rw [coeff_reflect, revAt_le (by omega : 3 ≤ 2 * N), hF3]
  rw [hPr0, hFr0, hFr1, hFr2, hFr3] at hraw
  simpa only [mul_zero, add_zero] using hraw

/-- The `L=3` terminal wall is incompatible with the all-nonzero cusp. -/
theorem fiveToSix_terminal_L_three_false68
    (alpha gamma epsilon zeta eta terminal : k)
    (A B c d e : k[X]) (N G : ℕ)
    (hNbig : 2 < N)
    (hpacket : FiveToSixContractedCuspPacket68
    ...
    (heTerminal : e.natDegree ≤ G - N)
    (hzeta : zeta = 0)
    (hL3 : 3 * G - 7 * N = 3) : False := by
  rcases hpacket with
    ⟨hN, hgt5, hle6, hA, hB, hc, _hd, _he, ha,
      ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
  have hAAF := fiveToSix_terminal_AAF_three68
    alpha gamma epsilon zeta eta terminal A B c d e N G hNbig
    ⟨hN, hgt5, hle6, hA, hB, hc, by omega, by omega, ha,
      ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
    hdTerminal heTerminal hzeta
  have hRne : 7 * N - 2 * G ≠ 3 := by omega
  rw [if_pos hL3, if_neg hRne, sub_zero] at hAAF
  have hmain := coeff_mul_mul_mul_derivative_at_bounds68 B c c A
    (3 * N - G) (4 * N - G) (4 * N - G) (2 * N) (by omega)
    hB hc hc hA
  rw [show (3 * N - G) + (4 * N - G) + (4 * N - G) + 2 * N - 1 =
    6 * N - 4 from by omega] at hmain
  have hmain' : (derivative A * B * c ^ 2).coeff (6 * N - 4) =
      A.coeff (2 * N) * B.coeff (3 * N - G) *
        c.coeff (4 * N - G) ^ 2 * (((2 * N : ℕ) : k)) := by
    simpa only [pow_two, mul_assoc, mul_comm, mul_left_comm] using hmain
  have hrow := fiveToSix_terminal_directLowerRowThree68
    alpha gamma epsilon zeta eta terminal A B c d e N G hNbig
    ⟨hN, hgt5, hle6, hA, hB, hc, by omega, by omega, ha,
      ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
  change (secondaryResidualRowZeroPolynomial68 A B c d e +
      cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon zeta eta
        A B c ((1 / 3 : k) • (A * B) + d) e).coeff (6 * N - 4) = 0 at hrow
  have hlow1 : (A * B ^ 2 * derivative d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow2 : (A * B * derivative B * d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow4 : (B * c * derivative e).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow5 : (B * d * derivative d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow6 : (derivative B * d ^ 2).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow7 : (c * derivative c * d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hdeLow : (derivative A * d * e).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload1 : (B * derivative A * c).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload2 : (B * derivative e).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload3 : (A * B * derivative A).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload4 : (A * derivative A * d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload5 : (d * derivative c).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hload6 : (d * derivative A).natDegree < 6 * N - 4 := by
    compute_degree; omega
  ...
  simp only [secondaryResidualRowZeroPolynomial68, coeff_smul, coeff_add,
    ...
    coeff_eq_zero_of_natDegree_lt hdeLow,
    ...
    sub_zero] at hrow
  have hcombine :
      (A * derivative A * B * e).coeff (6 * N - 4) +
        (A * derivative A * c * d).coeff (6 * N - 4) =
        (A * derivative A * (B * e + c * d)).coeff (6 * N - 4) := by
    rw [mul_add, coeff_add]
    ring
  have hcore : (-4 / 27 : k) *
      (A * derivative A * (B * e + c * d)).coeff (6 * N - 4) +
      (4 / 27 : k) *
        (derivative A * B * c ^ 2).coeff (6 * N - 4) = 0 := by
...
/-- On `R=3`, the I4 gamma load cancels the pinned row-zero gamma load;
the surviving `A'de` term contradicts the all-nonzero terminal edge. -/
theorem fiveToSix_terminal_R_three_false68
    (alpha gamma epsilon zeta eta terminal : k)
    (A B c d e : k[X]) (N G : ℕ)
    (hNbig : 2 < N)
    (hpacket : FiveToSixContractedCuspPacket68
    ...
    (heTerminal : e.natDegree ≤ G - N)
    (hzeta : zeta = 0)
    (hR3 : 7 * N - 2 * G = 3) : False := by
  rcases hpacket with
    ⟨hN, hgt5, hle6, hA, hB, hc, _hd, _he, ha,
      ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
  have hN3 : N = 3 := by omega
  have hG9 : G = 9 := by omega
  have hLne : 3 * G - 7 * N ≠ 3 := by omega
  have hAAF := fiveToSix_terminal_AAF_three68
    alpha gamma epsilon zeta eta terminal A B c d e N G hNbig
    ⟨hN, hgt5, hle6, hA, hB, hc, by omega, by omega, ha,
      ⟨hb, hc0, hds, hes, hcusp⟩, hadapter⟩
    hdTerminal heTerminal hzeta
  rw [if_neg hLne, if_pos hR3, zero_sub] at hAAF
  have hde := coeff_mul_mul_derivative_at_bounds68 d e A
    (G - 2 * N) (G - N) (2 * N) (by omega)
    hdTerminal heTerminal hA
  rw [show (G - 2 * N) + (G - N) + 2 * N - 1 = 6 * N - 4 from by
    omega] at hde
  have hde' : (derivative A * d * e).coeff (6 * N - 4) =
  ...
  rw [show 2 * N + (G - 2 * N) + 2 * N - 1 = 6 * N - 4 from by
    omega] at hAAd
  have hAAd' : (A * derivative A * d).coeff (6 * N - 4) =
  ...
  have hlow2 : (A * B * derivative B * d).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow3 : (derivative A * B * c ^ 2).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow4 : (B * c * derivative e).natDegree < 6 * N - 4 := by
    compute_degree; omega
  have hlow5 : (B * d * derivative d).natDegree < 6 * N - 4 := by
  ...
  have hcore : (-4 / 27 : k) *
      (A * derivative A * (B * e + c * d)).coeff (6 * N - 4) -
      (4 / 9 : k) * (derivative A * d * e).coeff (6 * N - 4) -
      (2 / 9 * gamma : k) *
        (A * derivative A * d).coeff (6 * N - 4) = 0 := by
    linear_combination hrow + (4 / 27 : k) * hcombine
...
/-- Terminal residual after consuming both genuine depth-three loads. -/
def FiveToSixTerminalCuspDepthFourResidual68
    (alpha gamma epsilon zeta eta terminal : k)
...
theorem fiveToSix_terminal_cusp_depth_four_residual68
    (alpha gamma epsilon zeta eta terminal : k)
    (A B c d e : k[X]) (N G : ℕ)
    (hres : FiveToSixTerminalCuspDepthThreeResidual68
      alpha gamma epsilon zeta eta terminal A B c d e N G) :
    FiveToSixTerminalCuspDepthFourResidual68
      alpha gamma epsilon zeta eta terminal A B c d e N G := by
  rcases hres with
    ⟨hbase, hR3, hL3, hi4two, hrowTwo⟩
  rcases hbase with
    ⟨hNbig, hdTerminal, heTerminal, hzeta, hR2, hL2, hpacket, hrowOne⟩
  have hR4 : 4 ≤ 7 * N - 2 * G := by
    by_contra hnot
    have heq : 7 * N - 2 * G = 3 := by omega
    exact fiveToSix_terminal_R_three_false68
      alpha gamma epsilon zeta eta terminal A B c d e N G hNbig hpacket
      hdTerminal heTerminal hzeta heq
  have hL4 : 4 ≤ 3 * G - 7 * N := by
    by_contra hnot
    have heq : 3 * G - 7 * N = 3 := by omega
    exact fiveToSix_terminal_L_three_false68
      alpha gamma epsilon zeta eta terminal A B c d e N G hNbig hpacket
      hdTerminal heTerminal hzeta heq
  exact ⟨⟨⟨hNbig, hdTerminal, heTerminal, hzeta, hR2, hL2, hpacket,
      hrowOne⟩, hR3, hL3, hi4two, hrowTwo⟩, hR4, hL4,
    fiveToSix_terminal_directLowerRowThree68
      alpha gamma epsilon zeta eta terminal A B c d e N G hNbig hpacket⟩

#print axioms coeff_mul_at_reflect_three68
#print axioms fiveToSix_terminal_i4_faceSum_three68
#print axioms fiveToSix_terminal_AAF_three68
#print axioms fiveToSix_terminal_L_three_false68
#print axioms fiveToSix_terminal_R_three_false68
#print axioms fiveToSix_terminal_cusp_depth_four_residual68

end FiveToSixCuspTerminalDepthThree68

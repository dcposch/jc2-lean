import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapCleanWindowIndexedScratch

/-! # Source backwire of T₁₅ and the first nonlinear T₁₆ correction -/
...
section AlignedSquareTerminalGBoundaryCapCleanEndpointBackwire68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareCapCleanEndpointBackwireHahnCharZero68 :
    CharZero (LaurentSeries k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℤ))

def RhoOneCapCleanEndpointPacket68
    (gamma : k) (A B CT F : LaurentSeries k) : Prop :=
  (rhoOneCapCoordinateDefectSeries68 B CT F).coeff 29 =
      -(2 / 3 : k) * (A * B ^ 2).coeff 29 ∧
    2 * F.coeff 3 *
        ((rhoOneCapCoordinateDefectSeries68 B CT F).coeff 30 +
          (2 / 3 : k) * (A * B ^ 2).coeff 30) =
      gamma * B.coeff 11 ^ 3

def RhoOneDoubleZeroTerminalGBoundaryCapCleanEndpointResidual68
  ...
  RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationNineResidual68
    ...
    ((B.coeff (1 : ℚ) = 0 ∧ 2 * C.coeff 0 + 3 * gamma = 0) →
      ...
      RhoOneCapCleanEndpointPacket68 gamma AL BL CT FL)

theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapCleanEndpointResidual
    [IsAlgClosed k]
  ...
  dsimp only [RhoOneDoubleZeroTerminalGBoundaryCapCleanEndpointResidual68]
  have hprevRaw :=
    T.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapPostRestorationNineResidual
      hp hH hp6 hp5 hp4 P hB hC hF hG hbeta hdelta hzeta
  rcases hprevRaw with hprev | htail
  · left
    refine ⟨hprev, ?_⟩
    intro hcoord hc1 hc2
    obtain ⟨i3, sold, cs, hnine⟩ := hprev.2 hcoord hc1 hc2
    have hseven := hnine.1
    ...
    have hsourcePrev := hprev.1.1.1.1
...
end AlignedSquareTerminalGBoundaryCapCleanEndpointBackwire68

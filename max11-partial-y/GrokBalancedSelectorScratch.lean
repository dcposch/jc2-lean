...
import GrokRowTwoFacesScratch
import GrokRowTwoSystemScratch
...
The `Q` degree cutoff and load degree cutoff are restated abstractly: the
tracked residual-descent module currently has the corresponding load theorem
in progress.  The row-two vanishing `residual + load = 0` is the checked
system-bridge shape.
-/
...
section GrokBalancedSelector68
...
theorem grokRowTwo_coeff_eq_zero_of_loadCutoff68
...
theorem grokDisc_imp_leadingA_ne_zero68 (a b c : k)
...
theorem grokRowTwo_not_leftChamber68
  ...
  have hcoeff0 := grokRowTwo_coeff_eq_zero_of_loadCutoff68
    A B c d e load (11 * n - 2 * h - 1) hload' hsum
  exact grokRowTwo_leftChamber_ne_zero68 A B c d e n g h hg hgh hh hsmall
    hleft hA hB hc hd he hQ hd0 he0 hcoeff0

set_option maxHeartbeats 2000000 in
/-- The right chamber `3g < 2h` is incompatible with residual-plus-load
vanishing under the cubic-face load cutoff. -/
theorem grokRowTwo_not_rightChamber68
    (A B c d e load : k[X]) (n g h : ℕ)
    (hg : 0 < g) (hgh : g < h) (hh : h < 2 * g)
...
theorem grokSelectedGap_eq_balanced68
  ...
  · exact (grokRowTwo_not_leftChamber68 A B c d e load n g h hg hgh hh
      hsmall hleft hA hB hc hd he hQ hload hsum hd0 he0).elim
  · exact heq
  · exact (grokRowTwo_not_rightChamber68 A B c d e load n g h hg hgh hh
...
theorem grokSelectedGap_eq_balanced68_of_rowTwoSystem
  ...
  exact grokSelectedGap_eq_balanced68 A B c d e
    (cubicLoadRowTwoPolynomial68 l alpha beta gamma delta epsilon zeta eta
      A B c ((1 / 3 : k) • (A * B) + d) e)
...
load cutoffs. -/
theorem grokCanonicalSelectedGap_eq_balanced68
    ...
    (hQ :
      ...
      (grokIncidenceQ68 Be ce d ee).natDegree < 9 * n - 3 * g)
    (hload :
      let n := weightedRadius68 A B C0 D E
      let Ae := expand k 60 A
      let Be := expand k 60 B
  ...
  change (grokIncidenceQ68 Be ce d ee).natDegree < 9 * n - 3 * g at hQ
  ...
  have hrowTwo :
    ...
    exact h2e
  exact grokSelectedGap_eq_balanced68_of_rowTwoSystem
...
end GrokBalancedSelector68

#print axioms grokRowTwo_coeff_eq_zero_of_loadCutoff68
#print axioms grokDisc_imp_leadingA_ne_zero68
#print axioms grokRowTwo_not_leftChamber68
#print axioms grokRowTwo_not_rightChamber68
#print axioms grokSelectedGap_eq_balanced68
#print axioms grokSelectedGap_eq_balanced68_of_rowTwoSystem
#print axioms grokCanonicalSelectedGap_eq_balanced68

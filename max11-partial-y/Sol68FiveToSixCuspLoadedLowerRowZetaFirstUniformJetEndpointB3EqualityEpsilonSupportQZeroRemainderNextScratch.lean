import Sol68FiveToSixCuspLoadedLowerRowZetaFirstUniformJetEndpointB3EqualityEpsilonSupportQZeroCompanionNextScratch
...
/-- The literal and normalized remainders agree at the next two source
coefficients consumed by the second companion. -/
theorem fiveToSix_zetaFirst_B3_equality_support_qZero_remainder_next_transport68
  ...
  have hfull0':
      (FiveToSixCuspZetaFirstB3EqualitySupportFullRowZero68
        alpha gamma epsilon 0 eta A B c d e).coeff (4*N-3)=0:=by
    simpa only [i0] using hfull0
  have hfull2':
      (FiveToSixCuspZetaFirstB3EqualitySupportFullRowTwo68
        alpha gamma epsilon 0 eta A B c d e).coeff (2*N-3)=0:=by

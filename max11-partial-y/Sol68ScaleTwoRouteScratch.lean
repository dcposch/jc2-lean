...
/-! # Scratch source reduction for the normalized `(6,8)`, `H = 2` leaf
...
theorem nextCoefficientJacobianRow_68_scratch {K : Type*}
...
theorem imprimitiveDiscriminatorRow_eq_zero_68_scratch {K : Type*}
  ...
  dsimp only
  have hrow := nextCoefficientJacobianRow_68_scratch hp hq hjac
  rw [hp6, hq8] at hrow
  have hscaled : H ^ 2 *
      ((2 : K[X]) * H *
          ((4 : K[X]) * p.coeff 5 * H -
...
theorem imprimitiveDiscriminatorPowerRelation_68_scratch {K : Type*}
    [Field K] [CharZero K] {p q : K[X][Y]} {j : K} {H : K[X]}
    (hp : p.natDegree = 6) (hq : q.natDegree = 8)
    (hjac : bivariateJacobian p q = Polynomial.C (Polynomial.C j))
    (hH : H ≠ 0)
  ...
  have hrow :
      (2 : K[X]) * H * N.derivative -
          (7 : K[X]) * N * H.derivative = 0 := by
    simpa only [N] using
      imprimitiveDiscriminatorRow_eq_zero_68_scratch hp hq hjac hH hp6 hq8
  have hrowC :
...
finishes the proof. -/
theorem commonCore_of_powerRelation_27_scratch {K : Type*}
...
theorem squareCore_of_nonzero_imprimitiveRelation_68_scratch
  ...
  obtain ⟨alpha, beta, h₀, halpha, hbeta, hh₀, hHform, hNform⟩ :=
    commonCore_of_powerRelation_27_scratch hH hN κ⁻¹
...
theorem normalized68ScaleTwo_discriminatorPowerRelation_scratch
  ...
  exact imprimitiveDiscriminatorPowerRelation_68_scratch
...
theorem normalized68ScaleTwo_discriminatorFirstFace_scratch
    {K : Type*} [Field K] [CharZero K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized68LeadingCoreSource P Q H 2) :
    let p := (Polynomial.Bivariate.equivMvPolynomial K).symm P
  ...
  dsimp only
  obtain ⟨κ, hrelation⟩ :=
    normalized68ScaleTwo_discriminatorPowerRelation_scratch hsource
  by_cases hκ : κ = 0
  · left
    have hNsquare :
        ((4 : K[X]) *
...
theorem normalized68ScaleTwo_imprimitiveDiscriminatorDichotomy_scratch
    {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized68LeadingCoreSource P Q H 2) :
    let p := (Polynomial.Bivariate.equivMvPolynomial K).symm P
  ...
  dsimp only
  rcases normalized68ScaleTwo_discriminatorFirstFace_scratch hsource with
    hzero | ⟨κ, hκ, hrelation⟩
  · exact Or.inl hzero
  · right
    obtain ⟨h, lambda, hh, hlambda, hH, hN, _hlambdaSq⟩ :=
      squareCore_of_nonzero_imprimitiveRelation_68_scratch
...
theorem normalized68ScaleTwo_nonzeroFace_has_linear_root_scratch
    {K : Type*} [Field K] [CharZero K] [IsAlgClosed K]
    {P Q : MvPolynomial (Fin 2) K} {H : K[X]}
    (hsource : Normalized68LeadingCoreSource P Q H 2)
    (hnonzero :
  ...
  rcases normalized68ScaleTwo_imprimitiveDiscriminatorDichotomy_scratch
...
#print axioms imprimitiveDiscriminatorPowerRelation_68_scratch
#print axioms commonCore_of_powerRelation_27_scratch
#print axioms normalized68ScaleTwo_discriminatorFirstFace_scratch
#print axioms normalized68ScaleTwo_imprimitiveDiscriminatorDichotomy_scratch
#print axioms normalized68ScaleTwo_nonzeroFace_has_linear_root_scratch

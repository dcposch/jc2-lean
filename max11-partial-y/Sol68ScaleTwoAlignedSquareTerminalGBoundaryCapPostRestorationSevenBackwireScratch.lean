import Sol68ScaleTwoAlignedSquareTerminalGBoundaryCapPostRestorationNextSourceScratch

/-! # Source backwire of the corrected post-restoration T₇ row -/
...
section AlignedSquareTerminalGBoundaryCapPostRestorationSevenBackwire68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareCapPostRestorationSevenBackwireHahnCharZero68 :
    CharZero (LaurentSeries k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℤ))

def rhoOneCapCoordinateDefectT7Core68
    (B C F : HahnSeries ℚ k) : k :=
  4 * C.coeff 7 * C.coeff 14 + 4 * C.coeff 8 * C.coeff 13 +
    4 * C.coeff 9 * C.coeff 12 + 4 * C.coeff 10 * C.coeff 11 -
    B.coeff 18 * F.coeff 3 - B.coeff 17 * F.coeff 4 -
    B.coeff 16 * F.coeff 5 - B.coeff 15 * F.coeff 6 -
    B.coeff 14 * F.coeff 7 - B.coeff 13 * F.coeff 8 -
    B.coeff 12 * F.coeff 9 - B.coeff 11 * F.coeff 10

def rhoOneCapCoordinateDefectT7LaurentCore68
...
theorem laurent_coeff_mul_lower_seven68
  ...
  have htail := laurent_coeff_mul_lower_six68 Xt Y (m + 1) n hXt hY
  ...
  have hXtpos (r : ℤ) (hr : r ≠ 0) : Xt.coeff (m + r) = X.coeff (m + r) := by
  ...
  calc
    ...
    _ = _ := by
      rw [add_mul, HahnSeries.coeff_add, hlead, htail',
        ...
        hXtpos 7 (by norm_num)]
      ring

theorem rhoOneCap_coordinate_defect_coeff_twenty_one_core68
    ...
    (rhoOneCapCoordinateDefectSeries68 B C F).coeff 21 =
      rhoOneCapCoordinateDefectT7LaurentCore68 B C F := by
  have hCC := laurent_coeff_mul_lower_seven68 C C 7 7 hC hC
  have hBF := laurent_coeff_mul_lower_seven68 B F 11 3 hB hF
  norm_num at hCC hBF
  ...
  have h2CC : ((2 : LaurentSeries k) * C ^ 2).coeff 21 =
      2 * (C * C).coeff 21 := by
    rw [pow_two, h2, hscale]
  dsimp only [rhoOneCapCoordinateDefectSeries68]
  rw [HahnSeries.coeff_sub, h2CC, hCC, hBF]
  dsimp only [rhoOneCapCoordinateDefectT7LaurentCore68]
  ring

def RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationSevenPacket68
    (gamma epsilon i3 sold cs source0 source1 source2 : k)
    (A B C F G dA : HahnSeries ℚ k) : Prop :=
  RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationPacket68
      gamma epsilon i3 sold cs source0 source1 source2 A B C F G dA ∧
    rhoOneCapCoordinateDefectT7Core68 B C F =
      -(4 / 3 : k) * A.coeff (-2 : ℚ) * B.coeff 11 * B.coeff 12 -
        (2 / 3 : k) * A.coeff (-1 : ℚ) * B.coeff 11 ^ 2

def RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationSevenResidual68
    (root j gamma epsilon : k) (A0 : k[X])
    (h0R AR BR CR FR GR : RatFunc k)
    (A B C F G : HahnSeries ℚ k) : Prop :=
  RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationResidual68
...
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapPostRestorationSevenResidual
    [IsAlgClosed k]
    ...
    RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationSevenResidual68
  ...
  dsimp only [RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationSevenResidual68]
  have hprevRaw :=
    T.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapPostRestorationResidual
  ...
  · left
    ...
    · rcases hboundary with ⟨hcurrent, hf2, hF3, hb4, hB5,
      ...
      have hT7core := rhoOneCap_coordinate_defect_coeff_twenty_one_core68
        BL CT FL hBL hCT hFL
      rw [hT7core] at hT7L
      have hT7 : rhoOneCapCoordinateDefectT7Core68
          (ratFuncAtHahn46 T.root T.source.B)
          (rhoOneCDefectHahn68 (ratFuncAtHahn46 T.root T.source.A)
            (ratFuncAtHahn46 T.root T.source.C0))
          (rhoOneTangentDDefectHahn68 (ratFuncAtHahn46 T.root T.source.A)
          ...
          -(4 / 3 : k) *
            ...
            (2 / 3 : k) *
              ...
              (ratFuncAtHahn46 T.root T.source.B).coeff 11 ^ 2 := by
        dsimp only [rhoOneCapCoordinateDefectT7LaurentCore68] at hT7L
        dsimp only [rhoOneCapCoordinateDefectT7Core68]
        rw [hCcoeff 7 (by norm_num), hCcoeff 14 (by norm_num),
          hCcoeff 8 (by norm_num), hCcoeff 13 (by norm_num),
          hCcoeff 9 (by norm_num), hCcoeff 12 (by norm_num),
          hCcoeff 10 (by norm_num), hCcoeff 11 (by norm_num),
          hBcoeff 18, hFcoeff 3, hBcoeff 17, hFcoeff 4,
          hBcoeff 16, hFcoeff 5, hBcoeff 15, hFcoeff 6,
          hBcoeff 14, hFcoeff 7, hBcoeff 13, hFcoeff 8,
          hBcoeff 12, hFcoeff 9, hBcoeff 11, hFcoeff 10,
          hAcoeff (-2), hAcoeff (-1)] at hT7L
        exact hT7L
      refine ⟨i3, sold, cs, ?_⟩
      dsimp only [RhoOneDoubleZeroTerminalGBoundaryCapPostRestorationSevenPacket68]
      exact ⟨hpacket, hT7⟩
    · exact (hrays.1 hcoord.1).elim
  · exact Or.inr htail

end AlignedSquareTerminalGBoundaryCapPostRestorationSevenBackwire68

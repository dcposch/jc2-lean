...
/-! # Refined faces after `beta = delta = zeta = 0` -/
...
section AlignedSquareRhoOneCubicExceptionalDoubleZeroRefinedFaces68

variable {k : Type*} [Field k] [CharZero k]

local instance exceptionalDoubleZeroRefinedFacesHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

def rhoOneZeroLCubicDoubleZeroRefinedFour68
    (gamma b c f g : k) : k :=
  2 * b * g + f * (2 * c + 3 * gamma)

def rhoOneZeroLCubicDoubleZeroRefinedThree68
    (gamma a b c f g : k) : k :=
  -2 * a * b * f + 3 * g * (2 * c + 3 * gamma) + 3 * f ^ 2

def rhoOneZeroLCubicDoubleZeroRefinedCompatibility68
    (a b f g : k) : k :=
  3 * f ^ 3 - 2 * a * b * f ^ 2 - 6 * b * g ^ 2

/-- Exact next `I₄(-1)` and `I₃(-2)` faces after the `G` order gain and
the vanishing of `zeta`. -/
theorem firstIntegral_mainTangent_doubleZero_refined_faces68
    (gamma epsilon : k)
    ...
    (firstIntegralFour68 0 A B (A ^ 2 / 3 + C) (A * B / 3 + F)
      ...
      (HahnSeries.C epsilon) (HahnSeries.C (0 : k))).coeff (-1 : ℚ) =
        (4 / 3 : k) * rhoOneZeroLCubicDoubleZeroRefinedFour68 gamma
          (B.coeff (1 : ℚ)) (C.coeff (0 : ℚ))
          (F.coeff (-1 : ℚ)) (G.coeff (-2 : ℚ)) ∧
    (firstIntegralThree68 0 A B (A ^ 2 / 3 + C) (A * B / 3 + F)
      (A ^ 3 / 27 + A * C / 3 + G) (HahnSeries.C (0 : k))
      (HahnSeries.C gamma) (HahnSeries.C (0 : k))
      (HahnSeries.C epsilon) (HahnSeries.C (0 : k))).coeff (-2 : ℚ) =
        (4 / 9 : k) * rhoOneZeroLCubicDoubleZeroRefinedThree68 gamma
  ...
  have h3decH :
    ...
    simp only [firstIntegralThreeMainTangentTop68,
      ...
      map_mul, map_neg, map_div₀, map_ofNat, map_one]
    ring
  have h4t1 := hz (-1) (-8 / 27 : k) B A C F G 3 0 0 0 0
    1 (-2) 0 (-1) (-2) hB hA hC hF hG (by norm_num)
  have h4t3 := lead (8 / 3 : k) B G A C F 1 1 0 0 0
    1 (-2) (-2) 0 (-1) hB hG hA hC hF
  have h4t4 := lead (8 / 3 : k) C F A B G 1 1 0 0 0
    0 (-1) (-2) 1 (-2) hC hF hA hB hG
  have h4l6 := lead (4 * gamma) F A B C G 1 0 0 0 0
    (-1) (-2) 1 0 (-2) hF hA hB hC hG
  have h4l8 := hz (-1) (2 * epsilon) B A C F G 1 0 0 0 0
    1 (-2) 0 (-1) (-2) hB hA hC hF hG (by norm_num)
  have h3t1 := lead (-8 / 9 : k) A B F C G 1 1 1 0 0
    (-2) 1 (-1) 0 (-2) hA hB hF hC hG
  have h3t2 := hz (-2) (-8 / 9 : k) B C A F G 2 1 0 0 0
    1 0 (-2) (-1) (-2) hB hC hA hF hG (by norm_num)
  have h3t3 := lead (8 / 3 : k) C G A B F 1 1 0 0 0
    0 (-2) (-2) 1 (-1) hC hG hA hB hF
  have h3l2 := lead (4 / 3 : k) F A B C G 2 0 0 0 0
    (-1) (-2) 1 0 (-2) hF hA hB hC hG
  have h3l6 := hz (-2) (-(2 / 3 : k) * gamma) B A C F G 2 0 0 0 0
    1 (-2) 0 (-1) (-2) hB hA hC hF hG (by norm_num)
  have h3l7 := lead (4 * gamma) G A B C F 1 0 0 0 0
    (-2) (-2) 1 0 (-1) hG hA hB hC hF
  have h3l8 := hz (-2) (2 * epsilon) C A B F G 1 0 0 0 0
    0 (-2) 1 (-1) (-2) hC hA hB hF hG (by norm_num)
  norm_num at h4t3 h4t4 h4l6 h3t1 h3t3 h3l2 h3l7
  have h3t1' :
  ...
  · rw [h4decH]
    ...
    simp only [zero_mul, HahnSeries.coeff_zero, add_zero,
      rhoOneZeroLCubicDoubleZeroRefinedFour68]
    ring
  · rw [h3decH]
    ...
    rw [hCzero]
    simp only [zero_mul, HahnSeries.coeff_zero, add_zero,
      rhoOneZeroLCubicDoubleZeroRefinedThree68]
    ring

/-- The two refined faces eliminate their common affine factor. -/
theorem rhoOneZeroLCubicDoubleZero_refined_compatibility68
    (gamma a b c f g : k)
    (h4 : rhoOneZeroLCubicDoubleZeroRefinedFour68 gamma b c f g = 0)
    (h3 : rhoOneZeroLCubicDoubleZeroRefinedThree68 gamma a b c f g = 0) :
    rhoOneZeroLCubicDoubleZeroRefinedCompatibility68 a b f g = 0 := by
  simp only [rhoOneZeroLCubicDoubleZeroRefinedFour68] at h4
  simp only [rhoOneZeroLCubicDoubleZeroRefinedThree68] at h3
  simp only [rhoOneZeroLCubicDoubleZeroRefinedCompatibility68]
  linear_combination f * h3 - 3 * g * h4

/-- Rational-function transport of both refined faces. -/
theorem ratFunc_doubleZero_refined_faces68
    ...
    rhoOneZeroLCubicDoubleZeroRefinedFour68 gamma
      ...
      ((ratFuncAtHahn46 root G).coeff (-2 : ℚ)) = 0 ∧
    rhoOneZeroLCubicDoubleZeroRefinedThree68 gamma
      ((ratFuncAtHahn46 root A).coeff (-2 : ℚ))
      ((ratFuncAtHahn46 root B).coeff (1 : ℚ))
      ((ratFuncAtHahn46 root C).coeff (0 : ℚ))
      ((ratFuncAtHahn46 root F).coeff (-1 : ℚ))
      ((ratFuncAtHahn46 root G).coeff (-2 : ℚ)) = 0 := by
  ...
  have hface := firstIntegral_mainTangent_doubleZero_refined_faces68
  ...
  have h3coeff := congrArg
    (fun X : HahnSeries ℚ k => X.coeff (-2 : ℚ)) hi3H'
  have h4const : (HahnSeries.C i4 : HahnSeries ℚ k).coeff (-1 : ℚ) = 0 := by
    simp [HahnSeries.C_apply]
  have h3const : (HahnSeries.C i3 : HahnSeries ℚ k).coeff (-2 : ℚ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hface.1, h4const] at h4coeff
  rw [hface.2, h3const] at h3coeff
  constructor
  · linear_combination (3 / 4 : k) * h4coeff
  · linear_combination (9 / 4 : k) * h3coeff

/-- Source-facing finite residual packet in the refined
`beta = delta = zeta = 0` box. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refined_faces
    ...
    rhoOneZeroLCubicDoubleZeroRefinedThree68 T.source.gamma
      (A.coeff (-2 : ℚ)) (B.coeff (1 : ℚ)) (C.coeff (0 : ℚ))
      (F.coeff (-1 : ℚ)) (G.coeff (-2 : ℚ)) = 0 ∧
    rhoOneZeroLCubicDoubleZeroRefinedCompatibility68
      (A.coeff (-2 : ℚ)) (B.coeff (1 : ℚ))
      (F.coeff (-1 : ℚ)) (G.coeff (-2 : ℚ)) = 0 := by
  ...
  have hi3T :
    ...
    simpa only [AR, BR, C0R, DR, E0R, RatFunc.algebraMap_eq_C,
      map_zero] using hi3
  have hfaces := ratFunc_doubleZero_refined_faces68 T.root AR BR CR FR GR
    T.source.gamma T.source.epsilon i4 i3 hAH hBH hCRH hFRH hGRH hi4T hi3T
  have hcompat := rhoOneZeroLCubicDoubleZero_refined_compatibility68
    T.source.gamma (AH.coeff (-2 : ℚ)) (BH.coeff (1 : ℚ))
      (CRH.coeff (0 : ℚ)) (FRH.coeff (-1 : ℚ)) (GRH.coeff (-2 : ℚ))
      hfaces.1 hfaces.2
  ...
  rw [hCRmap', hFRmap', hGRmap''] at hfaces
  dsimp only [AH, BH, FRH, GRH] at hcompat
  rw [hFRmap', hGRmap''] at hcompat
  simpa only [AH, BH, C0H, DH, E0H, AR, BR, C0R, DR, E0R] using
    ⟨hB, hC, hF, hG, hfaces.1, hfaces.2, hcompat⟩

end AlignedSquareRhoOneCubicExceptionalDoubleZeroRefinedFaces68

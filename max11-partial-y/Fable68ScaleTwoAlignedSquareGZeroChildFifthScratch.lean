import Fable68ScaleTwoAlignedSquareGZeroChildFourthScratch

/-! # The fifth unused rows on the refined `G[-1] = 0` children

The `G`-zero-child-fourth residual keeps the `G[-1] = 0` sibling as the
refined disjunction `BGainRefinedFourth ∨ FGainRefinedFourth`.  On that
sibling the exact next unused rows are the coefficient-five `I₄` row
and the coefficient-three `I₃` row, on both children:

* On the `B`-gain child (box `ord A ≥ -2`, `ord B ≥ 2`, `ord C ≥ 0`,
  `ord F ≥ 0`, `ord G ≥ 0`) the coefficient-five `I₄` row needs the
  six-combination jet on the `(2C+3γ)F` block and reads
  `4B[2]G[3] + 4B[3]G[2] + 4B[4]G[1] + 4B[5]G[0] + 2F[5]·(2C[0]+3γ) +
  4C[1]F[4] + 4C[2]F[3] + 4C[3]F[2] + 4C[4]F[1] + 4C[5]F[0] + 3εB[5] =
  0` (the cube stays silent through weight five), while the
  coefficient-three `I₃` row carries the ten `ABF` combinations
  `A[-2]B[2]F[3] + A[-2]B[3]F[2] + A[-1]B[2]F[2] + A[-2]B[4]F[1] +
  A[-1]B[3]F[1] + A[0]B[2]F[1] + A[-2]B[5]F[0] + A[-1]B[4]F[0] +
  A[0]B[3]F[0] + A[1]B[2]F[0]` — the first row of the lane to see
  `A[1]` — against `6G[3]·(2C[0]+3γ) + 12C[1]G[2] + 12C[2]G[1] +
  12C[3]G[0] + 12F[0]F[3] + 12F[1]F[2] + 9εC[3]`, with both `B²`
  blocks still silent.

* On the `F`-gain child (box `ord B ≥ 1`, `ord F ≥ 1`) the cube
  boundary deepens to `3B[1]²B[3] + 3B[1]B[2]²`:
  `-12B[1]²B[3] - 12B[1]B[2]² + 36B[1]G[4] + 36B[2]G[3] + 36B[3]G[2] +
  36B[4]G[1] + 36B[5]G[0] + 18F[5]·(2C[0]+3γ) + 36C[1]F[4] +
  36C[2]F[3] + 36C[3]F[2] + 36C[4]F[1] + 27εB[5] = 0`, and the
  coefficient-three `I₃` row carries the shifted ten-combination `ABF`
  jet (also first seeing `A[1]`, through `A[1]B[1]F[1]`) together with
  the deepened `B²C` and `γB²` loads `4C[1]B[1]² +
  2(4C[0]+3γ)B[1]B[2]`:
  `4A[-2]B[1]F[4] + 4A[-2]B[2]F[3] + 4A[-1]B[1]F[3] + 4A[-2]B[3]F[2] +
  4A[-1]B[2]F[2] + 4A[0]B[1]F[2] + 4A[-2]B[4]F[1] + 4A[-1]B[3]F[1] +
  4A[0]B[2]F[1] + 4A[1]B[1]F[1] + 4C[1]B[1]² + 2(4C[0]+3γ)B[1]B[2] =
  6G[3]·(2C[0]+3γ) + 12C[1]G[2] + 12C[2]G[1] + 12C[3]G[0] +
  12F[1]F[2] + 9εC[3]`.

Both `I₃` rows are consumed at the Laurent level through the new
reusable six-term coefficient-convolution lemma and the imported
...
eliminating `G[1]` and `G[0]` between the (imported) coefficient-two
and coefficient-three `I₄` rows and the new coefficient-three `I₃`
row yields
`B[2]²·(A[-2]B[2]F[3] + A[-2]B[3]F[2] + A[-1]B[2]F[2] + A[-2]B[4]F[1]
+ A[-1]B[3]F[1] + A[0]B[2]F[1] + A[-2]B[5]F[0] + A[-1]B[4]F[0] +
A[0]B[3]F[0] + A[1]B[2]F[0]) = 3B[2]²(F[0]F[3]+F[1]F[2]) +
3C[2]²(B[3]F[0]−B[2]F[1]) − 6C[2]C[3]B[2]F[0]` — the whole `G`
boundary jet and the ground constant `ε` both leave the new `I₃` row.
This is recorded as a standalone corollary and not wired into the
packet, so the tail disjunction of the imported refinement stays
verbatim.  Both children and every sibling are preserved: the nonzero
`G[-1]` boundary lane, the `C₂` face, and the quadratic rays remain
literally inside the imported derivative-next residual.
NEXT_UNUSED_ROW: the coefficient-six `I₄` row and the
coefficient-four `I₃` row, on both children. -/
...
section AlignedSquareGZeroChildFifth68

variable {k : Type*} [Field k] [CharZero k]

local instance alignedSquareGZeroChildFifthHahnCharZero68 :
    CharZero (HahnSeries ℚ k) :=
  charZero_of_injective_ringHom
    (HahnSeries.C_injective (R := k) (Γ := ℚ))

/-- The coefficient five integral jet above the lower face of a Laurent
product, restated locally for the `G`-zero-child-fifth lane. -/
theorem laurent_coeff_mul_lower_five_gZeroChildFifth68
  ...
  have htail := laurent_coeff_mul_lower_four_gZeroChildFourth68 Xt Y
    (m + 1) n hXt hY
...
/-- The coefficient-five `I₄` row on the `B`-gain child: with
`ord B ≥ 2` the `BG` block carries four integral combinations, the
`(2C+3γ)F` block carries all six, and the cube stays silent. -/
theorem rhoOneGZeroChildFifthI4Load_bGain_coeff_five68
    ...
    4 * (ratFuncAtHahn46 root B).coeff (2 : ℚ) *
      ...
      2 * (ratFuncAtHahn46 root F).coeff (5 : ℚ) *
      ...
      4 * (ratFuncAtHahn46 root C).coeff (5 : ℚ) *
  ...
  have hSLord : (↑(0 : ℤ) : WithTop ℤ) ≤ SL.orderTop := by
    ...
    exact le_trans (le_min h2 hC3g)
      HahnSeries.min_orderTop_le_orderTop_add
  have hSF := laurent_coeff_mul_lower_five_gZeroChildFifth68 SL FL 0 0
    hSLord hFL
  norm_num at hSF
  have hBG := laurent_coeff_mul_lower_three_gZeroChildThird68 BL Gser 2 0
    hBL hGL
  norm_num at hBG
  ...
  have hsc1 : ((-8 / 27 : LaurentSeries k) * BL ^ 3).coeff 5 =
      (-8 / 27 : k) * (BL ^ 3).coeff 5 := by
    rw [hc827, hscale]
  have hsc2 : ((8 / 3 : LaurentSeries k) * BL * Gser).coeff 5 =
      (8 / 3 : k) * (BL * Gser).coeff 5 := by
    rw [mul_assoc, hc83, hscale]
  have hsc3 : ((4 / 3 : LaurentSeries k) * SL * FL).coeff 5 =
      (4 / 3 : k) * (SL * FL).coeff 5 := by
    rw [mul_assoc, hc43, hscale]
  have hsc4 : ((2 : LaurentSeries k) * HahnSeries.C epsilon * BL).coeff 5 =
      (2 * epsilon) * BL.coeff 5 := by
    rw [hc2, ← map_mul, hscale]
  rw [hsc1, hsc2, hsc3, hsc4] at hc
  have hright : (HahnSeries.C i4 : LaurentSeries k).coeff (5 : ℤ) = 0 := by
  ...
  norm_num at hB2 hB3 hB4 hB5 hC0 hC1 hC2 hC3 hC4 hC5
  norm_num at hF0 hF1 hF2 hF3 hF4 hF5 hG0 hG1 hG2 hG3
  dsimp only [BL, CL, FL, Gser] at hc
  rw [← hB2, ← hB3, ← hB4, ← hB5, ← hC0, ← hC1, ← hC2, ← hC3, ← hC4,
    ← hC5, ← hF0, ← hF1, ← hF2, ← hF3, ← hF4, ← hF5, ← hG0, ← hG1,
    ← hG2, ← hG3] at hc
  linear_combination (3 / 2 : k) * hc

/-- The coefficient-five `I₄` row on the `F`-gain child: with
`ord B ≥ 1` and `ord F ≥ 1` the cube boundary deepens to
`3B[1]²B[3] + 3B[1]B[2]²` alongside the five-combination `BG` and
`(2C+3γ)F` jets. -/
theorem rhoOneGZeroChildFifthI4Load_fGain_coeff_five68
    (root gamma epsilon i4 : k) (B C F G : RatFunc k)
    (hB : (↑(1 : ℚ) : WithTop ℚ) ≤
      (ratFuncAtHahn46 root B).orderTop)
    (hC : (0 : WithTop ℚ) ≤ (ratFuncAtHahn46 root C).orderTop)
    ...
    -12 * (ratFuncAtHahn46 root B).coeff (1 : ℚ) ^ 2 *
      ...
      36 * (ratFuncAtHahn46 root B).coeff (5 : ℚ) *
        (ratFuncAtHahn46 root G).coeff (0 : ℚ) +
      18 * (ratFuncAtHahn46 root F).coeff (5 : ℚ) *
        (2 * (ratFuncAtHahn46 root C).coeff (0 : ℚ) + 3 * gamma) +
      ...
      36 * (ratFuncAtHahn46 root C).coeff (4 : ℚ) *
        (ratFuncAtHahn46 root F).coeff (1 : ℚ) +
      27 * epsilon * (ratFuncAtHahn46 root B).coeff (5 : ℚ) = 0 := by
  let BL : LaurentSeries k := ratFuncAtLaurent46 root B
  let CL : LaurentSeries k := ratFuncAtLaurent46 root C
  let FL : LaurentSeries k := ratFuncAtLaurent46 root F
  let Gser : LaurentSeries k := ratFuncAtLaurent46 root G
  ...
  have hSLord : (↑(0 : ℤ) : WithTop ℤ) ≤ SL.orderTop := by
    ...
    exact le_trans (le_min h2 hC3g)
      HahnSeries.min_orderTop_le_orderTop_add
  have hSF := laurent_coeff_mul_lower_four_gZeroChildFourth68 SL FL 0 1
    hSLord hFL
  norm_num at hSF
  have hBG := laurent_coeff_mul_lower_four_gZeroChildFourth68 BL Gser 1 0
    hBL hGL
  norm_num at hBG
  ...
  have hsq4 := laurent_coeff_mul_lower_two_gZeroChildNext68 BL BL 1 1
    hBL hBL
  norm_num at hsq4
  have hcubec := laurent_coeff_mul_lower_two_gZeroChildNext68 (BL * BL) BL
    2 1 hBsqOrder' hBL
  norm_num at hcubec
  have hBcube : (BL ^ 3).coeff (5 : ℤ) =
      3 * BL.coeff 1 ^ 2 * BL.coeff 3 +
        3 * BL.coeff 1 * BL.coeff 2 ^ 2 := by
    have hpow : BL ^ 3 = BL * BL * BL := by ring
    rw [hpow, hcubec, hsq2, hsq3, hsq4]
    ring
  have hfactorL := congrArg (ratFuncAtLaurent46 root) hfactor
  have hfactorL' : rhoOneGZeroChildI4LoadFactored68
      (HahnSeries.C gamma) (HahnSeries.C epsilon) BL CL FL Gser =
  ...
  norm_num at hB1 hB2 hB3 hB4 hB5 hC0 hC1 hC2 hC3 hC4
  norm_num at hF1 hF2 hF3 hF4 hF5 hG0 hG1 hG2 hG3 hG4
  dsimp only [BL, CL, FL, Gser] at hc
  rw [← hB1, ← hB2, ← hB3, ← hB4, ← hB5, ← hC0, ← hC1, ← hC2, ← hC3,
    ← hC4, ← hF1, ← hF2, ← hF3, ← hF4, ← hF5, ← hG0, ← hG1, ← hG2,
    ← hG3, ← hG4] at hc
  linear_combination (27 / 2 : k) * hc

/-- The coefficient-three `I₃` row on the `B`-gain child, consumed at
the Laurent level: the `ABF` block carries ten integral combinations —
the first appearance of `A[1]` in this lane — while both `B²` blocks
stay silent. -/
theorem rhoOneGZeroChildFifthI3Cap_bGain_coeff_three68
    ...
    4 * (ratFuncAtHahn46 root A).coeff (-2 : ℚ) *
      ...
      4 * (ratFuncAtHahn46 root A).coeff (1 : ℚ) *
        (ratFuncAtHahn46 root B).coeff (2 : ℚ) *
        (ratFuncAtHahn46 root F).coeff (0 : ℚ) -
      6 * (ratFuncAtHahn46 root G).coeff (3 : ℚ) *
        (2 * (ratFuncAtHahn46 root C).coeff (0 : ℚ) + 3 * gamma) -
      ...
      12 * (ratFuncAtHahn46 root F).coeff (1 : ℚ) *
        (ratFuncAtHahn46 root F).coeff (2 : ℚ) -
      9 * epsilon * (ratFuncAtHahn46 root C).coeff (3 : ℚ) = 0 := by
  ...
  have hABF := laurent_coeff_mul_lower_three_gZeroChildThird68 (AL * BL)
    FL 0 0 hABord hFL
  norm_num at hABF
  ...
  have hAB3 := laurent_coeff_mul_lower_three_gZeroChildThird68 AL BL (-2)
    2 hAL hBL
  norm_num at hAB3
  rw [hAB0, hAB1, hAB2, hAB3] at hABF
  have hSG := laurent_coeff_mul_lower_three_gZeroChildThird68 SL Gser 0 0
    hSLord hGL
  norm_num at hSG
  ...
  have hB2Cord : (↑(4 : ℤ) : WithTop ℤ) ≤ (BL ^ 2 * CL).orderTop := by
    ...
    exact le_trans (add_le_add hBsqOrder hCL)
      HahnSeries.orderTop_add_le_mul
  have hB2C : (BL ^ 2 * CL).coeff (3 : ℤ) = 0 :=
    HahnSeries.coeff_eq_zero_of_lt_orderTop
      (lt_of_lt_of_le (WithTop.coe_lt_coe.mpr (by norm_num)) hB2Cord)
  have hFsq := laurent_coeff_mul_lower_three_gZeroChildThird68 FL FL 0 0
    hFL hFL
  norm_num at hFsq
  have hF2 : (FL ^ 2).coeff (3 : ℤ) =
      FL.coeff 0 * FL.coeff 3 + FL.coeff 1 * FL.coeff 2 +
        FL.coeff 2 * FL.coeff 1 + FL.coeff 3 * FL.coeff 0 := by
    rw [pow_two, hFsq]
  have hfactorL := congrArg (ratFuncAtLaurent46 root) hfactor
  ...
  have hc2 : (2 : LaurentSeries k) = HahnSeries.C (2 : k) := by
    rw [HahnSeries.C_eq_algebraMap, map_ofNat]
  have hsc1 : ((-8 / 9 : LaurentSeries k) * AL * BL * FL).coeff (3 : ℤ) =
      (-8 / 9 : k) * (AL * BL * FL).coeff (3 : ℤ) := by
    have hform : (-8 / 9 : LaurentSeries k) * AL * BL * FL =
        HahnSeries.C (-8 / 9 : k) * (AL * BL * FL) := by
      rw [← hc89]
      ring
    rw [hform, hscale]
  have hsc2 : ((8 / 9 : LaurentSeries k) * BL ^ 2 * CL).coeff (3 : ℤ) =
      (8 / 9 : k) * (BL ^ 2 * CL).coeff (3 : ℤ) := by
    have hform : (8 / 9 : LaurentSeries k) * BL ^ 2 * CL =
        HahnSeries.C (8 / 9 : k) * (BL ^ 2 * CL) := by
      rw [← hc89']
      ring
    rw [hform, hscale]
  have hsc3 : ((4 / 3 : LaurentSeries k) * SL * Gser).coeff (3 : ℤ) =
      (4 / 3 : k) * (SL * Gser).coeff (3 : ℤ) := by
    rw [mul_assoc, hc43, hscale]
  have hsc4 : ((4 / 3 : LaurentSeries k) * FL ^ 2).coeff (3 : ℤ) =
      (4 / 3 : k) * (FL ^ 2).coeff (3 : ℤ) := by
    rw [hc43, hscale]
  have hsc5 : ((2 / 3 : LaurentSeries k) * HahnSeries.C gamma *
      BL ^ 2).coeff (3 : ℤ) =
      (2 / 3 * gamma : k) * (BL ^ 2).coeff (3 : ℤ) := by
    rw [hc23, ← map_mul, hscale]
  have hsc6 : ((2 : LaurentSeries k) * HahnSeries.C epsilon *
      CL).coeff (3 : ℤ) = (2 * epsilon) * CL.coeff (3 : ℤ) := by
    rw [hc2, ← map_mul, hscale]
  rw [hsc1, hsc2, hsc3, hsc4, hsc5, hsc6] at hc
  have hright : (HahnSeries.C i3 : LaurentSeries k).coeff (3 : ℤ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hABF, hB2C, hSG, hF2, hBsq3, hright] at hc
  have hSL0 : SL.coeff (0 : ℤ) = 2 * CL.coeff (0 : ℤ) + 3 * gamma := by
    dsimp only [SL]
    simp [HahnSeries.C_apply, two_mul]
  have hSL1 : SL.coeff (1 : ℤ) = 2 * CL.coeff (1 : ℤ) := by
  ...
  have hG2 := ratFuncAtHahn46_coeff_int68 root G 2
  have hG3 := ratFuncAtHahn46_coeff_int68 root G 3
  norm_num at hA2 hA1 hA0 hA1' hB2 hB3 hB4 hB5 hC0 hC1 hC2 hC3
  norm_num at hF0 hF1 hF2' hF3 hG0 hG1 hG2 hG3
  dsimp only [AL, BL, CL, FL, Gser] at hc
  rw [← hA2, ← hA1, ← hA0, ← hA1', ← hB2, ← hB3, ← hB4, ← hB5, ← hC0,
    ← hC1, ← hC2, ← hC3, ← hF0, ← hF1, ← hF2', ← hF3, ← hG0, ← hG1,
    ← hG2, ← hG3] at hc
  linear_combination (-9 / 2 : k) * hc

/-- The coefficient-three `I₃` row on the `F`-gain child, consumed at
the Laurent level: the `ABF` block carries the ten combinations of the
shifted box `ord B ≥ 1`, `ord F ≥ 1` — also first seeing `A[1]`,
through `A[1]B[1]F[1]` — while the `B²C` and `γB²` blocks deepen to
the load `4C[1]B[1]² + 2(4C[0]+3γ)B[1]B[2]`. -/
theorem rhoOneGZeroChildFifthI3Cap_fGain_coeff_three68
    (root gamma epsilon i3 : k) (A B C F G : RatFunc k)
    (hA : (↑(-2 : ℚ) : WithTop ℚ) ≤
      (ratFuncAtHahn46 root A).orderTop)
    (hB : (↑(1 : ℚ) : WithTop ℚ) ≤
  ...
  have hAB2 := laurent_coeff_mul_lower_three_gZeroChildThird68 AL BL (-2)
    1 hAL hBL
  norm_num at hAB2
  rw [hABm1, hAB0, hAB1, hAB2] at hABF
  have hSG := laurent_coeff_mul_lower_three_gZeroChildThird68 SL Gser 0 0
    hSLord hGL
  norm_num at hSG
  have hBsqOrder : (↑(2 : ℤ) : WithTop ℤ) ≤ (BL ^ 2).orderTop := by
  ...
  have hBsq3 : (BL ^ 2).coeff (3 : ℤ) =
    ...
    rw [pow_two, hsq3c]
  have hB2C := laurent_coeff_mul_lower_succ68 (BL ^ 2) CL 2 0 hBsqOrder
    hCL
  norm_num at hB2C
  have hFsq3c := laurent_coeff_mul_lower_succ68 FL FL 1 1 hFL hFL
  norm_num at hFsq3c
  have hF2 : (FL ^ 2).coeff (3 : ℤ) =
      FL.coeff 1 * FL.coeff 2 + FL.coeff 2 * FL.coeff 1 := by
    rw [pow_two, hFsq3c]
  have hfactorL := congrArg (ratFuncAtLaurent46 root) hfactor
  have hfactorL' : rhoOneI3CapFactored68 (HahnSeries.C gamma)
      (HahnSeries.C epsilon) AL BL CL FL Gser = HahnSeries.C i3 := by
  ...
  have hright : (HahnSeries.C i3 : LaurentSeries k).coeff (3 : ℤ) = 0 := by
    simp [HahnSeries.C_apply]
  rw [hABF, hB2C, hSG, hF2, hBsq2, hBsq3, hright] at hc
  have hSL0 : SL.coeff (0 : ℤ) = 2 * CL.coeff (0 : ℤ) + 3 * gamma := by
    dsimp only [SL]
    simp [HahnSeries.C_apply, two_mul]
  have hSL1 : SL.coeff (1 : ℤ) = 2 * CL.coeff (1 : ℤ) := by
  ...
  have hG2 := ratFuncAtHahn46_coeff_int68 root G 2
  have hG3 := ratFuncAtHahn46_coeff_int68 root G 3
  norm_num at hA2 hA1 hA0 hA1' hB1 hB2 hB3 hB4 hC0 hC1 hC2 hC3
  norm_num at hF1 hF2' hF3 hF4 hG0 hG1 hG2 hG3
  dsimp only [AL, BL, CL, FL, Gser] at hc
  rw [← hA2, ← hA1, ← hA0, ← hA1', ← hB1, ← hB2, ← hB3, ← hB4, ← hC0,
    ← hC1, ← hC2, ← hC3, ← hF1, ← hF2', ← hF3, ← hF4, ← hG0, ← hG1,
    ← hG2, ← hG3] at hc
  linear_combination (-9 / 2 : k) * hc
...
face, eliminating `G[1]` and `G[0]` between the imported coefficient-two
and coefficient-three `I₄` rows and the new coefficient-three `I₃` row
leaves a `G`-jet-free and `ε`-free minor tying the descended
ten-combination `ABF` jet to the `F` boundary jets and the `C[2]`,
`C[3]` defects.  Recorded as a standalone corollary; the packet keeps
the full rows. -/
theorem rhoOneGZeroChildFifth_bGain_affine_tail_minor68
    (gamma epsilon a2 a1 a0 aP b2 b3 b4 b5 c0 c1 c2 c3 f0 f1 f2 f3
      g0 g1 g2 g3 : k)
    ...
    (hrow : 4 * a2 * b2 * f3 + 4 * a2 * b3 * f2 + 4 * a1 * b2 * f2 +
      4 * a2 * b4 * f1 + 4 * a1 * b3 * f1 + 4 * a0 * b2 * f1 +
      4 * a2 * b5 * f0 + 4 * a1 * b4 * f0 + 4 * a0 * b3 * f0 +
      4 * aP * b2 * f0 -
      6 * g3 * (2 * c0 + 3 * gamma) - 12 * c1 * g2 - 12 * c2 * g1 -
      12 * c3 * g0 - 12 * f0 * f3 - 12 * f1 * f2 -
      9 * epsilon * c3 = 0)
    (hS0 : 2 * c0 + 3 * gamma = 0) (hc1 : c1 = 0) :
    b2 ^ 2 * (a2 * b2 * f3 + a2 * b3 * f2 + a1 * b2 * f2 +
        a2 * b4 * f1 + a1 * b3 * f1 + a0 * b2 * f1 +
        a2 * b5 * f0 + a1 * b4 * f0 + a0 * b3 * f0 + aP * b2 * f0) =
      3 * b2 ^ 2 * (f0 * f3 + f1 * f2) +
        3 * c2 ^ 2 * (b3 * f0 - b2 * f1) - 6 * c2 * c3 * b2 * f0 := by
  linear_combination (b2 ^ 2 / 4 : k) * hrow +
    (3 / 4 : k) * c2 * b2 * htie3 -
    (3 / 4 : k) * (c2 * b3 - c3 * b2) * htie2 +
    ((3 / 2 : k) * b2 ^ 2 * g3 - (3 / 2 : k) * b2 * c2 * f3 -
      (3 / 2 : k) * b2 * c3 * f2 + (3 / 2 : k) * b3 * c2 * f2) * hS0 +
    (3 * b2 ^ 2 * g2 - 3 * b2 * c2 * f2 - 3 * b2 * c3 * f1 +
      3 * b3 * c2 * f1) * hc1

/-- The `B`-gain child refined through its fifth unused rows: the
imported refinement is kept verbatim, and the coefficient-five `I₄`
row and the coefficient-three `I₃` row are added globally. -/
def RhoOneGZeroChildBGainRefinedFifth68
    (gamma epsilon : k) (A0R B0R C00R D0R E0R : RatFunc k)
    (A B C F G : HahnSeries ℚ k) : Prop :=
  RhoOneGZeroChildBGainRefinedFourth68 gamma epsilon A0R B0R C00R D0R E0R
      A B C F G ∧
    (4 * B.coeff (2 : ℚ) * G.coeff (3 : ℚ) +
        4 * B.coeff (3 : ℚ) * G.coeff (2 : ℚ) +
        4 * B.coeff (4 : ℚ) * G.coeff (1 : ℚ) +
        4 * B.coeff (5 : ℚ) * G.coeff (0 : ℚ) +
        2 * F.coeff (5 : ℚ) * (2 * C.coeff (0 : ℚ) + 3 * gamma) +
        4 * C.coeff (1 : ℚ) * F.coeff (4 : ℚ) +
        4 * C.coeff (2 : ℚ) * F.coeff (3 : ℚ) +
        4 * C.coeff (3 : ℚ) * F.coeff (2 : ℚ) +
        4 * C.coeff (4 : ℚ) * F.coeff (1 : ℚ) +
        4 * C.coeff (5 : ℚ) * F.coeff (0 : ℚ) +
        3 * epsilon * B.coeff (5 : ℚ) = 0) ∧
    (4 * A.coeff (-2 : ℚ) * B.coeff (2 : ℚ) * F.coeff (3 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (3 : ℚ) * F.coeff (2 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (2 : ℚ) * F.coeff (2 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (4 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (3 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (0 : ℚ) * B.coeff (2 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (5 : ℚ) * F.coeff (0 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (4 : ℚ) * F.coeff (0 : ℚ) +
        4 * A.coeff (0 : ℚ) * B.coeff (3 : ℚ) * F.coeff (0 : ℚ) +
        4 * A.coeff (1 : ℚ) * B.coeff (2 : ℚ) * F.coeff (0 : ℚ) -
        6 * G.coeff (3 : ℚ) * (2 * C.coeff (0 : ℚ) + 3 * gamma) -
        12 * C.coeff (1 : ℚ) * G.coeff (2 : ℚ) -
        12 * C.coeff (2 : ℚ) * G.coeff (1 : ℚ) -
        12 * C.coeff (3 : ℚ) * G.coeff (0 : ℚ) -
        12 * F.coeff (0 : ℚ) * F.coeff (3 : ℚ) -
        12 * F.coeff (1 : ℚ) * F.coeff (2 : ℚ) -
        9 * epsilon * C.coeff (3 : ℚ) = 0)

/-- The `F`-gain child refined through its fifth unused rows: the
imported refinement is kept verbatim, and the coefficient-five `I₄`
row (with the deepened cube boundary `3B[1]²B[3] + 3B[1]B[2]²`) and
the coefficient-three `I₃` row (with the deepened
`4C[1]B[1]² + 2(4C[0]+3γ)B[1]B[2]` load) are added globally. -/
def RhoOneGZeroChildFGainRefinedFifth68
    (gamma epsilon : k) (A0R B0R C00R D0R E0R : RatFunc k)
    (A B C F G : HahnSeries ℚ k) : Prop :=
  RhoOneGZeroChildFGainRefinedFourth68 gamma epsilon A0R B0R C00R D0R E0R
      A B C F G ∧
    (-12 * B.coeff (1 : ℚ) ^ 2 * B.coeff (3 : ℚ) -
        12 * B.coeff (1 : ℚ) * B.coeff (2 : ℚ) ^ 2 +
        36 * B.coeff (1 : ℚ) * G.coeff (4 : ℚ) +
        36 * B.coeff (2 : ℚ) * G.coeff (3 : ℚ) +
        36 * B.coeff (3 : ℚ) * G.coeff (2 : ℚ) +
        36 * B.coeff (4 : ℚ) * G.coeff (1 : ℚ) +
        36 * B.coeff (5 : ℚ) * G.coeff (0 : ℚ) +
        18 * F.coeff (5 : ℚ) * (2 * C.coeff (0 : ℚ) + 3 * gamma) +
        36 * C.coeff (1 : ℚ) * F.coeff (4 : ℚ) +
        36 * C.coeff (2 : ℚ) * F.coeff (3 : ℚ) +
        36 * C.coeff (3 : ℚ) * F.coeff (2 : ℚ) +
        36 * C.coeff (4 : ℚ) * F.coeff (1 : ℚ) +
        27 * epsilon * B.coeff (5 : ℚ) = 0) ∧
    (4 * A.coeff (-2 : ℚ) * B.coeff (1 : ℚ) * F.coeff (4 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (2 : ℚ) * F.coeff (3 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (1 : ℚ) * F.coeff (3 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (3 : ℚ) * F.coeff (2 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (2 : ℚ) * F.coeff (2 : ℚ) +
        4 * A.coeff (0 : ℚ) * B.coeff (1 : ℚ) * F.coeff (2 : ℚ) +
        4 * A.coeff (-2 : ℚ) * B.coeff (4 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (-1 : ℚ) * B.coeff (3 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (0 : ℚ) * B.coeff (2 : ℚ) * F.coeff (1 : ℚ) +
        4 * A.coeff (1 : ℚ) * B.coeff (1 : ℚ) * F.coeff (1 : ℚ) +
        4 * C.coeff (1 : ℚ) * B.coeff (1 : ℚ) ^ 2 +
        2 * (4 * C.coeff (0 : ℚ) + 3 * gamma) * B.coeff (1 : ℚ) *
          B.coeff (2 : ℚ) -
        6 * G.coeff (3 : ℚ) * (2 * C.coeff (0 : ℚ) + 3 * gamma) -
        12 * C.coeff (1 : ℚ) * G.coeff (2 : ℚ) -
        12 * C.coeff (2 : ℚ) * G.coeff (1 : ℚ) -
...
`G[-1] = 0` sibling refined child by child through the fifth unused
coefficient rows. -/
theorem SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapGZeroChildrenFifthResidual
    [IsAlgClosed k]
  ...
  · right
    ...
    · left
      ...
      have hB2H : (↑(2 : ℚ) : WithTop ℚ) ≤
        ...
        simpa only [BR] using hBgain'.1.1.1.1.1
      have htie4 := rhoOneGZeroChildFifthI4Load_bGain_coeff_five68
      ...
      have htie3 := rhoOneGZeroChildFifthI3Cap_bGain_coeff_three68
    ...
    · right
      have hFgain' := hFgain
      dsimp only [RhoOneGZeroChildFGainRefinedFourth68,
      ...
      have hFH1 : (↑(1 : ℚ) : WithTop ℚ) ≤
          (ratFuncAtHahn46 T.root FR).orderTop := by
        rw [hFRraw]
        exact hFgain'.1.1.1.1.1
      have htie4 := rhoOneGZeroChildFifthI4Load_fGain_coeff_five68
        T.root T.source.gamma T.source.epsilon i4 BR CR FR GR
        hBH hCH hFH1 hGH hload
      rw [hCRraw, hFRraw, hGRraw] at htie4
      simp only [BR] at htie4
      have htie3 := rhoOneGZeroChildFifthI3Cap_fGain_coeff_three68
        T.root T.source.gamma T.source.epsilon i3 AR BR CR FR GR
...
#print axioms laurent_coeff_mul_lower_five_gZeroChildFifth68
#print axioms rhoOneGZeroChildFifthI4Load_bGain_coeff_five68
#print axioms rhoOneGZeroChildFifthI4Load_fGain_coeff_five68
#print axioms rhoOneGZeroChildFifthI3Cap_bGain_coeff_three68
#print axioms rhoOneGZeroChildFifthI3Cap_fGain_coeff_three68
#print axioms rhoOneGZeroChildFifth_bGain_affine_tail_minor68
#print axioms SquareZeroLIntegratedLocalRootData68.rhoOne_exceptional_doubleZero_refinedGZero_terminalGBoundaryCapGZeroChildrenFifthResidual

end AlignedSquareGZeroChildFifth68

...
theorem alphaWallMixedRowZeroRowTwo_beta_eq_zero
  ...
  have hlin :
      -((35 / 192 : k) * beta * a ^ 4) -
        (35 / 1536 : k) * beta * a ^ 4 = 0 := by
    rw [← hX0, hX2, sub_self]
  have hfac :
      (-(35 / 192 : k) - (35 / 1536 : k)) * (beta * a ^ 4) = 0 := by
    have hre :
        (-(35 / 192 : k) - (35 / 1536 : k)) * (beta * a ^ 4) =
          -((35 / 192 : k) * beta * a ^ 4) -
            (35 / 1536 : k) * beta * a ^ 4 := by
      simp [sub_eq_add_neg, mul_add, add_mul, neg_mul, mul_assoc, mul_comm,
        mul_left_comm]
    rw [hre]
    exact hlin
  have hnz : (-(35 / 192 : k) - (35 / 1536 : k)) ≠ 0 := by norm_num
  exact (mul_eq_zero.mp hfac).resolve_left hnz

theorem alphaWall_nat_lt_double_pred {n : ℕ} (hn : 1 < n) :
    n < 2 * n - 1 := by
  have h1 : 1 ≤ 2 * n := by omega
  have hsum : n + 1 < 2 * n := by omega
  have hadd : n + 1 < (2 * n - 1) + 1 := by
...
theorem alphaWallMixedFibre_support_false
  ...
  · have hc3 : c ^ 3 = 0 := by
      ...
      simpa [pow_three, pow_two, mul_assoc] using this
  ...
  · have hab : a * b ^ 2 = (9 : k) * c ^ 2 := by linear_combination -hrel
    ...
    · have hsq' : c ^ 2 = a * b ^ 2 := by linear_combination hsq
      ...
      have hb2 : b ^ 2 = 0 :=
        (mul_eq_zero.mp hab0).resolve_left ha
      have hb : b = 0 := eq_zero_of_pow_eq_zero hb2
      have hc2 : c ^ 2 = 0 := by
        rw [hb] at hsq'
        have hz : a * (0 : k) ^ 2 = 0 := by
          rw [zero_pow (by decide : (2 : ℕ) ≠ 0)]
          exact mul_zero a
        rw [hz] at hsq'
        exact hsq'
      have hc0 : c = 0 := eq_zero_of_pow_eq_zero hc2
      rcases hsupport with hb' | hc'
      · exact hb' hb
      · exact hc' hc0
...
theorem maximalExpandedIntegratedPolynomialLowerSystem_alphaWallBalancedImpossible68
  ...
  have hmix2 := residualRowTwo_coeff_mixedCubic_of_alphaWall
    Ae Be ce d ee n g h hn hg hgh (le_of_lt hh) hwall
...
theorem maximalExpandedIntegratedPolynomialLowerSystem_alphaWallDoubleImpossible68
  ...
  have hnotleft : ¬ 2 * (2 * g) < 3 * g := by omega
  have h2e : lowerRowTwoPolynomial68
      (integratedSPolynomial68 l alpha beta delta Ae Be Ce De)
      (integratedTPolynomial68 l alpha beta gamma epsilon Ae Be Ce De Ee)
      (integratedUPolynomial68 l alpha beta gamma delta zeta Ae Be Ce De Ee)
  ...
  rw [coeff_add, coeff_zero] at hres2
  have hmix2 := residualRowTwo_coeff_mixedCubic_of_alphaWall
    Ae Be ce d ee n g (2 * g) hn hg hgh2 le_rfl hwall hnotleft
    hA hB hc hdouble.1 hdouble.2 hQdeg beta hq0
  have hne : ¬ 2 * (2 * g) = 3 * g := by omega
  have hrow2 :
      (-4 / 27 : k) *
          ((25 / 32 : k) * (n : k) * beta * Ae.coeff (2 * n) ^ 4 +
            (n : k) *
    ...
    rw [hDrec] at hL
    rw [hmix2, hL] at hres2
    simpa only [ite_eq_right hne, mul_zero, sub_zero, alphaWallRowTwoLoadScalar68]
      using hres2
  have h0e : lowerRowZeroPolynomial68
      (integratedUPolynomial68 l alpha beta gamma delta zeta Ae Be Ce De Ee)
      (integratedVPolynomial68 l alpha beta gamma delta epsilon eta
  ...
  have hrow0 :
    ...
    exact hres0
  have hN : (n : k) ≠ 0 := Nat.cast_ne_zero.mpr (by omega)
  -- Same cubic-index `de` in both rows: `(d * ee)'` lives at
  -- `≤ 11n-4g-1 < 8n-1`, so the contribution is `0` (not a claim that
  -- the Newton tops `d_(5n-2g)` and `e_(6n-2g)` vanish).
  have hrow0' :
      (4 / 27 : k) * ((2 : k) * (n : k) * Ae.coeff (2 * n) *
          ((5 / 192 : k) * beta * Ae.coeff (2 * n) ^ 4 +
            Be.coeff (3 * n - g) * ce.coeff (4 * n - g) ^ 2 -
  ...
  have hbeta := alphaWallMixedRowZeroRowTwo_beta_eq_zero
    (n : k) (Ae.coeff (2 * n)) (Be.coeff (3 * n - g))
    (ce.coeff (4 * n - g)) (1 : k) (0 : k) beta hN ha
    (by
      simpa [alphaWallRowZeroLoadScalar68, mul_zero] using hrow0')
    (by
      simpa [alphaWallRowTwoLoadScalar68, mul_zero] using hrow2')
  ...
  have hX :
    ...
    have hX0 := alphaWallMixedRowZeroRowTwo_X_eq_of_rowZero
      (n : k) (Ae.coeff (2 * n)) (Be.coeff (3 * n - g))
      (ce.coeff (4 * n - g)) (1 : k) (0 : k) beta hN ha
      (by simpa [alphaWallRowZeroLoadScalar68, mul_zero] using hrow0')
    have hβ : (35 / 192 : k) * beta * Ae.coeff (2 * n) ^ 4 = 0 := by
      rw [show (35 / 192 : k) * beta * Ae.coeff (2 * n) ^ 4 =

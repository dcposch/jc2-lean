import Sol68LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroRowZeroThreeScratch

/-! # Offset-240 nonzero row-zero coefficient 2

Contracted row-zero coefficient `2` (expanded index `179`) is exactly
dependent after the terminal I3 scalar and all four cusp jets are imposed.
This source-facing packet records that dependency without manufacturing a
new scalar wall.  The disjoint pair-zero successor remains imported.
-/
...
section LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroTwo68
...
theorem mul_derivative_coeff_two_twoForty68
    (A : k[X]) :
    (A * derivative A).coeff 2 =
      3 * A.coeff 3 * A.coeff 0 +
        3 * A.coeff 2 * A.coeff 1 := by
  rw [mul_coeff_two68]
  norm_num [coeff_derivative]
  ring

theorem secondaryResidualRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
    (A B c d e : k[X])
    ...
    (secondaryResidualRowZeroPolynomial68 A B c d e).coeff 2 =
      (4 / 27 : k) *
        (-(3 * A.coeff 3 * A.coeff 0 +
              3 * A.coeff 2 * A.coeff 1) *
            (B.coeff 0 * e.coeff 0 + c.coeff 0 * d.coeff 0) +
          A.coeff 1 * B.coeff 0 *
            (2 * c.coeff 0 * c.coeff 2 + c.coeff 1 ^ 2) +
          4 * A.coeff 2 * B.coeff 0 * c.coeff 0 * c.coeff 1 +
          3 * A.coeff 3 * B.coeff 0 * c.coeff 0 ^ 2 -
          3 * A.coeff 1 * d.coeff 0 * e.coeff 2 -
          6 * A.coeff 2 * d.coeff 0 * e.coeff 1 -
          9 * A.coeff 3 * d.coeff 0 * e.coeff 0 +
          6 * B.coeff 0 * c.coeff 1 * e.coeff 2 +
          3 * B.coeff 0 * c.coeff 2 * e.coeff 1 -
          9 * c.coeff 1 * c.coeff 2 * d.coeff 0) := by
  let F : k[X] := B * e + c * d
  ...
  have hG2 := mul_derivative_coeff_two_twoForty68 A
  have hAAFraw := mul_coeff_two68 (A * derivative A) F
  rw [hFpos 1 (by norm_num), hFpos 2 (by norm_num), hG2, hF0] at hAAFraw
  have hAAF : (A * derivative A * F).coeff 2 =
      (3 * A.coeff 3 * A.coeff 0 + 3 * A.coeff 2 * A.coeff 1) *
        (B.coeff 0 * e.coeff 0 + c.coeff 0 * d.coeff 0) := by
  ...
  have hcsq2 : (c ^ 2).coeff 2 =
    ...
    ring
  have hBcRaw := mul_coeff_two68 (derivative A * B) (c ^ 2)
  rw [hP0, hP1, hP2, hcsq0, hcsq1, hcsq2] at hBcRaw
  have hBc : (derivative A * B * c ^ 2).coeff 2 =
      A.coeff 1 * B.coeff 0 *
          (2 * c.coeff 0 * c.coeff 2 + c.coeff 1 ^ 2) +
        4 * A.coeff 2 * B.coeff 0 * c.coeff 0 * c.coeff 1 +
        3 * A.coeff 3 * B.coeff 0 * c.coeff 0 ^ 2 := by
  ...
  have hAdePoly : derivative A * d * e =
    ...
    calc
      ...
      _ = d.coeff 0 • (derivative A * e) := by
        ...
        ring
  have hAdeRaw := mul_coeff_two68 (derivative A) e
  norm_num [coeff_derivative] at hAdeRaw
  have hAde : (derivative A * d * e).coeff 2 =
      A.coeff 1 * d.coeff 0 * e.coeff 2 +
        2 * A.coeff 2 * d.coeff 0 * e.coeff 1 +
        3 * A.coeff 3 * d.coeff 0 * e.coeff 0 := by
    rw [hAdePoly, coeff_smul, smul_eq_mul, hAdeRaw]
    ring
  have he3 : e.coeff 3 = 0 :=
    coeff_eq_zero_of_natDegree_lt (he.trans_lt (by norm_num))
  have hceRaw := mul_coeff_two68 c (derivative e)
  norm_num [coeff_derivative, he3] at hceRaw
  have hBcePoly : B * c * derivative e =
    ...
    calc
      ...
      _ = B.coeff 0 • (c * derivative e) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h7 : (B * c * derivative e).coeff 2 =
      B.coeff 0 *
        (2 * c.coeff 1 * e.coeff 2 + c.coeff 2 * e.coeff 1) := by
    rw [hBcePoly, coeff_smul, smul_eq_mul, hceRaw]
    ring
  have hc3 : c.coeff 3 = 0 :=
    coeff_eq_zero_of_natDegree_lt (hc.trans_lt (by norm_num))
  have hccRaw := mul_coeff_two68 c (derivative c)
  norm_num [coeff_derivative, hc3] at hccRaw
  have hccdPoly : c * derivative c * d =
    ...
    calc
      ...
      _ = d.coeff 0 • (c * derivative c) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h10 : (c * derivative c * d).coeff 2 =
      3 * c.coeff 1 * c.coeff 2 * d.coeff 0 := by
  ...
  have hlow3 : (A * B ^ 2 * derivative d).natDegree < 2 := by
    rw [hdd]
    norm_num
  have hlow4 : (A * B * derivative B * d).natDegree < 2 := by
    rw [hdB]
    norm_num
  have hlow8 : (B * d * derivative d).natDegree < 2 := by
    rw [hdd]
    norm_num
  have hlow9 : (derivative B * d ^ 2).natDegree < 2 := by
    rw [hdB]
    norm_num
...
theorem cubicLoadRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
    ...
    (cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon 0 eta
      ...
      (2 / 9 : k) * gamma * B.coeff 0 *
          (A.coeff 1 * c.coeff 2 +
            2 * A.coeff 2 * c.coeff 1 + 3 * A.coeff 3 * c.coeff 0) -
        ...
        (2 / 9 : k) * gamma * d.coeff 0 *
          (3 * A.coeff 3 * A.coeff 0 +
            3 * A.coeff 2 * A.coeff 1) -
        epsilon * A.coeff 3 * d.coeff 0 := by
  ...
  have hBderAcPoly : B * derivative A * c =
    ...
    calc
      ...
      _ = B.coeff 0 • (derivative A * c) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have hderAcRaw := mul_coeff_two68 (derivative A) c
  norm_num [coeff_derivative] at hderAcRaw
  have h1 : (B * derivative A * c).coeff 2 =
      B.coeff 0 * (A.coeff 1 * c.coeff 2 +
        2 * A.coeff 2 * c.coeff 1 + 3 * A.coeff 3 * c.coeff 0) := by
    rw [hBderAcPoly, coeff_smul, smul_eq_mul, hderAcRaw]
    ring
  have hAA2 := mul_derivative_coeff_two_twoForty68 A
  have hABApoly : A * B * derivative A =
    ...
    calc
      ...
      _ = B.coeff 0 • (A * derivative A) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h3 : (A * B * derivative A).coeff 2 =
      B.coeff 0 * (3 * A.coeff 3 * A.coeff 0 +
        3 * A.coeff 2 * A.coeff 1) := by
    rw [hABApoly, coeff_smul, hAA2, smul_eq_mul]
  have hDC : d = C (d.coeff 0) := eq_C_of_natDegree_le_zero hd
  have hAAdpoly : A * derivative A * d =
    ...
    calc
      ...
      _ = d.coeff 0 • (A * derivative A) := by
        rw [Polynomial.smul_eq_C_mul]
        ring
  have h4 : (A * derivative A * d).coeff 2 =
      d.coeff 0 * (3 * A.coeff 3 * A.coeff 0 +
        3 * A.coeff 2 * A.coeff 1) := by
    rw [hAAdpoly, coeff_smul, hAA2, smul_eq_mul]
  have hdApoly : d * derivative A =
    ...
    calc
      ...
      _ = d.coeff 0 • derivative A := by
        rw [Polynomial.smul_eq_C_mul]
  have h6 : (d * derivative A).coeff 2 =
      3 * A.coeff 3 * d.coeff 0 := by
    rw [hdApoly, coeff_smul, smul_eq_mul]
    norm_num [coeff_derivative]
    ring
  have hlow2 : (B * derivative e).natDegree < 2 := by
    compute_degree; omega
  have hlow5 : (d * derivative c).natDegree < 2 := by
    compute_degree; omega
  change (cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon 0 eta
      A B c D0 e).coeff 2 = _
  rw [hloadEq]
...
theorem fiveToSix_ordinaryTwoForty_rowZero_two_dependent68
    ...
    (hfourth : a0 * b ^ 6 * d - 2 * b ^ 7 * c0 +
      3 * b ^ 4 * c0 ^ 2 * d + 18 * b ^ 2 * c0 * d ^ 3 +
        27 * d ^ 5 = 0) :
    (4 / 27 : k) *
          (-(3 * a3 * a0 + 3 * a2 * a1) * (b * e0 + c0 * d) +
            a1 * b * (2 * c0 * c + c1 ^ 2) +
            4 * a2 * b * c0 * c1 + 3 * a3 * b * c0 ^ 2 -
            3 * a1 * d * y - 6 * a2 * d * e1 - 9 * a3 * d * e0 +
            6 * b * c1 * y + 3 * b * c * e1 - 9 * c1 * c * d) +
        (2 / 9 : k) * gamma * b *
          (a1 * c + 2 * a2 * c1 + 3 * a3 * c0) -
        (1 / 9 : k) * epsilon * b * (3 * a3 * a0 + 3 * a2 * a1) -
        (2 / 9 : k) * gamma * d * (3 * a3 * a0 + 3 * a2 * a1) -
        epsilon * a3 * d = 0 := by
  ...
  rw [ha2', ha3', hepsilon, hgamma, ha1', ha0', hy', he1', ha']
  field_simp [hb, hc, hx]
  ring

set_option maxHeartbeats 18000000 in
theorem fiveToSix_offsetTwoForty_nonzero_rowZero_two_source_dependency68
    (alpha gamma epsilon eta : k) (A B c d e : k[X])
    (hA : A.natDegree ≤ 4) (hB : B.natDegree ≤ 0)
    (hc : c.natDegree ≤ 2) (hd : d.natDegree ≤ 0)
    (he : e.natDegree ≤ 2)
    ...
    (secondaryResidualRowZeroPolynomial68 A B c d e +
      cubicLoadRowZeroPolynomial68 0 alpha 0 gamma 0 epsilon 0 eta
        A B c ((1 / 3 : k) • (A * B) + d) e).coeff 2 = 0 := by
  rw [coeff_add,
    secondaryResidualRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
      A B c d e hA hB hc hd he hface hfirst,
    cubicLoadRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
      alpha gamma epsilon eta A B c d e hA hB hc hd he]
  have hz := fiveToSix_ordinaryTwoForty_rowZero_two_dependent68
    ...
    hsecond hscalar hthird hfourth
  linear_combination hz

#print axioms mul_derivative_coeff_two_twoForty68
#print axioms secondaryResidualRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
#print axioms cubicLoadRowZeroPolynomial68_coeff_two_ordinaryTwoForty68
#print axioms fiveToSix_ordinaryTwoForty_rowZero_two_dependent68
#print axioms fiveToSix_offsetTwoForty_nonzero_rowZero_two_source_dependency68

end LaterDeepLowerParentFiveToSixOffsetTwoFortyOrdinaryNonzeroTwo68
